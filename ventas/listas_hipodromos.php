...
$xbanca_Recordset1 = 2;
$ejeMinCar=4;
$query_Recordset1 = sprintf("/* PARSEADORES1 ventas\listas_hipodromos.php - QUERY 1 */ SELECT * FROM carrera WHERE carrera.fec_carrera = %s AND carrera.hor_carrera >= %s AND carrera.est_carrera = 1 AND carrera.cod_banca = %s AND can_caballos>=%s ORDER BY carrera.hor_carrera  LIMIT 0, 10", GetSQLValueString($fec, "date"), GetSQLValueString($hor, "date"), GetSQLValueString($xbanca_Recordset1, "int"), GetSQLValueString($ejeMinCar, "int"));
