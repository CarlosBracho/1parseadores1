...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 ventas\ventas_eliminar_ticket.php - QUERY 1 */ SELECT ve.ser_venta, ve.tra_codigo, ve.cod_cliente, tp.pag_codigo
...
if (isset($_POST["MM_update"]) && $_POST["MM_update"] == "form1" && isset($_POST["ticket"]) && isset($_POST["uVenta"])) {
    ...
    if ($totalRows_Recordset1>0) {
        $query_Recordset1 = sprintf("/* PARSEADORES1 ventas\ventas_eliminar_ticket.php - QUERY 2 */ SELECT us.tic_eliminados, us.cod_taquilla, tp.pag_codigo
        ...
        if (BuscarTicketEliminados($_POST["uVenta"], fechaactualbd())<$row_Recordset1['tic_eliminados']) {
            $query_Recordset2 = sprintf(
                "/* PARSEADORES1 ventas\ventas_eliminar_ticket.php - QUERY 3 */ SELECT 
					ve.est_ticket, 
					ve.id_usuario, 
					ve.ser_venta,
					ve.cod_taquilla,
                    ...
                    if (isset($_POST["ser_venta"]) && $_POST["ser_venta"]==substr($row_Recordset2['ser_venta'], 0, 3)) {
                        if ($row_Recordset2['est_ticket']==1) {
                            if ($row_Recordset2['est_carrera']==1 or $row_Recordset2['hor_carrera']>=horaactual()) {
                                ...
                                $query_Recordset3 = sprintf("/* PARSEADORES1 ventas\ventas_eliminar_ticket.php - QUERY 4 */ SELECT num_ticket 
									FROM venta ve 
                                ...
                                if ($totalRows_Recordset3>0) {
                                    ...
                                    do {
                                        ...
                                        if($tra_codigo=1){
                                        $updateSQL = sprintf(
                                            "/* PARSEADORES1 ventas\ventas_eliminar_ticket.php - QUERY 5 */ UPDATE venta 
                                    ...
                                    if($tra_codigo=0){
                                        $updateSQL = sprintf(
                                            "/* PARSEADORES1 ventas\ventas_eliminar_ticket.php - QUERY 6 */ UPDATE venta 
										   SET est_ticket=%s, fec_pago=%s, cod_usuario_pago=%s, ip_pago=%s, hor_pago=%s, tra_codigo=%s
											   WHERE num_ticket=%s",
                                            GetSQLValueString(0, "int"),
                                            GetSQLValueString($fecpago, "date"),
                                            GetSQLValueString($_POST["uVenta"], "int"),
                                        ...
                                        if ($tipo_pago==1) {
                                            $query_Recordset55 = sprintf(
                                                "
/* PARSEADORES1 ventas\ventas_eliminar_ticket.php - QUERY 7 */ SELECT * FROM tasadecambio
...
LIMIT 100",
                                            ...
                                            $insertSQL15 = sprintf(
                                                "/* PARSEADORES1 ventas\ventas_eliminar_ticket.php - QUERY 8 */ UPDATE taquilla 
				SET
				...
				cod_taquilla=%s",
                                        ...
                                        if ($tipo_pagoa==1) {
                                            if ($usdabss==0) {
                                                $query_Recordset55 = sprintf(
                                                    "
/* PARSEADORES1 ventas\ventas_eliminar_ticket.php - QUERY 9 */ SELECT * FROM tasadecambio
WHERE Idtasadecambio = %s 
LIMIT 100",
                                                    GetSQLValueString(1, "int")
                                                );
                                            ...
                                            $insertSQL155 = sprintf(
                                                "/* PARSEADORES1 ventas\ventas_eliminar_ticket.php - QUERY 10 */ UPDATE agencia 
				SET
...
<?php
    ...
    if (isset($_GET["pagoSIN"]) && $ver==1 && $pag_codigo==0) {
		...
		<form method="post" name="form1" action="<?php echo $editFormAction; ?>" onsubmit="return chequearEnvio();">
            ...
            <?php
            if($tra_codigo==1){
            if ($tra_codigo==0 && $row_Recordset1['cod_cliente']!="ANONIMO") {?>
	...
	<div style="width:<?php echo $width; ?>;float:left;text-align:right;color:#FFF;font-size:28px; text-align:center;">
		...
		class="btn-primary" 
		onclick="location.href = '../ventasmie/eli_tic_sincodigo.php?uVenta=<?php echo $usuarioPago;?>'"/>	
	</div>
    </div>
    </div>
</body>
