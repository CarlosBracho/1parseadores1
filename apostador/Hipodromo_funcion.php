...
bloq_hipodromo($hipodromo,$cod_taquilla);
 function bloq_hipodromo($hipodromo,$cod_taquilla)
 {
    ...
    global $conexionbanca;
     $query_Recordset1 = sprintf(
        "/* PARSEADORES1 apostador\Hipodromo_funcion.php - QUERY 1 */ SELECT 
...
if($row_Recordset1['hip_bloqueados']==''){
    $hipodromo=$hipodromo.',';
   $insertSQL1 = sprintf(
        "/* PARSEADORES1 apostador\Hipodromo_funcion.php - QUERY 2 */ UPDATE taquilla_opc_ame  
...
if($id==1){
$hipodromo=$hipodromo.',';
$ingresar=$row_Recordset1['hip_bloqueados'].$hipodromo;
$insertSQL1 = sprintf(
    "/* PARSEADORES1 apostador\Hipodromo_funcion.php - QUERY 3 */ UPDATE taquilla_opc_ame  
        SET hip_bloqueados=%s				
        WHERE cod_taquilla=%s",
    
    GetSQLValueString($ingresar, "text"),
    GetSQLValueString($cod_taquilla, "int")
);
$Result1 = mysqli_query($conexionbanca, $insertSQL1) or die(mysqli_error($conexionbanca));
}
}
     return array($cod_taquilla);
 }
