...
$codigoAgente=$_SESSION['MM_cod_agente'];
    ...
    if (!empty($_POST)) {
        ...
        if (!empty($_POST['telefonoa'])) {
            ...
            $insertSQL1 = sprintf(
                "/* PARSEADORES1 apostador\listaapostadoraa.php - QUERY 1 */ UPDATE agencia 
				SET tel_agencia=%s
    ...
    $query_Recordset1 = "/* PARSEADORES1 apostador\listaapostadoraa.php - QUERY 2 */ SELECT * FROM taquilla, agencia $where";
...
<th scope="col">Bloquear Hipodromos</th>
          </tr>
        </thead>
        <tbody>
          <?php
...
do {
    ...
    echo $row_Recordset1['tel_taquilla']; ?></td>
        ...
        "/* PARSEADORES1 apostador\listaapostadoraa.php - QUERY 3 */ SELECT MAX(Idbalancecli) 
    ...
    $query_Recordset14 = sprintf(
        "/* PARSEADORES1 apostador\listaapostadoraa.php - QUERY 4 */ SELECT saldoactualc
					FROM balanceclientes
					WHERE monedac = %s AND
					Idbalancecli = %s",
        GetSQLValueString(0, "int"),
    ...
    $query_Recordset15 = sprintf(
        "/* PARSEADORES1 apostador\listaapostadoraa.php - QUERY 5 */ SELECT MAX(Idbalancecli) 
					FROM balanceclientes
					WHERE monedac = %s AND 
					cod_taquilla = %s",
    ...
    $query_Recordset16 = sprintf(
        "/* PARSEADORES1 apostador\listaapostadoraa.php - QUERY 6 */ SELECT saldoactualc
					FROM balanceclientes
					WHERE monedac = %s AND
					Idbalancecli = %s",
        GetSQLValueString(3, "int"),
    ...
    $query_Recordset17 = sprintf(
        "/* PARSEADORES1 apostador\listaapostadoraa.php - QUERY 7 */ SELECT MAX(Idbalancecli) 
					FROM balanceclientes
					WHERE monedac = %s AND 
					cod_taquilla = %s",
    ...
    $query_Recordset18 = sprintf(
        "/* PARSEADORES1 apostador\listaapostadoraa.php - QUERY 8 */ SELECT saldoactualc
					FROM balanceclientes
					WHERE monedac = %s AND
					Idbalancecli = %s",
        GetSQLValueString(4, "int"),
    ...
    $query_Recordset19 = sprintf(
        "/* PARSEADORES1 apostador\listaapostadoraa.php - QUERY 9 */ SELECT MAX(Idbalancecli) 
					FROM balanceclientes
					WHERE monedac = %s AND 
					cod_taquilla = %s",
    ...
    $query_Recordset20 = sprintf(
        "/* PARSEADORES1 apostador\listaapostadoraa.php - QUERY 10 */ SELECT saldoactualc
					FROM balanceclientes
					WHERE monedac = %s AND
					Idbalancecli = %s",
        GetSQLValueString(5, "int"),
    ...
    <td><a href="bloqueo_ha.php?recordID=<?php echo $row_Recordset1['cod_taquilla']; ?>" class="btn btn-xs btn-success"><span class="glyphicon glyphicon-pencil"></span>Bloquear</a></td>
          </tr>
          <?php
