...
function ObtenerMontoEjeTaq($codT, $fecV, $numE, $codC, $codA)
{
    ...
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 apostador\t_grabajugadahipicoticket.php - QUERY 1 */ SELECT 
...
$query_Recordset7 = sprintf("/* PARSEADORES1 apostador\t_grabajugadahipicoticket.php - QUERY 2 */ SELECT est_control_ventas_ame FROM ctrol_ventpag_global_ame WHERE cod_ctrol_ventpag_global_ame = 1");
...
if (isset($_POST["MM_insert"]) && $_POST["MM_insert"]=="form1" && isset($_POST["cod_carrera"]) && $_POST["cod_carrera"]!=-1 && $est_control_ventas==0) {
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 apostador\t_grabajugadahipicoticket.php - QUERY 3 */ SELECT can_caballos, hor_carrera, fec_carrera, est_carrera, pau_ventas
								 FROM carrera
								 WHERE cod_carrera = %s 
								 LIMIT 1",
    ...
    if ($exito==1 && $pau_ventas==0) {// guarda e imprime ticket
        ...
        if ($valida==1 && $pau_ventas==0) {
            ...
            $query_Recordset55 = sprintf(
                "
/* PARSEADORES1 apostador\t_grabajugadahipicoticket.php - QUERY 4 */ SELECT 
tasadecambio.usdabss, tasadecambio.copabss, tasadecambio.solabss, 
agencia.tipo_pagoa, agencia.saldoactuala,
taquilla.tipo_pago, taquilla.saldoactual, taquilla.saldoactualusd, taquilla.saldoactualcop, taquilla.saldoactualsol
FROM 
...
tasadecambio.Idtasadecambio = %s",
            ...
            if ($_POST['efectivoO']<=2) {
                $tasa=1;
                if ($totalTicketx>$row_Recordset55['saldoactual']) {
                    $valida=0;
            ...
            if ($_POST['efectivoO']==3) {
                $tasa=$usdabss;
                if ($totalTicketx>$row_Recordset55['saldoactualusd']) {
                    $valida=0;
                    $menValida="Monto excede saldo recargue por favor";
                    $exito=0;
                }
            ...
            if ($_POST['efectivoO']==4) {
                $tasa=$copabss;
                if ($totalTicketx>$row_Recordset55['saldoactualcop']) {
                    $valida=0;
                    $menValida="Monto excede saldo recargue por favor";
                    $exito=0;
                }
            ...
            if ($_POST['efectivoO']==5) {
                $tasa=$solabss;
                if ($totalTicketx>$row_Recordset55['saldoactualsol']) {
                    $valida=0;
                    $menValida="Monto excede saldo recargue por favor";
                    $exito=0;
                }
        ...
        if ($sRetiro1==0 && $sRetiro2==0 && $sRetiro3==0 && $sRetiro4==0) { }else { $valida=0; $exito=0; }//esto

        if ($mensaje4!="" || $mensaje3!="" || $mensaje2!="" || $mensaje1!="") { //esto
        ...
        if ($valida==1 && $pau_ventas==0) {
            ...
            foreach ($apuesta as $apta) {
                ...
                $insertSQL = sprintf(
                    "/* PARSEADORES1 apostador\t_grabajugadahipicoticket.php - QUERY 5 */ INSERT INTO venta (ser_venta, ticket, cod_taquilla, fec_venta, hor_venta, cod_tventa,
                ...
                $insertSQL15 = sprintf(
                    "/* PARSEADORES1 apostador\t_grabajugadahipicoticket.php - QUERY 6 */ UPDATE taquilla 
				SET
                ...
                if ($tipo_pagoa==1) {
                    $insertSQL155 = sprintf(
                        "/* PARSEADORES1 apostador\t_grabajugadahipicoticket.php - QUERY 7 */ UPDATE agencia 
				SET
