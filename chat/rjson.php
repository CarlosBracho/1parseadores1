...
if (isset($_SESSION['MM_UserChat']) && $_SESSION['MM_UserChat']!="") {
    ...
    $query_Recordset102 = sprintf(
        "/* PARSEADORES1 chat\rjson.php - QUERY 1 */ SELECT ch.id, ch.connected	
		FROM chat ch 
		WHERE ch.from1=%s
		ORDER BY ch.id DESC
		LIMIT 1",
    ...
    if ($totalRows_Recordset102>0) {
        $insertSQL1 = sprintf(
            "/* PARSEADORES1 chat\rjson.php - QUERY 2 */ UPDATE chat 
			SET connected=%s
			WHERE id=%s",
        ...
        $insertSQL = sprintf(
            "/* PARSEADORES1 chat\rjson.php - QUERY 3 */ INSERT INTO chat (from1, to1, message, sentdate, senttime, recd) 
				VALUES (%s, %s, %s, %s, %s, %s)",
