...
$query_Recordset101 = sprintf(
    "/* PARSEADORES1 chat\pjson.php - QUERY 1 */ SELECT us.tip_usuario, us.id_usuario,
	CASE us.tip_usuario  
		WHEN 'U' THEN (/* PARSEADORES1 chat\pjson.php - QUERY 2 */ SELECT nom_taquilla FROM taquilla ta WHERE ta.cod_taquilla=us.cod_taquilla LIMIT 1)  
		WHEN 'G' THEN (/* PARSEADORES1 chat\pjson.php - QUERY 3 */ SELECT nom_agencia FROM agencia ag WHERE ag.cod_agencia=us.cod_taquilla LIMIT 1)
		WHEN 'D' THEN (/* PARSEADORES1 chat\pjson.php - QUERY 4 */ SELECT nom_banca FROM banca ba WHERE ba.cod_banca=us.cod_taquilla LIMIT 1)
		ELSE 'NO DEFINIDO'
	END AS nombre_chat, 
	(/* PARSEADORES1 chat\pjson.php - QUERY 5 */ SELECT COUNT(*) FROM chat ch 
		WHERE ch.to1=%s AND nombre_chat = ch.from1 AND recd=1
		ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC) AS no_leido,
	(/* PARSEADORES1 chat\pjson.php - QUERY 6 */ SELECT message FROM chat ch  
	...
	(/* PARSEADORES1 chat\pjson.php - QUERY 7 */ SELECT sentdate FROM chat ch 
		WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
		ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS sentdate,
	(/* PARSEADORES1 chat\pjson.php - QUERY 8 */ SELECT senttime FROM chat ch 
		WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
		ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS senttime,
	(/* PARSEADORES1 chat\pjson.php - QUERY 9 */ SELECT connected FROM chat ch 
...
$query_Recordset102 = sprintf("/* PARSEADORES1 chat\pjson.php - QUERY 10 */ SELECT cod_banca, nom_banca FROM banca ORDER BY nom_banca ASC");
...
do {
    ...
    $query_Recordset101 = sprintf(
        "/* PARSEADORES1 chat\pjson.php - QUERY 11 */ SELECT us.tip_usuario, us.id_usuario, 
		CASE us.tip_usuario  
			WHEN 'U' THEN (/* PARSEADORES1 chat\pjson.php - QUERY 12 */ SELECT nom_taquilla FROM taquilla ta, agencia ag WHERE ta.cod_taquilla=us.cod_taquilla AND
				ag.cod_agencia=ta.cod_agencia AND ag.cod_banca =%s LIMIT 1)  
			WHEN 'G' THEN (/* PARSEADORES1 chat\pjson.php - QUERY 13 */ SELECT nom_agencia FROM agencia ag WHERE ag.cod_agencia=us.cod_taquilla AND
		...
		CASE us.tip_usuario  
			...
			ELSE 'NO DEFINIDO'
		END AS orden,
		(/* PARSEADORES1 chat\pjson.php - QUERY 14 */ SELECT COUNT(*) FROM chat ch 
			WHERE ch.to1=%s AND nombre_chat = ch.from1 AND recd=1
			ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC) AS no_leido,
		(/* PARSEADORES1 chat\pjson.php - QUERY 15 */ SELECT message FROM chat ch  
			WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
			ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS message,
		(/* PARSEADORES1 chat\pjson.php - QUERY 16 */ SELECT sentdate FROM chat ch 
			WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
			ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS sentdate,
		(/* PARSEADORES1 chat\pjson.php - QUERY 17 */ SELECT senttime FROM chat ch 
			WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
			ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS senttime,
		(/* PARSEADORES1 chat\pjson.php - QUERY 18 */ SELECT connected FROM chat ch 
			WHERE nombre_chat = ch.from1
			ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS connected
	FROM 
		usuario us
...
$query_Recordset102 = sprintf("/* PARSEADORES1 chat\pjson.php - QUERY 19 */ SELECT cod_agencia, nom_agencia, cod_banca FROM agencia ORDER BY nom_agencia ASC");
$Recordset102 = mysqli_query($conexionbanca, $query_Recordset102) or die(mysqli_error($conexionbanca));
$row_Recordset102 = mysqli_fetch_assoc($Recordset102);
$totalRows_Recordset102 = mysqli_num_rows($Recordset102);
do {
    ...
    $query_Recordset101 = sprintf(
        "/* PARSEADORES1 chat\pjson.php - QUERY 20 */ SELECT us.tip_usuario, us.id_usuario,
			CASE us.tip_usuario  
				WHEN 'U' THEN (/* PARSEADORES1 chat\pjson.php - QUERY 21 */ SELECT nom_taquilla FROM taquilla ta WHERE ta.cod_taquilla=us.cod_taquilla 
					AND ta.cod_agencia=%s LIMIT 1)  
				WHEN 'D' THEN (/* PARSEADORES1 chat\pjson.php - QUERY 22 */ SELECT nom_banca FROM banca ba WHERE ba.cod_banca=%s LIMIT 1)
				WHEN 'A' THEN 'Soporte'
				ELSE 'NO DEFINIDO'
			END AS nombre_chat, 
			CASE us.tip_usuario  
				...
				ELSE 'NO DEFINIDO'
			END AS orden,
			(/* PARSEADORES1 chat\pjson.php - QUERY 23 */ SELECT COUNT(*) FROM chat ch 
				WHERE ch.to1=%s AND nombre_chat = ch.from1 AND recd=1
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC) AS no_leido,
			(/* PARSEADORES1 chat\pjson.php - QUERY 24 */ SELECT message FROM chat ch  
				WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS message,
			(/* PARSEADORES1 chat\pjson.php - QUERY 25 */ SELECT sentdate FROM chat ch 
				WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS sentdate,
			(/* PARSEADORES1 chat\pjson.php - QUERY 26 */ SELECT senttime FROM chat ch 
				WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS senttime,
			(/* PARSEADORES1 chat\pjson.php - QUERY 27 */ SELECT connected FROM chat ch 
				WHERE nombre_chat = ch.from1
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS connected
		FROM 
			usuario us
...
$query_Recordset102 = sprintf("/* PARSEADORES1 chat\pjson.php - QUERY 28 */ SELECT ta.cod_agencia, us.nom_usuario FROM usuario us, taquilla ta 
...
do {
    ...
    $query_Recordset101 = sprintf(
        "/* PARSEADORES1 chat\pjson.php - QUERY 29 */ SELECT us.tip_usuario, us.id_usuario,
			CASE us.tip_usuario  
				WHEN 'G' THEN (/* PARSEADORES1 chat\pjson.php - QUERY 30 */ SELECT nom_agencia FROM agencia ag WHERE ag.cod_agencia=%s LIMIT 1)
				WHEN 'A' THEN 'Soporte'
				ELSE 'NO DEFINIDO'
			END AS nombre_chat,
			CASE us.tip_usuario  
				...
				ELSE 'NO DEFINIDO'
			END AS orden,
			(/* PARSEADORES1 chat\pjson.php - QUERY 31 */ SELECT COUNT(*) FROM chat ch 
				WHERE ch.to1=%s AND nombre_chat = ch.from1 AND recd=1
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC) AS no_leido,
			(/* PARSEADORES1 chat\pjson.php - QUERY 32 */ SELECT message FROM chat ch  
				WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS message,
			(/* PARSEADORES1 chat\pjson.php - QUERY 33 */ SELECT sentdate FROM chat ch 
				WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS sentdate,
			(/* PARSEADORES1 chat\pjson.php - QUERY 34 */ SELECT senttime FROM chat ch 
				WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS senttime,
			(/* PARSEADORES1 chat\pjson.php - QUERY 35 */ SELECT connected FROM chat ch 
				WHERE nombre_chat = ch.from1
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS connected
		FROM 
			usuario us
