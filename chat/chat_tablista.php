...
if (isset($_SESSION['MM_id_usuario']) && $_SESSION['MM_id_usuario']!="" && isset($_SESSION['MM_UserChat']) && $_SESSION['MM_UserChat']!="") {
    ...
    $query_Recordset102 = sprintf(
        "/* PARSEADORES1 chat\chat_tablista.php - QUERY 1 */ SELECT ch.id, ch.connected	
		FROM chat ch 
		WHERE ch.from1=%s
		ORDER BY ch.id DESC
		LIMIT 1",
    ...
    if ($totalRows_Recordset102>0) {
        $insertSQL1 = sprintf(
            "/* PARSEADORES1 chat\chat_tablista.php - QUERY 2 */ UPDATE chat 
			SET connected=%s
			WHERE id=%s",
        ...
        $insertSQL = sprintf(
            "/* PARSEADORES1 chat\chat_tablista.php - QUERY 3 */ INSERT INTO chat (from1, to1, message, sentdate, senttime, recd) 
				VALUES (%s, %s, %s, %s, %s, %s)",
    ...
    if ($_SESSION['MM_UserGroup'] == "A") {
        $query_Recordset101 = sprintf(
            "/* PARSEADORES1 chat\chat_tablista.php - QUERY 4 */ SELECT us.nom_usuario, us.tip_usuario, us.id_usuario,
			CASE us.tip_usuario  
				WHEN 'U' THEN (/* PARSEADORES1 chat\chat_tablista.php - QUERY 5 */ SELECT nom_taquilla FROM taquilla ta WHERE ta.cod_taquilla=us.cod_taquilla LIMIT 1)  
				WHEN 'G' THEN (/* PARSEADORES1 chat\chat_tablista.php - QUERY 6 */ SELECT nom_agencia FROM agencia ag WHERE ag.cod_agencia=us.cod_taquilla LIMIT 1)
				WHEN 'D' THEN (/* PARSEADORES1 chat\chat_tablista.php - QUERY 7 */ SELECT nom_banca FROM banca ba WHERE ba.cod_banca=us.cod_taquilla LIMIT 1)
				ELSE 'NO DEFINIDO'
			END AS nombre_chat, 
			CASE us.tip_usuario  
			...
			END AS tipo_chat2,
			CASE us.tip_usuario  
				WHEN 'U' THEN (/* PARSEADORES1 chat\chat_tablista.php - QUERY 8 */ SELECT nom_agencia FROM taquilla taq, agencia age WHERE taq.cod_taquilla=us.cod_taquilla AND 
					age.cod_agencia = taq.cod_agencia LIMIT 1)  
				WHEN 'G' THEN (/* PARSEADORES1 chat\chat_tablista.php - QUERY 9 */ SELECT nom_banca FROM banca ban WHERE ban.cod_banca=us.cod_taquilla LIMIT 1)
				WHEN 'D' THEN ''
				ELSE 'NO DEFINIDO'
			END AS nombre_sup, 
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 10 */ SELECT COUNT(*) FROM chat ch 
				WHERE ch.to1=%s AND nombre_chat = ch.from1 AND recd=1
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC) AS no_leido,
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 11 */ SELECT message FROM chat ch  
			...
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 12 */ SELECT sentdate FROM chat ch 
				WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS sentdate,
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 13 */ SELECT senttime FROM chat ch 
				WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS senttime,
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 14 */ SELECT connected FROM chat ch 
        ...
        $query_Recordset101 = sprintf(
            "/* PARSEADORES1 chat\chat_tablista.php - QUERY 15 */ SELECT us.nom_usuario, us.tip_usuario, us.id_usuario,
			CASE us.tip_usuario  
				WHEN 'G' THEN (/* PARSEADORES1 chat\chat_tablista.php - QUERY 16 */ SELECT nom_agencia FROM agencia ag WHERE ag.cod_agencia=%s LIMIT 1)
				WHEN 'A' THEN 'Soporte'
				ELSE 'NO DEFINIDO'
			END AS nombre_chat,
			CASE us.tip_usuario  
			...
			END AS orden,
			CASE us.tip_usuario  
				WHEN 'G' THEN (/* PARSEADORES1 chat\chat_tablista.php - QUERY 17 */ SELECT nom_banca FROM banca ban WHERE ban.cod_banca=us.cod_taquilla LIMIT 1)
				WHEN 'A' THEN 'Soporte'
				ELSE 'NO DEFINIDO'
			END AS nombre_sup,
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 18 */ SELECT COUNT(*) FROM chat ch 
				WHERE ch.to1=%s AND nombre_chat = ch.from1 AND recd=1
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC) AS no_leido,
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 19 */ SELECT message FROM chat ch  
				WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS message,
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 20 */ SELECT sentdate FROM chat ch 
				WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS sentdate,
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 21 */ SELECT senttime FROM chat ch 
				WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS senttime,
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 22 */ SELECT connected FROM chat ch 
				WHERE nombre_chat = ch.from1
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS connected
		FROM 
			usuario us
        ...
        $query_Recordset101 = sprintf(
            "/* PARSEADORES1 chat\chat_tablista.php - QUERY 23 */ SELECT us.nom_usuario, us.tip_usuario, us.id_usuario,
			CASE us.tip_usuario  
				WHEN 'U' THEN (/* PARSEADORES1 chat\chat_tablista.php - QUERY 24 */ SELECT nom_taquilla FROM taquilla ta WHERE ta.cod_taquilla=us.cod_taquilla 
					AND ta.cod_agencia=%s LIMIT 1)  
				WHEN 'D' THEN (/* PARSEADORES1 chat\chat_tablista.php - QUERY 25 */ SELECT nom_banca FROM banca ba WHERE ba.cod_banca=%s LIMIT 1)
				WHEN 'A' THEN 'Soporte'
				ELSE 'NO DEFINIDO'
			END AS nombre_chat, 
			CASE us.tip_usuario  
			...
			END AS orden,
			CASE us.tip_usuario  
				WHEN 'U' THEN (/* PARSEADORES1 chat\chat_tablista.php - QUERY 26 */ SELECT nom_agencia FROM taquilla taq, agencia age WHERE taq.cod_taquilla=us.cod_taquilla AND 
					age.cod_agencia = taq.cod_agencia LIMIT 1)  
				WHEN 'D' THEN ''
				WHEN 'A' THEN 'Soporte'
				ELSE 'NO DEFINIDO'
			END AS nombre_sup, 
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 27 */ SELECT COUNT(*) FROM chat ch 
				WHERE ch.to1=%s AND nombre_chat = ch.from1 AND recd=1
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC) AS no_leido,
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 28 */ SELECT message FROM chat ch  
				WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS message,
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 29 */ SELECT sentdate FROM chat ch 
				WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS sentdate,
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 30 */ SELECT senttime FROM chat ch 
				WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS senttime,
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 31 */ SELECT connected FROM chat ch 
				WHERE nombre_chat = ch.from1
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS connected
		FROM 
			usuario us
        ...
        $query_Recordset101 = sprintf(
            "/* PARSEADORES1 chat\chat_tablista.php - QUERY 32 */ SELECT us.nom_usuario, us.tip_usuario, us.id_usuario,
			CASE us.tip_usuario  
				WHEN 'U' THEN (/* PARSEADORES1 chat\chat_tablista.php - QUERY 33 */ SELECT nom_taquilla FROM taquilla ta, agencia ag WHERE ta.cod_taquilla=us.cod_taquilla AND
					ag.cod_agencia=ta.cod_agencia AND ag.cod_banca =%s LIMIT 1)  
				WHEN 'G' THEN (/* PARSEADORES1 chat\chat_tablista.php - QUERY 34 */ SELECT nom_agencia FROM agencia ag WHERE ag.cod_agencia=us.cod_taquilla AND
			...
			END AS orden,
			CASE us.tip_usuario  
				WHEN 'U' THEN (/* PARSEADORES1 chat\chat_tablista.php - QUERY 35 */ SELECT nom_agencia FROM taquilla taq, agencia age WHERE taq.cod_taquilla=us.cod_taquilla AND 
					age.cod_agencia = taq.cod_agencia LIMIT 1)  
				WHEN 'G' THEN (/* PARSEADORES1 chat\chat_tablista.php - QUERY 36 */ SELECT nom_banca FROM banca ban WHERE ban.cod_banca=us.cod_taquilla LIMIT 1)
				WHEN 'A' THEN 'Soporte'
				ELSE 'NO DEFINIDO'
			END AS nombre_sup, 
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 37 */ SELECT COUNT(*) FROM chat ch 
				WHERE ch.to1=%s AND nombre_chat = ch.from1 AND recd=1
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC) AS no_leido,
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 38 */ SELECT message FROM chat ch  
				WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS message,
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 39 */ SELECT sentdate FROM chat ch 
				WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS sentdate,
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 40 */ SELECT senttime FROM chat ch 
				WHERE (nombre_chat = ch.from1 AND ch.to1=%s AND recd!=2) OR (nombre_chat = ch.to1 AND ch.from1=%s AND recd!=2)
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS senttime,
			(/* PARSEADORES1 chat\chat_tablista.php - QUERY 41 */ SELECT connected FROM chat ch 
				WHERE nombre_chat = ch.from1
				ORDER BY ch.id DESC, ch.sentdate DESC, ch.senttime DESC LIMIT 1) AS connected
		FROM 
			usuario us
