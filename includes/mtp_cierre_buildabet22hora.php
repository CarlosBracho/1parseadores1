...
if (!isset($_SESSION)) {
    session_start();
} require_once('../Connections/conexionbanca.php');set_time_limit(0);$fech=fechaactualbd();$horasistema=horaactual();
$query_Recordset1 = sprintf("/* PARSEADORES1 includes\mtp_cierre_buildabet22hora.php - QUERY 1 */ SELECT 
carrera.cod_carrera,
...
carrera.simulcast=0 AND
 carrera.mtp_control=9 AND
 carrera.mtp_tipo=1 AND
 eje_primero=0 AND
 fec_carrera=%s", 
            ...
            foreach ($nHipodro as $hip) {
                ...
                if ($id==$idCarrrera[$f] && $restan[$f]<=50) {
                    ...
                    if ($xEstado[$f]=="X") {
                        ...
                        $nm1="EL SISTEMA";
                        $updateSQL = sprintf(
                            "/* PARSEADORES1 includes\mtp_cierre_buildabet22hora.php - QUERY 2 */ UPDATE carrera 
							SET 
                        ...
                        $insertSQL2 = sprintf(
                            "/* PARSEADORES1 includes\mtp_cierre_buildabet22hora.php - QUERY 3 */ INSERT 
								INTO bitacora 
                    ...
                    if ($act==1) {
                        if($row_Recordset1['ABIERTOX']==0){$abiertox="BUILDA";}else{$abiertox=0;}
                        if($row_Recordset1['horaapertura']=='00:00:00'){$horaapertura=$horasistema;}else{$horaapertura=$row_Recordset1['horaapertura'];}
                        $updateSQL = sprintf(
                            "/* PARSEADORES1 includes\mtp_cierre_buildabet22hora.php - QUERY 4 */ UPDATE carrera SET 
											hor_carrera=%s, 
											hor_mtp=%s, 
											est_carrera=%s, 
											est_cierre=%s, 
                        ...
                        $Result1 = mysqli_query($conexionbanca, $updateSQL) or die(mysqli_error($conexionbanca));
                                  ...
                                  $fechaactual=fechaactualbd();
                $insertSQL = sprintf(
                    "/* PARSEADORES1 includes\mtp_cierre_buildabet22hora.php - QUERY 5 */ INSERT INTO quiencierrayabre 
					(codcarrera, 
					fechaquien, 
					que) 
					VALUES (%s, %s, %s)",
