...
$horasistema=horaactual();
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 includes\ameridatos_soloapertura.php - QUERY 1 */ SELECT * FROM carrera ca, hipodromo hi WHERE 
	 ca.est_cierre=3
	AND ca.fec_carrera=%s 
    AND hi.mtp_paribet=1
    ...
    do {
        ...
        if ($hipodomo[0]!="") {
            foreach ($hipodomo as $hip) {
                if (trim($hipodomo[$f])==trim($row_Recordset1['nom_hipodromo_sup']) && $numeroca[$f]==$row_Recordset1['num_carrera'] &&  $restante[$f]<=10) {
...
if ($row_Recordset1['ABIERTOX']=="") {
                    ...
                    $updateSQL = sprintf(
                        "/* PARSEADORES1 includes\ameridatos_soloapertura.php - QUERY 2 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_cierre=%s, est_carrera=%s, supercontrol=%s, mtp_control=%s , ABIERTOX=%s 
											  WHERE cod_carrera=%s",
                    ...
                    $insertSQL = sprintf(
                        "/* PARSEADORES1 includes\ameridatos_soloapertura.php - QUERY 3 */ INSERT INTO quiencierrayabre 
                        (codcarrera, 
                        fechaquien, 
                        que) 
                        VALUES (%s, %s, %s)",
