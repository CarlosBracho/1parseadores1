...
if (isset($_GET["hip"]) && isset($_GET["can"])) {
    ...
    if ($xhip!="" && $xcan>0) {
        ...
        if ($verif==0) {
            ...
            if ($acceso==1) {
                $updateSQL = sprintf(
                    "/* PARSEADORES1 includes\programar_carreras_guardar_buildtvg.php - QUERY 1 */ UPDATE hipodromo SET 
                ...
                $updateSQL = sprintf(
                    "/* PARSEADORES1 includes\programar_carreras_guardar_buildtvg.php - QUERY 2 */ UPDATE hipodromo SET 
  						cod_pagina_rb=%s 
						WHERE 
						cod_hipodromo=%s",
                    GetSQLValueString($id, "text"),
        ...
        for ($i = 1; $i <= $xcan; $i++) {
            ...
            if ($verif==0 && $cod!=-1) {
                $insertSQL = sprintf(
                    "/* PARSEADORES1 includes\programar_carreras_guardar_buildtvg.php - QUERY 3 */ INSERT INTO carrera 
					(cod_banca,
					cod_hipodromo, 
					nom_hipodromo, 
					nom_hipodromo_hpi, 
