...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 includes\admin_mtp_twinspires_cierre sin agregar minutos.php - QUERY 1 */ SELECT 
		hi.pre_twin,
		hi.nom_hipodromo,
		ca.cod_carrera,
		ca.num_carrera,
...
if ($totalRows_Recordset1>0) {
    ...
    do {
            ...
            foreach ($hipodomo as $hip) {
                if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && trim($hipodomo[$f])==trim($row_Recordset1['pre_twin']) && $numeroca[$f]==$row_Recordset1['num_carrera']) {
                    ...
                    if ($restante[$f]=="0") {
						...
						<?php
                        $updateSQL = sprintf(
                            "/* PARSEADORES1 includes\admin_mtp_twinspires_cierre sin agregar minutos.php - QUERY 2 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_carrera=%s, est_cierre=%s, mtp_control=%s, CERRADOX=%s  
										  WHERE cod_carrera=%s",
        ...
        if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $control==1) {
            ...
            $cie=1;
            $updateSQL = sprintf(
                "/* PARSEADORES1 includes\admin_mtp_twinspires_cierre sin agregar minutos.php - QUERY 3 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_carrera=%s, est_cierre=%s
