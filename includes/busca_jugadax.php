...
if ($tip_usuario=="A") {
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 includes\busca_jugadax.php - QUERY 1 */ SELECT *
		FROM 
			venta,
			carrera,
			usuario,
...
if ($tip_usuario=="U") {
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 includes\busca_jugadax.php - QUERY 2 */ SELECT venta.est_ticket, venta.ser_venta, venta.fec_venta, venta.hor_venta,
...
if ($tip_usuario=="G") {
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 includes\busca_jugadax.php - QUERY 3 */ SELECT 
			ve.est_ticket, 
			ve.ser_venta, 
			ve.fec_venta, 
			ve.hor_venta,
...
if ($tip_usuario=="D") {
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 includes\busca_jugadax.php - QUERY 4 */ SELECT 
			ve.est_ticket, 
			ve.ser_venta, 
			ve.fec_venta, 
			ve.hor_venta,
...
if ($ver==1) {
    if ($row_Recordset1['est_confirmacion']==0 && $row_Recordset1['est_carrera']==0 && $row_Recordset1['est_ticket']!=0) {
        $query_Recordset2 = sprintf(
            "/* PARSEADORES1 includes\busca_jugadax.php - QUERY 5 */ SELECT
...
$hora1=$row_Recordset1['hor_venta'];
    ...
    do { ?>
          ...
          <tr>
            ...
            <a href='buscar_ticketx.php?numeroticketreset=<?php echo $numerodelticket; ?>&tip_usuario=<?php echo $_SESSION['MM_UserGroup']; ?>&id_usuario=<?php echo $_SESSION['MM_id_usuario']; ?>'class="btn btn-info"> RESETEAR ESTE TICKET </a>
            </td>    
          <tr>
