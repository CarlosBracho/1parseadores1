...
$query_Recordset1_alertas = sprintf(
        "/* PARSEADORES1 includes\offtrack_apertura.php - QUERY 1 */ SELECT * FROM 
alertas
WHERE  
idalertas=18",
...
//inicio codigo buscar en la base de datos las carreras proximas a aperturar
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 includes\offtrack_apertura.php - QUERY 2 */ SELECT *
...
//fin codigo buscar en la base de datos las carreras proximas a aperturar
...
if($totalRows_Recordset1>0){
    ...
    if (strlen($str_datos) < 20) {
        $updateSQL = sprintf(
            "/* PARSEADORES1 includes\offtrack_apertura.php - QUERY 3 */ UPDATE alertas SET contadorfallos=1+contadorfallos
                              WHERE idalertas=%s",
        ...
        $updateSQL = sprintf(
            "/* PARSEADORES1 includes\offtrack_apertura.php - QUERY 4 */ UPDATE alertas SET contadorfallos=%s
                              WHERE idalertas=%s",
        ...
        $updateSQL = sprintf(
            "/* PARSEADORES1 includes\offtrack_apertura.php - QUERY 5 */ UPDATE alertas SET contadorfallos=0, ultima_bien=%s
                              WHERE idalertas=%s",
...
//aqui inicia el codigo donde las carreras pentientes por abrir se comparan con las abiertas en la pagina en estos momentos
if($totalRows_Recordset1>0){
do {
    $siestadisponibleparabarir=0;
    foreach ($carreras_abiertas_array as $carreras_abiertas_array2) {
if($carreras_abiertas_array2["hipodromo"]==$row_Recordset1['offtrack_cod'] && $carreras_abiertas_array2["numero_carrera"]==$row_Recordset1['num_carrera'] && $carreras_abiertas_array2["tiempo_para_inicio_min"]<46){
    $siestadisponibleparabarir=1;
}
if($siestadisponibleparabarir==1){
...
echo $carreras_abiertas_array2["numero_carrera"].'<br>';
  ...
  if ($row_Recordset1['delayapertura']<=1) {


    $updateSQL = sprintf(
        "/* PARSEADORES1 includes\offtrack_apertura.php - QUERY 6 */ UPDATE carrera SET delayapertura=delayapertura+1
                              WHERE cod_carrera=%s",
...
if ($row_Recordset1['delayapertura']>=2) {
    $updateSQL = sprintf(
        "/* PARSEADORES1 includes\offtrack_apertura.php - QUERY 7 */ UPDATE carrera SET 
        hor_carrera=%s, 
        hor_mtp=%s, 
        est_carrera=%s, 
        est_cierre=%s, 
    ...
    $insertSQL = sprintf(
        "/* PARSEADORES1 includes\offtrack_apertura.php - QUERY 8 */ INSERT INTO quiencierrayabre 
        (codcarrera, 
        fechaquien, 
        que) 
        VALUES (%s, %s, %s)",
...
//fin del codigo donde las carreras pentientes por abrir se comparan con las abiertas en la pagina en estos momentos
}}
