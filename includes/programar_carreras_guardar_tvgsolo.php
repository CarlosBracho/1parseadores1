...
if (isset($_GET["hip"]) && isset($_GET["can"])) {
    ...
    $query_Recordset1 =  sprintf(
        "/* PARSEADORES1 includes\programar_carreras_guardar_tvgsolo.php - QUERY 1 */ SELECT bus_auto
		FROM  
		hipodromo
		WHERE 
		cod_hipodromo = %s",
    ...
    if ($xhip!="" && $xcan>0) {
        ...
        $updateSQL = sprintf(
            "/* PARSEADORES1 includes\programar_carreras_guardar_tvgsolo.php - QUERY 2 */ UPDATE hipodromo SET 
        ...
        for ($i = 1; $i <= $xcan; $i++) {
            ...
            if ($verif==0) {
                $insertSQL = sprintf(
                    "/* PARSEADORES1 includes\programar_carreras_guardar_tvgsolo.php - QUERY 3 */ INSERT INTO carrera 
					(cod_banca,
					cod_hipodromo, 
					nom_hipodromo, 
					nom_hipodromo_hpi, 
