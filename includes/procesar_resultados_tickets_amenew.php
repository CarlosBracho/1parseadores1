...
if (isset($_GET["recordID"])) {
    ...
    function jpick($nCab, $pickresultados, $cVen, $mVen, $tope, $mas, $base)
    {
 ...
 $montoapagar[0]=0;
    ...
    // 6/2
    // ,,--dd--2/1--4.30--2.00,,
    // ,,[0]--dd[1]--2/1[2]--4.30[3]--2.00[4],,
    if(strpos($pickresultados, 'dd')) {
        ...
        if ((int)$cVen==22) {// DD
            ...
            $pickresultados=explode(',,', $pickresultados);
    // --dd--2/1--4.30--2.00
            //dd[1]
           // 2/1[2]
           // 4.30[3]
           // 2.00[4]
       foreach ($pickresultados as $pickresultadosx) {
        if(strpos($pickresultadosx, '-dd--')) {
            $pickresultadosx=explode('--', $pickresultadosx);
            $pickresultadosx[2]=str_replace('/', '-', $pickresultadosx[2]);
            $sihaydd=1;
            //$montoapagar[0]=$pickresultadosx[0];
            $ordenllegada=$pickresultadosx[2];
            $dividendo=$pickresultadosx[3];
            $factor=$pickresultadosx[4];
        }   
        if(strpos($pickresultadosx, '-dd2--')) {
            $pickresultadosx=explode('--', $pickresultadosx);
            $pickresultadosx[2]=str_replace('/', '-', $pickresultadosx[2]);
            $sihaydd=1;
            //$montoapagar[0]=$pickresultadosx[0];
       ...
       // var_dump($pickresultadosx);
            ...
            if($sihaydd==1) {
                ...
                if (($nCab==$ordenllegada)) {
                    
                    if ($factor<2) {
                        $ajuste=2/$factor;
                        $dividendo=$dividendo*$ajuste;
                        $factor=2;
                    }
                    $dTot=$dividendo+$mas;
                    if ($dTot>$tope) {
                    ...
                    $montoapagar[0]=(($base/$factor)*($dTot/$divisor))*$mVen;
                    echo 'si hay igual montoapagar[0] '.$montoapagar[0].' montoapagar[1] '.$montoapagar[1].'<br>';
                }
            } //if(strpos($pickresultadosx, '-dd--')) {
            else {
                $montoapagar[1]="5";
...
//print_r($montoapagar);
    ...
    if (isset($reset)) {// reset dividendos
        if ($reset==1) {// reset dividendos todos los ticket
            $query_Recordset12 = sprintf(
                "/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 1 */ SELECT num_ticket 
            ...
            if ($totalRows_Recordset12>0) {
                do {
                    ...
                    $insertSQL12 = sprintf(
                        "/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 2 */ UPDATE venta 
        ...
        if ($reset==2) {// reset dividendos todos los ticket
            $query_Recordset12 = sprintf(
                "/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 3 */ SELECT num_ticket 
            ...
            if ($totalRows_Recordset12>0) {
                do {
                    $num_ticket=$row_Recordset12['num_ticket'];
                    $insertSQL12 = sprintf(
                        "/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 4 */ UPDATE venta 
					SET pag_premio=%s, est_calculo=%s
						WHERE num_ticket=%s",
                        GetSQLValueString(0, "double"),
                        GetSQLValueString(0, "int"),
    ...
    if ($tipoProceso==2) { //proceso de premiacion
        $query_Recordset10 = sprintf("/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 5 */ SELECT cod_taquilla FROM taquilla");
        ...
        do {
            ...
            $query_Recordset11 = sprintf(
                "/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 6 */ SELECT 
				...
				tp.max_aganar_tri, tp.reg_tri, tp.max_aganar_sup, tp.reg_sup, tp.min_ejecarrera, tp.def_regdiv1, tp.div_pdes1, tp.div_phas1, tp.opc_ame, tp.pag_pdiv1, tp.div_pdes2, tp.div_phas2, tp.opc_ame2, tp.pag_pdiv2,
                tp.redondeo_ame, ca.pickresultados
				FROM 
					venta ve,
					carrera ca, 
					taquilla_opc_ame tp 
            ...
            if ($totalRows_Recordset11>0) {
                $redondeo_ame=$row_Recordset11['redondeo_ame'];
                ...
                do {
                    ...
                    if ($van>=$ejeMin) {
                            ...
                            if ($row_Recordset11['est_confirmacion']==0) {
                                ...
                                if ($row_Recordset11['cod_tventa']>=1 && $row_Recordset11['cod_tventa']<=3) {
                                    ...
                                    $pago=jNormal(
                                        ...
                                        $regalo,
                                        $anuReg,
                                        $row_Recordset11['def_regdiv1'],
                                        $row_Recordset11['div_pdes1'],
                                        $row_Recordset11['div_phas1'],
                                        $row_Recordset11['pag_pdiv1'],
                                        $row_Recordset11['opc_ame'],
                                        $row_Recordset11['div_pdes2'],
                                        $row_Recordset11['div_phas2'],
                                        $row_Recordset11['pag_pdiv2'],
                                        $row_Recordset11['opc_ame2'],
                                        $redondeo_ame
                                    );
                                    if ($pago[0]>0) {
                                        $_xPremio=$pago[0];
                                ...
                                if ($row_Recordset11['cod_tventa']>=22 && $row_Recordset11['cod_tventa']<=26) {
                                    if ($row_Recordset11['cod_tventa']==22) {
                                        $fact=$row_Recordset11['fac_exacta'];
                                        $topJugada=$row_Recordset11['max_aganar_exa'];
                                        $regalo=$row_Recordset11['reg_exa'];
                                    }
                                    if ($row_Recordset11['cod_tventa']==23) {
                                        $fact=$row_Recordset11['fac_trifecta'];
                                        $topJugada=$row_Recordset11['max_aganar_tri'];
                                        $regalo=$row_Recordset11['reg_tri'];
                                    }
                                    if ($row_Recordset11['cod_tventa']==24) {
                                        $fact=$row_Recordset11['fac_superfecta'];
                                        $topJugada=$row_Recordset11['max_aganar_sup'];
                                        $regalo=$row_Recordset11['reg_sup'];
                                    }
                                    if ($row_Recordset11['cod_tventa']==25) {
                                        $fact=$row_Recordset11['fac_superfecta'];
                                        $topJugada=$row_Recordset11['max_aganar_sup'];
                                        $regalo=$row_Recordset11['reg_sup'];
                                    }
                                    if ($row_Recordset11['cod_tventa']==26) {
                                        $fact=$row_Recordset11['fac_superfecta'];
                                        $topJugada=$row_Recordset11['max_aganar_sup'];
                                        $regalo=$row_Recordset11['reg_sup'];
                                    }
                                    $base=2;
                                    $pago=jpick(
                                        $row_Recordset11['num_caballo'],//listo
                                        $row_Recordset11['pickresultados'],//listo
                                        $row_Recordset11['cod_tventa'],//listo
                                        $row_Recordset11['mon_venta'],//listo
                                        $topJugada,//listo
                                        $regalo,//listo
                                        $base//listo
                                    );
                        ...
                        echo 'devolver cuando haya menos ejemplares<br>';
                        $_xPremio=$row_Recordset11['mon_venta'];
                        $_xEstado=5;
                    }
                    ///*
                    echo 'van '.$van.' ejeMin '.$ejeMin.' _xEstado '.$pago[1].' _xPremio '.$_xPremio.' _nTicket '.$_nTicket.'<br>';
                    $updateSQL = sprintf(
                        "/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 7 */ UPDATE venta 
        ...
        if ($tipoProceso==5) {// devolucion
            $query_Recordset12 = sprintf(
                "/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 8 */ SELECT num_ticket, mon_venta 
		  FROM venta WHERE cod_carrera = %s AND est_ticket=1 AND est_calculo=0",
            ...
            if ($totalRows_Recordset12 >0) {
                do {
                    ...
                    $insertSQL12 = sprintf(
                        "/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 9 */ UPDATE venta 
					SET pag_premio=%s, est_calculo=%s
					WHERE num_ticket=%s",
            ...
            if ($tipoProceso==4) {// retiro de caballo
                $query_Recordset12 = sprintf(
                    "/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 10 */ SELECT num_ticket, mon_venta, cod_tventa, num_caballo 
				  FROM venta 
                ...
                if ($totalRows_Recordset12>0) {
                    ...
                    do {
                        ...
                        if ($retiro==1) {
                            ...
                            $mon_venta=$row_Recordset12['mon_venta'];
                            $insertSQL12 = sprintf(
                                "/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 11 */ UPDATE venta 
								SET pag_premio=%s, est_calculo=%s
								WHERE num_ticket=%s",
                                GetSQLValueString($mon_venta, "double"),
                ...
                mysqli_free_result($Recordset12);
                $query_Recordset13 = sprintf("/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 12 */ SELECT cod_taquilla FROM taquilla");
                ...
                do {
                    ...
                    $query_Recordset14 = sprintf(
                        "/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 13 */ SELECT 
                    ...
                    do {
                        ...
                        if ($row_Recordset14['min_ejecarrera']>$van) {
                            ...
                            $insertSQL15 = sprintf(
                                "/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 14 */ UPDATE venta 
								SET pag_premio=%s, est_calculo=%s
								WHERE num_ticket=%s",
                                GetSQLValueString($mon_venta, "double"),
                                GetSQLValueString(5, "int"),
                ...
                if ($tipoProceso==1) {// reset dividendos
                    $query_Recordset12 = sprintf(
                        "/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 15 */ SELECT num_ticket 
				  FROM venta WHERE cod_carrera = %s AND est_ticket=1 AND est_calculo!=4",
                        GetSQLValueString($cod_carrera, "int")
                    );
                    $Recordset12 = mysqli_query($conexionbanca, $query_Recordset12) or die(mysqli_error($conexionbanca));
                    ...
                    if ($totalRows_Recordset12>0) {
                        do {
                            $num_ticket=$row_Recordset12['num_ticket'];
                            $insertSQL12 = sprintf(
                                "/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 16 */ UPDATE venta 
							SET pag_premio=%s, est_calculo=%s
							WHERE num_ticket=%s",
                                GetSQLValueString(0, "double"),
                                GetSQLValueString(0, "int"),
                    ...
                    if ($tipoProceso==3) {// reintegro de caballo
                        $query_Recordset12 = sprintf(
                            "/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 17 */ SELECT num_ticket, cod_tventa, num_caballo 
							  FROM venta 
                            ...
                            do {
                                ...
                                if ($acceso==1) {
                                    ...
                                    if ($acceso2==1) {
                                        $num_ticket=$row_Recordset12['num_ticket'];
                                        $insertSQL12 = sprintf(
                                            "/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 18 */ UPDATE venta 
										SET pag_premio=%s, est_calculo=%s
										WHERE num_ticket=%s",
                                            GetSQLValueString(0, "double"),
                                            GetSQLValueString(0, "int"),
                        ...
                        if ($tipoProceso==6) {// reintegro de TODOS caballo
                            $query_Recordset12 = sprintf(
                                "/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 19 */ SELECT num_ticket 
								  FROM venta 
                            ...
                            if ($totalRows_Recordset12>0) {
                                do {
                                    $num_ticket=$row_Recordset12['num_ticket'];
                                    $insertSQL12 = sprintf(
                                        "/* PARSEADORES1 includes\procesar_resultados_tickets_amenew.php - QUERY 20 */ UPDATE venta 
										SET pag_premio=%s, est_calculo=%s
										WHERE num_ticket=%s",
                                        GetSQLValueString(0, "double"),
                                        GetSQLValueString(0, "int"),
