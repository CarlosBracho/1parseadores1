...
function get_week_end_hnac($fecha)
{
    ...
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 includes\auto_resultado_buscar_lot   revisar.php - QUERY 1 */ SELECT 
        ...
        if (isset($rSorteo) or isset($rSorteoAn)) {
                        ...
                        for ($i = 0; $i < count($pTriple); ++$i) {
                            ...
                            if (strlen(trim($x_ganador[$i]))==3) {
                                $query_Recordset13 =  sprintf(
                                    "/* PARSEADORES1 includes\auto_resultado_buscar_lot   revisar.php - QUERY 2 */ SELECT re.num_resultado, re.sig_resultado, re.id_resultado
                                ...
                                if ($totalRows_Recordset13==0) {
                                    $insertSQL2 = sprintf(
                                        "/* PARSEADORES1 includes\auto_resultado_buscar_lot   revisar.php - QUERY 3 */ INSERT 
											INTO resultados_lot 
                                    ...
                                    $insertSQL1 = sprintf(
                                        "/* PARSEADORES1 includes\auto_resultado_buscar_lot   revisar.php - QUERY 4 */ UPDATE resultados_lot SET num_resultado=%s, sig_resultado=%s
            ...
            if (isset($rSorteoAn)) {
                ...
                foreach ($rSorteoAn as $xres2) {
                    ...
                    if ($cod_animal>0) {
                        $query_Recordset13 =  sprintf(
                            "/* PARSEADORES1 includes\auto_resultado_buscar_lot   revisar.php - QUERY 5 */ SELECT re.num_resultado, re.sig_resultado, re.id_resultado
								FROM resultados_lot re WHERE id_loteria=%s AND fec_resultado=%s AND id_banca=%s LIMIT 1",
                        ...
                        if ($totalRows_Recordset13==0) {
                            $insertSQL2 = sprintf(
                                "/* PARSEADORES1 includes\auto_resultado_buscar_lot   revisar.php - QUERY 6 */ INSERT 
									INTO resultados_lot 
                            ...
                            $insertSQL1 = sprintf(
                                "/* PARSEADORES1 includes\auto_resultado_buscar_lot   revisar.php - QUERY 7 */ UPDATE resultados_lot SET num_resultado=%s, sig_resultado=%s  
									WHERE id_resultado=%s",
                                GetSQLValueString(trim($termin), "text"),
                                GetSQLValueString($signo, "int"),
                                GetSQLValueString($row_Recordset13['id_resultado'], "int")
