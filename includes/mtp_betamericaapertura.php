...
$horasistema=horaactual();
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 includes\mtp_betamericaapertura.php - QUERY 1 */ SELECT * FROM carrera ca, hipodromo hi WHERE 
            ...
            foreach ($hipodomo as $hip) {
                if (trim($hipodomo[$f])==trim($row_Recordset1['mtp_betamericanom']) && $numeroca[$f]==$row_Recordset1['num_carrera'] && $horagmt[$f] <= $cuarentaminmas && $cerradoono[$f]==0) {
                    ...
                    if ($row_Recordset1['delayapertura']<=5) {
                        ...
                        $updateSQL = sprintf(
                            "/* PARSEADORES1 includes\mtp_betamericaapertura.php - QUERY 2 */ UPDATE carrera SET delayapertura=%s 
											  WHERE cod_carrera=%s",
                        ...
                        $updateSQL = sprintf(
                            "/* PARSEADORES1 includes\mtp_betamericaapertura.php - QUERY 3 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_cierre=%s, est_carrera=%s, supercontrol=%s, mtp_control=%s 
											  WHERE cod_carrera=%s",
