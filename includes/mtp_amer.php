...
$horasistema=horaactual();
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 includes\mtp_amer.php - QUERY 1 */ SELECT * FROM carrera ca, hipodromo hi WHERE hi.mtp_ameridatos=1 AND ca.eje_primero=0 AND ca.fec_carrera=%s AND 
    ...
    do {
        ...
        if ($hipodomo[0]!="") {
            foreach ($hipodomo as $hip) {
                if (trim($hipodomo[$f])==trim($row_Recordset1['nom_hipodromo_sup']) && $numeroca[$f]==$row_Recordset1['num_carrera'] &&  $restante[$f]<=50) {
					...
					<?php
                                         $updateSQL = sprintf(
                        "/* PARSEADORES1 includes\mtp_amer.php - QUERY 2 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_cierre=%s, est_carrera=%s, supercontrol=%s, CERRADOX=%s 
											  WHERE cod_carrera=%s",
        ...
        if ($cont==1) {
            if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $row_Recordset1['supercontrol']<=2) {
                ...
                $updateSQL = sprintf(
                    "/* PARSEADORES1 includes\mtp_amer.php - QUERY 3 */ UPDATE carrera SET supercontrol=%s
                                                          WHERE cod_carrera=%s",
        ...
        if ($cont==1) {
            if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $row_Recordset1['supercontrol']>=3) {
                ...
                $cie=1;
                $updateSQL = sprintf(
                    "/* PARSEADORES1 includes\mtp_amer.php - QUERY 4 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_carrera=%s, est_cierre=%s, CERRADOX=%s, contador_cierres=%s
							  WHERE cod_carrera=%s",
