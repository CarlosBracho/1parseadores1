...
function parseo_tvg()
{
    ...
    $hoy=fechaactualbd();
    $url = 'https://www.tvg.com/ajax/upcoming-races/id/upcomingList/date/2016-07-25/wp/AllTracks';
    ...
    if (is_array($datos)) {
        for ($i = 0; $i <= 400; $i++) {
            ...
            if ($actual["TrackName"]!="") {
                ...
                $hi=busHipodromo($n);
                if ($hi[0]==1) {
                    $hipodomo[$y]=strtoupper($actual["TrackName"]);
                    $numeroca[$y]=$actual["RaceNumber"];
                }
            } else {
                break;
            }
...
function paribet()
{
    ...
    if (!empty($matches1[3])) {
        ...
        foreach ($matches1[3] as $datos) {
            if ($matches1[5][$y]==trim("RED") or $matches1[5][$y]==trim("RED")) {
                $carrer[$x]=$matches1[6][$y];
                $estado[$x]=$matches1[5][$y];
    ...
    return array($hipodr, $carrer, $estado);
}
function consultabetbird()
{
    set_time_limit(0);
    date_default_timezone_set("Pacific/Honolulu");
    $url='https://www.betbird.com/next_events.php';
    $ch = curl_init();
    ...
    $result = curl_exec($ch);
    curl_close($ch);
    preg_match_all("(alt=\"\" style=\"margin-right:4px;position:relative; top:1px;\">(.*)</td>
		<td nowrap style=\"padding-left: 4px;\">(.*).&nbsp;Race</td>)siU", $result, $matches1);
    $hipodr[0]="";
    $carrer[0]="";
    if (!empty($matches1[1])) {
        $carrer=$matches1[2];
...
function consultaPacificnaRetirados()
{
    set_time_limit(0);
    date_default_timezone_set("Pacific/Honolulu") ;
    $url="http://web1.ameridatos.com:6850/retirados.asp";
    $existe=1;
    $hipoCar[0]=0;
    $caballo[0]=0;
    if ($existe==1) {
