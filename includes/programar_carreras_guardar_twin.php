...
if (isset($_GET["cHip"]) && isset($_GET["can"])) {
    ...
    $query_Recordset1 =  sprintf(
        "/* PARSEADORES1 includes\programar_carreras_guardar_twin.php - QUERY 1 */ SELECT bus_auto, nom_hipodromo
		FROM  
		hipodromo
		WHERE 
		cod_hipodromo = %s",
    ...
    if ($xhip!="" && $xcan>0) {
        ...
        for ($i = 1; $i <= $xcan; $i++) {
            ...
            if ($verif==0) {
                $insertSQL = sprintf(
                    "/* PARSEADORES1 includes\programar_carreras_guardar_twin.php - QUERY 2 */ INSERT INTO carrera 
					(cod_banca,
					cod_hipodromo, 
					nom_hipodromo, 
					nom_hipodromo_hpi, 
