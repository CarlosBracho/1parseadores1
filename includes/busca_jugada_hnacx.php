...
if ($tip_usuario=="A") {
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 includes\busca_jugada_hnacx.php - QUERY 1 */ SELECT *
		FROM 
			venta_hnac,
			carrera_hnac,
			usuario,
...
if ($tip_usuario=="U") {
    $query_Recordset2 = sprintf(
        "/* PARSEADORES1 includes\busca_jugada_hnacx.php - QUERY 2 */ SELECT venta.est_ticket, venta.ser_venta, carrera.est_carrera, carrera.est_confirmacion 
			 FROM venta, carrera 
...
if ($tip_usuario=="G") {
    $query_Recordset2 = sprintf(
        "/* PARSEADORES1 includes\busca_jugada_hnacx.php - QUERY 3 */ SELECT venta.est_ticket, venta.ser_venta, carrera.est_carrera, carrera.est_confirmacion 
...
if ($totalRows_Recordset1>0 && $controlSer==1 && $row_Recordset1['est_ticket_hnac']!=0
    ...
    $query_Recordset3 = sprintf(
        "/* PARSEADORES1 includes\busca_jugada_hnacx.php - QUERY 4 */ SELECT *
		FROM 
			resultados_hnac
		WHERE 
...
if ($ver==1) {
    if ($row_Recordset1['est_carrera_hnac']==0 && $row_Recordset1['est_cierre_hnac']==1
        ...
        $query_Recordset2 = sprintf(
            "/* PARSEADORES1 includes\busca_jugada_hnacx.php - QUERY 5 */ SELECT *
		FROM 
			venta_hnac,
			carrera_hnac,
			usuario,
            ...
            if ($retiro==0) {
                if ($row_Recordset2['est_ticket_hnac']==1 && $row_Recordset2['est_carrera_hnac']==0
                    ...
                    $query_Recordset21 = sprintf(
                        "/* PARSEADORES1 includes\busca_jugada_hnacx.php - QUERY 6 */ SELECT resultados_hnac.div_pago_hnac, inscritos.est_favorito_hnac,
...
<a href='buscar_ticket_hnacx.php?numeroticketreset=<?php echo $numerodelticket; ?>&tip_usuario=<?php echo $_SESSION['MM_UserGroup']; ?>&id_usuario=<?php echo $_SESSION['MM_id_usuario']; ?>'class="btn btn-info"> RESETEAR ESTE TICKET </a>
</td>
