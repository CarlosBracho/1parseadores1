    ...
    do {
        ...
        if ($cont==1) {
            if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $row_Recordset1['supercontrol']==0) {
                $cod_carrera=$row_Recordset1['cod_carrera'];
                $updateSQL = sprintf(
                    "/* PARSEADORES1 includes\mtp_tvgsin agrega tiempo.php - QUERY 2 */ UPDATE carrera SET supercontrol=%s
                                                          WHERE cod_carrera=%s",
        ...
        if ($cont==1) {
            if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $row_Recordset1['supercontrol']==1) {
                $cod_carrera=$row_Recordset1['cod_carrera'];
                $updateSQL = sprintf(
                    "/* PARSEADORES1 includes\mtp_tvgsin agrega tiempo.php - QUERY 3 */ UPDATE carrera SET supercontrol=%s
                                                          WHERE cod_carrera=%s",
        ...
        if ($cont==1) {
            if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $row_Recordset1['supercontrol']>=2) {
                ...
                $mtp=0;
                $updateSQL = sprintf(
                    "/* PARSEADORES1 includes\mtp_tvgsin agrega tiempo.php - QUERY 4 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_carrera=%s, est_cierre=%s, mtp_control=%s, CERRADOX=%s
							  WHERE cod_carrera=%s",
