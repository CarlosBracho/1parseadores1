...
if (isset($_GET["hip"]) && isset($_GET["can"])) {
    ...
    if ($xhip!="" && $xcan>0) {
        ...
        if ($verif==0 && $mtp_control!=-1) {
            ...
            $updateSQL = sprintf(
                "/* PARSEADORES1 includes\programar_carreras_guardar_buildabet2.php - QUERY 1 */ UPDATE hipodromo SET 
  					cod_pagina_rb=%s 
					WHERE 
					cod_hipodromo=%s",
        ...
        for ($i = 1; $i <= $xcan; $i++) {
            ...
            if ($verif==0 && $hipodomo!="") {
                $insertSQL = sprintf(
                    "/* PARSEADORES1 includes\programar_carreras_guardar_buildabet2.php - QUERY 2 */ INSERT INTO carrera 
					(cod_banca,
					cod_hipodromo, 
					nom_hipodromo, 
					nom_hipodromo_hpi, 
