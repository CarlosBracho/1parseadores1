...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 includes\mtp_paribetx.php - QUERY 1 */ SELECT carrera.cod_carrera,carrera.nom_hipodromo,hipodromo.mtp_paribetnom,carrera.num_carrera,carrera.hor_carrera,
carrera.hor_mtp,
carrera.est_carrera,
carrera.est_cierre,
    ...
    do {
        ...
        if ($hipodr[0]!="") {
            foreach ($hipodr as $hip) {
                ...
                if (($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $faltan2<="00:00:30" && $last!=0) or ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $row_Recordset1['hor_carrera']<=$horasistema && $last!=0)) {
                    ...
                    $updateSQL = sprintf(
                        "/* PARSEADORES1 includes\mtp_paribetx.php - QUERY 2 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s 
											  WHERE cod_carrera=%s",
        ...
        if ($control==1) {
            if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $last!=0) {
                ...
                $cie=1;
                $updateSQL = sprintf(
                    "/* PARSEADORES1 includes\mtp_paribetx.php - QUERY 3 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_carrera=%s, est_cierre=%s, CERRADOX=%s, contador_cierres=%s
							  WHERE cod_carrera=%s",
