...
$mensaje2="";
$t=0;
function buscaHipBuild5solo($identificador) { 
	$tipo=-1;
	$nomb2="";
	global $conexionbanca; 
	$query_ConsultaFuncion = sprintf("/* PARSEADORES1 includes\programar_carreras_guardar_buildabet2solo.php - QUERY 1 */ SELECT nom_hipodromo, bus_auto 
		FROM hipodromo 
...
function verificarCarrera2solo($cod, $num, $fec) {  
	global $conexionbanca;  
	$query_ConsultaFuncion = sprintf("/* PARSEADORES1 includes\programar_carreras_guardar_buildabet2solo.php - QUERY 2 */ SELECT est_carrera 
	FROM carrera  
	WHERE cod_hipodromo = %s 
	AND num_carrera = %s 
	AND  fec_carrera = %s 
...
if (isset($_GET["hip"]) && isset($_GET["can"])) {
	...
	if ($xhip!="" && $xcan>0) {
		list($mtp_control, $hipodomo)=buscaHipBuild5solo($xhip);
		$verif=verificarCarrera2solo($xhip,1,$hoy);
		if ($verif==0 && $mtp_control!=-1) {
			//cod_pagina_rb ahora es para buildabet2
			$updateSQL = sprintf("/* PARSEADORES1 includes\programar_carreras_guardar_buildabet2solo.php - QUERY 3 */ UPDATE hipodromo SET 
  					cod_pagina_rb=%s 
					WHERE 
					cod_hipodromo=%s",
		...
		for ($i = 1; $i <= $xcan; $i++) {
			...
			if ($verif==0 && $hipodomo!="") {
				$insertSQL = sprintf("/* PARSEADORES1 includes\programar_carreras_guardar_buildabet2solo.php - QUERY 4 */ INSERT INTO carrera 
					(cod_banca,
					cod_hipodromo, 
					nom_hipodromo, 
					nom_hipodromo_hpi, 
					...
					VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
					   ...
					   GetSQLValueString($Runner[$i-1], "int"));
				$Result1 = mysqli_query($conexionbanca, $insertSQL) or die(mysqli_error($conexionbanca));
				$mensaje1="Carrera(s) creada(s)";
			} else {
				$t++;
				if ($hipodomo!=1) $mensaje2="hipódromo no existe"; else {
