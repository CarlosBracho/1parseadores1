...
<?php
                $horaactualcarrera2=horaactual(); $faltan2=restahoras($horaactualcarrera2, $row_Recordset1['hor_carrera']);
                        ...
                        if (($faltan2<="00:01:30") or ($row_Recordset1['hor_carrera']<=$horasistema)) {
                            ...
                            $updateSQL = sprintf(
                                "/* PARSEADORES1 includes\mtp_tvgx.php - QUERY 2 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s 
											  WHERE cod_carrera=%s",
...
?>
        ...
        if ($cont==1) {
            if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $last!=0) {
                ...
                $mtp=3;
                $updateSQL = sprintf(
                    "/* PARSEADORES1 includes\mtp_tvgx.php - QUERY 3 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_carrera=%s, est_cierre=%s, CERRADOX=%s, contador_cierres=%s  
							  WHERE cod_carrera=%s",
