...
if(isset($_POST['ticket'])){
    $insertSQL1 = sprintf(
      "/* PARSEADORES1 includes\mensajes_chat.php - QUERY 1 */ UPDATE chat2
...
/* PARSEADORES1 includes\mensajes_chat.php - QUERY 2 */ SELECT * 
FROM 
chat2
WHERE 
    ...
    if ($totalRows_Recordset2>0) {
        if (trim($row_Recordset2['from1'])!="Soporte") {
            ...
            <form method="POST" name="Finalizar" action="<?php echo $editFormAction; ?>" onsubmit="return chequearEnvio();">
                <input type="hidden" name="ticket" value="<?php echo $row_Recordset2['id_taquilla']; ?>">    
                <input type="hidden" name="recd" value="0">
                <input type="hidden" name="FinalizarReabrir" value="0">
                <div class="d-grid gap-2">
                    <button style="float:right; font-size:46px; margin:15px 5px 0px 0px; color:#C00; border:antiquewhite" class="noLeido">*</button>
                </div>
            </form>
