...
$cod_carrera=$_GET['carrera'];
    ...
    if (isset($reset)) {// reset dividendos
        if ($reset==1) {// reset dividendos todos los ticket
            $query_Recordset12 = sprintf(
                "/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 1 */ SELECT num_ticket 
            ...
            if ($totalRows_Recordset12>0) {
                do {
                    ...
                    $insertSQL12 = sprintf(
                        "/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 2 */ UPDATE venta 
        ...
        if ($reset==2) {// reset dividendos todos los ticket
            $query_Recordset12 = sprintf(
                "/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 3 */ SELECT num_ticket 
            ...
            if ($totalRows_Recordset12>0) {
                do {
                    $num_ticket=$row_Recordset12['num_ticket'];
                    $insertSQL12 = sprintf(
                        "/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 4 */ UPDATE venta 
					SET pag_premio=%s, est_calculo=%s
						WHERE num_ticket=%s",
                        GetSQLValueString(0, "double"),
                        GetSQLValueString(0, "int"),
    ...
    if ($tipoProceso==2) { //proceso de premiacion
        $query_Recordset10 = sprintf("/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 5 */ SELECT cod_taquilla FROM taquilla");
        ...
        do {
            ...
            $query_Recordset11 = sprintf(
                "/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 6 */ SELECT 
            ...
            if ($totalRows_Recordset11>0) {
                ...
                do {
                    ...
                    $updateSQL = sprintf(
                        "/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 7 */ UPDATE venta 
        ...
        if ($tipoProceso==5) {// devolucion
            $query_Recordset12 = sprintf(
                "/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 8 */ SELECT num_ticket, mon_venta 
		  FROM venta WHERE cod_carrera = %s AND est_ticket=1 AND est_calculo=0",
            ...
            if ($totalRows_Recordset12 >0) {
                do {
                    ...
                    $insertSQL12 = sprintf(
                        "/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 9 */ UPDATE venta 
					SET pag_premio=%s, est_calculo=%s
					WHERE num_ticket=%s",
            ...
            if ($tipoProceso==4) {// retiro de caballo
                $query_Recordset12 = sprintf(
                    "/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 10 */ SELECT num_ticket, mon_venta, cod_tventa, num_caballo 
				  FROM venta 
                ...
                if ($totalRows_Recordset12>0) {
                    ...
                    do {
                        ...
                        if ($retiro==1) {
                            ...
                            $mon_venta=$row_Recordset12['mon_venta'];
                            $insertSQL12 = sprintf(
                                "/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 11 */ UPDATE venta 
								SET pag_premio=%s, est_calculo=%s
								WHERE num_ticket=%s",
                                GetSQLValueString($mon_venta, "double"),
                ...
                mysqli_free_result($Recordset12);
                $query_Recordset13 = sprintf("/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 12 */ SELECT cod_taquilla FROM taquilla");
                ...
                do {
                    ...
                    $query_Recordset14 = sprintf(
                        "/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 13 */ SELECT 
                    ...
                    do {
                        ...
                        if ($row_Recordset14['min_ejecarrera']>$van) {
                            ...
                            $insertSQL15 = sprintf(
                                "/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 14 */ UPDATE venta 
								SET pag_premio=%s, est_calculo=%s
								WHERE num_ticket=%s",
                                GetSQLValueString($mon_venta, "double"),
                                GetSQLValueString(5, "int"),
                ...
                if ($tipoProceso==1) {// reset dividendos
                    $query_Recordset12 = sprintf(
                        "/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 15 */ SELECT num_ticket 
				  FROM venta WHERE cod_carrera = %s AND est_ticket=1 AND est_calculo!=4",
                        GetSQLValueString($cod_carrera, "int")
                    );
                    $Recordset12 = mysqli_query($conexionbanca, $query_Recordset12) or die(mysqli_error($conexionbanca));
                    ...
                    if ($totalRows_Recordset12>0) {
                        do {
                            $num_ticket=$row_Recordset12['num_ticket'];
                            $insertSQL12 = sprintf(
                                "/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 16 */ UPDATE venta 
							SET pag_premio=%s, est_calculo=%s
							WHERE num_ticket=%s",
                                GetSQLValueString(0, "double"),
                                GetSQLValueString(0, "int"),
                    ...
                    if ($tipoProceso==3) {// reintegro de caballo
                        $query_Recordset12 = sprintf(
                            "/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 17 */ SELECT num_ticket, cod_tventa, num_caballo 
							  FROM venta 
                            ...
                            do {
                                ...
                                if ($acceso==1) {
                                    ...
                                    if ($acceso2==1) {
                                        $num_ticket=$row_Recordset12['num_ticket'];
                                        $insertSQL12 = sprintf(
                                            "/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 18 */ UPDATE venta 
										SET pag_premio=%s, est_calculo=%s
										WHERE num_ticket=%s",
                                            GetSQLValueString(0, "double"),
                                            GetSQLValueString(0, "int"),
                        ...
                        if ($tipoProceso==6) {// reintegro de TODOS caballo
                            $query_Recordset12 = sprintf(
                                "/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 19 */ SELECT num_ticket 
								  FROM venta 
                            ...
                            if ($totalRows_Recordset12>0) {
                                do {
                                    $num_ticket=$row_Recordset12['num_ticket'];
                                    $insertSQL12 = sprintf(
                                        "/* PARSEADORES1 includes\procesar_resultados_tickets_ame_lejos.php - QUERY 20 */ UPDATE venta 
										SET pag_premio=%s, est_calculo=%s
										WHERE num_ticket=%s",
                                        GetSQLValueString(0, "double"),
                                        GetSQLValueString(0, "int"),
