...
$query_Recordset1 = sprintf("/* PARSEADORES1 includes\offtrack_confirmacion_nuevo.php - QUERY 1 */ SELECT * 
	FROM 
	hipodromo,
	carrera 
	WHERE
...
(carrera.confirmandox = 0 OR carrera.confirmandox = 10) AND	
(hipodromo.bus_resultado_tip = 11 OR hipodromo.bus_resultado_tip = 18) AND
...
//$url = 'prueba.bot.nu/includes/108.json';
...
$headers = array();
$headers[] = 'Authority: us-west-2.aws.data.mongodb-api.com';
$headers[] = 'Accept-language: es-US,es-419;q=0.9,es;q=0.8,en;q=0.7';
$headers[] = 'Accept: application/json, text/javascript, */*; q=0.01';
$headers[] = 'Origin: https://www.offtrackbetting.com';
$headers[] = 'User-agent: Mozilla/5.0 (iPhone; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.3 Mobile/15E148 Safari/604.1';
$headers[] = 'Sec-Fetch-Site: cross-site';
$headers[] = 'Sec-Fetch-Mode: cors';
$headers[] = 'Sec-Fetch-Dest: empty';
$headers[] = 'Referer: https://www.offtrackbetting.com/';
...
if (isset($fulldatos)) {
  ...
  //var_dump($fulldatos["events"][$num_carrera]["postTime"]);
    echo '<br><br>';
   if (isset($fulldatos["events"][$num_carrera]["postTime"])) {
    echo $fulldatos["events"][$num_carrera]["postTime"].'<br>';
    $hor_carrera = date('Y-m-d H:i:s', $fulldatos["events"][$num_carrera]["postTime"]); //date('Y-m-d =2023-11-02 Y-m-j=2023-11-2
    $fecha_carrera = date('Y-m-d', $fulldatos["events"][$num_carrera]["postTime"]); //date('Y-m-d =2023-11-02 Y-m-j=2023-11-2
...
$eje_cuarto=$eje_cuarto; echo '$eje4LugarSimple '.$eje_cuarto.'<br>';
...
if($exoticas2["betType"]=='EX' && $exa==0){
    $div_exacta=$exoticas2["amount"];
    $fac_exacta=$exoticas2["baseAmount"]/100;
...
if($exoticas2["betType"]=='EX' && $exa==1){
    $div_exacta_doble=$exoticas2["amount"];
...
if($exoticas2["betType"]=='EX' && $exa==2){
    $div_exacta_triple=$exoticas2["amount"];
...
if($exoticas2["betType"]=='TR' && $tri==0){
    $div_trifecta=$exoticas2["amount"];
    $fac_trifecta=$exoticas2["baseAmount"]/100;
...
if($exoticas2["betType"]=='TR' && $tri==1){
    $div_trifecta_doble=$exoticas2["amount"];
...
if($exoticas2["betType"]=='TR' && $tri==2){
    $div_trifecta_triple=$exoticas2["amount"];
...
if($exoticas2["betType"]=='SU' && $sup==0){
    $div_superfecta=$exoticas2["amount"];
    $fac_superfecta=$exoticas2["baseAmount"]/100;
...
if($exoticas2["betType"]=='SU' && $sup==1){
    $div_superfecta_doble=$exoticas2["amount"];
...
if($exoticas2["betType"]=='SU' && $sup==2){
    $div_superfecta_triple=$exoticas2["amount"];
...
echo $exoticas2["baseAmount"].'<br>';
echo $exoticas2["amount"].'<br><br>';
}
...
$updateSQL = sprintf(
    "/* PARSEADORES1 includes\offtrack_confirmacion_nuevo.php - QUERY 2 */ UPDATE carrera 
SET 
confirmandox=%s,
est_confirmacion=%s,
control_dividendo=%s,
...
$insertSQL = sprintf(
"/* PARSEADORES1 includes\offtrack_confirmacion_nuevo.php - QUERY 3 */ INSERT INTO quiencierrayabre 
(codcarrera, 
fechaquien, 
que) 
VALUES (%s, %s, %s)",
...
if ($est_confirmacion==0) {
    ...
    echo "<h3><font color='#027BAD'>Proceso de cálculo culminado! ".$row_Recordset1['num_carrera']."</font></h3>";


$insertSQL = sprintf(
"/* PARSEADORES1 includes\offtrack_confirmacion_nuevo.php - QUERY 4 */ INSERT INTO quiencierrayabre 
(codcarrera, 
fechaquien, 
que) 
VALUES (%s, %s, %s)",
