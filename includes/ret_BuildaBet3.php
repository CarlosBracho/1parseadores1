...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 includes\ret_BuildaBet3.php - QUERY 1 */ SELECT * FROM carrera, hipodromo 
	WHERE
	carrera.nom_hipodromo=hipodromo.nom_hipodromo AND
	carrera.est_confirmacion=1 AND
        carrera.est_cierre!=3 AND
	(hipodromo.bus_retirado=2 OR hipodromo.bus_retirado=4 OR hipodromo.bus_retirado=5 OR hipodromo.bus_retirado=7 OR hipodromo.bus_retirado=8) AND
        carrera.mtp_tipo=1 AND
	carrera.fec_carrera=%s  ORDER BY RAND() LIMIT 0, 7",
...
if ($totalRows_Recordset1>0) {
    do {
        ...
        if ($cantProgramados!=$row_Recordset1['can_caballos'] && $cantProgramados>3) {
            ...
            $updateSQL2 = sprintf(
                "/* PARSEADORES1 includes\ret_BuildaBet3.php - QUERY 2 */ UPDATE carrera SET can_caballos=%s 
								  WHERE cod_carrera=%s",
        ...
        if (isset($retiro[0]) && $retiro[0]!="") {
            ...
            foreach ($retiro as $ejem) {
                ...
                if ($estado==0) { // retirar caballo
                    $insertSQL = sprintf(
                        "/* PARSEADORES1 includes\ret_BuildaBet3.php - QUERY 3 */ INSERT INTO retirados 
					  	(quienretiro, cod_carrera, num_rcaballo) VALUES (%s, %s, %s)",
                        GetSQLValueString('buil', "text"),
                        GetSQLValueString($cod_carrera, "int"),
                        GetSQLValueString($ejem, "int"));
                    $Result = mysqli_query($conexionbanca, $insertSQL) or die(mysqli_error($conexionbanca));
                    $insertSQL = sprintf(
                        "/* PARSEADORES1 includes\ret_BuildaBet3.php - QUERY 4 */ INSERT INTO quiencierrayabre 
                        (codcarrera, 
                        fechaquien, 
                        que) 
                        VALUES (%s, %s, %s)",
