...
if ($_GET['buscar']=="hijos") {
    ...
    $consulta = sprintf("/* PARSEADORES1 includes\combobox.php - QUERY 1 */ SELECT * FROM taquilla WHERE taquilla.cod_agencia = %s order by nom_taquilla", $_GET["idpadre"]);
...
if ($_GET['buscar']=="nietos") {
    global $conexionbanca;
    $consulta="/* PARSEADORES1 includes\combobox.php - QUERY 2 */ SELECT * FROM usuario WHERE cod_taquilla='".mysql_real_escape_string(intval($_GET['idhijo']))."' order by nom_usuario";
    mysql_select_db($database_conexionbanca);
