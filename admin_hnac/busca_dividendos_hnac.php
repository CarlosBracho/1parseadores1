...
var loadData = function() {
    $.ajax({  
        type: "GET",
        url: "../includes/dividendos_total_hnac.php",             
        dataType: "html",
		beforeSend: function() {
			clearTimeout(loadData);
			var espera1='<i class="fa fa-spinner fa-spin fa-2x"></i><br/>';
			var espera2='Iniciando busqueda de resultados<br/>Por favor espere un momento...';
...
function confirmarDiv(codCarrera, conf, rese, modi) {
	...
	$.ajax({ data:parametros1, url:'../includes/confirma_dividendos_hnac.php', type:'post',
...
function cancelaCar1(codC, e1, e2, e3, e4, diws1, dips1, diss1, dips2, diss2, diss3, exac, trif, supe, cerr, conf, rese, modi) {
	...
	$.ajax({ data:parametros3, url:'admin_apertura_cancelar_hnac.php', type:'GET',
		success:function (response) { $(conf).html(response); },
		error: function(){ 
			$(".divControl").html(mError);
			setTimeout(loadData, tiempo);
...
function ModiDiv1(codModi,tip) {
	...
	$.ajax({ data:parametros2, url:'confirma_dividendos_edit_hnac.php', type:'GET',
...
function resetDiv1(codC, e1, e2, div1, div2, div3, divss3, exac, trif, supe, botConf) {
	$(e1).html("0"); $(e2).html("0"); $(div1).html("0.0"); $(div2).html("0.0"); 
	$(div3).html("0.0"); $(exac).html(""); $(trif).html(""); $(supe).html(""); $(botConf).html("");
	var parametros3 = { "eM":Math.random(), "recordID":codC };
	$.ajax({ data:parametros3, url:'../includes/dividendos_reset_hnac.php', type:'GET',
...
<div class="container1">
    <div style="background: #75003A; height:25px; color:#FFFFFF; padding:25px 15px 10px 0px; text-align: center; font-size:34px"
    	id="datosUsuario">
        RESULTADOS/DIVIDENDOS MAQUINA AZUL 
        <p style="padding:2px; font-size:14px">HIPISMO NACIONAL</p>
	</div>
</div>
