...
foreach ($matches10[0] as $datos) {
    ...
    $query_Recordset21 = sprintf(
        "/* PARSEADORES1 new\includes\spcrearjuegos.php - QUERY 1 */ SELECT *
FROM p1equipos
WHERE  
nomequipop1 = %s",
    ...
    if ($totalRows_Recordset21==0) {
        $insertSQL = sprintf(
            "/* PARSEADORES1 new\includes\spcrearjuegos.php - QUERY 2 */ INSERT 
				INTO p1equipos
        ...
        $query_Recordset21 = sprintf(
            "/* PARSEADORES1 new\includes\spcrearjuegos.php - QUERY 3 */ SELECT *
FROM p1equipos
WHERE  
nomequipop1 = %s",
            GetSQLValueString(trim(strtoupper($matches10[15][$x])), "text")
    ...
    $query_Recordset22 = sprintf(
        "/* PARSEADORES1 new\includes\spcrearjuegos.php - QUERY 4 */ SELECT *
FROM p1equipos
WHERE  
nomequipop1 = %s",
    ...
    if ($totalRows_Recordset22==0) {
        $insertSQL = sprintf(
            "/* PARSEADORES1 new\includes\spcrearjuegos.php - QUERY 5 */ INSERT 
				INTO p1equipos
				(nomequipop1, nomdimp1, deportep1, ligap1, ordenp1) 
				VALUES (%s, %s, %s, %s, %s)",
            GetSQLValueString(trim(strtoupper($matches10[17][$x])), "text"),
        ...
        $Result1 = mysqli_query($conexionbanca, $insertSQL) or die(mysqli_error($conexionbanca));
        $query_Recordset22 = sprintf(
            "/* PARSEADORES1 new\includes\spcrearjuegos.php - QUERY 6 */ SELECT *
FROM p1equipos
WHERE  
nomequipop1 = %s",
            GetSQLValueString(trim(strtoupper($matches10[17][$x])), "text")
    ...
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 new\includes\spcrearjuegos.php - QUERY 7 */ SELECT * 
FROM p2juegos 
WHERE  
idequipo1p2 = %s  AND
pichee1p2 = %s  AND
...
iniciodtp2 <= %s",
    ...
    if ($totalRows_Recordset1==0) {
        $insertSQL = sprintf(
            "/* PARSEADORES1 new\includes\spcrearjuegos.php - QUERY 8 */ INSERT 
				INTO p2juegos
