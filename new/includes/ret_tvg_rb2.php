...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 new\includes\ret_tvg_rb2.php - QUERY 1 */ SELECT * FROM carrera, hipodromo 
	WHERE
    ...
    do {
        if ($row_Recordset1['bus_retirado']==1) { //BASIC TVG
            ...
            if ($pos !== false) {
                ...
                if ($cantProg!=$row_Recordset1['can_caballos'] && $cantProg>0) {
                    ...
                    $updateSQL2 = sprintf(
                        "/* PARSEADORES1 new\includes\ret_tvg_rb2.php - QUERY 2 */ UPDATE carrera SET can_caballos=%s 
										  WHERE cod_carrera=%s",
                ...
                if (isset($retiro[0])) {
                    ...
                    foreach ($retiro as $ejem) {
                        ...
                        if ($estado==0) { // retirar caballo
                            $insertSQL = sprintf(
                                "/* PARSEADORES1 new\includes\ret_tvg_rb2.php - QUERY 3 */ INSERT INTO retirados 
        ...
        if ($row_Recordset1['bus_retirado']==2) { //RACEBETS
            ...
            if ($cantProgramados!=$row_Recordset1['can_caballos']) {
                $updateSQL2 = sprintf(
                    "/* PARSEADORES1 new\includes\ret_tvg_rb2.php - QUERY 4 */ UPDATE carrera SET can_caballos=%s 
									  WHERE cod_carrera=%s",
            ...
            if (isset($retiro[0]) && $retiro[0]!="") {
                ...
                foreach ($retiro as $ejem) {
                    ...
                    if ($estado==0) { // retirar caballo
                        $insertSQL = sprintf(
                            "/* PARSEADORES1 new\includes\ret_tvg_rb2.php - QUERY 5 */ INSERT INTO retirados 
									(cod_carrera, num_rcaballo) VALUES (%s, %s)",
                            GetSQLValueString($cod_carrera, "int"),
                            GetSQLValueString($ejem, "int")
                        );
