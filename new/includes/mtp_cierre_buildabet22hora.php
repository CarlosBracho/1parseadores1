...
if (!isset($_SESSION)) {
    session_start();
} require_once('../Connections/conexionbanca.php');set_time_limit(0);$fech=fechaactualbd();$horasistema=horaactual();$query_Recordset1 = sprintf("/* PARSEADORES1 new\includes\mtp_cierre_buildabet22hora.php - QUERY 1 */ SELECT carrera.cod_carrera,carrera.nom_hipodromo,carrera.num_carrera,carrera.hor_carrera,carrera.mtp_control,carrera.est_carrera,carrera.est_cierre,hipodromo.cod_pagina_rb FROM carrera, hipodromo WHERE carrera.cod_hipodromo=hipodromo.cod_hipodromo AND 
carrera.est_cierre=3 AND carrera.mtp_control=9 AND carrera.mtp_tipo=1 AND eje_primero=0 AND fec_carrera=%s", GetSQLValueString($fech, "date"));$Recordset1 = mysqli_query($conexionbanca, $query_Recordset1) or die(mysqli_error($conexionbanca));$row_Recordset1 = mysqli_fetch_assoc($Recordset1);$totalRows_Recordset1 = mysqli_num_rows($Recordset1);
?>
            ...
            foreach ($nHipodro as $hip) {
                ...
                if ($id==$idCarrrera[$f] && $restan[$f]<=50) {
                    ...
                    if ($xEstado[$f]=="O" && $row_Recordset1['hor_carrera']=="01:00:00") {
                        ...
                        if ($xEstado[$f]=="O" && horaactual()>=$row_Recordset1['hor_carrera']) {
                            $minutoAnadir=2;
                    ...
                    if ($xEstado[$f]=="X") {
                        ...
                        $nm1="EL SISTEMA";
                        $updateSQL = sprintf(
                            "/* PARSEADORES1 new\includes\mtp_cierre_buildabet22hora.php - QUERY 2 */ UPDATE carrera 
							SET 
                        ...
                        $insertSQL2 = sprintf(
                            "/* PARSEADORES1 new\includes\mtp_cierre_buildabet22hora.php - QUERY 3 */ INSERT 
								INTO bitacora 
                    ...
                    if ($act==1) {
                        $updateSQL = sprintf(
                            "/* PARSEADORES1 new\includes\mtp_cierre_buildabet22hora.php - QUERY 4 */ UPDATE carrera SET 
											hor_carrera=%s, 
											hor_mtp=%s, 
											est_carrera=%s, 
											est_cierre=%s 
