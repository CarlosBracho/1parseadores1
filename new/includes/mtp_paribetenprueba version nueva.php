    ...
    do {
        ...
        if ($hipodr[0]!="") {
            foreach ($hipodr as $hip) {
                ...
                if (($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $faltan2<="00:00:30") or ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $row_Recordset1['hor_carrera']<=$horasistema)) {
                    ...
                    $updateSQL = sprintf(
                        "/* PARSEADORES1 new\includes\mtp_paribetenprueba version nueva.php - QUERY 2 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s 
											  WHERE cod_carrera=%s",
        ...
        if ($control==1) {
            if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2) {
                ...
                $cie=1;
                $updateSQL = sprintf(
                    "/* PARSEADORES1 new\includes\mtp_paribetenprueba version nueva.php - QUERY 3 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_carrera=%s, est_cierre=%s, CERRADOX=%s, contador_cierres=%s
							  WHERE cod_carrera=%s",
