...
$horasistema=horaactual();
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 new\includes\mtp_tvgsin agrega tiempo.php - QUERY 1 */ SELECT ca.cod_carrera,hi.pre_hipodromo,ca.num_carrera,ca.hor_carrera,ca.est_carrera,ca.est_cierre,ca.supercontrol FROM carrera ca, hipodromo hi WHERE ca.est_cierre=2 AND ca.eje_primero=0 AND ca.fec_carrera=%s AND (ca.mtp_control=0 OR ca.mtp_control=1 OR ca.mtp_control=2 OR ca.mtp_control=3 OR ca.mtp_control=4 OR ca.mtp_control=7 OR ca.mtp_control=8) AND ca.cod_hipodromo=hi.cod_hipodromo AND (hi.mtp_betbird=0 OR hi.mtp_betbird=1) ORDER BY hor_carrera",
...
if ($totalRows_Recordset1>0) {
    ...
    do {
        ...
        if ($cont==1) {
            if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $row_Recordset1['supercontrol']==0) {
                $cod_carrera=$row_Recordset1['cod_carrera'];
                $updateSQL = sprintf(
                    "/* PARSEADORES1 new\includes\mtp_tvgsin agrega tiempo.php - QUERY 2 */ UPDATE carrera SET supercontrol=%s
                                                          WHERE cod_carrera=%s",
        ...
        if ($cont==1) {
            if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $row_Recordset1['supercontrol']==1) {
                $cod_carrera=$row_Recordset1['cod_carrera'];
                $updateSQL = sprintf(
                    "/* PARSEADORES1 new\includes\mtp_tvgsin agrega tiempo.php - QUERY 3 */ UPDATE carrera SET supercontrol=%s
                                                          WHERE cod_carrera=%s",
        ...
        if ($cont==1) {
            if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $row_Recordset1['supercontrol']>=2) {
                ...
                $mtp=0;
                $updateSQL = sprintf(
                    "/* PARSEADORES1 new\includes\mtp_tvgsin agrega tiempo.php - QUERY 4 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_carrera=%s, est_cierre=%s, mtp_control=%s, CERRADOX=%s
							  WHERE cod_carrera=%s",
