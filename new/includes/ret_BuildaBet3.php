...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 new\includes\ret_BuildaBet3.php - QUERY 1 */ SELECT * FROM carrera, hipodromo 
	WHERE
	carrera.nom_hipodromo=hipodromo.nom_hipodromo AND
	carrera.est_confirmacion=1 AND
        (carrera.est_cierre=2 OR carrera.est_cierre=1 OR carrera.est_cierre=0) AND
	(hipodromo.bus_retirado=2 OR hipodromo.bus_retirado=4 OR hipodromo.bus_retirado=5 OR hipodromo.bus_retirado=7) AND
...
if ($totalRows_Recordset1>0) {
    do {
        ...
        if ($cantProgramados!=$row_Recordset1['can_caballos'] && $cantProgramados>0) {
            ...
            $updateSQL2 = sprintf(
                "/* PARSEADORES1 new\includes\ret_BuildaBet3.php - QUERY 2 */ UPDATE carrera SET can_caballos=%s 
								  WHERE cod_carrera=%s",
        ...
        if (isset($retiro[0]) && $retiro[0]!="") {
            ...
            foreach ($retiro as $ejem) {
                ...
                if ($estado==0) { // retirar caballo
                    $insertSQL = sprintf(
                        "/* PARSEADORES1 new\includes\ret_BuildaBet3.php - QUERY 3 */ INSERT INTO retirados 
