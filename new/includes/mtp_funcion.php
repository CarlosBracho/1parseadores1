...
function consultaPacificnaRetirados()
{
    set_time_limit(0);
    date_default_timezone_set("Pacific/Honolulu") ;
    $url="http://web1.ameridatos.com:5530/retirados.asp";
    $existe=1;
    $hipoCar[0]=0;
    $caballo[0]=0;
    if ($existe==1) {
...
function consultaCierreTwinspires()
{
    $horacarr[0]="";
    $BrisCode[0]="";
    $numeroca[0]="";
    $restante[0]="";
    $horacier[0]="";
    $url = 'http://www.twinspires.com/php/fw/php_BRIS_BatchAPI/2.3/Tote/CurrentRace?ip=71.212.122.168&affid=2800&debug=off&username=my_sports&password=Gltbatm&output=json';
    ...
    return array($horacarr, $BrisCode, $numeroca, $restante, $horacier);
}
function consultaCierreTwinspiresx()
{
    $horacarr[0]="";
    $BrisCode[0]="";
    $numeroca[0]="";
    ...
    if (isset($fulldatos["CurrentRace"])) {
        ...
        foreach ($fulldatos["CurrentRace"] as $CurrentRace) {
            ...
            $horacier[$x]=$nuevaHora;
            $horacarr[$x]=$nuevaHora;
            $Canceled[$x]=$CurrentRace["TrackCanceled"];

            $x++;
        }
    }
    return array($horacarr, $BrisCode, $numeroca, $restante, $horacier, $Canceled);
}

function consultaTwinRetirados($ncar, $fech, $TrackType, $BrisCode)
{
    ...
    $urlC =  'http://www.twinspires.com/php/fw/php_BRIS_BatchAPI/2.3/Pgm/Entry?ip=71.212.122.168&affid=2800&debug=off&cDate='.$fecUrl.'&password=Gltbatm&track='.$BrisCode.'&output=json&username=my_sports&race='.$ncar.'&type='.$hType;
    $str_datos = get_url_contents($urlC);
    $fulldatos = json_decode($str_datos, true);
    if (isset($fulldatos["RaceList"]["DataThru"])) {
        $programados=$fulldatos["RaceList"]["DataThru"];
    }
    $url = 'http://www.twinspires.com/php/fw/php_BRIS_BatchAPI/2.3/Tote/OddsMtpPost?affid=2800&debug=off&password=Gltbatm&output=json&track='.$BrisCode.'&username=my_sports&race='.$ncar.'&ip=71.212.122.168&cDate='.$fecUrl.'&type='.$hType;
    $str_datos = get_url_contents($url);
    $fulldatos = json_decode($str_datos, true);
    if (isset($fulldatos["WinOdds"]["Entries"])) {
        $j=0;
        foreach ($fulldatos["WinOdds"]["Entries"] as $Entries) {
            if ($Entries["TextOdds"]=="SCR") {
