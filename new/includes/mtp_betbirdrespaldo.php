    ...
    do {
        ...
        if ($cont==1) {
            if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $faltan1=restahoras($horaactualcarrera, $row_Recordset1['hor_carrera'])<="00:03:00"  && $last!=0) {
                ...
                $mtp=1;
                $updateSQL = sprintf(
                    "/* PARSEADORES1 new\includes\mtp_betbirdrespaldo.php - QUERY 2 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_carrera=%s, est_cierre=%s, mtp_control=%s, CERRADOX=%s
							  WHERE cod_carrera=%s",
