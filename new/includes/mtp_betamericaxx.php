...
$horasistema=horaactual();
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 new\includes\mtp_betamericaxx.php - QUERY 1 */ SELECT 
	ca.cod_carrera,
	hi.mtp_betamericanom,
	ca.num_carrera,
	ca.hor_carrera,
	ca.est_carrera,
	ca.est_cierre,
...
if ($totalRows_Recordset1>0) {
    function betamerica($p)
    {
        ...
        return array($nHipodro,$nCarrera,$cerradoono);
    }
    list($nHipodro, $nCarrera, $cerradoono)=betamerica(1);
    $last = end($nCarrera);
    echo $last;
    echo "hola";
    $t=0;
    ...
    do {
        ...
        if ($last==0 && $horasistema<="17:00:00") {
            $msj="mtp betamerica esta fallando" . "\n";
    ...
    'text'=>$msjx,
            ...
            echo "mtp betamerica esta fallando";
        }
        $f=0;
        $cont=1;
        list($h, $m, $s)=restahoraVenta(horaactual(), $row_Recordset1['hor_carrera']);
        ...
        if ($nHipodro[0]!="") {
            foreach ($nHipodro as $hip) {
                if (trim($nHipodro[$f])==trim($row_Recordset1['mtp_betamericanom']) && $nCarrera[$f]==$row_Recordset1['num_carrera'] && $last!=0 && $cerradoono[$f]==0) {
...
<?php
                ...
                $horaactualcarrera2=horaactual(); $faltan2=restahoras($horaactualcarrera2, $row_Recordset1['hor_carrera']);
                        ...
                        if (($faltan2<="00:01:30") or ($row_Recordset1['hor_carrera']<=$horasistema)) {
                            ...
                            echo "<br/>";
                            $updateSQL = sprintf(
                                "/* PARSEADORES1 new\includes\mtp_betamericaxx.php - QUERY 2 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s 
											  WHERE cod_carrera=%s",
...
?>
        ...
        if ($cont==1) {
            if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $last!=0) {
                ...
                $mtp=3;
                $updateSQL = sprintf(
                    "/* PARSEADORES1 new\includes\mtp_betamericaxx.php - QUERY 3 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_carrera=%s, est_cierre=%s, CERRADOX=%s, contador_cierres=%s  
							  WHERE cod_carrera=%s",
                    GetSQLValueString($horasistema, "date"),
                    GetSQLValueString($horasistema, "date"),
                    GetSQLValueString(0, "int"),
                    GetSQLValueString(1, "int"),
                    GetSQLValueString("BETAMERICA", "text"),
