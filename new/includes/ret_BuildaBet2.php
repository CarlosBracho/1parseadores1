...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 new\includes\ret_BuildaBet2.php - QUERY 1 */ SELECT * FROM carrera, hipodromo 
	WHERE
	carrera.nom_hipodromo=hipodromo.nom_hipodromo AND
	carrera.est_confirmacion=1 AND
	carrera.est_cierre!=3 AND
	hipodromo.bus_retirado=2 AND
...
if ($totalRows_Recordset1>0) {
    do {
        ...
        if ($cantProgramados!=$row_Recordset1['can_caballos']) {
            ...
            $updateSQL2 = sprintf(
                "/* PARSEADORES1 new\includes\ret_BuildaBet2.php - QUERY 2 */ UPDATE carrera SET can_caballos=%s 
								  WHERE cod_carrera=%s",
        ...
        if (isset($retiro[0]) && $retiro[0]!="") {
            ...
            foreach ($retiro as $ejem) {
                ...
                if ($estado==0) { // retirar caballo
                    $insertSQL = sprintf(
                        "/* PARSEADORES1 new\includes\ret_BuildaBet2.php - QUERY 3 */ INSERT INTO retirados 
