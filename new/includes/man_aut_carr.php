...
if (isset($_POST["codCar"]) && isset($_POST["modo"])) {
    $query_Recordset1 = sprintf("/* PARSEADORES1 new\includes\man_aut_carr.php - QUERY 1 */ SELECT * FROM carrera WHERE carrera.cod_carrera = %s", GetSQLValueString($_POST["codCar"], "int"));
    ...
    if ($_POST["modo"]>=0 && $_POST["modo"]<=9) {
        $updateSQL = sprintf(
            "/* PARSEADORES1 new\includes\man_aut_carr.php - QUERY 2 */ UPDATE carrera SET mtp_control=%s 
				  WHERE cod_carrera=%s",
        ...
        $insertSQL2 = sprintf(
            "/* PARSEADORES1 new\includes\man_aut_carr.php - QUERY 3 */ INSERT 
						INTO bitacora 
