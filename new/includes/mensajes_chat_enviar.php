...
if ($_POST["txtMensaje"]!="") {
    ...
    $query_Recordset11 = sprintf(
        "/* PARSEADORES1 new\includes\mensajes_chat_enviar.php - QUERY 1 */ SELECT id FROM chat2 
				WHERE 
    ...
    if ($totalRows_Recordset11>0) {
        do {
            $insertSQL1 = sprintf(
                "/* PARSEADORES1 new\includes\mensajes_chat_enviar.php - QUERY 2 */ UPDATE chat2 
						SET recd=%s
						WHERE id=%s",
    ...
    $insertSQL = sprintf(
        "/* PARSEADORES1 new\includes\mensajes_chat_enviar.php - QUERY 3 */ INSERT INTO chat2 (from1, to1, message, sentdate, senttime, recd, id_taquilla) 
