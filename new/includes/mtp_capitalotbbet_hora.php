...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 new\includes\mtp_capitalotbbet_hora.php - QUERY 1 */ SELECT 
		hi.pre_twin,
		hi.nom_hipodromo,
				hi.mtp_paribetnom,
		ca.cod_carrera,
...
function consultaCierrecapitalotbbet()
{
    ...
    if (isset($fulldatos["CurrentRace"])) {
        ...
        foreach ($fulldatos["CurrentRace"] as $data) {
            ...
            $Mtp[$g]=$data["Mtp"]+1;
            $h1="+".$Mtp[$g]." minute";
            $horacier[$g]=date("H:i:s", strtotime($h1));

            $g++;
        }
    }
    return array($BrisCode, $DisplayName, $RaceNum, $RaceStatus, $Mtp, $horacier);
}
...
if ($totalRows_Recordset1>0) {
    list($BrisCode, $DisplayName, $RaceNum, $RaceStatus, $Mtp, $horacier)=consultaCierrecapitalotbbet();
    ...
    do {
        ...
        if ($DisplayName[0]!="") {
            foreach ($DisplayName as $Dis) {
                if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==3 && trim($DisplayName[$f])==trim($row_Recordset1['mtp_paribetnom']) && $RaceNum[$f]==$row_Recordset1['num_carrera'] && $last!=0 &&  $Mtp[$f]<=50) {
                    ...
                    if ($RaceStatus[$f]=="OPEN") {
						...
						<?php
                                         $updateSQL = sprintf(
                            "/* PARSEADORES1 new\includes\mtp_capitalotbbet_hora.php - QUERY 2 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_cierre=%s, est_carrera=%s, supercontrol=%s, mtp_control=%s 
											  WHERE cod_carrera=%s",
