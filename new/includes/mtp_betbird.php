...
if ($totalRows_Recordset1>0) {
    ...
    if ($last<1 && $horasistema<="17:00:00"  && $row_Recordset1['contadoralerta']<6) {
        ...
        echo $alertas3;
        $updateSQL = sprintf(
            "/* PARSEADORES1 new\includes\mtp_betbird.php - QUERY 2 */ UPDATE alertas SET contadoralerta=%s
							  WHERE idalertas=%s",
    ...
    'text'=>$msjx,
        ...
        $updateSQL = sprintf(
            "/* PARSEADORES1 new\includes\mtp_betbird.php - QUERY 3 */ UPDATE alertas SET contadoralerta=%s
							  WHERE idalertas=%s",
    ...
    do {
                ...
                if (trim($hipodomo[$f])==trim($row_Recordset1['nom_betbird']) && $numeroca[$f]==$row_Recordset1['num_carrera'] && $mtp[$f]<=47 && $last>0) {
                    if ($row_Recordset1['contadoralerta']>0) {
                        $updateSQL222 = sprintf(
                            "/* PARSEADORES1 new\includes\mtp_betbird.php - QUERY 4 */ UPDATE alertas SET contadoralerta=%s
							  WHERE idalertas=%s",
                            GetSQLValueString(0, "int"),
                            GetSQLValueString(1, "int")
                        );
                    ...
                    if (trim($hipodomo[$f])==trim($row_Recordset1['nom_betbird']) && $numeroca[$f]==$row_Recordset1['num_carrera'] && $row_Recordset1['betbirdvista']==0) {
                        ...
                        $updateSQL = sprintf(
                            "/* PARSEADORES1 new\includes\mtp_betbird.php - QUERY 5 */ UPDATE carrera SET betbirdvista=%s
							  WHERE cod_carrera=%s",
                            GetSQLValueString(1, "int"),
                    ...
                    if (($faltan2<="00:01:30") or ($row_Recordset1['hor_carrera']<=$horasistema)) {
                        ...
                        echo "<br/>";
                        $updateSQL = sprintf(
                            "/* PARSEADORES1 new\includes\mtp_betbird.php - QUERY 6 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s 
											  WHERE cod_carrera=%s",
        ...
        if ($cont==1) {
            if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $row_Recordset1['betbirdvista']==1 && $mtp[$f]<=3 && $last>=1) {
                $cod_carrera=$row_Recordset1['cod_carrera'];
                $est=0;
                $cie=1;
                $mtp=1;
                $updateSQL = sprintf(
                    "/* PARSEADORES1 new\includes\mtp_betbird.php - QUERY 7 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_carrera=%s, est_cierre=%s, mtp_control=%s, CERRADOX=%s
							  WHERE cod_carrera=%s",
                ...
                $insertSQL = sprintf(
                    "/* PARSEADORES1 new\includes\mtp_betbird.php - QUERY 8 */ INSERT INTO quiencierrayabre 
					(codcarrera, 
					fechaquien, 
					cerro) 
					VALUES (%s, %s, %s)",
