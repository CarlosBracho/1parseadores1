...
$horasistema=horaactual();
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 new\includes\mtp_amercierre.php - QUERY 1 */ SELECT * FROM carrera ca, hipodromo hi WHERE ca.est_cierre=2 AND ca.eje_primero=0 AND ca.fec_carrera=%s AND (ca.mtp_control=1 OR ca.mtp_control=2 OR ca.mtp_control=5 OR ca.mtp_control=6 OR ca.mtp_control=7) AND ca.cod_hipodromo=hi.cod_hipodromo ORDER BY hor_carrera",
    ...
    do {
        ...
        if ($hipodomo[0]!="") {
            foreach ($hipodomo as $hip) {
                if (trim($hipodomo[$f])==trim($row_Recordset1['nom_hipodromo_sup']) && $numeroca[$f]==$row_Recordset1['num_carrera'] &&  $restante[$f]<=50) {
					...
					<?php
                                         $updateSQL = sprintf(
                        "/* PARSEADORES1 new\includes\mtp_amercierre.php - QUERY 2 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_cierre=%s, est_carrera=%s, supercontrol=%s 
											  WHERE cod_carrera=%s",
        ...
        if ($cont==1) {
            if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $row_Recordset1['supercontrol']==0) {
                $cod_carrera=$row_Recordset1['cod_carrera'];
                $updateSQL = sprintf(
                    "/* PARSEADORES1 new\includes\mtp_amercierre.php - QUERY 3 */ UPDATE carrera SET supercontrol=%s
                                                          WHERE cod_carrera=%s",
                    GetSQLValueString(1, "int"),
                    GetSQLValueString($cod_carrera, "int")
                );
        ...
        if ($cont==1) {
            if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $row_Recordset1['supercontrol']==1) {
                $cod_carrera=$row_Recordset1['cod_carrera'];
                $updateSQL = sprintf(
                    "/* PARSEADORES1 new\includes\mtp_amercierre.php - QUERY 4 */ UPDATE carrera SET supercontrol=%s
                                                          WHERE cod_carrera=%s",
                    GetSQLValueString(2, "int"),
                    GetSQLValueString($cod_carrera, "int")
                );
        ...
        if ($cont==1) {
            if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $row_Recordset1['supercontrol']==2) {
                $cod_carrera=$row_Recordset1['cod_carrera'];
                $updateSQL = sprintf(
                    "/* PARSEADORES1 new\includes\mtp_amercierre.php - QUERY 5 */ UPDATE carrera SET supercontrol=%s
                                                          WHERE cod_carrera=%s",
        ...
        if ($cont==1) {
            if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $row_Recordset1['supercontrol']==3) {
                ...
                $cie=1;
                $updateSQL = sprintf(
                    "/* PARSEADORES1 new\includes\mtp_amercierre.php - QUERY 6 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_carrera=%s, est_cierre=%s, CERRADOX=%s
							  WHERE cod_carrera=%s",
