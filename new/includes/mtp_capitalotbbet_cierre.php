...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 new\includes\mtp_capitalotbbet_cierre.php - QUERY 1 */ SELECT 
		hi.pre_twin,
		hi.nom_hipodromo,
				hi.mtp_paribetnom,
		ca.cod_carrera,
...
function consultaCierrecapitalotbbet()
{
    ...
    if (isset($fulldatos["CurrentRace"])) {
        ...
        foreach ($fulldatos["CurrentRace"] as $data) {
            $BrisCode[$g]=strtoupper($data["BrisCode"]);
            $DisplayName[$g]=strtoupper($data["DisplayName"]);
            $RaceNum[$g]=$data["RaceNum"];
            $RaceStatus[$g]=strtoupper($data["RaceStatus"]);
            $Mtp[$g]=$data["Mtp"]+1;
...
if ($totalRows_Recordset1>0) {
    ...
    do {
            ...
            foreach ($DisplayName as $Dis) {
                if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && trim($DisplayName[$f])==trim($row_Recordset1['mtp_paribetnom']) && $RaceNum[$f]==$row_Recordset1['num_carrera'] && $last!=0) {
                    ...
                    if ($RaceStatus[$f]<>"OPEN") {
						...
						<?php
                        $updateSQL = sprintf(
                            "/* PARSEADORES1 new\includes\mtp_capitalotbbet_cierre.php - QUERY 2 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_carrera=%s, est_cierre=%s, mtp_control=%s, CERRADOX=%s, contador_cierres=%s, mtp1=%s  
										  WHERE cod_carrera=%s",
                        ...
                        if (($faltan2<="00:00:30" && $last!=0) or ($row_Recordset1['hor_carrera']<=$horasistema && $last!=0)) {
                            ...
                            $updateSQL = sprintf(
                                "/* PARSEADORES1 new\includes\mtp_capitalotbbet_cierre.php - QUERY 3 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s 
											  WHERE cod_carrera=%s",
        ...
        if ($row_Recordset1['est_carrera']==1 && $row_Recordset1['est_cierre']==2 && $control==1 && $last!=0) {
            ...
            $cie=1;
            $updateSQL = sprintf(
                "/* PARSEADORES1 new\includes\mtp_capitalotbbet_cierre.php - QUERY 4 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_carrera=%s, est_cierre=%s, CERRADOX=%s, contador_cierres=%s, mtp1=%s  
