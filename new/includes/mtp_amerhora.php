...
$horasistema=horaactual();
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 new\includes\mtp_amerhora.php - QUERY 1 */ SELECT * FROM carrera, hipodromo WHERE carrera.eje_primero=0 AND carrera.fec_carrera=%s AND (carrera.mtp_control=5 OR carrera.mtp_control=6) AND carrera.cod_hipodromo=hipodromo.cod_hipodromo",
        ...
        if ($hipodomo[0]!="") {
            foreach ($hipodomo as $hip) {
                if (trim($hipodomo[$f])==trim($row_Recordset1['nom_hipodromo_sup']) && $numeroca[$f]==$row_Recordset1['num_carrera']) {
					...
					<?php
                        $updateSQL = sprintf(
                        "/* PARSEADORES1 new\includes\mtp_amerhora.php - QUERY 2 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_cierre=%s, est_carrera=%s, mtp_control=%s 
											  WHERE cod_carrera=%s",
