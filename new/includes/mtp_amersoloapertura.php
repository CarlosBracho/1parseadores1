...
$horasistema=horaactual();
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 new\includes\mtp_amersoloapertura.php - QUERY 1 */ SELECT * FROM carrera ca, hipodromo hi WHERE 
	(ca.est_cierre=0 OR ca.est_cierre=1 OR ca.est_cierre=2 OR ca.est_cierre=3 OR ca.est_cierre=4) 
...
if ($totalRows_Recordset1>0) {
    function consultaPacificna()
    {
        ...
        $url='http://web1.ameridatos.com:5530/program1.asp';
        $ch = curl_init();
    ...
    do {
        ...
        if ($hipodomo[0]!="") {
            foreach ($hipodomo as $hip) {
                if (trim($hipodomo[$f])==trim($row_Recordset1['nom_hipodromo_sup']) && $numeroca[$f]==$row_Recordset1['num_carrera'] &&  $restante[$f]<=30) {
					...
					<?php
if ($row_Recordset1['ABIERTOX']=="") {
                        $ABIERTOX="ameridatos";
                    } else {
                        $ABIERTOX=$row_Recordset1['ABIERTOX'];
                    }

                    
                    $updateSQL = sprintf(
                        "/* PARSEADORES1 new\includes\mtp_amersoloapertura.php - QUERY 2 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_cierre=%s, est_carrera=%s, supercontrol=%s, mtp_control=%s , ABIERTOX=%s 
											  WHERE cod_carrera=%s",
