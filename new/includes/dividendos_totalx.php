...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 new\includes\dividendos_totalx.php - QUERY 1 */ SELECT * 
	FROM 
	hipodromo,
	carrera 
	WHERE
            ...
            if (isset($cAct) && isset($cAct[0]) && $cAct[0]>0) {
                foreach ($cAct as $hip) {
					...
					<?php
                        ...
                        if ($eje1LugarSimple[$hip]>0 && $DivWinPriLugar[$hip]>0) {
                            if ($row_Recordset1['control_dividendo']==0) {
                                $control_dividendo=1;
                            }
                            $updateSQL = sprintf(
                                "/* PARSEADORES1 new\includes\dividendos_totalx.php - QUERY 2 */ UPDATE carrera 
							SET 
								est_confirmacion=%s,
								control_dividendo=%s,
								eje_primero=%s, 
