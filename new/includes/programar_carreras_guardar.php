...
if (isset($_GET["hip"]) && isset($_GET["can"])) {
    ...
    if ($xhip!="" && $xcan>0) {
        ...
        if ($verif==0) {
            $updateSQL = sprintf(
                "/* PARSEADORES1 new\includes\programar_carreras_guardar.php - QUERY 1 */ UPDATE hipodromo SET 
  						cod_pagina_rb=%s 
						WHERE 
						cod_hipodromo=%s",
        ...
        for ($i = 1; $i <= $xcan; $i++) {
            ...
            if ($verif==0 && $cod!=-1) {
                $insertSQL = sprintf(
                    "/* PARSEADORES1 new\includes\programar_carreras_guardar.php - QUERY 2 */ INSERT INTO carrera 
					(cod_banca,
					cod_hipodromo, 
					nom_hipodromo, 
					nom_hipodromo_hpi, 
