...
?>
<link rel="stylesheet" href="../iconos/styleIcon.css" />
...
function imprimeTicket(cod, acceso){
	if (acceso==1) {
		ajax = nuevoAjax();
...
if (isset($_POST["MM_insert"]) && $_POST["MM_insert"]=="form1" && isset($_POST["cod_carrera"]) && $_POST["cod_carrera"]!=-1) {
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 new\Templates\t_grabajugadahipicoticket.php - QUERY 1 */ SELECT can_caballos, hor_carrera, fec_carrera, est_carrera
								 FROM carrera
								 WHERE cod_carrera = %s 
								 LIMIT 1",
    ...
    $_POST["monPla1"]=$_POST["monPla1"]/1;
    $_POST["monSho1"]=$_POST["monSho1"]/1;
    $_POST["monGan2"]=$_POST["monGan2"]/1;
    $_POST["monPla2"]=$_POST["monPla2"]/1;
    $_POST["monSho2"]=$_POST["monSho2"]/1;
    $_POST["monGan3"]=$_POST["monGan3"]/1;
    $_POST["monPla3"]=$_POST["monPla3"]/1;
    $_POST["monSho3"]=$_POST["monSho3"]/1;
    $_POST["monGan4"]=$_POST["monGan4"]/1;
    $_POST["monPla4"]=$_POST["monPla4"]/1;
    $_POST["monSho4"]=$_POST["monSho4"]/1;
...
/************************************************************* 4 ************************************************************/
    ...
    if ($exito==1) {// guarda e imprime ticket
        $t=0;
        $u=0;
        $apuesta2=$apuesta;
        $tipo2=$tipo;
        ...
        foreach ($apTotCab as $busca) {
            ...
            if ($busca[1]==4 || $busca[1]==7) {
                if ($busca[0]>$_POST["apMaxExa"]) {
                    $valida=0;
                    $menValida="Monto en exacta excedido";
                    $exito=0;
                    break;
                }
            }
            if ($busca[1]==5 || $busca[1]==8) {
            ...
            foreach ($apuesta as $busca) {  //busca exceso o minimos de montos
                ...
                if ($tipo[$t-1]==1) { // ganador
                    if ($monto[$t-1]<$_POST["apMinGan"]) {
                        $valida=0;
                        $menValida="Monto mínimo a ganador";
                        $exito=0;
                        break;
                    }
                }
        ...
        if ($valida==1) {
            ...
            foreach ($apuesta as $apta) {
                $yu=$f+1;
                $insertSQL = sprintf(
                    "/* PARSEADORES1 new\Templates\t_grabajugadahipicoticket.php - QUERY 2 */ INSERT INTO venta (ser_venta, ticket, cod_taquilla, fec_venta, hor_venta, cod_tventa,
					 num_caballo, mon_venta, cod_carrera, id_usuario, est_ticket, can_ticket, ip_venta, lin_ticket) 
...
$mensaje1='¡ERROR! TICKET NO GUARDADO. '.$menMaxCab.$menRetCab.$menFueFec.$menValida;
...
imprimeTicket(<?php echo $numerotiket2; ?>,<?php echo $exito; ?>);
</script>