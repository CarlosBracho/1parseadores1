...
$timestamp = strtotime('+1 day', strtotime($final));
$final2 = date("Y-m-d", $timestamp );
//echo $final2;
...
$query_Recordset13 = sprintf(
  "/* PARSEADORES1 new\parley\adpaprobar.php - QUERY 1 */ SELECT *
  FROM p4jugadas
  WHERE
  p4jugadas.jugadadtp4 >= %s 
  AND p4jugadas.jugadadtp4 <= %s  
  AND  p4jugadas.premioapagarp4 > 0  
  AND p4jugadas.pverificado = 0  
  AND (p4jugadas.estadoticketp4=1 OR  p4jugadas.estadoticketp4=3)
  AND p4jugadas.lineatp4= 1 ORDER BY p4jugadas.jugadadtp4 DESC",
...
//solo toma en cuenta la linea ticket 1
      ...
      <table class="table">
        <thead class="thead-dark">
          <tr>
            ...
            <th scope="col">Devolver ticket</th>
...
if ($totalRows_Recordset13>0) {
    do {
      $query_Recordset19 = sprintf(
        "/* PARSEADORES1 new\parley\adpaprobar.php - QUERY 2 */ SELECT 
        *
        FROM
    p4jugadas, p2juegos
    WHERE
    ...
    p4jugadas.nticketp4 = %s   ORDER BY p4jugadas.lineatp4 DESC",
        GetSQLValueString($row_Recordset13['nticketp4'], "int"));
               ...
               $query_Recordset18 = sprintf(
                  "/* PARSEADORES1 new\parley\adpaprobar.php - QUERY 3 */ SELECT 
...
Agente:<?php echo $row_Recordset18['nom_agencia']; ?> <br/>  
Distri:<?php echo $row_Recordset18['nom_banca']; ?> <br/>     
</br>
			<?php
?>
...
<?php
        ...
        $query_Recordset14 = sprintf(
            "/* PARSEADORES1 new\parley\adpaprobar.php - QUERY 4 */ SELECT *
                FROM p4jugadas
                WHERE
        ...
        do {
            ...
            if ($row_Recordset14['jugadadtp4']<$final2) { $juegoiniciado++; }
        ...
        <center><input type="submit" value="CONFIRME QUE TODO ESTA CORRECTO" class="btn-success" title="CONFIRME QUE TODO ESTA CORRECTO" 
          data-toggle="modal" data-target="#exampleModal" href="detalle_ticket.php" title="" 
onclick="detalle_ticket(<?php echo $row_Recordset13['nticketp4']; ?>, 0, 2); return false"   /></center>
<?php } ?>
      </td>
...
<td><?php echo $row_Recordset13['mon_ventap4'];
...
if ($row_Recordset13['monedap4']==5) {echo ' SOL';}} ?>
     ...
     <td>
      <input type="submit" value="DEVOLVER TICKET" style="font-size: 10px; background:grey; color:white;"  title="CONFIRME QUE TODO ESTA CORRECTO" 
          data-toggle="modal" data-target="#exampleModal" href="detalle_ticket.php" title="" 
onclick="detalle_ticket(<?php echo $row_Recordset13['nticketp4']; ?>, 0, 18); return false"   />
</td> 
          </tr> 
<?php 
}
