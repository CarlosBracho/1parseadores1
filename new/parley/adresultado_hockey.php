...
do {
	...
	$query_Recordset301 = sprintf(
		"/* PARSEADORES1 new\parley\adresultado_hockey.php - QUERY 1 */ SELECT * FROM p2juegos, p5resultadosj WHERE 
...
p2juegos.Id_p2juegosp2 = %s 
	...
	GetSQLValueString($iniciof, "date"), GetSQLValueString($finalf, "date"),
		GetSQLValueString($row_Recordset1hockey['juegop5'], "int")
	);
...
if($row_Recordset1hockey['quienmete']<>0 && $row_Recordset1hockey['quienmete']<>''){
    $query_Recordset301d = sprintf(
        "/* PARSEADORES1 new\parley\adresultado_hockey.php - QUERY 2 */ SELECT nom_usuario FROM usuario WHERE 
    id_usuario = %s
    ",
