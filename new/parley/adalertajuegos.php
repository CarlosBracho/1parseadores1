...
"/* PARSEADORES1 new\parley\adalertajuegos.php - QUERY 1 */ SELECT *
FROM p2juegos
WHERE
iniciodtp2 >= %s
...
do {
	$query_Recordset14 = sprintf(
		"/* PARSEADORES1 new\parley\adalertajuegos.php - QUERY 2 */ SELECT Id_p5resultadosjp5
		FROM p5resultadosj
		WHERE
		iniciodtp5 >= %s AND 
		iniciodtp5 <= %s AND 
		...
		if($totalRows_Recordset14==0){
			...
			$query_Recordset144 = sprintf(
				"/* PARSEADORES1 new\parley\adalertajuegos.php - QUERY 3 */ SELECT p2juegos.deportep2, p2juegos.iniciodtp2
...
if($row_Recordset144['deportep2']=='hockey'){
	...
	$datetime = date('Y-m-d H:i:s', $datetime);
				...
				$query_Recordset21 = sprintf(
					"/* PARSEADORES1 new\parley\adalertajuegos.php - QUERY 4 */ SELECT nomequipop1
					FROM p1equipos
					WHERE  
					...
					"/* PARSEADORES1 new\parley\adalertajuegos.php - QUERY 5 */ SELECT nomequipop1
					FROM p1equipos
					WHERE  
