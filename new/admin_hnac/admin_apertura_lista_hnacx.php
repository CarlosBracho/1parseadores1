...
$MM_authorizedUsers = "A"; $editFormAction = $_SERVER['PHP_SELF'];
...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 new\admin_hnac\admin_apertura_lista_hnacx.php - QUERY 1 */ SELECT 
...
if ($totalRows_Recordset1>0) {
    $query_Recordset2 = sprintf(
        "/* PARSEADORES1 new\admin_hnac\admin_apertura_lista_hnacx.php - QUERY 2 */ SELECT dv.cod_dividendo_macu, dv.div_pago_macu
		FROM 
...
$query_Recordset3 = sprintf("/* PARSEADORES1 new\admin_hnac\admin_apertura_lista_hnacx.php - QUERY 3 */ SELECT * FROM ctrol_ventpag_global_hnac WHERE cod_ctrol_ventpag_global_hnac = 1");
	...
	if(confirma==true){
		...
		$.ajax({ data:parametros, url:'../includes/man_aut_carr_hnac.php', type:'post',
			success:function (response) { 
				...
				window.location='admin_apertura_lista_hnacx.php';
			}
		}); 
	...
	$(document).ready(function(){
		...
		$("#cVentas").change(function(){
			...
			$.ajax({ url:"../admin_hnac/ctrol_ventaspagos_global_hnac.php", type: "POST", data:parametros,
				success: function(){ window.location='admin_apertura_lista_hnacx.php';}
			});
		});
