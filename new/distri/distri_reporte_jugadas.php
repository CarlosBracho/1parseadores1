        ...
        if ($_POST['fecha_inicio']!="" && $_POST['fecha_fin']!="") {
            ...
            if ($_POST['id_agente']!="todos") {
                $query_Recordset1 = sprintf(
                    "/* PARSEADORES1 new\distri\distri_reporte_jugadas.php - QUERY 1 */ SELECT 
...
USE INDEX(id_us_fe_fe)
				...
				agencia.cod_agencia = %s AND agencia.cod_banca = %s 
...
if (((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1") && isset($_POST['id_agente']) && $_POST['id_agente']=="todos") || (!isset($_POST["MM_update"]))) {
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 new\distri\distri_reporte_jugadas.php - QUERY 2 */ SELECT 
					venta.ticket, venta.ip_venta, venta.fec_venta, venta.hor_venta, venta.mon_venta, venta.cod_usuario_pago, 
					venta.pag_premio, venta.est_ticket, venta.est_calculo, venta.can_ticket, venta.hor_pago,
					taquilla.nom_taquilla, usuario.nom_usuario, 
					venta.cod_tventa, venta.fec_pago, venta.ip_pago,
...
USE INDEX(id_us_fe_fe)
	...
	usuario.cod_taquilla = taquilla.cod_taquilla AND usuario.id_usuario = venta.id_usuario AND
	(venta.fec_venta >= %s AND venta.fec_venta <= %s OR venta.fec_pago >= %s AND venta.fec_pago <= %s) AND 
	taquilla.cod_agencia = agencia.cod_agencia AND venta.cod_carrera = carrera.cod_carrera AND agencia.cod_banca = %s
	ORDER BY venta.fec_venta, venta.cod_taquilla, venta.num_ticket ASC",
        GetSQLValueString($in, "date"),
        GetSQLValueString($fi, "date"),
        GetSQLValueString($in, "date"),
...
$query_Recordset2 = sprintf(
    "/* PARSEADORES1 new\distri\distri_reporte_jugadas.php - QUERY 3 */ SELECT cod_agencia, nom_agencia FROM agencia WHERE agencia.cod_banca = %s	 
