...
/* PARSEADORES1 new\chat_ad\mensajes_control_ad.php - QUERY 1 */ SELECT 
	us.nom_usuario,
	us.tip_usuario,
	CASE us.tip_usuario
		WHEN 'G' THEN 'Agente'
	...
	END AS tipo,
	CASE us.tip_usuario
		WHEN 'G' THEN (/* PARSEADORES1 new\chat_ad\mensajes_control_ad.php - QUERY 2 */ SELECT nom_agencia FROM agencia ag WHERE ag.cod_agencia=us.cod_taquilla LIMIT 1)
		WHEN 'D' THEN (/* PARSEADORES1 new\chat_ad\mensajes_control_ad.php - QUERY 3 */ SELECT nom_banca FROM banca ba WHERE ba.cod_banca=us.cod_taquilla LIMIT 1)
	END AS nombre
FROM usuario us
WHERE us.tip_usuario='G' OR us.tip_usuario='D' ORDER BY tipo, us.nom_usuario");
...
$(document).ready(function() {
	$("#enviarMen").click(function(){
		var url = "../chat_ad/mensajes_chat_enviar_ad.php"; // El script en dónde se realizará la petición.
	...
	var refreshId1 = null;	
		function startChat() {
			refreshId1 = setInterval(function() {
				...
				$.ajax({ data:parametros, url:'../chat_ad/mensajes_chat_ad.php', type:'post',
	...
	<script>
		...
		$(document).ready(function() {
			$('.botonChat').click(function() {
				...
				$.ajax({ data:parametros, url:'../chat_ad/mensajes_chat_mostrar_ad.php', type:'post',
					success:function (response) { 
...
<div class="container1" style="background: #333;">
	<div style="height:700px; width:100%; padding:0px 0px 0px 0px; text-align:left;">
		<div style="background: #333; width:100%; float:left; padding:12px 0px 15px 0px;
			...
			MENSAJES AMERICANAS DISTRIBUIDOR - AGENTES
		</div><!-- end .container -->            
		<div style="height:100%; width:20%; padding:5px 0px 10px 0px; text-align:left; background:#0099CC; float:left; 
                    ...
                    <tr align="left">
                        ...
                        <td width="99%">
                        	<a href="#" style="font-size:14px;text-decoration:none;" class="botonChat" data-type="zoomout" 
                                ...
                                '<?php echo $row_Recordset1['tip_usuario'] ?>')">
			                        <div class="btn btn-large" 
                                    	style="width:74%; text-align:left; margin: 4px 0px 0px 0px; height:22px;">
		                                <?php echo $row_Recordset1['nom_usuario'] ?>
        		                        <br/>
                		                <font style="font-size:10px">
                        		        	(<?php echo $row_Recordset1['tipo'].": ".$row_Recordset1['nombre'] ?>)
                                		</font>
			                        </div>
            	            </a>
                        </td>
		...
		<div style="height:100%; width:80%; padding:5px 0px 10px 0px; text-align:left; background:#F90; float:left;
   			color:#FFF; font-size:16px;text-align: center;">
    			CHAT
				<div style="height:94%; width:99%; margin:15px 1px 2px 2px; text-align:left; background: #FFF; 
                    ...
                    onmouseout="startChat()" id="mensajes" >
      				<?php
                    include("../chat_ad/mensajes_chat_ad.php");?>
      			</div>
		</div>
		<div class="overlay-container">
			<div class="window-container zoomout" style="height:400px;" id="window-container">
     			<div style="height:16px; width:100%; background:#F90; margin:-10px 0px 0px -20px; padding:5px 40px 0px 2px;
