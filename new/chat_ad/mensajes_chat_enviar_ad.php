...
if ($_POST["txtMensaje"]!="") {
    ...
    $query_Recordset11 = sprintf(
        "/* PARSEADORES1 new\chat_ad\mensajes_chat_enviar_ad.php - QUERY 1 */ SELECT id FROM chat7 
				WHERE 
    ...
    if ($totalRows_Recordset11>0) {
        do {
            $insertSQL1 = sprintf(
                "/* PARSEADORES1 new\chat_ad\mensajes_chat_enviar_ad.php - QUERY 2 */ UPDATE chat7 
						SET recd=%s
						WHERE id=%s",
    ...
    $insertSQL = sprintf(
        "/* PARSEADORES1 new\chat_ad\mensajes_chat_enviar_ad.php - QUERY 3 */ INSERT INTO chat7 (from1, to1, message, sentdate, senttime, recd) 
			VALUES (%s, %s, %s, %s, %s, %s)",
