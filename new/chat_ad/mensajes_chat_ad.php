...
$query_Recordset2 = sprintf(
    "
/* PARSEADORES1 new\chat_ad\mensajes_chat_ad.php - QUERY 1 */ SELECT 
	ch.from1,
	ch.to1,
	ch.message,
	ch.sentdate,
...
USE INDEX(sentdate) 
WHERE 
(ch.sentdate >= date_add(NOW(), INTERVAL -7 DAY) AND ch.sentdate <= %s)
ORDER BY ch.id desc",
        ...
        if (trim($row_Recordset2['from1'])!="Soporte") {
            if (!in_array($row_Recordset2['from1'], $os)) {
                ...
                $query_Recordset222 = sprintf(
                    "/* PARSEADORES1 new\chat_ad\mensajes_chat_ad.php - QUERY 2 */ SELECT 
	us.cod_taquilla,
	us.tip_usuario
FROM 
usuario us
USE INDEX(nom_usuario) 
WHERE 
us.nom_usuario = %s",
                    GetSQLValueString($row_Recordset2['from1'], "text")
                );
                ...
                if ($row_Recordset2['recd']==1) {?>
                    ...
                    onClick="haga('<?php echo $row_Recordset2['from1'];?>','<?php echo $row_Recordset222['cod_taquilla'];?>')">
                    <?php
                } ?>
                <table width="100%" border="0">
                    ...
                    <tr>
                        <td align="left"><?php echo " [".$row_Recordset2['from1']."]"; ?></td>
                    </tr>
                    <tr>
                ...
                -webkit-border-radius: 5px 5px 5px 5px; box-shadow: #aaa 1px 1px 6px; border: 1px solid #d4d4d4;">
                    <table width="100%" border="0">
                        ...
                        <tr>
                            ...
                            <?php echo " a ".$row_Recordset2['to1']." [".$row_Recordset2['from1']."]"; ?> 
                            <em><?php
                            if ($fechasistema==$row_Recordset2['sentdate']) {
                                $dia="hoy";
                            } else {
