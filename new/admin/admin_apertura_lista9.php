...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 new\admin\admin_apertura_lista9.php - QUERY 1 */ SELECT * FROM carrera 
	WHERE  
...
function cStatus(id, cCar, nHip) {
	...
	if (id==5) modo=" MTP WATCHANDWAGER?";
	if (id==6) modo=" MTP SUPERAMERICANA?";
	if (id>=0 && id<=6) {
		swal({
		  title: nHip,
...
<div class="container">
  ...
  <tr style="background:#5EAEFF; color:#FFFFFF; height:30px">
    <td width="42"></td>
    <td colspan="2">HIPÓDROMO</td>
    <td width="36" style="font-size:8px">CORREN</td>
    <td width="118">STATUS</td>
    <td width="44">MTP</td>
  ...
  <tr bgcolor="#F2F2F2" onmouseover="cambiacolor_over(this)" onmouseout="cambiacolor_out(this)"; style="font-size:14px">
    ...
    <td width="241" align="left"><strong><?php echo $carrA; ?></strong></td>
    <td align="center" width="60">
		<button type="button" style="font-size:22px; width:50px; height:40px; text-align:center; text-decoration:none;" 
		class="btn btn-danger" onclick='accionAumentar("<?php echo $row_Recordset1['nom_hipodromo'].': ...'.$row_Recordset1['num_carrera']; ?>","¿Está seguro de AUMENTAR 1 MINUTOS a la carrera?","<?php echo $row_Recordset1['cod_carrera']; ?>",1,"info",1)'>
        	+1<span style="font-size:9px">m</span>
		</button>    
	</td>
    <?php
    $status="";
    ...
    <td width="155" align="center" valign="middle">
      <select name="mtp_control" class="alert-success2"  style="width:155px; height:32px; font-size:12px; margin:1px 0px 0px 0px" tabindex="4" 
        ...
        <option value="1" <?php if (!(strcmp(1, htmlentities($row_Recordset1['mtp_control'], ENT_COMPAT, 'utf-8')))) {
            echo "SELECTED";
        } ?> style="background:#000;color:#CCC">MTP EQUIBASE</option>
        <option value="2" <?php if (!(strcmp(2, htmlentities($row_Recordset1['mtp_control'], ENT_COMPAT, 'utf-8')))) {
            echo "SELECTED";
        } ?> style="background:#000;color:#CCC">TRACK INFO</option>
        ...
        <option value="4" <?php if (!(strcmp(4, htmlentities($row_Recordset1['mtp_control'], ENT_COMPAT, 'utf-8')))) {
            echo "SELECTED";
        } ?> style="background:#000;color:#CCC">BASIC TVG</option>
        ...
        <option value="6" <?php if (!(strcmp(6, htmlentities($row_Recordset1['mtp_control'], ENT_COMPAT, 'utf-8')))) {
            echo "SELECTED";
        } ?> style="background:#000;color:#CCC">SUPERAMERICANA</option>
      </select>
    </td>
    ...
    <td width="33" align="center">
		<a title="editar carrera" href="admin_apertura_lista.php">
  ...
  <?php
    ...
    <?php } else {?>
        <table width="100%" border="0" align="center" style="background:#5EAEFF; color:#FFFFFF; height:30px">
          <tr style="background:#5EAEFF; color:#FFFFFF; height:30px">
            <td width="30"></td>
            <td width="289">HIPÓDROMO</td>
            <td width="59">&nbsp;</td>
            <td width="35" style="font-size:8px">CORREN</td>
            <td width="112">STATUS</td>
            <td width="90">FECHA</td>
            <td width="61" style="font-size:12px">HORA CIERRE</td>
            <td width="230" colspan="6">&nbsp;</td>
          </tr>
        </table>
