...
$query_Recordset1 =  sprintf(
    "/* PARSEADORES1 new\admin\taquillas_edit_ame2021.php - QUERY 1 */ SELECT  
...
$query_Recordset4 =  sprintf(
    "/* PARSEADORES1 new\admin\taquillas_edit_ame2021.php - QUERY 2 */ SELECT  
	ta.pag_codigo, tp.apu_maxgan, tp.apu_maxpla, tp.apu_maxsho, tp.apu_maxexa, tp.apu_maxtri, tp.apu_maxsup, tp.apu_mingan,
...
if ($totalRows_Recordset4==0 && (!isset($_POST["MM_insert2"]))) {
    ...
    if ((isset($_POST["MM_insert2"])) && ($_POST["MM_insert2"] == "form2")) {
        if (isset($_POST["exp_agencia"]) && $_POST["exp_agencia"]>0) {
            ...
            $query_Recordset4 =  sprintf(
                "/* PARSEADORES1 new\admin\taquillas_edit_ame2021.php - QUERY 3 */ SELECT  
				...
				tp.est_impresion, tp.anu_regalia, tp.tic_caduca, tp.tip_ticket, tp.tie_reclamo, tp.ver_porpagar
				FROM  taquilla ta, taquilla_opc_ame tp 
				WHERE ta.cod_taquilla = %s AND tp.cod_taquilla = ta.cod_taquilla
				LIMIT 1",
    ...
    if ($graba==31) {
        if (isset($row_Recordset4['reg_gan'])) {
            if ($_POST['reg_gan']<>$row_Recordset4['reg_gan'] or $_POST['reg_pla']<>$row_Recordset4['reg_pla'] or $_POST['pag_codigo']<>$row_Recordset4['pag_codigo']) {
                ...
                $insertSQL2 = sprintf(
                    "/* PARSEADORES1 new\admin\taquillas_edit_ame2021.php - QUERY 4 */ INSERT 
					INTO bitacora 
        ...
        if ($_POST['existe']==1) {
            $insertSQL2 = sprintf(
                "/* PARSEADORES1 new\admin\taquillas_edit_ame2021.php - QUERY 5 */ UPDATE taquilla_opc_ame
					SET
					pag_codigo=%s, 
					apu_maxgan=%s, 
					apu_maxpla=%s, 
            ...
            $query_Recordset22 =  sprintf(
                "/* PARSEADORES1 new\admin\taquillas_edit_ame2021.php - QUERY 6 */ SELECT  
tp.apu_mingan
	...
	LIMIT 1",
            ...
            if ($apuestasminimaausar > $row_Recordset22['apu_mingan']) {
                $insertSQL11 = sprintf(
                    "/* PARSEADORES1 new\admin\taquillas_edit_ame2021.php - QUERY 7 */ UPDATE taquilla_opc_ame
					SET
					apu_mingan=%s
            ...
            $insertSQL2 = sprintf(
                "/* PARSEADORES1 new\admin\taquillas_edit_ame2021.php - QUERY 8 */ INSERT 
			INTO 
			taquilla_opc_ame 
            ...
            $query_Recordset22 =  sprintf(
                "/* PARSEADORES1 new\admin\taquillas_edit_ame2021.php - QUERY 9 */ SELECT  
tp.apu_mingan
	FROM  taquilla_opc_ame tp 
	WHERE tp.cod_taquilla = %s
	LIMIT 1",
            ...
            if ($apuestasminimaausar > $row_Recordset22['apu_mingan']) {
                $insertSQL11 = sprintf(
                    "/* PARSEADORES1 new\admin\taquillas_edit_ame2021.php - QUERY 10 */ UPDATE taquilla_opc_ame
					SET
					apu_mingan=%s
					WHERE cod_taquilla=%s",
                    GetSQLValueString($apuestasminimaausar, "int"),
...
$query_Recordset3 = sprintf(
    "/* PARSEADORES1 new\admin\taquillas_edit_ame2021.php - QUERY 11 */ SELECT ta.cod_taquilla, ta.nom_taquilla 
              ...
              <table width="920" align="center" border="0"  style="line-height:11px" cellpadding="0" cellspacing="0">
                ...
                <tr valign="baseline">
                  ...
                  <td>
                    ...
                    onKeyUp="return handleEnter(this, event)" tabindex="7" required   min="0" max="20" />
                ...
                <tr valign="baseline">
                  ...
                  <td>
                    ...
                    onKeyUp="return handleEnter(this, event)" tabindex="12" required   min="0" max="20" />
                ...
                <tr valign="baseline">
                  ...
                  <td>
                      <input type="number" name="reg_sho" class="textboxsmal" style="height:auto" onclick="ocultaDiv('Info15');"
                      value="<?php echo htmlentities($reg_sho, ENT_COMPAT, 'utf-8'); ?>" 
                      size="10" onkeypress="ValidaSoloNumeros()" title="regalia a esta jugada esta desativada"
                    onKeyUp="return handleEnter(this, event)" tabindex="16" required   min="0" max="20" />
