...
$query_Recordset3 =  sprintf(
  "/* PARSEADORES1 new\admin\usuarios_ve_desactivados.php - QUERY 1 */ SELECT * 
FROM 
usuario
WHERE id_usuario = %s AND est_usuario=0",
...
$query_Recordset1 =  sprintf(
    "/* PARSEADORES1 new\admin\usuarios_ve_desactivados.php - QUERY 2 */ SELECT
...
$query_Recordset94 =  sprintf(
  "/* PARSEADORES1 new\admin\usuarios_ve_desactivados.php - QUERY 3 */ SELECT
  nom_banca
FROM  taquilla ta, 
      banca ba,
      agencia ag
...
$est_taquilla=$row_Recordset1['est_taquilla']/1;
    ...
    $taq_cob_hnac=$row_Recordset1['taq_cob_hnac']/1;
        ...
        $query_Recordset11 = sprintf(
            "/* PARSEADORES1 new\admin\usuarios_ve_desactivados.php - QUERY 4 */ SELECT * FROM usuario 
	WHERE tip_usuario='U' 
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
    ...
    if ($graba==31) {
        $insertSQL1 = sprintf(
            "/* PARSEADORES1 new\admin\usuarios_ve_desactivados.php - QUERY 5 */ UPDATE taquilla 
...
curl_close ($ch);
        ...
        $query_Recordset22 =  sprintf(
            "/* PARSEADORES1 new\admin\usuarios_ve_desactivados.php - QUERY 6 */ SELECT  
tp.apu_mingan
	...
	LIMIT 1",
        ...
        if ($apuestasminimaausar > $row_Recordset22['apu_mingan']) {
            $insertSQL11 = sprintf(
                "/* PARSEADORES1 new\admin\usuarios_ve_desactivados.php - QUERY 7 */ UPDATE taquilla_opc_ame
					SET
					apu_mingan=%s
					WHERE cod_taquilla=%s",
...
$query_Recordset3 = sprintf(
    "/* PARSEADORES1 new\admin\usuarios_ve_desactivados.php - QUERY 8 */ SELECT ta.cod_taquilla, ta.nom_taquilla 
	...
	<div style="padding:70px 10px 20px 10px; text-align:left; font-size:18px; height: auto">
        ...
        <div style="height:100%; font-size:28px; padding:10px 10px 20px 10px; float:left;">
            <a href="taquillas_edit.php?recordID=<?php echo $row_Recordset1['cod_taquilla']; ?>" class="btn alert-success" 
            	style="font-size:18px; width:265px; height:40px; padding:5px 0px 0px 0px; text-align:center; background:success;
                text-decoration:none;" title="crear nuevo usuario vendedor">
                 Ver Lista de Vendedores Activos
            </a>
