...
$query_Recordsetl02 = sprintf(
  "/* PARSEADORES1 new\admin\caballos_lista.php - QUERY 1 */ SELECT DISTINCT
nom_hipodromo  
FROM 
carrera
...
if($D==0){
$query_Recordset1 = sprintf("/* PARSEADORES1 new\admin\caballos_lista.php - QUERY 2 */ SELECT * FROM carrera WHERE carrera.fec_carrera = %s", GetSQLValueString($xFechaCarrera_Recordset1, "date"));
}else{
$query_Recordset1 = sprintf("/* PARSEADORES1 new\admin\caballos_lista.php - QUERY 3 */ SELECT * FROM carrera WHERE carrera.fec_carrera = %s AND nom_hipodromo= %s", GetSQLValueString($xFechaCarrera_Recordset1, "date"),GetSQLValueString($_POST['leito'], "text"));  
}
...
function bRetirados2($identificador)
{
    ...
    $query_Recordset1 = sprintf("/* PARSEADORES1 new\admin\caballos_lista.php - QUERY 4 */ SELECT * FROM retirados WHERE retirados.cod_carrera = %s ORDER BY retirados.num_rcaballo ASC", $identificador, "int");
       ...
       <div style="height:40px; font-size:18px; padding:4px 0px 0px 4px; background: #333; color: #fff ">
           <form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1" autocomplete="off"  
                ...
                <select name="leito" id="soflow" style="height:40px; width:280px; margin:-9px 0px 0px 0px ">
                      <option value="">SELECIONE EL HIPODROMO</option>
                      <?php
                do {
                    ?>
               <option value="<?php echo $row_Recordsetl02['nom_hipodromo']?>"
               <?php if (strtoupper($row_Recordsetl02['nom_hipodromo'])) {
                        echo "SELECTED";
                    } ?>>
