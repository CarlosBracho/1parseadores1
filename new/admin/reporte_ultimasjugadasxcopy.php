        ...
        if ($_POST['fecha_inicio']!="" && $_POST['fecha_fin']!="") {
            ...
            if ($_POST['id_agente']!="todos") {
                $query_Recordset1 = sprintf(
                    "/* PARSEADORES1 new\admin\reporte_ultimasjugadasxcopy.php - QUERY 1 */ SELECT 
					...
					agencia.nom_agencia												
				FROM agencia, taquilla, venta, usuario, carrera 
				WHERE
				venta.cod_carrera = 525283 AND usuario.cod_taquilla = taquilla.cod_taquilla AND usuario.id_usuario = venta.id_usuario AND
				(venta.fec_venta >= %s AND venta.fec_venta <= %s OR venta.fec_pago >= %s AND venta.fec_pago <= %s) AND 
				taquilla.cod_agencia = agencia.cod_agencia AND venta.cod_carrera = carrera.cod_carrera AND agencia.cod_agencia = %s 
				ORDER BY venta.fec_pago, venta.hor_pago DESC LIMIT 0,100000",
...
if (((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1") && isset($_POST['id_agente']) && $_POST['id_agente']=="todos") || (!isset($_POST["MM_update"]))) {
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 new\admin\reporte_ultimasjugadasxcopy.php - QUERY 2 */ SELECT 
					venta.ticket, venta.ip_venta, venta.fec_venta, venta.hor_venta, venta.mon_venta, venta.cod_usuario_pago, 
					venta.pag_premio, venta.est_ticket, venta.est_calculo, venta.can_ticket, venta.hor_pago,
					taquilla.nom_taquilla, usuario.nom_usuario, 
					venta.cod_tventa, venta.fec_pago, venta.ip_pago,
					...
					carrera.num_carrera, carrera.nom_hipodromo,
					agencia.nom_agencia												
	FROM agencia, taquilla, carrera, usuario, venta

	WHERE
	venta.cod_carrera = 525283 AND usuario.cod_taquilla = taquilla.cod_taquilla AND usuario.id_usuario = venta.id_usuario AND
	(venta.fec_venta >= %s AND venta.fec_venta <= %s OR venta.fec_pago >= %s AND venta.fec_pago <= %s) AND 
	taquilla.cod_agencia = agencia.cod_agencia AND venta.cod_carrera = carrera.cod_carrera 
	ORDER BY venta.fec_pago, venta.hor_pago DESC LIMIT 0,8000",
        GetSQLValueString($in, "date"),
        GetSQLValueString($fi, "date"),
        GetSQLValueString($in, "date"),
        GetSQLValueString($fi, "date")
...
$query_Recordset2 = "/* PARSEADORES1 new\admin\reporte_ultimasjugadasxcopy.php - QUERY 3 */ SELECT cod_agencia, nom_agencia FROM agencia WHERE cod_agencia > 0 ORDER BY agencia.nom_agencia";
...
<body onload="Javascript:history.go(1);" onunload="Javascript:history.go(1);">
   <div style="background: #005232; width:114.8%; float:left; padding:50px 2px 2px 2px;
   		color:#FFF; font-size:28px; text-align:center">
        REPORTE DE ULTIMAS JUGADAS
   </div><!-- end .container -->
   ...
   <div id="tgeneral" style="background: #005232; width:98.5%; float:left; padding:12px 13px 2px 12px;
   		color:#FFF; font-size:18px;">
        <div></div>
   </div><!-- end .container -->
   <div style="background: #005232; width:98.5%; float:left; padding:12px 13px 2px 12px;
   		color:#FFF; font-size:18px;">
...
<table width="100%" border="0" style="font-size:10px" cellpadding="0" cellspacing="0">
  <tr style="background:#333; color:#FFFFFF; font-size:11px" valign="bottom">
...
$hora1=$row_Recordset1['hor_venta'];
    ...
    <td align="center"><?php echo $row_Recordset1['hor_pago']; ?></td>
...
<div style="background: #005232; width:113.3%; float:left; padding:12px 13px 2px 12px; text-align:center;
