...
$query_Recordset1 = sprintf(
	"/* PARSEADORES1 new\admin\taquillas_listadesactivadas.php - QUERY 1 */ SELECT 
	ta.nom_representante, ta.tel_taquilla, ta.tel_taquilla2, ta.tel_taquilla3, ta.cod_taquilla, ta.nom_taquilla, ag.nom_agencia, ta.est_taquilla, ba.nom_banca  
	FROM 
	taquilla ta,
	agencia ag,
  banca ba
	WHERE ba.cod_banca = ag.cod_banca AND ag.cod_agencia = ta.cod_agencia AND ta.est_taquilla = 0
...
<div style="height:100%; font-size:18px;" class="xfirefox">
        <div style="height:100%; font-size:28px; padding:10px 10px 10px 10px; float:right;">
            <a href="taquillas_add.php" class="btn alert-success" 
		        ...
		        <div style="height:100%; font-size:28px; padding:10px 10px 10px 10px; float:right;">
            <a href="taquillas_lista.php" class="btn alert-success" 
            	style="font-size:18px; width:165px; height:40px; padding:5px 0px 0px 0px; text-align:center; background:#9C0;
  ...
  <?php do { ?>
    <tr class="brillo" style="border-bottom:1px solid  #D5D5D5">
      ...
      <td align="left"><?php echo $row_Recordset1['nom_representante']; ?>
      </td>
