...
$query_mensajesyalertas = "/* PARSEADORES1 new\admin\admin_mensajesyalertas.php - QUERY 1 */ SELECT me.tipo, me.para, me.mensaje, me.mostrarhasta, me.Id_mensajesyalertas  FROM mensajesyalertas me WHERE (me.mostrarhasta >= CURDATE()) AND me.creadopor = 0 ORDER BY me.Id_mensajesyalertas DESC";
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
    ...
    if ($graba==31) {
        ...
        $updateSQL = sprintf(
            "/* PARSEADORES1 new\admin\admin_mensajesyalertas.php - QUERY 2 */ UPDATE mensajesyalertas 
...
<div class="container">
  <div class="header" style="height:100px; background:#333">
    ...
    <table width="100%" border="2" align="center" >
			<tr>
		<td>
		  <div class="contentAdmin2"><!-- InstanceBeginEditable name="Contenido" -->
  	<div style="height:100%; font-size:18px;" class="xfirefox">
            ...
            <a href="crear_mensaje_aagente.php" class="btn alert-success" 
            	style="font-size:18px; width:200px; height:70px; padding:5px 0px 0px 0px; text-align:center; background:#ffbf00;
                text-decoration:none;" title="Añadir Nuevo Mensaje A Taquilla">
                 Añadir Nuevo Mensaje A agente
            </a>
        </div>
			        <div style="height:100%; font-size:28px; padding:10px 10px 20px 10px; float:right;">
            <a href="crear_mensaje_aagentesdeundistribuidor.php" class="btn alert-success" 
            	style="font-size:18px; width:200px; height:70px; padding:5px 0px 0px 0px; text-align:center; background:#ffbf00;
                text-decoration:none;" title="Añadir Nuevo Mensaje A Taquillas De Un Agente">
                 Añadir Nuevo Mensaje A Agentes De Un Distribuidor
            </a>
        </div>
        <div style="height:100%; font-size:28px; padding:10px 10px 20px 10px; float:right;">
            <a href="crear_mensaje_aundistribuidor.php" class="btn alert-success" 
            	style="font-size:18px; width:200px; height:70px; padding:5px 0px 0px 0px; text-align:center; background:#ffbf00;
                text-decoration:none;" title="Añadir Nuevo Mensaje A Taquillas De Un Distribuidor">
                 Añadir Nuevo Mensaje A Un Distribuidor
            </a>
        </div>
		</td>
		      </tr>
  ...
  <div class="contentAdmin"><!-- InstanceBeginEditable name="Contenido" -->
  	<div style="height:100%; font-size:18px;" class="xfirefox">
				        <div style="height:100%; font-size:28px; padding:10px 10px 20px 10px; float:right;">
            <a href="crear_mensaje_ataquilla.php" class="btn alert-success" 
                ...
                text-decoration:none;" title="Añadir Nuevo Mensaje A Taquilla">
                 Añadir Nuevo Mensaje A Taquilla
			        ...
			        <div style="height:100%; font-size:28px; padding:10px 10px 20px 10px; float:right;">
            <a href="crear_mensaje_ataquillasdelagente.php" class="btn alert-success" 
            	style="font-size:18px; width:200px; height:70px; padding:5px 0px 0px 0px; text-align:center; background:#9C0;
                text-decoration:none;" title="Añadir Nuevo Mensaje A Taquillas De Un Agente">
        ...
        <div style="height:100%; font-size:28px; padding:10px 10px 20px 10px; float:right;">
            <a href="crear_mensaje_ataquillasdeldistribuidor.php" class="btn alert-success" 
            	style="font-size:18px; width:200px; height:70px; padding:5px 0px 0px 0px; text-align:center; background:#9C0;
                text-decoration:none;" title="Añadir Nuevo Mensaje A Taquillas De Un Distribuidor">
                 Añadir Nuevo Mensaje A Taquillas De Un Distribuidor
            </a>
        </div>
		</td>
		</tr>
...
if ($row_mensajesyalertas['tipo']==1) {
    $query_Recordset4 = sprintf(
        "/* PARSEADORES1 new\admin\admin_mensajesyalertas.php - QUERY 3 */ SELECT nom_taquilla FROM taquilla
...
if ($row_mensajesyalertas['tipo']==2) {
    $query_Recordset4 = sprintf(
        "/* PARSEADORES1 new\admin\admin_mensajesyalertas.php - QUERY 4 */ SELECT ag.nom_agencia FROM agencia ag
...
if ($row_mensajesyalertas['tipo']==3) {
    $query_Recordset4 = sprintf(
        "/* PARSEADORES1 new\admin\admin_mensajesyalertas.php - QUERY 5 */ SELECT ba.nom_banca FROM banca ba
...
if ($row_mensajesyalertas['tipo']==4) {
    $query_Recordset4 = sprintf(
        "/* PARSEADORES1 new\admin\admin_mensajesyalertas.php - QUERY 6 */ SELECT ag.nom_agencia FROM agencia ag
WHERE ag.cod_agencia = %s
ORDER BY RAND() LIMIT 1",
        GetSQLValueString($row_mensajesyalertas['para'], "int")
    );
    ...
    echo 'Agente'."<br/><FONT FACE='times new roman' SIZE=4>".$nomdedeagente."</FONT>";
}
if ($row_mensajesyalertas['tipo']==5) {
    $query_Recordset4 = sprintf(
        "/* PARSEADORES1 new\admin\admin_mensajesyalertas.php - QUERY 7 */ SELECT ba.nom_banca FROM banca ba
WHERE ba.cod_banca = %s
ORDER BY RAND() LIMIT 1",
        GetSQLValueString($row_mensajesyalertas['para'], "int")
    );
    ...
    echo 'Agentes del Distribuidor'."<br/><FONT FACE='times new roman' SIZE=4>".$nomdedistribuidor."</FONT>";
}
if ($row_mensajesyalertas['tipo']==6) {
    $query_Recordset4 = sprintf(
        "/* PARSEADORES1 new\admin\admin_mensajesyalertas.php - QUERY 8 */ SELECT ba.nom_banca FROM banca ba
WHERE ba.cod_banca = %s
ORDER BY RAND() LIMIT 1",
        GetSQLValueString($row_mensajesyalertas['para'], "int")
    );
    ...
    echo 'Distribuidor'."<br/><FONT FACE='times new roman' SIZE=4>".$nomdedistribuidor."</FONT>";
}
 ?>
