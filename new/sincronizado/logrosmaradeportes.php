...
$query_delete =  sprintf(
  "DELETE FROM p3logros WHERE logrodtp3 >= %s",
  GetSQLValueString('2022-08-04 00:00:01', "date")
);
$Result1 = mysqli_query($conexionbanca, $query_delete) or die(mysqli_error($conexionbanca));
*/
$query_Recordset1_sino_seejecuta = sprintf(
  "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 1 */ SELECT * FROM 
alertas 
WHERE  
idalertas=10");
$Recordset1_sino_seejecuta = mysqli_query($conexionbanca, $query_Recordset1_sino_seejecuta) or die(mysqli_error($conexionbanca));
$row_Recordset1_sino_seejecuta = mysqli_fetch_assoc($Recordset1_sino_seejecuta);
$totalRows_Recordset1_sino_seejecuta = mysqli_num_rows($Recordset1_sino_seejecuta);
if($totalRows_Recordset1_sino_seejecuta>=1 && $row_Recordset1_sino_seejecuta['activo_archivo']==0){
...
$query_Recordset18D =  sprintf(
  "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 2 */ SELECT  
*
...
if($row_Recordset18D['Swicht']==1){
    //inicio funciones1
    function verificarlogro($yaestaequipo1cod, $logro, $tipojugada, $idp10juego, $equipo1o2, $logrodate, $ArrayLogros2, $Fechahoramarafun, $valorabrl){
        ...
        foreach ($ArrayLogros2 as $clave=>$valor2) {
          if($valor2["idjuegop3"]==$idp10juego && $valor2["equipop3"]==$equipo1o2 && $valor2["tipojugadap3"]==$tipojugada){  $yaestalg=1; $Id_p3logrosp3=$valor2["Id_p3logrosp3"]; }
          
          }
    
          if($idp10juego>0 && $yaestalg==1){
    
    //var_dump($ArrayLogros2);
          foreach ($ArrayLogros2 as $clave=>$valor2) {
            if($valor2["idjuegop3"]==$idp10juego && $valor2["equipop3"]==$equipo1o2 && $valor2["tipojugadap3"]==$tipojugada && $valor2["logrop3"]==$logro  && $valor2["logroABoRLp3"]==$valorabrl){  $yaestalgperonoigual=1; $Id_p3logrosp3=$valor2["Id_p3logrosp3"];
    ...
    if($idp10juego>0 && $yaestalg==0 && $yaestaequipo1cod>0){
    
    
    $insertSQL = sprintf(
      "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 3 */ INSERT 
    INTO p3logros
    ...
    $insertSQL = sprintf(
      "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 4 */ INSERT 
    INTO p7histolg
    ...
    //echo 'error1 '.$idp10juego.' error2 '.$yaestalgperonoigual.' error2 '.$Id_p3logrosp3;
    if($idp10juego>0 && $yaestalgperonoigual==0 &&  $Id_p3logrosp3>0){
    //echo ' es distinto';
    $logroactualizado=1;
      $insertSQL1 = sprintf(
          "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 5 */ UPDATE p3logros 
      ...
      $insertSQL = sprintf(
        "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 6 */ INSERT 
      INTO p7histolg
      (idjuegop7, Id_p1equiposp7, equipop7, tipojugadap7, logrop7, logroABoRLp7, actxp7) 
    VALUES (%s, %s, %s, %s, %s, %s, %s)",
    GetSQLValueString($idp10juego, "int"),
    ...
    //inicio funciones2   Id_p3logrosp3, idjuegop3, equipop3, tipojugadap3, logroABoRLp3, logrop3

    function verificarlogro($yaestaequipo1cod, $logro, $tipojugada, $idp10juego, $equipo1o2, $logrodate, $ArrayLogros2, $Fechahoramarafun, $valorabrl){
        //var_dump($ArrayLogros2[5]); echo '<br>';
        //echo $yaestaequipo1cod.' - '.$logro.' - '.$tipojugada.' - '.$idp10juego.' - '.$equipo1o2.' - '.$logrodate.' - '.$Fechahoramarafun.' - '.$valorabrl.' -<br> ';
    ...
    $insertSQL = sprintf(
    "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 7 */ INSERT 
    INTO p10logros
    (idjuegop3, Id_p1equiposp3, equipop3, tipojugadap3, logrop3, logrodtp3, logroABoRLp3, actxp3) 
    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
    GetSQLValueString($idp10juego, "int"),
    ...
    $insertSQL = sprintf(
    "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 8 */ INSERT 
    INTO p7histolg
    (idjuegop7, Id_p1equiposp7, equipop7, tipojugadap7, logrop7, logroABoRLp7, actxp7) 
    VALUES (%s, %s, %s, %s, %s, %s, %s)",
    GetSQLValueString($idp10juego, "int"),
    ...
    $insertSQL1 = sprintf(
        "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 9 */ UPDATE p10logros 
      SET logrop3=%s, logroABoRLp3=%s	
      WHERE 
      Id_p3logrosp3=%s",
    ...
    $insertSQL = sprintf(
      "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 10 */ INSERT 
    INTO p7histolg
    (idjuegop7, Id_p1equiposp7, equipop7, tipojugadap7, logrop7, logroABoRLp7, actxp7) 
    VALUES (%s, %s, %s, %s, %s, %s, %s)",
    GetSQLValueString($idp10juego, "int"),
...
$query_Recordset18D =  sprintf(
    "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 11 */ SELECT  
*
            FROM  opciones_parley
            WHERE id_opcionp=%s
            LIMIT 1",
...
$query_RecordsetAE =  sprintf(
    "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 12 */ SELECT
...
$fp = fopen('maradeporteslogros.pdf', 'w');
fwrite($fp, $datoscurl);
fclose($fp);
*/
...
if(strpos($datoscurl, 'HOJAs7sDEs7sLOGROSs7sDELs7sDIAs7s')) { 

  //inicio regitro alerta
echo 'si es la pagina correcta de logros<br>';


$updateSQL = sprintf("/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 13 */ UPDATE alertas SET contadoralerta=contadoralerta+1
WHERE idalertas=%s",
...
if($datoscurl2ini[5][0]=='SEPTIEMBRE'){$mes='09';} //SETIEMBRE
if($datoscurl2ini[5][0]=='SETIEMBRE'){$mes='09';}
...
if($row_Recordset18D['Swicht']==1){
    ...
    $query_RecordsetAJ =  sprintf(
        "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 14 */ SELECT
      Id_p2juegosp2, idequipo1p2, idequipo2p2, deportep2, competicionp2, iniciodtp2
      FROM  
      p2juegos
      WHERE 
      iniciodtp2 >= %s  ORDER BY Id_p2juegosp2 DESC",
      GetSQLValueString($FechaTxtMARA.' 00:00:00', "date"));
    ...
    $idequipo2p2=$valorAJ["idequipo2p2"];
     ...
     $ArrayJuegosCN[] = array('Id_p2juegosp2' => $Id_p2juegosp2, 'nommara1' => $nommara1, 'nommarapais1' => $nommarapais1, 'idequipo1p2' => $idequipo1p2, 'nommara2' => $nommara2, 'nommarapais2' => $nommarapais2, 'idequipo2p2' => $idequipo2p2, 'iniciodtp2' => $iniciodtp2, 'deportep2' => $deportep2);
    ...
    $query_RecordsetLG =  sprintf(
        "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 15 */ SELECT
    ...
    //inicio array de juegos
    $query_RecordsetAJ =  sprintf(
        "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 16 */ SELECT
      Id_p2juegosp2, idequipo1p2, idequipo2p2, deportep2, competicionp2, iniciodtp2
      FROM  
      p10juegos
      WHERE 
    ...
    $query_RecordsetLG =  sprintf(
        "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 17 */ SELECT
      Id_p3logrosp3, idjuegop3, equipop3, tipojugadap3, logroABoRLp3, logrop3
      FROM  
      p10logros
      WHERE logrodtp3 >= %s AND Id_p3logrosp3 >= 0 ORDER BY Id_p3logrosp3 DESC",
...
//sts0brstss7sbFUTBOLs7sAMERICANOs7s..ZNFLZ..stsbbrstss7s

if(strpos($datoscurl222, 'UTBOLs7sAMERICANOs7s..ZNFLZ..')){
  preg_match_all("((.*)sts0brstss7sbFUTBOLs7sAMERICANOs7s..ZNFLZ..stsbbrstss7s(.*))siU", $datoscurl222, $datoscurl223);
  $hora = substr($datoscurl222, -7);
  $hora=str_replace('sxs', ':', $hora);
  $competicion='NFL';
  $deporte='FUTBOLAMERICANO';

}
//FUTBOL AMERICANO COLEGIAL
//rstss7sbFUTBOLs7sAMERICANOs7sCOLEGIALstsbbrstss7s05sxs00
//brstss7s0sts0brstss7sbFUTBOLs7sAMERICANOs7sCOLEGIALstsbbrstss7s05sxs00corteaquipmbrstss7sL
if(strpos($datoscurl222, 'UTBOLs7sAMERICANOs7sCOLEGIALsts')){
  preg_match_all("((.*)sts0brstss7sbFUTBOLs7sAMERICANOs7sCOLEGIALstsbbrstss7s(.*))siU", $datoscurl222, $datoscurl223);
  $hora = substr($datoscurl222, -7);
  $hora=str_replace('sxs', ':', $hora);
  $competicion='NCAA';
  $deporte='FUTBOLAMERICANO';
...
//if(strpos($datoscurl222, 'FUTBOL')){
  if(!strpos($datoscurl222, 'COPA')){
    if(!strpos($datoscurl222, 'AMERICANO')){
  ...
  //fin futbol con copa
  //inicio FUTBOL CLASIFICACION INTERCONTINENTAL MUNDIAL
  //FUTBOLs7sCLASIFICACIONs7sINTERCONTINENTALs7sMUNDIALstsbbrstss7s
  if(strpos($datoscurl222, 'INTERCONTINENTALs7sMUNDIAL')){
    if(strpos($datoscurl222, 'UTBOL')){

    if(strpos($datoscurl222, 'stsbbrstss7s')){
      //preg_match_all("((.*)s7sCANADAs7s(.*)stsbbrstss(.*))siU", $datoscurl222, $datoscurl223);

      preg_match_all("((.*)s7sCLASIFICACIONs7sINTERCONTINENTALs7sMUNDIALstsbbrstss7s(.*))siU", $datoscurl222, $datoscurl223);
  
    $hora = substr($datoscurl222, -7);
    $hora=str_replace('sxs', ':', $hora);
    $competicion=str_replace('s7s', ' ', 's7sCLASIFICACIONs7sINTERCONTINENTALs7sMUNDIAL');
    $deporte=$datoscurl223[1][0];
  //echo 'Deporte '.$deporte.' competicion '.$competicion.' hora '.$hora.'<br>';
  ...
  //fin FUTBOL CLASIFICACION INTERCONTINENTAL MUNDIAL
...
$Fechahoramara = date('Y-m-d H:i:s', $Fechahoramara);
//if($deporte==0 && $competicion==0){ echo $datoscurl222;    }
...
if($deporte=='FUTBOLAMERICANO'){$deportecod=4; $deportenombre='futbolamericano'; }
if($deporte=='HOCKEY'){$deportecod=5; $deportenombre='hockey'; }
$ArrayEquiposEnMara[] =  array('equipo1' => $equipo1, 'equipo2' => $equipo2, 'deporte' => $deportenombre, 'deportecod' => $deportecod);

$yaestaequipo1=0;
$yaestaequipo2=0;
$yaestaequipo1cod=0;
$yaestaequipo2cod=0;
foreach ($ArrayEquipos as $clave=>$valore) {


  if($valore["nommara"]==$equipo1 && $valore["deportep1"]==$deportecod){    $yaestaequipo1=1; $yaestaequipo1cod=$valore["Id_p1equiposp1"];  }
  if($valore["nommara"]==$equipo2 && $valore["deportep1"]==$deportecod){    $yaestaequipo2=1; $yaestaequipo2cod=$valore["Id_p1equiposp1"];  }

  if($yaestaequipo1==0){
    $equipo112=$valore["nommara"].' ('.$valore["nommarapais"].')';
    if($equipo112==$equipo1 && $valore["deportep1"]==$deportecod){  $yaestaequipo1=1; $yaestaequipo1cod=$valore["Id_p1equiposp1"];  }
  }
  if($yaestaequipo2==0){
    $equipo114=$valore["nommara"].' ('.$valore["nommarapais"].')';
    if($equipo114==$equipo2 && $valore["deportep1"]==$deportecod){  $yaestaequipo2=1; $yaestaequipo2cod=$valore["Id_p1equiposp1"];  }
  }
...
foreach ($ArrayJuegosCN as $clave=>$valor2) {
  ...
  if($valor2["iniciodtp2"]<>$Fechahoramara){ 
    ...
    echo 'se cambiara la hora <br>'.$Fechahoramara.' '.$valor2["iniciodtp2"];
...
$fechasistemamas305 = strtotime('+305 minute', strtotime($fechahora));
$fechasistemamas305  = date('Y-m-d', $fechasistemamas305);


if($yaestajuego==0 && $FechaTxtMARA==$fechasistemamas305){ echo 'se creara el juego<br>'; 
//esta es la fecha de mara $FechaTxtMARA
//$fechahora del sistema
  ...
  if($row_Recordset18D['Swicht']==1){
    ...
    $query_Recordsetvrfjuego =  sprintf(
      "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 18 */ SELECT  
  deportep2
              FROM  p2juegos
              WHERE 
              iniciodtp2=%s
...
$insertSQL = sprintf(
"/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 19 */ INSERT 
INTO p2juegos
(idequipo1p2, idequipo2p2, deportep2, competicionp2, iniciodtp2, jexterno ) 
VALUES (%s, %s, %s, %s, %s, %s)",
    GetSQLValueString($yaestaequipo1cod, "int"),
    GetSQLValueString($yaestaequipo2cod, "int"),
...
$Result1 = mysqli_query($conexionbanca, $insertSQL) or die(mysqli_error($conexionbanca));
  ...
  $query_Recordsetvrfjuego =  sprintf(
    "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 20 */ SELECT  
deportep2
            FROM  p10juegos
            WHERE 
            iniciodtp2=%s
  ...
  if($totalRows_Recordsetvrfjuego==0){
  $insertSQL = sprintf(
    "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 21 */ INSERT 
    INTO p10juegos
    (idequipo1p2, idequipo2p2, deportep2, competicionp2, iniciodtp2, jexterno) 
    VALUES (%s, %s, %s, %s, %s, %s)",
        GetSQLValueString($yaestaequipo1cod, "int"),
...
}else{ echo 'ya esta creara el juego >>> '.$equipo1.' '.$equipo2.' '.$idp10juego.'<br>';
  $juegos++;
  ...
  if($row_Recordset18D['Swicht']==1){
    ...
    $insertSQL24 = sprintf(
      "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 22 */ UPDATE p2juegos 
SET iniciodtp2=%s			
WHERE 
Id_p2juegosp2 = %s", 
...
echo $Result24.'Result24';
$updateSQL = sprintf("/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 23 */ UPDATE p2juegos SET p2vecesactualizado=p2vecesactualizado+1
WHERE  
Id_p2juegosp2 = %s ",
GetSQLValueString($idp10juego, "int"));
$Result1 = mysqli_query($conexionbanca, $updateSQL) or die(mysqli_error($conexionbanca));
    ...
    $insertSQL24 = sprintf(
      "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 24 */ UPDATE p10juegos 
SET iniciodtp2=%s			
WHERE 
Id_p2juegosp2 = %s", 
...
if(strlen($deporte)<='2'){ 
  $error2 ='hay un error de codigo con el juego que contiene los equipos '.$equipo1.' '.$equipo2.' '.$idp10juego;
  $error=$error+$error2;
}
//  if(strlen($datoscurl2)>='50') {
...
if($yaestajuego==1 && $yaestajuegohorajuego==1){ echo 'se actualiza hora del juego<br>'; 
  ...
  if($row_Recordset18D['Swicht']==1){
    ...
    $insertSQL24 = sprintf(
      "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 25 */ UPDATE p2juegos 
SET iniciodtp2=%s			
WHERE 
Id_p2juegosp2 = %s", 
  ...
  $Result24 = mysqli_query($conexionbanca, $insertSQL24);
    ...
    $insertSQL24 = sprintf(
      "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 26 */ UPDATE p10juegos 
SET iniciodtp2=%s			
WHERE 
Id_p2juegosp2 = %s", 
...
if($JuegosQEstan==','){ $JuegosQEstan=$idp10juego; }else{$JuegosQEstan=','.$JuegosQEstan.','.$idp10juego;}

  $tipojugada='ML';
  list($logroactualizado)=verificarlogro($yaestaequipo1cod, $datoscurl2223[3][0], $tipojugada, $idp10juego, 1, $logrodate, $ArrayLogros, $Fechahoramara, $nov); if($logroactualizado==1){ $la++; $logroactualizado=0; } 


  $tipojugada='ML';
  list($logroactualizado)=verificarlogro($yaestaequipo2cod, $datoscurl2223[20][0], $tipojugada, $idp10juego, 2, $logrodate, $ArrayLogros, $Fechahoramara, $nov); if($logroactualizado==1){ $la++; $logroactualizado=0; } 


  $tipojugada='RL';
  list($logroactualizado)=verificarlogro($yaestaequipo1cod, $datoscurl2223[5][0], $tipojugada, $idp10juego, 1, $logrodate, $ArrayLogros, $Fechahoramara, $datoscurl2223[4][0]); if($logroactualizado==1){ $la++; $logroactualizado=0; }   


  $tipojugada='RL';
  list($logroactualizado)=verificarlogro($yaestaequipo2cod, $datoscurl2223[22][0], $tipojugada, $idp10juego, 2, $logrodate, $ArrayLogros, $Fechahoramara, $datoscurl2223[21][0]); if($logroactualizado==1){ $la++; $logroactualizado=0; }   


  $tipojugada='A';
  list($logroactualizado)=verificarlogro($yaestaequipo1cod, $datoscurl2223[7][0], $tipojugada, $idp10juego, 1, $logrodate, $ArrayLogros, $Fechahoramara, $datoscurl2223[6][0]); if($logroactualizado==1){ $la++; $logroactualizado=0; }  
  $tipojugada='B';
  list($logroactualizado)=verificarlogro($yaestaequipo2cod, $datoscurl2223[24][0], $tipojugada, $idp10juego, 2, $logrodate, $ArrayLogros, $Fechahoramara, $datoscurl2223[23][0]); if($logroactualizado==1){ $la++; $logroactualizado=0; }   


  $tipojugada='SI';
  list($logroactualizado)=verificarlogro($yaestaequipo1cod, $datoscurl2223[8][0], $tipojugada, $idp10juego, 1, $logrodate, $ArrayLogros, $Fechahoramara, $nov); if($logroactualizado==1){ $la++; $logroactualizado=0; }  

  $tipojugada='NO';
  list($logroactualizado)=verificarlogro($yaestaequipo1cod, $datoscurl2223[25][0], $tipojugada, $idp10juego, 2, $logrodate, $ArrayLogros, $Fechahoramara, $nov); if($logroactualizado==1){ $la++; $logroactualizado=0; }  

    $tipojugada='AP';
    list($logroactualizado)=verificarlogro($yaestaequipo1cod, $datoscurl2223[9][0], $tipojugada, $idp10juego, 1, $logrodate, $ArrayLogros, $Fechahoramara, $nov); if($logroactualizado==1){ $la++; $logroactualizado=0; }  

    $tipojugada='AP';
    list($logroactualizado)=verificarlogro($yaestaequipo1cod, $datoscurl2223[26][0], $tipojugada, $idp10juego, 2, $logrodate, $ArrayLogros, $Fechahoramara, $nov); if($logroactualizado==1){ $la++; $logroactualizado=0; }  
    


      $tipojugada='AG';
      list($logroactualizado)=verificarlogro($yaestaequipo1cod, $datoscurl2223[11][0], $tipojugada, $idp10juego, 1, $logrodate, $ArrayLogros, $Fechahoramara, $datoscurl2223[10][0]); if($logroactualizado==1){ $la++; $logroactualizado=0; }  

      $tipojugada='BG';
      list($logroactualizado)=verificarlogro($yaestaequipo1cod, $datoscurl2223[28][0], $tipojugada, $idp10juego, 2, $logrodate, $ArrayLogros, $Fechahoramara, $datoscurl2223[27][0]); if($logroactualizado==1){ $la++; $logroactualizado=0; }  

      $tipojugada='SRL';
    list($logroactualizado)=verificarlogro($yaestaequipo1cod, 0, $tipojugada, $idp10juego, 1, $logrodate, $ArrayLogros, $Fechahoramara, 0); if($logroactualizado==1){ $la++; $logroactualizado=0; }  


    $tipojugada='SRL';
    list($logroactualizado)=verificarlogro($yaestaequipo2cod, 0, $tipojugada, $idp10juego, 2, $logrodate, $ArrayLogros, $Fechahoramara, 0); if($logroactualizado==1){ $la++; $logroactualizado=0; }  
...
//var_dump($datoscurl2223[25][0]);
  ...
  $tipojugada='EML';
  list($logroactualizado)=verificarlogro($yaestaequipo1cod, $datoscurl2223[18][0], $tipojugada, $idp10juego, 0, $logrodate, $ArrayLogros, $Fechahoramara, $nov); if($logroactualizado==1){ $la++; $logroactualizado=0; }  
  ...
  $tipojugada='5ML';
  list($logroactualizado)=verificarlogro($yaestaequipo1cod, $datoscurl2223[13][0], $tipojugada, $idp10juego, 1, $logrodate, $ArrayLogros, $Fechahoramara, $nov); if($logroactualizado==1){ $la++; $logroactualizado=0; }  


  $tipojugada='5ML';
  list($logroactualizado)=verificarlogro($yaestaequipo2cod, $datoscurl2223[30][0], $tipojugada, $idp10juego, 2, $logrodate, $ArrayLogros, $Fechahoramara, $nov); if($logroactualizado==1){ $la++; $logroactualizado=0; }  


    $tipojugada='5RL';
    list($logroactualizado)=verificarlogro($yaestaequipo1cod, $datoscurl2223[17][0], $tipojugada, $idp10juego, 1, $logrodate, $ArrayLogros, $Fechahoramara, $datoscurl2223[16][0]); if($logroactualizado==1){ $la++; $logroactualizado=0; }  


    $tipojugada='5RL';
    list($logroactualizado)=verificarlogro($yaestaequipo2cod, $datoscurl2223[17][0], $tipojugada, $idp10juego, 2, $logrodate, $ArrayLogros, $Fechahoramara, $datoscurl2223[33][0]); if($logroactualizado==1){ $la++; $logroactualizado=0; }  


    $tipojugada='5A';
    list($logroactualizado)=verificarlogro($yaestaequipo1cod, $datoscurl2223[15][0], $tipojugada, $idp10juego, 1, $logrodate, $ArrayLogros, $Fechahoramara, $datoscurl2223[14][0]); if($logroactualizado==1){ $la++; $logroactualizado=0; }  


    $tipojugada='5B';
    list($logroactualizado)=verificarlogro($yaestaequipo2cod, $datoscurl2223[32][0], $tipojugada, $idp10juego, 2, $logrodate, $ArrayLogros, $Fechahoramara, $datoscurl2223[31][0]); if($logroactualizado==1){ $la++; $logroactualizado=0; }  
...
if($juegos <>0) {
  ...
  $query_Recordset1b = sprintf(
      "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 27 */ SELECT Id_p2juegosp2 FROM p2juegos WHERE 
  iniciodtp2 > %s AND
  idequipo1p2 > 0 AND
  idequipo1p2 > 0 AND
    jexterno = 0 
  ...
  DESC",
      ...
      $query_Recordset1b = sprintf(
          "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 28 */ SELECT Id_p2juegosp2 FROM p10juegos WHERE 
      iniciodtp2 > %s AND
      idequipo1p2 > 0 AND
      idequipo1p2 > 0 AND
        jexterno = 0
...
if(!strpos($JuegosQEstan, $row_Recordset1b['Id_p2juegosp2'])){
  ...
  if($row_Recordset18D['Swicht']==1){
      $insertSQL24 = sprintf(
        "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 29 */ UPDATE p2juegos 
  SET iniciodtp2 = %s			
  WHERE 
  Id_p2juegosp2 = %s", 
    ...
    $Result24 = mysqli_query($conexionbanca, $insertSQL24);
      ...
      $insertSQL24 = sprintf(
        "/* PARSEADORES1 new\sincronizado\logrosmaradeportes.php - QUERY 30 */ UPDATE p10juegos 
  SET iniciodtp2 = %s			
  WHERE 
  Id_p2juegosp2 = %s", 
    ...
    $Result24 = mysqli_query($conexionbanca, $insertSQL24);
      ...
      echo 'se cierra<br>';
      $msj='Mara 9999 no tiene el siguiente juego por lo tanto se cerrara'.$row_Recordset1b['Id_p2juegosp2'];
...
echo '<br>si es la apgina de logros correcta<br>';
}else{ 
  $errorpagina=1;
  
  echo '<br>no es la apgina de logros correcta<br>';}//fin de inicio si es la pagina de logros correcta
...
$sitelegram=0;
if($juegos>0){
    $msj='Marad BD'.$bd.' Se act '.$juegos.' jue y '.$la.' lg  fatan '.$noestaequipo.' equipos t '.$tiempo.' V3.1 '.$error;
    $sitelegram=1;
  }
  if($errorpagina==1){
    $msj='Marad BD'.$bd.' Maradeportes esta fallando en cargar la apgina correctamente  V3.1 '.$error;
    $sitelegram=1;
  }
