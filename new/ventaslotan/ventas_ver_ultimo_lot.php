...
$query_Recordset12 = sprintf("/* PARSEADORES1 new\ventaslotan\ventas_ver_ultimo_lot.php - QUERY 1 */ SELECT ticket_lot FROM venta_lot WHERE id_usuario = %s AND fec_venta_lot = %s AND est_ticket_lot >= 0 AND est_ticket_lot <= 1 AND tip_loteria_lot >= 4
...
$query_Recordset11 = sprintf(
    "/* PARSEADORES1 new\ventaslotan\ventas_ver_ultimo_lot.php - QUERY 2 */ SELECT ser_ticket_lot, est_ticket_lot, mon_apuesta_lot,
...
if ($totalRows_Recordset11>0 && $row_Recordset11['tot_apuesta']>0) {
    ...
    if ($estadoTicket==1) { ?>
            ...
            value="REIMPRIMIR" onclick="javascript:window.open('../ventaslot/ventas_reimprimir_ultimo_lot.php?tID=2','_blank','width=230,height=620,left=0, top=0,toolbar=no,menubar=no,scrollbars=no,status=no,Aresizable=no,location=no,fullscreen=no,dependent=yes');return false;"/>
	<?php
        }
    if ($estadoTicket==0) {
