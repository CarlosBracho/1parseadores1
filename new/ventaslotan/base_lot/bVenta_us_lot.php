<?php
    ...
    "/* PARSEADORES1 new\ventaslotan\base_lot\bVenta_us_lot.php - QUERY 1 */ SELECT 
					us.nom_usuario,
					ta.nom_taquilla,
					ta.cod_taquilla,
					tp.por_taquilla_lot,
					...
					ve.fec_venta_lot <= %s OR 
					ve.fec_pago_lot >= %s AND 
					ve.fec_pago_lot <= %s) AND 
    ...
    GetSQLValueString($codigoUsuario, "int")
                ...
                $query_Recordset = sprintf(
                    "/* PARSEADORES1 new\ventaslotan\base_lot\bVenta_us_lot.php - QUERY 2 */ SELECT cod_taquilla FROM taquilla_opc_lot WHERE cod_taquilla = %s LIMIT 1",
                ...
                $ApagoSistema=(($row_Recordset81['total_venta_lot']-$totAnuTaquilla)*$row_Recordset81['por_taquilla_lot'])/100;
