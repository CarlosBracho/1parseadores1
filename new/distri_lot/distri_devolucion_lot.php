...
$cDia=loteriaHoyAdmin($diahoy);
$query_Recordset2 = "/* PARSEADORES1 new\distri_lot\distri_devolucion_lot.php - QUERY 1 */ SELECT id_signo, nom_corto FROM signos ORDER BY id_signo";
$Recordset2 = mysqli_query($conexionbanca, $query_Recordset2) or die(mysqli_error($conexionbanca));
$row_Recordset2 = mysqli_fetch_assoc($Recordset2);
$totalRows_Recordset2 = mysqli_num_rows($Recordset2);
$query_Recordset3 =  sprintf("/* PARSEADORES1 new\distri_lot\distri_devolucion_lot.php - QUERY 2 */ SELECT so.id_sorteo, so.nom_sorteo
	FROM loterias lo, sorteos so, bancaloterias bl 
	WHERE ((lo.tip_loteria = 1 OR lo.tip_loteria>=3) AND lo.est_loteria=1) AND 
	so.id_sorteo = lo.id_sorteo  AND so.est_sorteo=1 AND bl.id_banca = %s AND bl.id_loteria = lo.id_loteria
	GROUP BY so.id_sorteo
	ORDER BY lo.nom_loteria ASC", GetSQLValueString($id_banca, "int"));
$Recordset3 = mysqli_query($conexionbanca, $query_Recordset3) or die(mysqli_error($conexionbanca));
$row_Recordset3 = mysqli_fetch_assoc($Recordset3);
$totalRows_Recordset3 = mysqli_num_rows($Recordset3);

$query_Recordset4 =  sprintf("/* PARSEADORES1 new\distri_lot\distri_devolucion_lot.php - QUERY 3 */ SELECT ag.cod_agencia, ag.nom_agencia 
	FROM agencia ag, agencialoterias al, taquilla ta, taquilla_opc_lot tp 
	WHERE ag.cod_agencia = al.id_agencia AND ta.cod_agencia = ag.cod_agencia AND 
		ta.cod_taquilla = tp.cod_taquilla AND ag.cod_banca = %s
	GROUP BY ag.cod_agencia
	ORDER BY ag.nom_agencia ASC", GetSQLValueString($id_banca, "int"));
...
$(document).ready(function() {$("#reloj").load('../includes/reloj.php?&js='+Math.random());var refreshId1 = setInterval(function() {$("#reloj").load('../includes/reloj.php?&js='+Math.random());}, 60000);$('#divS').fadeOut(12000);$('#divE').fadeOut(12000);
	...
	$('#procesar').click(function(){
		...
		if (resp == true) {
			var url = "../admin_lot/procesar_devolucion_lot.php"; form1=this.form;
