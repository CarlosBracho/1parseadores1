        ...
        if ($_POST['fecha_inicio']!="" && $_POST['fecha_fin']!="") {
            ...
            if ($_POST['id_agente']!="todos") {
                $query_Recordset1 = sprintf(
                    "/* PARSEADORES1 new\distri_lot\distri_reporte_ultimasjugadas_lot.php - QUERY 1 */ SELECT 
					ve.ip_venta_lot,
					ve.est_ticket_lot,
					ve.ser_ticket_lot,
					ve.est_calculo_lot,
					...
					(/* PARSEADORES1 new\distri_lot\distri_reporte_ultimasjugadas_lot.php - QUERY 2 */ SELECT usu.nom_usuario FROM usuario usu WHERE usu.id_usuario = ve.cod_usuario_pago_lot) AS nom_pago_lot,
					CASE ve.tip_loteria_lot  
						WHEN 2 THEN (/* PARSEADORES1 new\distri_lot\distri_reporte_ultimasjugadas_lot.php - QUERY 3 */ SELECT sor.nom_corto FROM signos sor WHERE sor.id_signo = ve.id_signo LIMIT 1)
						WHEN 3 THEN (/* PARSEADORES1 new\distri_lot\distri_reporte_ultimasjugadas_lot.php - QUERY 4 */ SELECT sor.nom_corto FROM signos sor WHERE sor.id_signo = ve.id_signo LIMIT 1)
						WHEN 4 THEN (/* PARSEADORES1 new\distri_lot\distri_reporte_ultimasjugadas_lot.php - QUERY 5 */ SELECT ani.nom_animal FROM animales ani WHERE ani.num_animal = ve.num_apuesta_lot LIMIT 1)
						WHEN 5 THEN (/* PARSEADORES1 new\distri_lot\distri_reporte_ultimasjugadas_lot.php - QUERY 6 */ SELECT fru.nom_corto FROM frutas fru WHERE fru.id_fruta = ve.id_signo LIMIT 1)
						WHEN 6 THEN (/* PARSEADORES1 new\distri_lot\distri_reporte_ultimasjugadas_lot.php - QUERY 7 */ SELECT pal.nom_corto FROM palos_cartas pal WHERE pal.id_palo = ve.id_signo LIMIT 1)
						ELSE ''
					END AS nsigno,
					lo.nom_loteria,
					ta.nom_taquilla,
...
if (((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1") && $_POST['id_agente']=="todos") ||
    ...
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 new\distri_lot\distri_reporte_ultimasjugadas_lot.php - QUERY 8 */ SELECT 
		ve.ip_venta_lot,
		ve.est_ticket_lot,
		ve.ser_ticket_lot,
		ve.est_calculo_lot,
		...
		ve.hor_pago_lot,
		ve.ip_pago_lot,
		(/* PARSEADORES1 new\distri_lot\distri_reporte_ultimasjugadas_lot.php - QUERY 9 */ SELECT usu.nom_usuario FROM usuario usu WHERE usu.id_usuario = ve.cod_usuario_pago_lot) AS nom_pago_lot,
		CASE ve.tip_loteria_lot  
			WHEN 2 THEN (/* PARSEADORES1 new\distri_lot\distri_reporte_ultimasjugadas_lot.php - QUERY 10 */ SELECT sor.nom_corto FROM signos sor WHERE sor.id_signo = ve.id_signo LIMIT 1)
			WHEN 3 THEN (/* PARSEADORES1 new\distri_lot\distri_reporte_ultimasjugadas_lot.php - QUERY 11 */ SELECT sor.nom_corto FROM signos sor WHERE sor.id_signo = ve.id_signo LIMIT 1)
			WHEN 4 THEN (/* PARSEADORES1 new\distri_lot\distri_reporte_ultimasjugadas_lot.php - QUERY 12 */ SELECT ani.nom_animal FROM animales ani WHERE ani.num_animal = ve.num_apuesta_lot LIMIT 1)
			WHEN 5 THEN (/* PARSEADORES1 new\distri_lot\distri_reporte_ultimasjugadas_lot.php - QUERY 13 */ SELECT fru.nom_corto FROM frutas fru WHERE fru.id_fruta = ve.id_signo LIMIT 1)
			WHEN 6 THEN (/* PARSEADORES1 new\distri_lot\distri_reporte_ultimasjugadas_lot.php - QUERY 14 */ SELECT pal.nom_corto FROM palos_cartas pal WHERE pal.id_palo = ve.id_signo LIMIT 1)
			ELSE ''
		END AS nsigno,
		lo.nom_loteria,
		ta.nom_taquilla,
...
$query_Recordset2 = sprintf(
    "/* PARSEADORES1 new\distri_lot\distri_reporte_ultimasjugadas_lot.php - QUERY 15 */ SELECT ag.cod_agencia, ag.nom_agencia 
	FROM 
		agencia ag, agencialoterias al 
	WHERE ag.cod_agencia = al.id_agencia AND ag.cod_banca = %s
...
<script language="javascript"> 
	...
	function cStatus(cTic) {
		...
		if(confirma==true){
			...
			$.ajax({ data:parametros, url:'../includes/distri_elimina_ticket_lot.php', type:'post',
...
if ($totalRows_Recordset1>0) {
                ...
                do {
                    ...
                    $rest = $rest.$ticket2;
                    $insertGoTo = "distri_reimprimir_ticket_lot.php?recordID=$ticket2"; ?> 
