...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1" && isset($_POST["guardarAge"]))) {
    ...
    if ($graba==31) {
        $insertSQL1 = sprintf(
            "/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 1 */ UPDATE agencia SET por_agencia_lot=%s WHERE cod_agencia=%s",
...
if (isset($_POST["guardar"])) {
    ...
    if ($graba==31) {
        ...
        foreach ($_POST["idtr_agelot"] as $idl) {
            ...
            $insertSQL2 = sprintf(
                "/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 2 */ UPDATE agencialoterias 
					SET
            ...
            if ($tip_loteria[$cta]<4) {
                $insertSQL3 = sprintf(
                    "/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 3 */ UPDATE agencialoterias 
						SET
						est_agelot=%s, lun_loteria=%s, mar_loteria=%s, mie_loteria=%s, jue_loteria=%s, vie_loteria=%s, sab_loteria=%s,
						dom_loteria=%s, top_ventaage=%s
						WHERE id_agelot=%s",
...
if ((isset($_POST["MM_insert2"])) && ($_POST["MM_insert2"] == "form2")) {
    $query_Recordset6 =  sprintf(
        "/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 4 */ SELECT 
			lo.id_loteria, 
			lo.tip_loteria, 
			lo.id_triple, 
			lo.id_terminal, 
    ...
    if ($totalRows_Recordset6>0) {
        ...
        do {
            $query_Recordset7 =  sprintf(
                "/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 5 */ SELECT id_loteria
            ...
            if ($totalRows_Recordset7==0) {
                $insertSQL2 = sprintf(
                    "/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 6 */ INSERT 
    ...
    if (isset($_POST["tipA"]) && $_POST["tipA"]>=0) {
        if ($_POST["tipA"]>=0 && $_POST["tipA"]<=4 && isset($_POST["od_loteria"])) {
            ...
            $query_Recordset1 =  sprintf(
                "/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 7 */ SELECT 
				al.id_agelot, lo.tip_loteria, id_agencia,
				CASE lo.tip_loteria
					WHEN 1 THEN (/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 8 */ SELECT alo.id_agelot FROM agencialoterias alo WHERE alo.id_loteria = lo.id_terminal AND 
						ag.cod_agencia = alo.id_agencia LIMIT 1)
					WHEN 3 THEN (/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 9 */ SELECT alo.id_agelot FROM agencialoterias alo WHERE alo.id_loteria = lo.id_terminal AND 
						ag.cod_agencia = alo.id_agencia LIMIT 1)
					ELSE 0
				END AS id_terminal	
            ...
            if ($totalRows_Recordset1>0) {
                if ($_POST["tipA"]==0) {//dias
                    ...
                    do {
                        $insertSQL2 = sprintf(
                            "/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 10 */ UPDATE agencialoterias 
                        ...
                        if ($row_Recordset1['tip_loteria']==1 or $row_Recordset1['tip_loteria']==3) {
                            $insertSQL2 = sprintf(
                                "/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 11 */ UPDATE agencialoterias 
								SET lun_loteria=%s, mar_loteria=%s, mie_loteria=%s, jue_loteria=%s,
									vie_loteria=%s, sab_loteria=%s, dom_loteria=%s
									WHERE id_agelot=%s",
                                GetSQLValueString($lun, "int"),
                    ...
                    do {
                        $insertSQL2 = sprintf(
                            "/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 12 */ UPDATE agencialoterias SET est_agelot=%s WHERE id_agelot=%s",
                        ...
                        if ($row_Recordset1['tip_loteria']==1 or $row_Recordset1['tip_loteria']==3) {
                            $insertSQL2 = sprintf(
                                "/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 13 */ UPDATE agencialoterias SET est_agelot=%s WHERE id_agelot=%s",
                                GetSQLValueString($_POST['staA'], "int"),
                                GetSQLValueString($row_Recordset1['id_terminal'], "int")
                            );
                            $Result2 = mysqli_query($conexionbanca, $insertSQL2) or die(mysqli_error($conexionbanca));
                    ...
                    if ($_POST['toptrA']>0) {
                        do {
                            $insertSQL3 = sprintf(
                                "/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 14 */ UPDATE agencialoterias 
								SET top_ventaage=%s
								WHERE id_agelot=%s",
                    ...
                    if ($_POST['topteA']>0) {
                        do {
                            if ($row_Recordset1['tip_loteria']==1 or $row_Recordset1['tip_loteria']==3) {
                                $insertSQL3 = sprintf(
                                    "/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 15 */ UPDATE agencialoterias 
									SET top_ventaage=%s
									WHERE id_agelot=%s",
...
if ((isset($_POST["MM_inserto"])) && ($_POST["MM_inserto"] == "formo") && (isset($_POST["od_sorteo"]))) {
    ...
    $query_Recordset1 =  sprintf(
        "/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 16 */ SELECT 
		...
		so.nom_sorteo,
		CASE lo.tip_loteria
			WHEN 1 THEN (/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 17 */ SELECT blo.pre_loteria FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal AND 
				ba.cod_banca = blo.id_banca LIMIT 1)
			WHEN 3 THEN (/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 18 */ SELECT blo.pre_loteria FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal AND 
				ba.cod_banca = blo.id_banca LIMIT 1)
			ELSE 0
		END AS pre_terminal,	
		CASE lo.tip_loteria
			WHEN 1 THEN (/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 19 */ SELECT al2.id_agelot FROM agencialoterias al2 WHERE al2.id_loteria = lo.id_terminal AND 
				ag.cod_agencia = al2.id_agencia LIMIT 1)
			WHEN 3 THEN (/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 20 */ SELECT al2.id_agelot FROM agencialoterias al2 WHERE al2.id_loteria = lo.id_terminal AND 
				ag.cod_agencia = al2.id_agencia LIMIT 1)
			ELSE 0
		END AS id_agelot_ter,	
		CASE lo.tip_loteria
			WHEN 1 THEN (/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 21 */ SELECT alo.top_ventaage FROM agencialoterias alo WHERE alo.id_loteria = lo.id_terminal AND 
				ag.cod_agencia = alo.id_agencia LIMIT 1)
			WHEN 3 THEN (/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 22 */ SELECT alo.top_ventaage FROM agencialoterias alo WHERE alo.id_loteria = lo.id_terminal AND 
				ag.cod_agencia = alo.id_agencia LIMIT 1)
			ELSE 0
		END top_terminal	
		FROM 
		...
		WHERE 
			...
			so.est_sorteo = 1 AND 
			bl.est_banlot = 1 AND 
			so.id_sorteo IN ('$osorteo') AND ag.cod_banca = %s
    ...
    $osorteo=array();
    $query_Recordset1 =  sprintf(
        "/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 23 */ SELECT 
		ag.cod_agencia, ag.nom_agencia, ag.por_agencia_lot,
		al.lun_loteria, al.mar_loteria, al.mie_loteria, al.jue_loteria, al.vie_loteria, 
		al.sab_loteria, al.dom_loteria, al.id_agelot, al.est_agelot, al.top_ventaage,
		lo.nom_loteria, lo.tip_loteria, bl.id_banlot, bl.est_banlot, ba.con_tope,
		...
		so.nom_sorteo,
		CASE lo.tip_loteria
			WHEN 1 THEN (/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 24 */ SELECT blo.pre_loteria FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal AND 
				ba.cod_banca = blo.id_banca LIMIT 1)
			WHEN 3 THEN (/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 25 */ SELECT blo.pre_loteria FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal AND 
				ba.cod_banca = blo.id_banca LIMIT 1)
			ELSE 0
		END AS pre_terminal,	
		CASE lo.tip_loteria
			WHEN 1 THEN (/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 26 */ SELECT al2.id_agelot FROM agencialoterias al2 WHERE al2.id_loteria = lo.id_terminal AND 
				ag.cod_agencia = al2.id_agencia LIMIT 1)
			WHEN 3 THEN (/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 27 */ SELECT al2.id_agelot FROM agencialoterias al2 WHERE al2.id_loteria = lo.id_terminal AND 
				ag.cod_agencia = al2.id_agencia LIMIT 1)
			ELSE 0
		END AS id_agelot_ter,	
		CASE lo.tip_loteria
			WHEN 1 THEN (/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 28 */ SELECT alo.top_ventaage FROM agencialoterias alo WHERE alo.id_loteria = lo.id_terminal AND 
				ag.cod_agencia = alo.id_agencia LIMIT 1)
			WHEN 3 THEN (/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 29 */ SELECT alo.top_ventaage FROM agencialoterias alo WHERE alo.id_loteria = lo.id_terminal AND 
				ag.cod_agencia = alo.id_agencia LIMIT 1)
			ELSE 0
		END top_terminal	
		FROM 
...
$query_Recordset2 =  sprintf(
    "/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 30 */ SELECT 
...
$query_Recordset3 =  sprintf(
    "/* PARSEADORES1 new\distri_lot\distri_agente_edit_lot.php - QUERY 31 */ SELECT 
	...
	FROM 
		...
		loterias lo
	WHERE ag.cod_agencia = %s AND al.id_agencia = ag.cod_agencia AND lo.id_loteria = al.id_loteria AND lo.tip_loteria != 2 AND
		lo.id_loteria = bl.id_loteria AND lo.est_loteria = 1 AND bl.est_banlot = 1 AND ag.cod_banca = bl.id_banca AND
		ag.cod_banca = %s
