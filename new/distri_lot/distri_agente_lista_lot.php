...
$query_Recordset1 =  sprintf(
    "/* PARSEADORES1 new\distri_lot\distri_agente_lista_lot.php - QUERY 1 */ SELECT 
		...
		CASE ag.est_agencia  
			...
			ELSE 'NO DEFINIDO'
		END AS estado,
		(/* PARSEADORES1 new\distri_lot\distri_agente_lista_lot.php - QUERY 2 */ SELECT bl.id_banlot FROM bancaloterias bl WHERE ag.cod_banca = bl.id_banca LIMIT 1) AS idbanlot,
		(/* PARSEADORES1 new\distri_lot\distri_agente_lista_lot.php - QUERY 3 */ SELECT al.id_agelot FROM agencialoterias al WHERE ag.cod_agencia = al.id_agencia LIMIT 1) AS idagelot
	FROM 
	agencia ag, banca ba
	WHERE ag.cod_banca = ba.cod_banca AND ba.cod_banca = %s
	ORDER BY 
