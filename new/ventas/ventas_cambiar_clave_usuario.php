...
$MM_authorizedUsers = "A,G,D,U"; $MM_restrictGoTo = "../index.php"; include("../includes/comprobar_acceso.php");
$idUsuario=$_SESSION['MM_id_usuario'];
$editFormAction = $_SERVER['PHP_SELF'];
if (isset($_SERVER['QUERY_STRING'])) {
    $editFormAction .= "?" . htmlentities($_SERVER['QUERY_STRING']);
}
$query_Recordset1 = sprintf("/* PARSEADORES1 new\ventas\ventas_cambiar_clave_usuario.php - QUERY 1 */ SELECT * FROM usuario WHERE usuario.id_usuario = %s", GetSQLValueString($idUsuario, "int"));
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
    ...
    if ($_POST['pas_usuario']!="" && strlen($_POST['pas_usuario'])>=3 && strlen($_POST['pas_usuario'])<=21 &&
        ...
        $updateSQL = sprintf(
            "/* PARSEADORES1 new\ventas\ventas_cambiar_clave_usuario.php - QUERY 2 */ UPDATE usuario SET pas_usuario=%s WHERE id_usuario=%s",
