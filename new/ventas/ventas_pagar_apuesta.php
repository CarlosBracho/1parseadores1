...
$query_Recordset1 = sprintf("/* PARSEADORES1 new\ventas\ventas_pagar_apuesta.php - QUERY 1 */ SELECT * FROM venta WHERE venta.cod_taquilla = %s", GetSQLValueString($codigotaquilla, "int"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1") && ($_POST["ticket"]!="") && ($_POST["ser_venta"]!="")) {
    ...
    $query_Recordset2 = sprintf("/* PARSEADORES1 new\ventas\ventas_pagar_apuesta.php - QUERY 2 */ SELECT * FROM venta, carrera WHERE venta.cod_carrera = carrera.cod_carrera AND venta.ticket = %s AND venta.ser_venta = %s AND venta.cod_taquilla = %s", GetSQLValueString($_POST['ticket'], "int"), GetSQLValueString($_POST['ser_venta'], "text"), GetSQLValueString($codigotaquilla, "int"));
