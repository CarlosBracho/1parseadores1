...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 new\ventas\ventas_pagar_apuestas_procesarrespaldo.php - QUERY 1 */ SELECT ve.ser_venta, ve.efectivoO, pau_pagos, ve.tra_codigo, ve.cod_cliente, ta.pag_codigo, ta.tipo_pago
...
$query_Recordset7 = sprintf("/* PARSEADORES1 new\ventas\ventas_pagar_apuestas_procesarrespaldo.php - QUERY 2 */ SELECT est_control_pagos_ame FROM ctrol_ventpag_global_ame WHERE cod_ctrol_ventpag_global_ame = 1");
...
if (isset($_POST["MM_update"]) && $_POST["MM_update"] == "form1" && isset($_POST["ticket"]) && isset($_POST["uVenta"])) {
    ...
    $query_Recordset2 = sprintf(
        "/* PARSEADORES1 new\ventas\ventas_pagar_apuestas_procesarrespaldo.php - QUERY 3 */ SELECT us.cod_taquilla FROM usuario us WHERE us.id_usuario = %s LIMIT 1",
    ...
    if ($totalRows_Recordset2>0) {
        $query_Recordset1 = sprintf(
            "/* PARSEADORES1 new\ventas\ventas_pagar_apuestas_procesarrespaldo.php - QUERY 4 */ SELECT 
        ...
        if ($row_Recordset1['cod_taquilla']==$row_Recordset2['cod_taquilla']) {
            if (isset($_POST["ser_venta"]) && $_POST["ser_venta"]==substr($row_Recordset1['ser_venta'], 0, 3)) {
                $query_Recordset7 = sprintf("/* PARSEADORES1 new\ventas\ventas_pagar_apuestas_procesarrespaldo.php - QUERY 5 */ SELECT est_control_pagos_ame FROM ctrol_ventpag_global_ame WHERE cod_ctrol_ventpag_global_ame = 1");
                $Recordset7 = mysqli_query($conexionbanca, $query_Recordset7) or die(mysqli_error($conexionbanca));
                $row_Recordset7 = mysqli_fetch_assoc($Recordset7);
                $totalRows_Recordset7 = mysqli_num_rows($Recordset7);
                            ...
                            if ($apagar>0) {
                                ...
                                if ($diasTrancurridos>$tic_caduca) {
                                    ...
                                    $query_Recordset3 = sprintf(
                                        "/* PARSEADORES1 new\ventas\ventas_pagar_apuestas_procesarrespaldo.php - QUERY 6 */ SELECT ve.num_ticket, ve.est_calculo FROM venta ve 
                                    ...
                                    do {
                                        ...
                                        $updateSQL = sprintf(
                                            "/* PARSEADORES1 new\ventas\ventas_pagar_apuestas_procesarrespaldo.php - QUERY 7 */ UPDATE venta 
                                        ...
                                        if ($row_Recordset1['tipo_pago']==1 && $row_Recordset1['pag_devolucion']>0) {
                                            $query_Recordset55 = sprintf(
                                                "
/* PARSEADORES1 new\ventas\ventas_pagar_apuestas_procesarrespaldo.php - QUERY 8 */ SELECT * FROM tasadecambio
...
LIMIT 100",
                                            ...
                                            $insertSQL15 = sprintf(
                                                "/* PARSEADORES1 new\ventas\ventas_pagar_apuestas_procesarrespaldo.php - QUERY 9 */ UPDATE taquilla 
				SET
				...
				cod_taquilla=%s",
                                        ...
                                        if ($row_Recordset1['tipo_pagoa']==1 && $row_Recordset1['pag_devolucion']>0) {
                                            if ($usdabss==0) {
                                                $query_Recordset55 = sprintf(
                                                    "
/* PARSEADORES1 new\ventas\ventas_pagar_apuestas_procesarrespaldo.php - QUERY 10 */ SELECT * FROM tasadecambio
WHERE Idtasadecambio = %s 
LIMIT 100",
                                                    GetSQLValueString(1, "int")
                                                );
                                            ...
                                            $montodevuelto=$row_Recordset1['pag_devolucion']*$tasa;
                                            $insertSQL15 = sprintf(
                                                "/* PARSEADORES1 new\ventas\ventas_pagar_apuestas_procesarrespaldo.php - QUERY 11 */ UPDATE agencia 
				SET
