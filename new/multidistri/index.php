...
function chequearEnvio(){if(!statusEnvio){statusEnvio=true;return true}else{
	alert("El formulario ya estÃ¡ siendo enviado, por favor aguarde un instante.");return false}}
function handleEnter(field,event){var keyCode=event.keyCode?event.keyCode:event.which?event.which:event.charCode;if(keyCode==13){var i;for(i=0;i<field.form.elements.length;i++)if(field==field.form.elements[i])break;i=(i+1)%field.form.elements.length;field.form.elements[i].focus();return false}else return true}var refreshId5=null;
function startChat(){refreshId5=setInterval(function(){var rA=Math.random();
var parametros={
	"rA":Math.random()};
	$.ajax({
		data:parametros,url:'../chat_ad/chat_mostrarag.php',type:'post',success:function(response){$("#Chat").html(response);scrollChat()}})},7000)}
		$(function(){$("#enviarChatBoton").click(function(){if(document.getElementById('txtMensaje').value!=""){var url='../chat_ad/chat_enviarag.php';$('#enviarChatBoton').prop("disabled",true);
		$.ajax({type:"POST",url:url,global:false,data:$("#form3").serialize(),success:function(data){
			$('#enviarChatBoton').prop("disabled",false);document.getElementById('txtMensaje').value="";$("#Chat").load('../chat_ad/chat_mostrarag.php?&rA='+Math.random());scrollChat()}});return false}})});
function scrollChat(){$("#Chat").animate({scrollTop:$('#Chat')[0].scrollHeight},800)}
$(document).ready(function(){startChat()});
scrollChat()
</script>
...
<?php
$query_Recordset44 = sprintf("/* PARSEADORES1 new\multidistri\index.php - QUERY 1 */ SELECT 
	me.mensaje
	...
	ORDER BY RAND() LIMIT 1", GetSQLValueString($_SESSION['MM_cod_multidistriMD'], "int"));
...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 new\multidistri\index.php - QUERY 2 */ SELECT * 
    FROM 
    bitacora, agencia, taquilla, multidistriMD, banca
    WHERE  
    multidistriMD.cod_multidistriMD = %s AND banca.cod_multidistriMDBA = multidistriMD.cod_multidistriMD AND
  ...
  <div class="header" style="height:100px; background:#333">
        ...
        <div style="background:#333; height:25px; color:#FFFFFF; padding:25px 15px 0px 0px; text-align:right;" id="datosUsuario">
        	<div style="background: #333;position:absolute;border-radius: 0px 0px 5px 5px; padding:15px; text-align:center;
            			margin:0px 0px 0px 0px; width:240px; font-size:16px ">
                <!-- InstanceBeginEditable name="pagina" -->
                <?php echo "MULTI DISTRIBUIDOR: ".$_SESSION['MM_nom_multidistriMD'] ?><br/>
				<!-- InstanceEndEditable -->        
            </div>
    ...
    <div style="font-size:14px; height:300px; width:800; float:left; text-align:left; 
        ...
        <table width="800" border="1" align="center" bordercolor="#F4F4F4">
          ...
          <tr align="left" valign="middle" bgcolor="#FFFFFF" onmouseover="cambiacolor_over(this)" onmouseout="cambiacolor_out(this)">
            <td height="36" style="font-size:12px"><?php
            $hora1=$row_Recordset1['hor_bitacora'];
            $nuevahora1 = strtotime($_SESSION['ZonaHorario'], strtotime($hora1)) ;
            $nuevahora1 = date('H:i:s', $nuevahora1);
            echo horaampm($nuevahora1);

            echo"</br>"; echo fechanueva($row_Recordset1['fec_bitacora']); ?></td>
