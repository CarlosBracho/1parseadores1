...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
    ...
    if ($graba==31) {
        $query_Recordset111 =  sprintf(
            "/* PARSEADORES1 new\multidistri\agencias_agregarsaldo.php - QUERY 1 */ SELECT
agencia.saldoactuala
	FROM  
	agencia
	WHERE 
	agencia.cod_agencia = %s",
        ...
        $agregarsaldoahay=$row_Recordset111['saldoactuala']/1;



        $insertSQL1 = sprintf(
            "/* PARSEADORES1 new\multidistri\agencias_agregarsaldo.php - QUERY 2 */ UPDATE agencia 
				SET tipo_pagoa=%s, saldoactuala=%s, nom_representante=%s, tel_agencia=%s, dir_agencia=%s, 

				por_agencia_lot=%s, por_agencia_macu=%s, est_agencia=%s 
				WHERE cod_agencia=%s",
            GetSQLValueString($_POST['tipo_pagoa']/1, "int"),
            GetSQLValueString($_POST['agregarsaldoa']+$agregarsaldoahay, "double"),
...
$query_Recordset1 =  sprintf(
    "/* PARSEADORES1 new\multidistri\agencias_agregarsaldo.php - QUERY 3 */ SELECT
agencia.tipo_pagoa,
	agencia.nom_agencia,
	agencia.cod_banca,
...
$query_Recordset11 = sprintf(
    "/* PARSEADORES1 new\multidistri\agencias_agregarsaldo.php - QUERY 4 */ SELECT * FROM usuario 
	WHERE tip_usuario='G' 
...
$tipo_pagoa=$row_Recordset1['tipo_pagoa']/1;
...
<tr valign="baseline">
                 ...
                 <tr valign="baseline">
				  ...
				  :<br />
				                      <select name="tipo_pagoa" style="width:140px; height: auto" class="textbox" tabindex="4"> 
                      <option value="1" <?php if (!(strcmp(1, htmlentities($tipo_pagoa/1, ENT_COMPAT, 'utf-8')))) {
    echo "SELECTED";
} ?>>PRE PAGO</option>
                      <option value="0" <?php if (!(strcmp(0, htmlentities($tipo_pagoa/1, ENT_COMPAT, 'utf-8')))) {
    echo "SELECTED";
} ?>>POST PAGO</option>
						  </select>
						    <br />SALDO A AGREGAR:<br/>
                  	<input type="text" name="agregarsaldoa" class="textbox" style="height:auto; width:100px" 
