...
$cod_banca=$_SESSION['MM_cod_multidistriMD'];
    ...
    if ($_POST['id_hipod']!="-1") {
        if ($_POST['id_hipod']!="-1" && $_POST['id_carrera']=="todas") {
            $query_Recordset4 = sprintf(
                "/* PARSEADORES1 new\multidistri\distri_reporte_carrera_carrera.php - QUERY 1 */ SELECT 
				nom_hipodromo
			FROM hipodromo
            ...
            $xhipodromo=strtoupper($row_Recordset4['nom_hipodromo']);
            $codcarrera="todas";
            $num_carrera="&nbsp;TODAS";
        } else {
            $query_Recordset4 = sprintf(
                "/* PARSEADORES1 new\multidistri\distri_reporte_carrera_carrera.php - QUERY 2 */ SELECT 
				cod_carrera, nom_hipodromo, num_carrera
			FROM carrera
            ...
            $totalRows_Recordset4 = mysqli_num_rows($Recordset4);
            $xhipodromo=strtoupper($row_Recordset4['nom_hipodromo']);
            $codcarrera=$row_Recordset4['cod_carrera'];
            $num_carrera=$row_Recordset4['num_carrera'];
        }
        $query_Recordset6 = sprintf(
            "/* PARSEADORES1 new\multidistri\distri_reporte_carrera_carrera.php - QUERY 3 */ SELECT 
			hi.nom_hipodromo, hi.cod_hipodromo, ca.cod_carrera, ca.num_carrera
		FROM 
			carrera ca,
			hipodromo hi
		WHERE 	
        ...
        if ($_POST['id_agente']!="todos") {
            $query_Recordset5 = sprintf(
                "/* PARSEADORES1 new\multidistri\distri_reporte_carrera_carrera.php - QUERY 4 */ SELECT nom_taquilla, cod_taquilla FROM taquilla WHERE cod_taquilla = %s",
        ...
        $vendedor=$vendedor." / ".strtoupper($row_Recordset4['nom_hipodromo'])." Carr#".$num_carrera;
        $codHipodromo=$_POST['id_hipod'];
    }
}
$query_Recordset2 = sprintf(
    "/* PARSEADORES1 new\multidistri\distri_reporte_carrera_carrera.php - QUERY 5 */ SELECT ta.nom_taquilla, ta.est_taquilla, ta.cod_taquilla, ag.nom_agencia
	FROM
		banca ba, 
		taquilla ta,
		agencia ag,
	...
	WHERE 
		...
		ag.cod_banca = ba.cod_banca AND mu.cod_multidistriMD = ba.cod_multidistriMDBA AND
		mu.cod_multidistriMD = %s AND ta.est_taquilla = 1 AND ta.tipotaquilla = 0
	ORDER BY ta.nom_taquilla ASC, ag.nom_agencia ASC",
...
$query_Recordset3 = sprintf(
    "/* PARSEADORES1 new\multidistri\distri_reporte_carrera_carrera.php - QUERY 6 */ SELECT 
...
<script language="javascript">
	...
	function ver2() {
		...
		$.ajax({ data:parametros1, url:'../admin/filt_carrera_fecha.php', type:'post',
			success: function(opciones) {                    
				...
				$.ajax({ url:"../admin/filt_carrera_hipodromo.php", type: "POST", data:parametros,
	...
	function verInfo() {
		var parametros1 = { 
			...
			"codBan":document.getElementById("cod_multidistriMD").value,
			"fecCar":document.getElementById("datepickeri").value,
			"codHipodromo":document.getElementById("id_hipod").value};
		$.ajax({ data:parametros1, url:'distri_reca_carrera_carrera.php', type:'post',
			beforeSend: function() {
...
$(document).ready(function(){
	...
	$("#refrescar").change(function(){
		if ($(this).prop('checked')==true) refreshIntervalId = setInterval("verInfo()", 15000);
...
<div class="container">
    ...
    <div class="contentAdmin">
		<div style="background: #099;width:99.6%;float:left;padding:15px 2px 10px 2px;color:#FFF;font-size:28px;text-align:center; 
			font-family:'Lucida Grande', 'Lucida Sans Unicode', 'Lucida Sans', 'DejaVu Sans', Verdana, sans-serif;">
			REPORTE CARRERA POR CARRERA AMERICANAS
		</div><!-- end .container -->
    ...
    echo "SELECTED";
                      ...
                      <td width="30%" align="left" valign="bottom">
                        ...
                        if ($totalRows_Recordset3>0) {?>
                           ...
                           <option value="<?php echo $row_Recordset3['cod_hipodromo']?>"
								<?php if ($row_Recordset3['nom_hipodromo']==$xhipodromo) {
                                    echo "SELECTED";
                                } ?>>
                      ...
                      <?php } else {?>
                            <select id="nrocarrera" name="id_carrera" style="height:40px; width:99%; font-size:16px" onchange="form1.submit()">
                           <option value="todas" <?php if ($row_Recordset6['cod_carrera']==$codcarrera) {
                          echo "SELECTED";
                      } ?>>
                                <?php echo "TODAS"?>
                           </option>
				    <?php
                            do {?>
                           <option value="<?php echo $row_Recordset6['cod_carrera'];?>"
								<?php if ($row_Recordset6['cod_carrera']==$codcarrera) {
                                echo "SELECTED";
                            } ?>>
                                <?php echo $row_Recordset6['num_carrera']?>
                           </option>
                                  <?php
                ...
                <input type="hidden" name="cod_multidistriMD" id="cod_multidistriMD" value="<?php echo $cod_banca;?>" />
		  ...
		  <div id="gener" style="width:100%; float:left; height:750px">
                ...
                <div id="reporte" style="width:100%; float:left; height:400px">
					<?php include("distri_reca_carrera_carrera.php"); ?>
                </div>
			</div>
        </div>
