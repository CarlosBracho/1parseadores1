...
$totSistemaA=0;
$query_Recordset555 = sprintf("/* PARSEADORES1 new\multidistri\distri_reporte_general_total_prueba.php - QUERY 1 */ SELECT * FROM tasadecambio WHERE Idtasadecambio = 1");
...
$query_Recordset12 = sprintf(
    "/* PARSEADORES1 new\multidistri\distri_reporte_general_total_prueba.php - QUERY 2 */ SELECT dist_por_ame, dist_cob_hnac, dist_por_parley FROM banca WHERE cod_banca = %s LIMIT 1",
...
$query_Recordset3 = sprintf(
    "/* PARSEADORES1 new\multidistri\distri_reporte_general_total_prueba.php - QUERY 3 */ SELECT *
	FROM agencia ag
    ...
    if (isset($_POST['fecha_inicio']) && isset($_POST['fecha_fin'])) {
        if ($_POST['fecha_inicio']!="" && $_POST['fecha_fin']!="") {
            ...
            if ($_POST['id_agencia']!="todos") {
                $query_Recordset3 = sprintf(
                    "/* PARSEADORES1 new\multidistri\distri_reporte_general_total_prueba.php - QUERY 4 */ SELECT *
					FROM agencia ag
...
$query_Recordset2 = sprintf(
    "/* PARSEADORES1 new\multidistri\distri_reporte_general_total_prueba.php - QUERY 5 */ SELECT ag.cod_agencia, ag.nom_agencia
	FROM agencia ag
	WHERE ag.cod_banca = %s
	ORDER BY ag.nom_agencia ASC",
    GetSQLValueString($_SESSION['MM_cod_banca'], "int")
...
<?php
$query_Recordset44 = sprintf("/* PARSEADORES1 new\multidistri\distri_reporte_general_total_prueba.php - QUERY 6 */ SELECT 
	me.mensaje
		...
		<table width="100%" border="0" style="color:#000; font-size:12px;" cellpadding="0" cellspacing="0">
			<tr style="font-size:16px" valign="middle" align="center">
				...
				<td width="33%" colspan="-2" bgcolor="#333" style="color:#FFF"><br/>
                    ...
                    NACIONALES: <?php echo number_format($row_Recordset12['dist_cob_hnac'], 2, ",", ".")."%<br/>"; ?>
           ...
           <td width="21%">TAQUILLA</td>
            ...
            <td width="19%">TOTAL A COBRAR Al<br/> TAQUILLA AMERICANA (%)<br/> NACIONALES(X) TOTAL ( )</td>
                    ...
                    $query_Recordset30 = sprintf(
                       "/* PARSEADORES1 new\multidistri\distri_reporte_general_total_prueba.php - QUERY 7 */ SELECT cod_taquilla, nom_taquilla
						FROM taquilla
						WHERE cod_agencia = %s
                   ...
                   if ($totalRows_Recordset30>0) {
                       do {
                           ...
                           $query_Recordset1 = sprintf(
                               "/* PARSEADORES1 new\multidistri\distri_reporte_general_total_prueba.php - QUERY 8 */ SELECT
					...
					SUM(CASE WHEN ve.est_ticket = 0 AND ve.fec_venta >= %s AND ve.fec_venta <= %s AND ve.lin_ticket = 1  AND ve.efectivoO = 5
						THEN 1 ELSE 0 END) AS con_tic_elisol
                           ...
                           $query_Recordset21 = sprintf(
                               "/* PARSEADORES1 new\multidistri\distri_reporte_general_total_prueba.php - QUERY 9 */ SELECT
					...
					SUM(CASE WHEN veN.est_ticket_hnac = 0 AND veN.fec_venta_hnac >= %s AND veN.fec_venta_hnac <= %s AND
						...
						THEN 1 ELSE 0 END) AS con_tic_eliNsol
                           ...
                           $query_Recordset22 =  sprintf(
                               "/* PARSEADORES1 new\multidistri\distri_reporte_general_total_prueba.php - QUERY 10 */ SELECT cob_taquilla_hnac
								FROM  
                           ...
                           $query_Recordset50 = sprintf(
                            "/* PARSEADORES1 new\multidistri\distri_reporte_general_total_prueba.php - QUERY 11 */ SELECT 
