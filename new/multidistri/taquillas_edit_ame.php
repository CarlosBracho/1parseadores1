...
$query_Recordset1 =  sprintf(
    "/* PARSEADORES1 new\multidistri\taquillas_edit_ame.php - QUERY 1 */ SELECT  
...
$query_Recordset4 =  sprintf(
    "/* PARSEADORES1 new\multidistri\taquillas_edit_ame.php - QUERY 2 */ SELECT  
    ...
    tp.div_pdes2, tp.div_phas2, tp.pag_pdiv2, tp.opc_ame2, tp.redondeo_ame
...
if ($totalRows_Recordset4==0 && (!isset($_POST["MM_insert2"]))) {
    ...
    if ((isset($_POST["MM_insert2"])) && ($_POST["MM_insert2"] == "form2")) {
        if (isset($_POST["exp_agencia"]) && $_POST["exp_agencia"]>0) {
            ...
            $query_Recordset4 =  sprintf(
                "/* PARSEADORES1 new\multidistri\taquillas_edit_ame.php - QUERY 3 */ SELECT  
				...
				tp.est_impresion, tp.anu_regalia, tp.tic_caduca, tp.tip_ticket, tp.tie_reclamo, tp.ver_porpagar, tp.def_regdiv1, tp.div_pdes1, tp.div_phas1,
                 tp.pag_pdiv1, tp.opc_ame, tp.div_pdes2, tp.div_phas2, tp.pag_pdiv2, tp.opc_ame2, tp.redondeo_ame
				FROM  taquilla ta, taquilla_opc_ame tp 
				WHERE ta.cod_taquilla = %s AND tp.cod_taquilla = ta.cod_taquilla
				LIMIT 1",
    ...
    if ($graba==31) {
        if (isset($row_Recordset4['reg_gan'])) {
            if ($_POST['reg_gan']<>$row_Recordset4['reg_gan'] or $_POST['reg_pla']<>$row_Recordset4['reg_pla'] or $_POST['pag_codigo']<>$row_Recordset4['pag_codigo']) {
                ...
                $insertSQL2 = sprintf(
                    "/* PARSEADORES1 new\multidistri\taquillas_edit_ame.php - QUERY 4 */ INSERT 
					INTO bitacora 
        ...
        if ($_POST['existe']==1) {
            $insertSQL2 = sprintf(
                "/* PARSEADORES1 new\multidistri\taquillas_edit_ame.php - QUERY 5 */ UPDATE taquilla_opc_ame
					SET
					pag_codigo=%s, 
					apu_maxgan=%s, 
					apu_maxpla=%s, 
        ...
        curl_close ($ch);
            ...
            if($_POST['def_regdiv1']<>$row_Recordset4['def_regdiv1']){
                ...
                $insertGoTo = "../multidistri/taquillas_edit_ame.php?recordID=".$row_Recordset1['cod_taquilla'];
                if (isset($_SERVER['QUERY_STRING'])) {
                ...
                header(sprintf("Location: %s", $insertGoTo));
                    ...
                    $insertGoTo = "../multidistri/taquillas_edit.php?recordID=".$row_Recordset1['cod_taquilla'];
                    if (isset($_SERVER['QUERY_STRING'])) {
                        $insertGoTo .= (strpos($insertGoTo, '?')) ? "&" : "?";
                        $insertGoTo .= $_SERVER['QUERY_STRING'];
                    }
            ...
            $query_Recordset22 =  sprintf(
                "/* PARSEADORES1 new\multidistri\taquillas_edit_ame.php - QUERY 6 */ SELECT  
tp.apu_mingan
	...
	LIMIT 1",
            ...
            if ($apuestasminimaausar > $row_Recordset22['apu_mingan']) {
                $insertSQL11 = sprintf(
                    "/* PARSEADORES1 new\multidistri\taquillas_edit_ame.php - QUERY 7 */ UPDATE taquilla_opc_ame
					SET
					apu_mingan=%s
            ...
            $insertSQL2 = sprintf(
                "/* PARSEADORES1 new\multidistri\taquillas_edit_ame.php - QUERY 8 */ INSERT 
			INTO 
			taquilla_opc_ame 
            ...
            $query_Recordset22 =  sprintf(
                "/* PARSEADORES1 new\multidistri\taquillas_edit_ame.php - QUERY 9 */ SELECT  
tp.apu_mingan
	FROM  taquilla_opc_ame tp 
	WHERE tp.cod_taquilla = %s
	LIMIT 1",
            ...
            if ($apuestasminimaausar > $row_Recordset22['apu_mingan']) {
                $insertSQL11 = sprintf(
                    "/* PARSEADORES1 new\multidistri\taquillas_edit_ame.php - QUERY 10 */ UPDATE taquilla_opc_ame
					SET
					apu_mingan=%s
					WHERE cod_taquilla=%s",
...
$query_Recordset3 = sprintf(
    "/* PARSEADORES1 new\multidistri\taquillas_edit_ame.php - QUERY 11 */ SELECT ta.cod_taquilla, ta.nom_taquilla
        	...
        	<div style="width:920px; text-align:left; font-size:18px; background: #E1E1E1">
              ...
              <table width="924" align="center">
                <tr valign="baseline">
                  ...
                  <a href='../multidistri/taquillas_edit.php?recordID=<?php echo $row_Recordset1['cod_taquilla']; ?>'
