...
$totSistemaA=0;
$query_Recordset555 = sprintf("/* PARSEADORES1 new\multidistri\distri_reporte_general_total_malo.php - QUERY 1 */ SELECT * FROM tasadecambio WHERE Idtasadecambio = 1");
...
$query_Recordset12 = sprintf(
    "/* PARSEADORES1 new\multidistri\distri_reporte_general_total_malo.php - QUERY 2 */ SELECT dist_por_ame, dist_cob_hnac FROM banca WHERE cod_banca = %s LIMIT 1",
...
$query_Recordset3 = sprintf(
    "/* PARSEADORES1 new\multidistri\distri_reporte_general_total_malo.php - QUERY 3 */ SELECT *
	FROM agencia ag
    ...
    if (isset($_POST['fecha_inicio']) && isset($_POST['fecha_fin'])) {
        if ($_POST['fecha_inicio']!="" && $_POST['fecha_fin']!="") {
            ...
            if ($_POST['id_agencia']!="todos") {
                $query_Recordset3 = sprintf(
                    "/* PARSEADORES1 new\multidistri\distri_reporte_general_total_malo.php - QUERY 4 */ SELECT *
					FROM agencia ag
...
$query_Recordset2 = sprintf(
    "/* PARSEADORES1 new\multidistri\distri_reporte_general_total_malo.php - QUERY 5 */ SELECT ag.cod_agencia, ag.nom_agencia
	FROM agencia ag
	WHERE ag.cod_banca = %s
	ORDER BY ag.nom_agencia ASC",
    GetSQLValueString($_SESSION['MM_cod_banca'], "int")
...
<?php
$query_Recordset44 = sprintf("/* PARSEADORES1 new\multidistri\distri_reporte_general_total_malo.php - QUERY 6 */ SELECT 
	me.mensaje
		...
		<table width="100%" border="0" style="color:#000; font-size:12px;" cellpadding="0" cellspacing="0">
			<tr style="font-size:16px" valign="middle" align="center">
				...
				<td width="33%" colspan="-2" bgcolor="#333" style="color:#FFF"><br/>
                    ...
                    NACIONALES: <?php echo number_format($row_Recordset12['dist_cob_hnac'], 2, ",", ".")."%<br/><br/><br/>"; ?>
           ...
           <td width="21%">TAQUILLA</td>
            ...
            <td width="19%">TOTAL A COBRAR Al<br/> TAQUILLA AMERICANA (%)<br/> NACIONALES(X) TOTAL ( )</td>
                    ...
                    $query_Recordset30 = sprintf(
                       "/* PARSEADORES1 new\multidistri\distri_reporte_general_total_malo.php - QUERY 7 */ SELECT cod_taquilla, nom_taquilla
						FROM taquilla
						WHERE cod_agencia = %s
                   ...
                   $tCantTickElimAgPAR=0;
                   $tMontTickElimAgAme=0;
                   $tMontTickElimAgNac=0;
                   $tMontTickElimAgPAR=0;
                   $tCobrAgAme=0;
                   $tCobrAgNac=0;
                   $tCobrAgPAR=0;
                   ...
                   if ($totalRows_Recordset30>0) {
                       do {
                           ...
                           $query_Recordset1 = sprintf(
                               "/* PARSEADORES1 new\multidistri\distri_reporte_general_total_malo.php - QUERY 8 */ SELECT
					...
					SUM(CASE WHEN ve.est_ticket = 0 AND ve.fec_venta >= %s AND ve.fec_venta <= %s AND ve.lin_ticket = 1  AND ve.efectivoO = 5
						THEN 1 ELSE 0 END) AS con_tic_elisol
                           ...
                           $query_Recordset21 = sprintf(
                               "/* PARSEADORES1 new\multidistri\distri_reporte_general_total_malo.php - QUERY 9 */ SELECT
					...
					SUM(CASE WHEN veN.est_ticket_hnac = 0 AND veN.fec_venta_hnac >= %s AND veN.fec_venta_hnac <= %s AND
						...
						THEN 1 ELSE 0 END) AS con_tic_eliNsol
                           ...
                           $query_Recordset22 =  sprintf(
                               "/* PARSEADORES1 new\multidistri\distri_reporte_general_total_malo.php - QUERY 10 */ SELECT cob_taquilla_hnac
								FROM  
                           ...
                           $tCobroAgenteN=$row_Recordset21['taq_cob_hnac']*$totalRows_Recordset22;
                           ...
                           $query_Recordset50 = sprintf(
                            "/* PARSEADORES1 new\multidistri\distri_reporte_general_total_malo.php - QUERY 11 */ SELECT 
...
SUM(CASE WHEN p4jugadas.lineatp4 = 1 AND p4jugadas.jugadadtp4pago >= %s AND p4jugadas.jugadadtp4pago <= %s AND (p4jugadas.estadoticketp4 = 1) AND p4jugadas.monedap4 <= 2  THEN p4jugadas.premioapagarp4 ELSE 0 END) AS total_ganadorbss,
...
SUM(CASE WHEN p4jugadas.lineatp4 = 1 AND p4jugadas.jugadadtp4pago >= %s AND p4jugadas.jugadadtp4pago <= %s AND (p4jugadas.estadoticketp4 = 1) AND p4jugadas.monedap4 = 3  THEN p4jugadas.premioapagarp4 ELSE 0 END) AS total_ganadorusd,
...
SUM(CASE WHEN p4jugadas.lineatp4 = 1 AND p4jugadas.jugadadtp4pago >= %s AND p4jugadas.jugadadtp4pago <= %s AND (p4jugadas.estadoticketp4 = 1) AND p4jugadas.monedap4 = 4  THEN p4jugadas.premioapagarp4 ELSE 0 END) AS total_ganadorcop,
...
SUM(CASE WHEN p4jugadas.lineatp4 = 1 AND p4jugadas.jugadadtp4pago >= %s AND p4jugadas.jugadadtp4pago <= %s AND (p4jugadas.estadoticketp4 = 1) AND p4jugadas.monedap4 = 5  THEN p4jugadas.premioapagarp4 ELSE 0 END) AS total_ganadorsol
        FROM 
        ...
        GetSQLValueString($in, "date"), GetSQLValueString($fi, "date"),
        GetSQLValueString($in, "date"), GetSQLValueString($fi, "date"),
        GetSQLValueString($in, "date"), GetSQLValueString($fi, "date"),
        GetSQLValueString($in, "date"), GetSQLValueString($fi, "date"),
        GetSQLValueString($in, "date"), GetSQLValueString($fi, "date"),
...
if ($totVentaTaq==0) {
    echo "AM(".number_format($row_Recordset1['taq_por_ame'], 1, ",", ".")."%)";
    echo "NA(".number_format($row_Recordset2['agen_cob_hna'], 0, ",", ".");
    echo "x".$totalRows_Recordset22.")";
    $totalpagosistema=$tCobroAgenteN+$tCobroAgente;
}
?>					
...
?></font>
					...
					<?php
                           ...
                           $tVtasAgPAR=$tVtasAgPAR+$totVentaTaqP;
                           $tVtasAg=$tVtasAgAme+$tVtasAgNac+$tVtasAgPAR;
                            
                           $tPremPagosAgAme=$tPremPagosAgAme+$totPremiTaq;
                           $tPremPagosAgNac=$tPremPagosAgNac+$totPremiTaqN;
                           $tPremPagosAgPAR=$tPremPagosAgPAR+$totPremiTaqP;
                           $tPremPagosAg=$tPremPagosAgNac+$tPremPagosAgAme+$tPremPagosAgPAR;
                            
                           $tAnulAgAme=$tAnulAgAme+$totAnulaTaq;
                           $tAnulAgNac=$tAnulAgNac+$totAnulaTaqN;
                           $tAnulAgPAR=$tAnulAgPAR+$totAnulaTaqP;
                           $tAnulAg=$tAnulAgAme+$tAnulAgPAR+$tAnulAgNac;
                            
                           $tCajaAgAme=$tCajaAgAme+$totTaquilla;
                           $tCajaAgNac=$tCajaAgNac+$totTaquillaN;
                           $tCajaAgPAR=$tCajaAgPAR+$totTaquillaP;
                           $tCajaAg=$tCajaAgAme+$tCajaAgNac+$tCajaAgPAR;
                            
                           $tPremPagarAgAme=$tPremPagarAgAme+$totPTaqPpag;
                           $tPremPagarAgNac=$tPremPagarAgNac+$totPTaqPpagN;
                           $tPremPagarAgPAR=$tPremPagarAgPAR+$totPTaqPpagP;
                           $tPremPagarAg=$tPremPagarAgAme+$tPremPagarAgNac+$tPremPagarAgPAR;
                            
                           $tAnulPagarAgAme=$tAnulPagarAgAme+$porPagarEliTaq;
                           $tAnulPagarAgNac=$tAnulPagarAgNac+$porPagarEliTaqN;
                           $tAnulPagarAgPAR=$tAnulPagarAgPAR+$porPagarEliTaqP;
                           $tAnulPagarAg=$tAnulPagarAgAme+$tAnulPagarAgNac+$tAnulPagarAgPAR;
                            
                           $totalAgAme=$totalAgAme+$totGanPerTaq;
                           $totalAgNac=$totalAgNac+$totGanPerTaqN;
                           $totalAgPAR=$totalAgPAR+$totGanPerTaqP;
                           $totalAg=$totalAgAme+$totalAgNac+$totalAgPAR;
                            
                           $tCantTickElimAgAme=$tCantTickElimAgAme+$tot;
                           $tCantTickElimAgNac=$tCantTickElimAgNac+$totN;
                           $tCantTickElimAgPAR=$tCantTickElimAgPAR+$totP;
                           $tCantTickElimAg=$tCantTickElimAgAme+$tCantTickElimAgNac+$tCantTickElimAgPAR;
                           ...
                           $tMontTickElimAgPAR=$tMontTickElimAgPAR+$row_Recordset50['total_eliminadosbss'];
                           
                           $tMontTickElimAg=$tMontTickElimAgAme+$tMontTickElimAgNac+$tMontTickElimAgPAR;
                            
    
                            
                           $tCobrAgAme=$tCobrAgAme+$tCobroAgente;
                           $tCobrAgNac=$tCobrAgNac+$tCobroAgenteN;
                           $tCobrAgPAR=$tCobrAgPAR+$tCobroAgentePBS;
                           $tCobrAg=$tCobrAgAme+$tCobrAgNac+$tCobrAgPAR;
                           ...
                           $subTotAnulaP=$subTotAnulaP+$totAnulaTaqP;
                            
                           $subTotVentaA=$subTotVentaA+$totVentaTaq+$totVentaTaqP;//total ventas ame
                           $subTotVentaPUSD=$subTotVentaPUSD+$totVentaTaqPUSD;
                           ...
                           $eliminadosAgenteP=$eliminadosAgenteP+$totalAnuladosP;
                            
                           $eliminadosAgenteAusd=$eliminadosAgenteAusd+$totalAnuladosusd+$eliminadosAgenteP;

                           $subTotVentaAusd=$subTotVentaAusd+$totVentaTaqusd+$subTotVentaPUSD;//total ventas ame
                           
                            
                           $eliminadosAgenteAcop=$eliminadosAgenteAcop+$totalAnuladoscop+$totalAnuladosPCOP;

                           $subTotVentaAcop=$subTotVentaAcop+$totVentaTaqcop+$totVentaTaqPCOP;//total ventas ame

                           $eliminadosAgenteAsol=$eliminadosAgenteAsol+$totalAnuladossol+$totalAnuladosPSOL;

                           $subTotVentaAsol=$subTotVentaAsol+$totVentaTaqsol+$totVentaTaqPSOL;//total ventas ame
                           ...
                           $subTotAnupPagar=$subTotAnupPagar+$porPagarEliTaq+$porPagarEliTaqN+$porPagarEliTaqP;
                           $subTotEliminados=$subTotEliminados+$row_Recordset1['tot_eliminad']+$row_Recordset2['tot_eliminadN']+$row_Recordset50['total_eliminadosbss'];
                           $subTotCantEli=$subTotCantEli+$tot+$totN+$totP;
                           $subTotVenta=$subTotVenta+$totVentaTaq+$totVentaTaqN+$totVentaTaqP;
    
                           //$subTotVentaA=$subTotVentaA+$totVentaTaq;
                
                           $subTotPremi=$subTotPremi+$totPremiTaq+$totPremiTaqN+$totPremiTaqP;
                           $subTotAnula=$subTotAnula+$totAnulaTaq+$totAnulaTaqN+$totAnulaTaqP;
                           $subTotTaquilla=$subTotTaquilla+$totTaquilla+$totTaquillaN+$totTaquillaP;
                           $subPTaqPpag=$subPTaqPpag+$totPTaqPpag+$totPTaqPpagN+$totPTaqPpagP;
                           $subGenGanPerTaq=$subGenGanPerTaq+$totGanPerTaq+$totGanPerTaqN+$totGanPerTaqP;
                           $cobroAgente=$cobroAgente+$tCobroAgente+$tCobroAgenteN+$tCobroAgenteP;
                           $eliminadosAgente=$eliminadosAgente+$totalAnulados+$totalAnuladosN+$totalAnuladosP;
                


                
                           $subCobroAgenteN=$subCobroAgenteN+$tCobroAgenteN+$tCobroAgenteP;
                           $tPuntosAg=$tPuntosAg+$totalRows_Recordset22;
                           ...
                           $tVtasAgPUSD=$tVtasAgPUSD+$totVentaTaqPUSD;
                           $tVtasAgusd=$tVtasAgAmeusd+$tVtasAgNacusd+$tVtasAgPUSD;
                
                           $tPremPagosAgAmeusd=$tPremPagosAgAmeusd+$totPremiTaqusd;
                           $tPremPagosAgNacusd=$tPremPagosAgNacusd+$totPremiTaqNusd;
                           $tPremPagosAgPUSD=$tPremPagosAgPUSD+$totPremiTaqPUSD;
                           $tPremPagosAgusd=$tPremPagosAgNacusd+$tPremPagosAgAmeusd+$tPremPagosAgPUSD;
                
                           $tAnulAgAmeusd=$tAnulAgAmeusd+$totAnulaTaqusd;
                           $tAnulAgNacusd=$tAnulAgNacusd+$totAnulaTaqNusd;
                           $tAnulAgPUSD=$tAnulAgPUSD+$totAnulaTaqPUSD;
                           $tAnulAgusd=$tAnulAgAmeusd+$tAnulAgNacusd+$tAnulAgPUSD;
                
                           $tCajaAgAmeusd=$tCajaAgAmeusd+$totTaquillausd;
                           $tCajaAgNacusd=$tCajaAgNacusd+$totTaquillaNusd;
                           $tCajaAgPUSD=$tCajaAgPUSD+$totTaquillaPUSD;
                           $tCajaAgusd=$tCajaAgAmeusd+$tCajaAgNacusd+$tCajaAgPUSD;

                           $tPremPagarAgAmeusd=$tPremPagarAgAmeusd+$totPTaqPpagusd;
                           $tPremPagarAgNacusd=$tPremPagarAgNacusd+$totPTaqPpagNusd;
                           $tPremPagarAgPUSD=$tPremPagarAgPUSD+$totPTaqPpagPUSD;
                           $tPremPagarAgusd=$tPremPagarAgAmeusd+$tPremPagarAgNacusd+$tPremPagarAgPUSD;
                
                            
                           $tAnulPagarAgAmeusd=$tAnulPagarAgAmeusd+$porPagarEliTaqusd;
                           $tAnulPagarAgNacusd=$tAnulPagarAgNacusd+$porPagarEliTaqNusd;
                           $tAnulPagarAgPUSD=$tAnulPagarAgPUSD+$porPagarEliTaqPUSD;
                           $tAnulPagarAgusd=$tAnulPagarAgAmeusd+$tAnulPagarAgNacusd+$tAnulPagarAgPUSD;
                            
                           $totalAgAmeusd=$totalAgAmeusd+$totGanPerTaqusd;
                           $totalAgNacusd=$totalAgNacusd+$totGanPerTaqNusd;
                           $totalAgPUSD=$totalAgPUSD+$totGanPerTaqPUSD;
                           $totalAgusd=$totalAgAmeusd+$totalAgNacusd+$totalAgPUSD;
                
                           $tCantTickElimAgAmeusd=$tCantTickElimAgAmeusd+$totusd;
                           $tCantTickElimAgNacusd=$tCantTickElimAgNacusd+$totNusd;
                           $tCantTickElimAgPUSD=$tCantTickElimAgPUSD+$totPUSD;
                           $tCantTickElimAgusd=$tCantTickElimAgAmeusd+$tCantTickElimAgNacusd+$tCantTickElimAgPUSD;
                           ...
                           $tMontTickElimAgPUSD=$tMontTickElimAgPUSD+$row_Recordset50['total_eliminadosusd'];
                           $tMontTickElimAgusd=$tMontTickElimAgAmeusd+$tMontTickElimAgNacusd+$tMontTickElimAgPUSD;
                
                           $tCobrAgAmeusd=$tCobrAgAmeusd+$tCobroAgenteusd;
                        
                           $tCobrAgPUSD=$tCobrAgPUSD+$tCobroAgentePUSD;
                           $tCobrAgusd=$tCobrAgAmeusd+$tCobrAgPUSD;
                           ...
                           $tVtasAgPCOP=$tVtasAgPCOP+$totVentaTaqPCOP;
                           $tVtasAgcop=$tVtasAgAmecop+$tVtasAgPCOP+$tVtasAgNaccop;
                
                           $tPremPagosAgAmecop=$tPremPagosAgAmecop+$totPremiTaqcop;
                           $tPremPagosAgNaccop=$tPremPagosAgNaccop+$totPremiTaqNcop;
                           $tPremPagosAgPCOP=$tPremPagosAgPCOP+$totPremiTaqPCOP;
                           $tPremPagosAgcop=$tPremPagosAgNaccop+$tPremPagosAgAmecop+$tPremPagosAgPCOP;
                
                           $tAnulAgAmecop=$tAnulAgAmecop+$totAnulaTaqcop;
                           $tAnulAgNaccop=$tAnulAgNaccop+$totAnulaTaqNcop;
                           $tAnulAgPCOP=$tAnulAgPCOP+$totAnulaTaqPCOP;
                           $tAnulAgcop=$tAnulAgAmecop+$tAnulAgPCOP+$tAnulAgNaccop;
                
                           $tCajaAgAmecop=$tCajaAgAmecop+$totTaquillacop;
                           $tCajaAgNaccop=$tCajaAgNaccop+$totTaquillaNcop;
                           $tCajaAgPCOP=$tCajaAgPCOP+$totTaquillaPCOP;
                           $tCajaAgcop=$tCajaAgAmecop+$tCajaAgNaccop+$tCajaAgPCOP;

                           $tPremPagarAgAmecop=$tPremPagarAgAmecop+$totPTaqPpagcop;
                           $tPremPagarAgNaccop=$tPremPagarAgNaccop+$totPTaqPpagNcop;
                           $tPremPagarAgPCOP=$tPremPagarAgPCOP+$totPTaqPpagPCOP;
                           $tPremPagarAgcop=$tPremPagarAgAmecop+$tPremPagarAgNaccop+$tPremPagarAgPCOP;
                
                            
                           $tAnulPagarAgAmecop=$tAnulPagarAgAmecop+$porPagarEliTaqcop;
                           $tAnulPagarAgNaccop=$tAnulPagarAgNaccop+$porPagarEliTaqNcop;
                           $tAnulPagarAgPCOP=$tAnulPagarAgPCOP+$porPagarEliTaqPCOP;
                           $tAnulPagarAgcop=$tAnulPagarAgAmecop+$tAnulPagarAgNaccop+$tAnulPagarAgPCOP;
                            
                           $totalAgAmecop=$totalAgAmecop+$totGanPerTaqcop;
                           $totalAgNaccop=$totalAgNaccop+$totGanPerTaqNcop;
                           $totalAgPCOP=$totalAgPCOP+$totGanPerTaqPCOP;
                           $totalAgcop=$totalAgAmecop+$totalAgNaccop+$totalAgPCOP;
                
                           $tCantTickElimAgAmecop=$tCantTickElimAgAmecop+$totcop;
                           $tCantTickElimAgNaccop=$tCantTickElimAgNaccop+$totNcop;
                           $tCantTickElimAgPCOP=$tCantTickElimAgPCOP+$totPCOP;
                           $tCantTickElimAgcop=$tCantTickElimAgAmecop+$tCantTickElimAgNaccop+$tCantTickElimAgPCOP;
                           ...
                           $tMontTickElimAgPCOP=$tMontTickElimAgPCOP+$row_Recordset50['total_eliminadoscop'];
                           $tMontTickElimAgcop=$tMontTickElimAgAmecop+$tMontTickElimAgNaccop+$tMontTickElimAgPCOP;
                
                           $tCobrAgAmecop=$tCobrAgAmecop+$tCobroAgentecop;
                           $tCobrAgPCOP=$tCobrAgPCOP+$tCobroAgentePCOP;
                           $tCobrAgcop=$tCobrAgAmecop+$tCobrAgNaccop+$tCobrAgPCOP;
                           //cop fin n
                           ...
                           $tVtasAgPSOL=$tVtasAgPSOL+$totVentaTaqPSOL;
                           $tVtasAgsol=$tVtasAgAmesol+$tVtasAgNacsol+$tVtasAgPSOL;
                
                           $tPremPagosAgAmesol=$tPremPagosAgAmesol+$totPremiTaqsol;
                           $tPremPagosAgNacsol=$tPremPagosAgNacsol+$totPremiTaqNsol;
                           $tPremPagosAgPSOL=$tPremPagosAgPSOL+$totPremiTaqPSOL;
                           $tPremPagosAgsol=$tPremPagosAgNacsol+$tPremPagosAgAmesol+$tPremPagosAgPSOL;
                
                           $tAnulAgAmesol=$tAnulAgAmesol+$totAnulaTaqsol;
                           $tAnulAgNacsol=$tAnulAgNacsol+$totAnulaTaqNsol;
                           $tAnulAgPSOL=$tAnulAgPSOL+$totAnulaTaqPSOL;
                           $tAnulAgsol=$tAnulAgAmesol+$tAnulAgNacsol+$tAnulAgPSOL;
                
                           $tCajaAgAmesol=$tCajaAgAmesol+$totTaquillasol;
                           $tCajaAgNacsol=$tCajaAgNacsol+$totTaquillaNsol;
                           $tCajaAgPSOL=$tCajaAgPSOL+$totTaquillaPSOL;
                           $tCajaAgsol=$tCajaAgAmesol+$tCajaAgNacsol+$tCajaAgPSOL;

                           $tPremPagarAgAmesol=$tPremPagarAgAmesol+$totPTaqPpagsol;
                           $tPremPagarAgNacsol=$tPremPagarAgNacsol+$totPTaqPpagNsol;
                           $tPremPagarAgPSOL=$tPremPagarAgPSOL+$totPTaqPpagPSOL;
                           $tPremPagarAgsol=$tPremPagarAgAmesol+$tPremPagarAgNacsol+$tPremPagarAgPSOL;
                           ...
                           $totalAgPSOL=$totalAgPSOL+$totGanPerTaqPSOL;
                           $totalAgsol=$totalAgAmesol+$totalAgNacsol+$totalAgPSOL;
                
                           $tCantTickElimAgAmesol=$tCantTickElimAgAmesol+$totsol;
                           $tCantTickElimAgNacsol=$tCantTickElimAgNacsol+$totNsol;
                           $tCantTickElimAgPSOL=$tCantTickElimAgPSOL+$totPSOL;
                           $tCantTickElimAgsol=$tCantTickElimAgAmesol+$tCantTickElimAgNacsol+$tCantTickElimAgPSOL;
                           ...
                           $tMontTickElimAgPSOL=$tMontTickElimAgPSOL+$row_Recordset50['total_eliminadossol'];
                           $tMontTickElimAgsol=$tMontTickElimAgAmesol+$tMontTickElimAgNacsol+$tMontTickElimAgPSOL;
                
                           $tCobrAgAmesol=$tCobrAgAmesol+$tCobroAgentesol;
                           $tCobrAgPSOL=$tCobrAgPSOL+$tCobroAgentePSOL;
                           $tCobrAgsol=$tCobrAgAmesol+$tCobrAgNacsol+$tCobrAgPSOL;
                           //sol fin n
				...
				<?php } ?>
                   ...
                   $totalacobrar=$totalPagarSistemaA+$totCobraBancausdx+$totCobraBancacopx+$totCobraBancasolx; ?>  
                    ...
                    PARLEY: <?php echo number_format($porTaquillaP, 2, ",", "."); ?>%
                   </div>
      			   <div id="costo1" style="width:100%; float:left; padding:0px 0px 0px 0px">
														<?php
                            $tVtasAgAmez=((($tVtasAgAme-$tAnulAgAme-$tAnulPagarAgAme + $tVtasAgPAR-$tAnulAgPAR-$tAnulPagarAgPAR)/100)*($porcentaje));
                   $tVtasAgusdz=((($tVtasAgusd-$tAnulAgusd-$tAnulPagarAgusd + $tVtasAgPUSD-$tAnulAgPUSD-$tAnulPagarAgPUSD)/100)*($porcentaje));
                   $tVtasAgusd2z=$tVtasAgusdz*$row_Recordset555['usdabss'];
                   $tVtasAgcopz=((($tVtasAgcop-$tAnulAgcop-$tAnulPagarAgcop + $tVtasAgPCOP-$tAnulAgPCOP-$tAnulPagarAgPCOP)/100)*($porcentaje));
                   $tVtasAgcop2z=$tVtasAgcopz*$row_Recordset555['copabss'];
                   $tVtasAgsolz=((($tVtasAgsol-$tAnulAgsol-$tAnulPagarAgsol + $tVtasAgPSOL-$tAnulAgPSOL-$tAnulPagarAgPSOL)/100)*($porcentaje));
