...
$query_Recordset1 = sprintf(
    '/* PARSEADORES1 new\multidistri\usuarios_ag_lista.php - QUERY 1 */ SELECT usuario.nom_usuario, usuario.id_usuario, usuario.est_usuario, agencia.nom_agencia
	FROM 
		banca, 
		agencia,
		usuario
	WHERE 
	usuario.tip_usuario = "G" AND
	usuario.cod_taquilla = agencia.cod_agencia AND
	agencia.cod_banca = banca.cod_banca AND
	banca.cod_banca = %s
	
	ORDER BY usuario.nom_usuario',
...
<div class="container">
  ...
  <div class="contentAdmin"><!-- InstanceBeginEditable name="Contenido" -->
  	<div style="height:100%; font-size:18px; padding:0px 0px 0px 0px ">
        <div style="height:100%; font-size:28px; padding:10px 10px 20px 10px; float:right;">
            <a href="usuarios_ag_add.php" class="btn alert-success" 
            	style="font-size:24px; width:370px; height:30px; padding:10px 0px 0px 0px; text-align:center ">
                Añadir Nuevo Usuario Agente
          </a>
        </div>
    <?php if ($totalRows_Recordset1>0) {?>    
    <table width="100%" border="0" align="center" style="background: #333; color:#FFF; font-size:14px" >
        <tr>
          <td width="594" align="right" class="diezpunto">Usuarios Agentes <?php echo($startRow_Recordset1 + 1) ?>-<?php echo min($startRow_Recordset1 + $maxRows_Recordset1, $totalRows_Recordset1) ?> de <?php echo $totalRows_Recordset1 ?></td>
    ...
    <td width="305">NOMBRE</td>
    <td width="296">AGENTE</td>
  ...
  <?php do { ?>
    <tr class="brillo">
      ...
      <td align="center" class="brillo">
      	<a href='usuarios_ag_edit.php?recordID=<?php echo $row_Recordset1['id_usuario']; ?>'class="btn btn-info"> EDITAR </a>
      </td>
      </tr>
