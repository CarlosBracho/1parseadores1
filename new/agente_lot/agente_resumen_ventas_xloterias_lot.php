...
if (isset($_POST["MM_inserto"])&&$_POST["MM_inserto"]=="formo"&&isset($_POST["buscar"])) {
    if (isset($_POST["od_sorteo"])&&isset($_POST["od_taquilla"])&&isset($_POST["fechai"])&&isset($_POST["fechaf"])) {
        ...
        $taquilla=$_POST["od_taquilla"];
        $xsorteo = implode("','", $_POST["od_sorteo"]);
        $xtaquilla = implode("','", $_POST["od_taquilla"]);
        $query_Recordset1 = sprintf(
            "/* PARSEADORES1 new\agente_lot\agente_resumen_ventas_xloterias_lot.php - QUERY 1 */ SELECT 
			lo.nom_loteria, lo.id_triple, nom_sorteo, lo.id_sorteo, 
			SUM(mon_apuesta_lot) AS ven_triple,
			CASE lo.tip_loteria
				WHEN 1 THEN (/* PARSEADORES1 new\agente_lot\agente_resumen_ventas_xloterias_lot.php - QUERY 2 */ SELECT SUM(mon_apuesta_lot) FROM venta_lot ven WHERE ven.id_loteria=lo.id_terminal AND
					us.id_usuario=ve.id_usuario)
				WHEN 3 THEN (/* PARSEADORES1 new\agente_lot\agente_resumen_ventas_xloterias_lot.php - QUERY 3 */ SELECT SUM(mon_apuesta_lot) FROM venta_lot ven WHERE ven.id_loteria=lo.id_terminal AND
					us.id_usuario=ve.id_usuario)
				WHEN 4 THEN '0'
				WHEN 5 THEN '0'
				WHEN 6 THEN '0'
			...
			LEFT JOIN taquilla ta ON ta.cod_taquilla IN ('$xtaquilla') AND us.cod_taquilla = ta.cod_taquilla
			...
			ag.cod_agencia = ta.cod_agencia AND ag.cod_banca = %s AND
			ta.cod_agencia = %s
...
$query_Recordset3 =  sprintf(
    "/* PARSEADORES1 new\agente_lot\agente_resumen_ventas_xloterias_lot.php - QUERY 4 */ SELECT so.id_sorteo, so.nom_sorteo
	FROM loterias lo, sorteos so, bancaloterias bl, agencialoterias al
	WHERE ((lo.tip_loteria = 1 OR lo.tip_loteria>=3) AND lo.est_loteria=1) AND 
	so.id_sorteo = lo.id_sorteo  AND so.est_sorteo=1 AND bl.id_loteria = lo.id_loteria AND bl.est_banlot = 1 AND
	bl.id_banca = %s AND al.id_agencia = %s AND al.est_agelot = 1 
...
$query_Recordset4 =  sprintf(
    "/* PARSEADORES1 new\agente_lot\agente_resumen_ventas_xloterias_lot.php - QUERY 5 */ SELECT ta.cod_taquilla, ta.nom_taquilla 
...
<script>
 $(document).ready(function() {$("#reloj").load('../includes/reloj.php?&js='+Math.random());var refreshId1 = setInterval(function() {$("#reloj").load('../includes/reloj.php?&js='+Math.random());}, 60000);$('#divE').fadeOut(12000);
 	$('#od_sorteo, #od_taquilla').multiselect({
	...
	$( "#od_sorteo, #od_taquilla" ).change(function() {
		if ($("#od_sorteo option:selected").text()=="" || $("#od_taquilla option:selected").text()=="") 
					...
					<tr valign="baseline" style="border-bottom:1px solid  #D5D5D5; background:#D5D5D5; text-align:center">
						<form action="<?php echo $editFormAction; ?>" method="POST" name="formo" id="formo" 
							...
							<td width="35%" align="left" valign="bottom" nowrap>
                                    ...
                                    <select multiple="multiple" name="od_taquilla[]" id="od_taquilla"
                                        style="height:50px; font-size:16px;margin:2px 0 2px 10px;float:left;"><?php
                                        $ag="TAQUILLAS: ";
                                        do {?>
                                            <option value="<?php echo $row_Recordset4['cod_taquilla']?>"<?php
                                            if (in_array($row_Recordset4['cod_taquilla'], $taquilla)) {
                                                echo"selected=\"selected\"";
                                            }?>>
                                            <?php echo $row_Recordset4['nom_taquilla'];?></option><?php
                                            if (isset($totalRows_Recordset1) && $totalRows_Recordset1>0) {
                                                $ag.="-". $row_Recordset4['nom_taquilla'];
                                            }
            ...
            <div id="mostrar" style="width:100%; float:left; border-top: 1px solid #C1BDBE;font-family:'Lucida Grande','Lucida Sans Unicode','Lucida Sans','DejaVu Sans',Verdana,sans-serif;"><?php
                ...
                <?php
                    ...
                    if (isset($totalRows_Recordset1)) {
                        ...
                        echo 'SELECCIONE SORTEOS, ELIJA TAQUILLAS, INDIQUE RANGO DE FECHA<br/>Y PRESIONE EL BOTON BUSCAR';
                    }
                    echo '</div>';
                }?>
            </div>
