...
$query_Recordset1 =  sprintf(
    "/* PARSEADORES1 new\agente_lot\agente_taquillas_lista_lot.php - QUERY 1 */ SELECT 
		...
		CASE ta.est_taquilla 
			...
			ELSE 'NO DEFINIDO' 
		END AS estado,
		(/* PARSEADORES1 new\agente_lot\agente_taquillas_lista_lot.php - QUERY 2 */ SELECT cod_taopclot FROM taquilla_opc_lot tp WHERE tp.cod_taquilla = ta.cod_taquilla LIMIT 1) AS codtaopclot,
		(/* PARSEADORES1 new\agente_lot\agente_taquillas_lista_lot.php - QUERY 3 */ SELECT al.id_agelot FROM agencialoterias al WHERE ag.cod_agencia = al.id_agencia LIMIT 1) AS idagelot,
		(/* PARSEADORES1 new\agente_lot\agente_taquillas_lista_lot.php - QUERY 4 */ SELECT bl.id_banlot FROM bancaloterias bl WHERE ag.cod_banca = bl.id_banca LIMIT 1) AS idbanlot
	FROM 
	taquilla ta, agencia ag, banca ba
	WHERE ta.cod_agencia = ag.cod_agencia AND ag.cod_banca = ba.cod_banca  AND ag.cod_agencia = %s
	ORDER BY 
    ...
    <tr class="brillo" style="border-bottom:1px solid #D5D5D5" valign="bottom">
      ...
      <td align="center" valign="middle">
          ...
          if ($row_Recordset1['idagelot']==0) {
              $url='agente_edit_lot.php?recordID='.$row_Recordset1['cod_agencia'];
              $acc='<font size="1.5">CREE LAS OPCIONES DEL AGENTE PRIMERO</font>';
          } else {
              $url='agente_taquilla_edit_lot.php?recordID='.$row_Recordset1['cod_taquilla'];
