...
$GanPer=0;
//echo $in." agencia:".$codagencia." - carrera:".$codcarrera." - hipodromo:".$codHipodromo;
if (($codagencia>0 or $codagencia=="todos") && ($codcarrera>0 or $codcarrera=="todas") && $in!="" && $codHipodromo!=-1 && $codHipodromo!="") {
    ...
    if ($codcarrera!="todas") {
        $query_Recordset7 = sprintf(
            "/* PARSEADORES1 new\agente\agente_reca_carrera_carrera.php - QUERY 1 */ SELECT 
        ...
        $query_Recordset7 = sprintf(
            "/* PARSEADORES1 new\agente\agente_reca_carrera_carrera.php - QUERY 2 */ SELECT 
            ...
            for ($numCab = 1; $numCab <= $row_Recordset7['can_caballos']; $numCab++) {
                if ($codagencia!="todos") {
                    $query_Recordset8 = sprintf(
                        "/* PARSEADORES1 new\agente\agente_reca_carrera_carrera.php - QUERY 3 */ SELECT 
                    ...
                    $query_Recordset8 = sprintf(
                        "/* PARSEADORES1 new\agente\agente_reca_carrera_carrera.php - QUERY 4 */ SELECT 
                ...
                //echo $row_Recordset8['apuestas']." * ".$cod_banca;
            }
            if ($codagencia!="todos") {
                $query_Recordset9 = sprintf(
                    "/* PARSEADORES1 new\agente\agente_reca_carrera_carrera.php - QUERY 5 */ SELECT 
					SUM(mon_venta) AS apuestas,
					SUM(CASE WHEN est_ticket = 2 
						THEN pag_premio ELSE 0 END) AS tpremios,
					SUM(CASE WHEN est_ticket >= 4 AND est_ticket <= 5
                ...
                $query_Recordset9 = sprintf(
                    "/* PARSEADORES1 new\agente\agente_reca_carrera_carrera.php - QUERY 6 */ SELECT 
					SUM(ve.mon_venta) AS apuestas,
					SUM(CASE WHEN ve.est_ticket = 2 
						THEN ve.pag_premio ELSE 0 END) AS tpremios,
					SUM(CASE WHEN ve.est_ticket >= 4 AND ve.est_ticket <= 5
