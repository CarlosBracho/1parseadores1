...
$query_Recordset12 = sprintf("/* PARSEADORES1 new\agente\hipodromos_agen.php - QUERY 1 */ SELECT cod_hipodromo, nom_hipodromo, hor_carrera FROM carrera WHERE carrera.fec_carrera = %s AND carrera.hor_carrera >= %s AND carrera.est_carrera = 1 AND carrera.cod_banca = %s ORDER BY carrera.hor_carrera  LIMIT 0, 20", 
...
$query_Recordset18 = sprintf(
  "/* PARSEADORES1 new\agente\hipodromos_agen.php - QUERY 2 */ SELECT 
...
$agenteagente=$row_Recordset18['cod_agencia'];
          ...
          $query_Recordset3 = sprintf(
            "/* PARSEADORES1 new\agente\hipodromos_agen.php - QUERY 3 */ SELECT ta.cod_taquilla, ta.nom_taquilla 
          ...
          $query_Recordset38 = sprintf(
            "/* PARSEADORES1 new\agente\hipodromos_agen.php - QUERY 4 */ SELECT ta.cod_taquilla, ta.nom_taquilla 
          FROM taquilla ta, taquilla_opc_ame tp WHERE ta.cod_agencia = %s AND ta.cod_taquilla = tp.cod_taquilla AND ta.cod_taquilla != %s ORDER BY nom_taquilla",
              GetSQLValueString($agenteagente, "int"),
              GetSQLValueString($xCodigo2, "int")
          );
...
$query_Recordset1 = "/* PARSEADORES1 new\agente\hipodromos_agen.php - QUERY 5 */ SELECT cod_hipodromo, nom_hipodromo, est_hipodromo, bus_auto FROM hipodromo ORDER BY nom_hipodromo ASC";
...
if(isset($_POST['taq_bloq'])){
  ...
  $query_Recordset32 = sprintf(
    "/* PARSEADORES1 new\agente\hipodromos_agen.php - QUERY 6 */ SELECT hip_bloqueados
...
$insertSQL1 = sprintf(
  "/* PARSEADORES1 new\agente\hipodromos_agen.php - QUERY 7 */ UPDATE taquilla_opc_ame  
...
<div class="container">
  ...
  <div style="width:98%; float:right; text-align:right; padding:1.5% 2% 0 0; height:40px; font-size:16px;font-family:'Lucida Grande','Lucida Sans Unicode','Lucida Sans','DejaVu Sans',Verdana,sans-serif;">
        <form method="post" name="form2" action="<?php echo $editFormAction; ?>" onsubmit="return chequearEnvio();">
			...
			<input name="Exportar" id="botExp" type="submit"  value="Exportar" class=" btn-danger" 
