...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 new\ventaslot\ventas_eliminar_ticket_lot.php - QUERY 1 */ SELECT ve.ticket_lot, ve.ser_ticket_lot, tp.pag_codigo_lot
	FROM 
		venta_lot ve, 
		usuario us,
		taquilla ta, 
...
if (isset($_POST["MM_update"]) && $_POST["MM_update"] == "form1" && isset($_POST["ticket"]) && isset($_POST["uVenta"])) {
    ...
    if ($totalRows_Recordset1>0) {
        $query_Recordset1 = sprintf("/* PARSEADORES1 new\ventaslot\ventas_eliminar_ticket_lot.php - QUERY 2 */ SELECT us.tic_eliminados, us.cod_taquilla, tp.pag_codigo_lot
			FROM usuario us, taquilla_opc_lot tp WHERE id_usuario = %s AND us.cod_taquilla = tp.cod_taquilla  
			LIMIT 1", GetSQLValueString($_POST["uVenta"], "int"));
        $Recordset1 = mysqli_query($conexionbanca, $query_Recordset1) or die(mysqli_error($conexionbanca));
        $row_Recordset1 = mysqli_fetch_assoc($Recordset1);
        $totalRows_Recordset1 = mysqli_num_rows($Recordset1);
        $pag_codigo=$row_Recordset1['pag_codigo_lot'];
        if (BuscarTicketEliminadosLOT($_POST["uVenta"], fechaactualbd())<$row_Recordset1['tic_eliminados']) {
            $query_Recordset2 = sprintf(
                "/* PARSEADORES1 new\ventaslot\ventas_eliminar_ticket_lot.php - QUERY 3 */ SELECT 
				ve.est_ticket_lot, 
				ve.id_usuario,
				ve.ticket_lot, 
				ve.ser_ticket_lot,
                ...
                if ($row_Recordset1['cod_taquilla']==$row_Recordset2['cod_taquilla']) {
                    ...
                    if (isset($_POST["ser_ticket_lot"]) && $_POST["ser_ticket_lot"]==$serial) {
                        if ($row_Recordset2['est_ticket_lot']==1) {
                            if ($row_Recordset2['est_cierre']==0) {
                                $mensaje="<font color='red'>¡SE ELIMINO CORRECTAMENTE!<br/></font>";
                                $query_Recordset3 = sprintf("/* PARSEADORES1 new\ventaslot\ventas_eliminar_ticket_lot.php - QUERY 4 */ SELECT num_ticket_lot 
									FROM venta_lot ve 
									WHERE  ve.ticket_lot = %s", GetSQLValueString($_POST["ticket"], "int"));
                                ...
                                if ($totalRows_Recordset3>0) {
                                    ...
                                    do {
                                        $numTicket=$row_Recordset3['num_ticket_lot'];
                                        $updateSQL = sprintf(
                                            "/* PARSEADORES1 new\ventaslot\ventas_eliminar_ticket_lot.php - QUERY 5 */ UPDATE venta_lot 
                                    ...
                                    $aE=$row_Recordset1['tic_eliminados']-BuscarTicketEliminadosLOT($_POST["uVenta"], fechaactualbd());
                                ...
                                $mensaje="<font color='red'>¡NO PUEDE SER ELIMINADO!<br/><p style='font-size:12px;color:#000000;'>ALGUNAS JUGADAS POSEEN SORTEOS YA CERRADOS!</p></font>";
                            }
                        } elseif ($row_Recordset2['est_ticket_lot']==0) {
                            $mensaje="<font color='red'>¡HA SIDO ELIMINADO ANTERIORMENTE!</font>";
                        } else {
                            $mensaje="<font color='red'>¡NO PUEDE SER ELIMINADO!</font>";
                        }
                    } else {
...
<?php
	...
	<div style="width:<?php echo $width; ?>;float:left;text-align:right;color:#FFF;font-size:28px; text-align:center;">
		...
		class="btn-primary" 
		onclick="location.href = '../ventaslot/eliminar_ticket_lot.php?uVenta=<?php echo $usuarioPago;?>'"/>	
	</div>
    </div>
    </div>
</body>
