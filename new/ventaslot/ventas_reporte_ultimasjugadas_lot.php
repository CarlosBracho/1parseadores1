...
$xCodigo = $_SESSION['MM_cod_taquilla'];
set_time_limit(0);
        ...
        if ($_POST['fecha_inicio']!="" && $_POST['fecha_fin']!="") {
            ...
            if ($_POST['id_agente']!="todos") {
                $query_Recordset1 = sprintf(
                    "/* PARSEADORES1 new\ventaslot\ventas_reporte_ultimasjugadas_lot.php - QUERY 1 */ SELECT 
					ve.ip_venta_lot,
					ve.est_ticket_lot,
					ve.ser_ticket_lot,
					ve.est_calculo_lot,
					...
					ve.hor_pago_lot,
					ve.ip_pago_lot,
					(/* PARSEADORES1 new\ventaslot\ventas_reporte_ultimasjugadas_lot.php - QUERY 2 */ SELECT usu.nom_usuario FROM usuario usu WHERE usu.id_usuario = ve.cod_usuario_pago_lot) AS nom_pago_lot,
					CASE ve.tip_loteria_lot  
						WHEN 2 THEN (/* PARSEADORES1 new\ventaslot\ventas_reporte_ultimasjugadas_lot.php - QUERY 3 */ SELECT sor.nom_corto FROM signos sor WHERE sor.id_signo = ve.id_signo LIMIT 1)
						WHEN 3 THEN (/* PARSEADORES1 new\ventaslot\ventas_reporte_ultimasjugadas_lot.php - QUERY 4 */ SELECT sor.nom_corto FROM signos sor WHERE sor.id_signo = ve.id_signo LIMIT 1)
						WHEN 4 THEN (/* PARSEADORES1 new\ventaslot\ventas_reporte_ultimasjugadas_lot.php - QUERY 5 */ SELECT ani.nom_animal FROM animales ani WHERE ani.num_animal = ve.num_apuesta_lot LIMIT 1)
						WHEN 5 THEN (/* PARSEADORES1 new\ventaslot\ventas_reporte_ultimasjugadas_lot.php - QUERY 6 */ SELECT fru.nom_corto FROM frutas fru WHERE fru.id_fruta = ve.id_signo LIMIT 1)
						WHEN 6 THEN (/* PARSEADORES1 new\ventaslot\ventas_reporte_ultimasjugadas_lot.php - QUERY 7 */ SELECT pal.nom_corto FROM palos_cartas pal WHERE pal.id_palo = ve.id_signo LIMIT 1)
						ELSE ''
					END AS nsigno,
					lo.nom_loteria,
					ta.nom_taquilla,
				...
				WHERE
					...
					ta.cod_taquilla = %s AND us.id_usuario = %s 
...
if (((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1") && $_POST['id_agente']=="todos") ||
    ...
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 new\ventaslot\ventas_reporte_ultimasjugadas_lot.php - QUERY 8 */ SELECT 
		ve.ip_venta_lot,
		ve.est_ticket_lot,
		ve.ser_ticket_lot,
		ve.est_calculo_lot,
		...
		ve.hor_pago_lot,
		ve.ip_pago_lot,
		(/* PARSEADORES1 new\ventaslot\ventas_reporte_ultimasjugadas_lot.php - QUERY 9 */ SELECT usu.nom_usuario FROM usuario usu WHERE usu.id_usuario = ve.cod_usuario_pago_lot) AS nom_pago_lot,
		CASE ve.tip_loteria_lot  
			WHEN 2 THEN (/* PARSEADORES1 new\ventaslot\ventas_reporte_ultimasjugadas_lot.php - QUERY 10 */ SELECT sor.nom_corto FROM signos sor WHERE sor.id_signo = ve.id_signo LIMIT 1)
			WHEN 3 THEN (/* PARSEADORES1 new\ventaslot\ventas_reporte_ultimasjugadas_lot.php - QUERY 11 */ SELECT sor.nom_corto FROM signos sor WHERE sor.id_signo = ve.id_signo LIMIT 1)
			WHEN 4 THEN (/* PARSEADORES1 new\ventaslot\ventas_reporte_ultimasjugadas_lot.php - QUERY 12 */ SELECT ani.nom_animal FROM animales ani WHERE ani.num_animal = ve.num_apuesta_lot LIMIT 1)
			WHEN 5 THEN (/* PARSEADORES1 new\ventaslot\ventas_reporte_ultimasjugadas_lot.php - QUERY 13 */ SELECT fru.nom_corto FROM frutas fru WHERE fru.id_fruta = ve.id_signo LIMIT 1)
			WHEN 6 THEN (/* PARSEADORES1 new\ventaslot\ventas_reporte_ultimasjugadas_lot.php - QUERY 14 */ SELECT pal.nom_corto FROM palos_cartas pal WHERE pal.id_palo = ve.id_signo LIMIT 1)
			ELSE ''
		END AS nsigno,
		lo.nom_loteria,
		ta.nom_taquilla,
...
if ($est==1) {
    ...
    $vendedor="VENDEDOR: ".strtoupper($row_Recordset1['nom_taquilla']);
}
$query_Recordset2 = sprintf(
    "/* PARSEADORES1 new\ventaslot\ventas_reporte_ultimasjugadas_lot.php - QUERY 15 */ SELECT us.id_usuario, us.nom_usuario 
	FROM 
		usuario us, taquilla ta, taquilla_opc_lot tp
	WHERE ta.cod_taquilla = %s AND ta.cod_taquilla = us.cod_taquilla AND tp.cod_taquilla = ta.cod_taquilla
   ...
   <div style="background: #FFF; width:100%; float:left; padding:25px 0px 0px 10px;
       ...
       <form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1" autocomplete="off"  
           ...
           <option value="<?php echo $row_Recordset2['id_usuario']?>"
                ...
                <?php echo $row_Recordset2['nom_usuario']; ?>
           </option>
                  <?php
...
if ($totalRows_Recordset1>0) {
  ...
  <tr onmouseover="cambiacolor_over(this)" onmouseout="cambiacolor_out(this)">
    ...
    <td align="center" style="font-size:12px"><?php echo fechanueva($row_Recordset1['fec_venta_lot']); ?></td>
				...
				<?php
$hora1=$row_Recordset1['hor_venta_lot'];
                    $nuevahora1 = strtotime($_SESSION['ZonaHorario'], strtotime($hora1)) ;
                    $nuevahora1 = date('H:i:s', $nuevahora1); ?>


    <td align="center" style="font-size:12px"><?php echo horaampm($nuevahora1); ?></td>
...
$hora1=$row_Recordset1['hor_pago_lot'];
$nuevahora1 = strtotime($_SESSION['ZonaHorario'], strtotime($hora1)) ;
$nuevahora1 = date('H:i:s', $nuevahora1);
...
?></td>
    <td align="left"><?php echo horaampm($nuevahora1); ?></td>
    <td align="center" ></td>
    <?php } else {?>
     <td><?php echo ""; ?></td>
