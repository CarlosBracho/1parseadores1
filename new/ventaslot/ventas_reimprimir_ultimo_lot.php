...
if (isset($_GET["recordID"])) {
    $query_Recordset2 = sprintf(
        "/* PARSEADORES1 new\ventaslot\ventas_reimprimir_ultimo_lot.php - QUERY 1 */ SELECT 
		ve.ticket_lot 
		FROM 
			venta_lot ve
		WHERE 
    ...
    if (isset($_GET["tID"])&&$_GET["tID"]==1) {
        $query_Recordset2 = sprintf("/* PARSEADORES1 new\ventaslot\ventas_reimprimir_ultimo_lot.php - QUERY 2 */ SELECT ve.ticket_lot 
        ...
        $query_Recordset2 = sprintf("/* PARSEADORES1 new\ventaslot\ventas_reimprimir_ultimo_lot.php - QUERY 3 */ SELECT ve.ticket_lot 
...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 new\ventaslot\ventas_reimprimir_ultimo_lot.php - QUERY 4 */ SELECT 
	ta.nom_taquilla,
	ta.nom_taquilla,
 	ve.fec_venta_lot,
	ve.hor_venta_lot,
	...
	lt.nom_loteria,
	CASE lt.tip_loteria  
		WHEN 2 THEN (/* PARSEADORES1 new\ventaslot\ventas_reimprimir_ultimo_lot.php - QUERY 5 */ SELECT sor.nom_corto FROM signos sor WHERE sor.id_signo = ve.id_signo LIMIT 1)
		WHEN 3 THEN (/* PARSEADORES1 new\ventaslot\ventas_reimprimir_ultimo_lot.php - QUERY 6 */ SELECT sor.nom_corto FROM signos sor WHERE sor.id_signo = ve.id_signo LIMIT 1)
		WHEN 4 THEN (/* PARSEADORES1 new\ventaslot\ventas_reimprimir_ultimo_lot.php - QUERY 7 */ SELECT ani.nom_animal FROM animales ani WHERE ani.num_animal = ve.num_apuesta_lot LIMIT 1)
		WHEN 5 THEN (/* PARSEADORES1 new\ventaslot\ventas_reimprimir_ultimo_lot.php - QUERY 8 */ SELECT fru.nom_corto FROM frutas fru WHERE fru.id_fruta = ve.id_signo LIMIT 1)
		WHEN 6 THEN (/* PARSEADORES1 new\ventaslot\ventas_reimprimir_ultimo_lot.php - QUERY 9 */ SELECT pal.nom_corto FROM palos_cartas pal WHERE pal.id_palo = ve.id_signo LIMIT 1)
		ELSE ''
	END AS nsigno,
	ba.hor_cierre
FROM 
...
if ($horacarrerabd >= horaactual() && $row_Recordset1['fec_venta_lot']==fechaactualbd() && $totalRows_Recordset1>0 &&
    ...
    if ($rimp==1 && $row_Recordset1['rei_ticket_lot']==0) {
        if ($conte==1) {
            $insertSQL = sprintf(
                "/* PARSEADORES1 new\ventaslot\ventas_reimprimir_ultimo_lot.php - QUERY 10 */ INSERT 
					INTO reimpresion 
            ...
            $insertSQL1 = sprintf(
                "/* PARSEADORES1 new\ventaslot\ventas_reimprimir_ultimo_lot.php - QUERY 11 */ UPDATE reimpresion 
					SET can_actual=%s
        ...
        $insertSQL2 = sprintf(
            "/* PARSEADORES1 new\ventaslot\ventas_reimprimir_ultimo_lot.php - QUERY 12 */ UPDATE venta_lot
