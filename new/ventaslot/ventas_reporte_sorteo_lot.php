...
$query_Recordset2 = sprintf("/* PARSEADORES1 new\ventaslot\ventas_reporte_sorteo_lot.php - QUERY 1 */ SELECT us.nom_usuario, us.id_usuario, ta.nom_taquilla, ag.cod_agencia, ba.cod_banca, ta.cod_taquilla 
...
if ((isset($_POST["MM_inserto"])) && ($_POST["MM_inserto"] == "formo") && (isset($_POST["od_sorteo"])) &&
    ...
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 new\ventaslot\ventas_reporte_sorteo_lot.php - QUERY 2 */ SELECT 
		lo.nom_loteria, lo.id_triple, nom_sorteo, lo.id_sorteo,
		SUM(mon_apuesta_lot) AS ven_triple,
		CASE lo.tip_loteria
			WHEN 1 THEN (/* PARSEADORES1 new\ventaslot\ventas_reporte_sorteo_lot.php - QUERY 3 */ SELECT SUM(mon_apuesta_lot) FROM venta_lot ven WHERE ven.id_loteria=lo.id_terminal)
			WHEN 3 THEN (/* PARSEADORES1 new\ventaslot\ventas_reporte_sorteo_lot.php - QUERY 4 */ SELECT SUM(mon_apuesta_lot) FROM venta_lot ven WHERE ven.id_loteria=lo.id_terminal)
			ELSE 0
		END AS ven_terminal
		FROM
			bancaloterias bl,
...
$query_Recordset3 =  sprintf("/* PARSEADORES1 new\ventaslot\ventas_reporte_sorteo_lot.php - QUERY 5 */ SELECT so.id_sorteo, so.nom_sorteo
    ...
    if (isset($totalRows_Recordset1) && $totalRows_Recordset1>0) {?>
     ...
     <div style="background: #333; width:99%; float:left; color:#FFF; text-align:right; font-size:16px; 
        ...
        <?php }?>  
            ...
            <script>
                $(function() {
                    $('/* PARSEADORES1 new\ventaslot\ventas_reporte_sorteo_lot.php - QUERY 6 */ select').selectric({
