...
$query_Recordset1 = sprintf("/* PARSEADORES1 new\ventashnac_mie\ventas_eliminar_ticket.php - QUERY 1 */ SELECT tic_eliminados 
...
if ($totaleliminados<$maximo) {
    $query_Recordset8 = sprintf(
        "/* PARSEADORES1 new\ventashnac_mie\ventas_eliminar_ticket.php - QUERY 2 */ SELECT cod_taquilla 
								 FROM usuario 
    ...
    $query_Recordset2 = sprintf(
        "/* PARSEADORES1 new\ventashnac_mie\ventas_eliminar_ticket.php - QUERY 3 */ SELECT 
			venta.est_ticket, 
			venta.id_usuario, 
			venta.ser_venta,
			venta.cod_taquilla,
    ...
    if ($totalRows_Recordset2>=1 && $row_Recordset2['est_ticket']==1 && $row_Recordset2['est_carrera']==1 && $acceso==1 &&
        ...
        $query_Recordset3 = sprintf(
            "/* PARSEADORES1 new\ventashnac_mie\ventas_eliminar_ticket.php - QUERY 4 */ SELECT num_ticket 
			FROM 
			venta 
			WHERE 
			venta.ticket = %s",
        ...
        if ($totalRows_Recordset3>0) {
            ...
            do {
                $updateSQL3 = sprintf(
                    "/* PARSEADORES1 new\ventashnac_mie\ventas_eliminar_ticket.php - QUERY 5 */ UPDATE venta SET est_ticket=%s, fec_pago=%s, hor_pago=%s WHERE num_ticket=%s",
