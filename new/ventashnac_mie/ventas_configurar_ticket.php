...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 new\ventashnac_mie\ventas_configurar_ticket.php - QUERY 1 */ SELECT taquilla_opc_hnac.tip_ticket_hnac, taquilla_opc_hnac.lar_ticket_hnac, 
	taquilla_opc_hnac.cod_taopc_hnac 
	FROM 
		usuario,
		taquilla, 
		taquilla_opc_hnac 
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
    $updateSQL = sprintf(
        "/* PARSEADORES1 new\ventashnac_mie\ventas_configurar_ticket.php - QUERY 2 */ UPDATE taquilla_opc_hnac 
			SET 
			taquilla_opc_hnac.tip_ticket_hnac=%s,
			taquilla_opc_hnac.lar_ticket_hnac=%s  
			WHERE 
			cod_taopc_hnac=%s",
        GetSQLValueString($_POST['tip_ticket_hnac'], "int"),
        GetSQLValueString($_POST['lar_ticket_hnac'], "int"),
    ...
    <?php if ($guardar!=2) { ?>
     <form action="<?php echo $editFormAction; ?>" method="post" 
        ...
        <div style="background: #FFF; width:100%; float:left; padding:25px 0px 0px 40px;">
         	Tipo Ticket:
			<select name="tip_ticket_hnac" style="width:140px; height: auto" class="textbox">
			<?php
            for ($i = 0;  $i <= 3; $i++) {?>
				<option value="<?php echo $i; ?>" 
				<?php if (!(strcmp($i, htmlentities($row_Recordset1['tip_ticket_hnac'], ENT_COMPAT, 'utf-8')))) {
                echo "SELECTED";
            } ?>>
				<?php echo $i; ?>
				</option>
       ...
       <div style="background: #FFF; width:100%; float:left; padding:0px 0px 0px 36px;">
        	Ajuste largo: 
            <select name="lar_ticket_hnac" style="width:140px; height: auto" class="textbox">
            <?php for ($i = 0; $i < 10; ++$i) {?>
                <option value="<?php echo $i; ?>"
                <?php if (!(strcmp($i, htmlentities($row_Recordset1['lar_ticket_hnac'], ENT_COMPAT, 'utf-8')))) {
                echo "SELECTED";
            } ?>>
                <?php echo $i; ?>
                </option>
       ...
       <input type="hidden" name="cod_taopc_hnac" value="<?php echo $row_Recordset1['cod_taopc_hnac']; ?>" />
