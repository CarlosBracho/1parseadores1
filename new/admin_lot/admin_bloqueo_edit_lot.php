...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1") && isset($_POST["agregar"])) {
    ...
    if (isset($_POST['tip_bloqueo'])&&isset($_POST['cod_banca'])&&isset($_POST['num_bloqueo'])&&$_POST['num_bloqueo']!=""
        ...
        if ($guarda==80) {
            $insertSQL1 = sprintf(
                "/* PARSEADORES1 new\admin_lot\admin_bloqueo_edit_lot.php - QUERY 1 */ INSERT 
            ...
            $menS="<br/>&nbsp;&nbsp;DATOS GUARDADOS CORECTAMENTE&nbsp;&nbsp;<br/>&nbsp;";
        }
    } else {
        if (!isset($_POST['tip_bloqueo'])) {
            $menE="<br/>&nbsp;&nbsp;INDIQUE TIPO DE BLOQUEO&nbsp;&nbsp;<br/>&nbsp;";
        }
        if ($_POST['num_bloqueo']=="") {
            $menE="<br/>&nbsp;&nbsp;INDIQUE TRIPLE O TERMINAL&nbsp;&nbsp;<br/>&nbsp;";
        }
        if (!isset($_POST['ran_fecha'])) {
            $menE="<br/>&nbsp;&nbsp;INDIQUE FECHA&nbsp;&nbsp;<br/>&nbsp;";
        }
    }
}
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1") && isset($_POST["eliminar"])) {
    $query_delete =  sprintf(
        "/* PARSEADORES1 new\admin_lot\admin_bloqueo_edit_lot.php - QUERY 2 */ DELETE FROM bloqueadoloterias WHERE id_bloqueado = %s",
...
$query_Recordset1 =  sprintf(
    "/* PARSEADORES1 new\admin_lot\admin_bloqueo_edit_lot.php - QUERY 3 */ SELECT 
...
$query_Recordset2 =  sprintf(
    "/* PARSEADORES1 new\admin_lot\admin_bloqueo_edit_lot.php - QUERY 4 */ SELECT 
...
$query_Recordset3 =  sprintf(
    "/* PARSEADORES1 new\admin_lot\admin_bloqueo_edit_lot.php - QUERY 5 */ SELECT 
...
$query_Recordset4 =  sprintf(
    "/* PARSEADORES1 new\admin_lot\admin_bloqueo_edit_lot.php - QUERY 6 */ SELECT 
...
$query_Recordset5 =  sprintf(
    "/* PARSEADORES1 new\admin_lot\admin_bloqueo_edit_lot.php - QUERY 7 */ SELECT 
...
$query_Recordset6 =  sprintf(
    "/* PARSEADORES1 new\admin_lot\admin_bloqueo_edit_lot.php - QUERY 8 */ SELECT 
	bl.id_bloqueado, bl.num_bloqueado, bl.des_bloqueado, bl.has_bloqueado, bl.tip_rango, bl.max_bloqueo, bl.min_bloqueo,
	(CASE bl.id_sorteo
		WHEN 0 THEN 
			(CASE bl.id_loteria
				WHEN -1 THEN 'TODAS'
				WHEN 0 THEN 'TODAS'
				ELSE (/* PARSEADORES1 new\admin_lot\admin_bloqueo_edit_lot.php - QUERY 9 */ SELECT CONCAT('LOTERIA: ',lo.nom_loteria) FROM loterias lo WHERE lo.id_loteria = bl.id_loteria LIMIT 1)
			END)
		ELSE (/* PARSEADORES1 new\admin_lot\admin_bloqueo_edit_lot.php - QUERY 10 */ SELECT CONCAT('SORTEO: ',so.nom_sorteo) FROM sorteos so WHERE so.id_sorteo = bl.id_sorteo LIMIT 1)
	END) AS nomloteria,
	
	(CASE bl.id_taquilla
		WHEN 0 THEN 'TODAS'
		ELSE (/* PARSEADORES1 new\admin_lot\admin_bloqueo_edit_lot.php - QUERY 11 */ SELECT ta.nom_taquilla FROM taquilla ta WHERE bl.id_taquilla = ta.cod_taquilla LIMIT 1)
	END) AS nomtaquilla,
	
	(CASE bl.id_agencia
		WHEN 0 THEN 'TODAS'
		ELSE (/* PARSEADORES1 new\admin_lot\admin_bloqueo_edit_lot.php - QUERY 12 */ SELECT ag.nom_agencia FROM agencia ag WHERE ag.cod_agencia = bl.id_agencia LIMIT 1)
	END) AS nomagencia
...
<div class="container">
	...
	<div style="background:#0084B4; height:25px; color:#FFFFFF; padding:25px 15px 0px 0px; text-align:right;" 
		...
		<div style="background:#0084B4;position:absolute;border-radius: 0px 0px 5px 5px; padding:15px; text-align:center;
			...
			INCLUIR O EDITAR <br/>BLOQUEOS Y TOPES
		</div>
                ...
                <form method="post" name="form1" action="<?php echo $editFormAction; ?>"  onsubmit="return chequearEnvio();">
                    ...
                    <table width="100%" border="0" cellpadding="0" cellspacing="0">
                        <tr align="center">
                            <td>&nbsp;</td>
                            <td>&nbsp;</td>
                            <td>NUMEROS CON BLOQUEOS O TOPES</td>
                        </tr>
                        <tr align="left">
                            <td width="44%" style="font-size:14px; background: #EDEDED" valign="top">
								...
								<div style="width:98%;padding:5px 1px 0 0;float:left;border-bottom:1px solid #D5D5D5; display:none;
                                	text-align: center; font-size:12px;line-height:1.1" id="pa6">
                                        <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                            <tr>
                                                <td><br/>&nbsp;
                                                    NUMERO:<br/>&nbsp;
                                                    <input style="height:30px; width:63px; font-size:26px" name="num_bloqueo" 
                                                    id="num_bloqueo" onKeyPress="return validar_numero(event)" type="text" 
                                                    value="" size="3" maxlength="3" title=" indique triple o terminal "/>
                                                </td>
                                                <td>
                                                    VENTA <br/>MINIMA:<br/>&nbsp;
                                                    <input style="height:30px; width:63px; font-size:26px" name="min_bloqueo" 
                                                    id="min_bloqueo" onKeyPress="return validar_numero(event)" type="text" 
                                                    value="" size="3" maxlength="3" title=" indique triple o terminal "/>
                                                </td>
                                                <td>&nbsp;
                                                    VENTA <br/>MAXIMA:<br/>&nbsp;
                                                    <input style="height:30px; width:63px; font-size:26px" name="max_bloqueo" 
                                                    id="max_bloqueo" onKeyPress="return validar_numero(event)" type="text" 
                                                    value="" size="3" maxlength="3" title=" indique triple o terminal "/>
                                                </td>
                                                <td>
                                                    <input type="submit" value="Agregar" name="agregar" id="agregar" 
                                                    class="btn btn-primary" title="agregar numero"
                                                    style="font-size:14px; cursor:pointer; height:42px;"/>
                                                </td>
                                            </tr>
                                        </table>
                                </div>
                            ...
                            <td width="55%" valign="top">
                                <table width="100%" border="0" style="font-size:12px;" cellpadding="0" cellspacing="0">
                                    <tr style="font-size:10px;background:#333333;color:#FFF" align="center">
                                        <td width="9%">Numero</td>
                                        <td width="51%">Loteria/Agente/Taquilla</td>
                                        <td width="13%">Fecha</td>
                                        <td width="19%">Tope Venta</td>
                                    ...
                                    if ($totalRows_Recordset3>0&&$row_Recordset6['des_bloqueado']!="") {
                                        do {?>
                                            <tr class="brillo" style="border-bottom:1px solid  #D5D5D5" align="center">
                                                ...
                                                if ($row_Recordset6['tip_rango']==2) {?>
													<td style="font-size:10px;line-height:1"><?php
                                                        echo fechanueva($row_Recordset6['des_bloqueado'])."<br/>";
                                                        echo fechanueva($row_Recordset6['has_bloqueado']);
                                                        ?>
                                                    </td>
													<?php
                                                } else {
                                                    echo '<td style="font-size:10px">PERMANENTE</td>';
                                                }?>
												<td style="font-size:10px;line-height:1" align="right">
													Min:<?php echo $row_Recordset6['min_bloqueo']."<br/>";?>
                                                    Max:<?php echo $row_Recordset6['max_bloqueo'];?>
												</td>
                                                <td>
										...
										<tr class="brillo" style="border-bottom:1px solid  #D5D5D5" align="center">
											<td colspan="5" align="center" height="50" style='font-size:18px;'>
                                               	No existen numeros con bloqueos o topes
											</td>
										</tr><?php
                                    }?>
                                </table>
