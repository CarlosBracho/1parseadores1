...
if ($MM_authorizedUsers=="D") {
    $query_Recordset1 =  sprintf("/* PARSEADORES1 new\admin_lot\admin_usuarios_ven_lista_lot.php - QUERY 1 */ SELECT 
			us.id_usuario, us.nom_usuario, ta.nom_taquilla, ta.tel_taquilla, ag.nom_agencia,
			CASE us.est_usuario 
		...
		WHERE us.cod_taquilla = ta.cod_taquilla AND us.tip_usuario = 'U' AND ta.cod_agencia = ag.cod_agencia
		ORDER BY 
		us.nom_usuario ASC, ta.nom_taquilla ASC");
} else {
    $query_Recordset1 =  sprintf("/* PARSEADORES1 new\admin_lot\admin_usuarios_ven_lista_lot.php - QUERY 2 */ SELECT 
			us.id_usuario, us.nom_usuario, ta.nom_taquilla, ta.tel_taquilla, ag.nom_agencia,
			CASE us.est_usuario 
				WHEN 0 THEN '<font color=red>INACTIVO</font>' 
				WHEN 1 THEN 'ACTIVO' 
  ...
  <div class="contentAdmin">
	<div style="height:100%; font-size:18px;" class="xfirefox">
        <div style="height:100%; font-size:28px; padding:10px 10px 10px 10px; float:right;">
            <a href="admin_usuario_ven_add_lot.php" class="btn alert-success" 
	...
	<tr style="background:#0084B4; color:#FFFFFF; height:30px; font-family:'Lucida Grande','Lucida Sans Unicode','Lucida Sans','DejaVu Sans',Verdana,sans-serif;" valign="bottom">
		...
		<td width="33%" align="center">TAQUILLA/AGENTE</td>
  ...
  <?php do { ?>
		<tr class="brillo" style="border-bottom:1px solid #D5D5D5" valign="bottom">
			<td align="left" style="line-height:1.1"><?php
                ...
                echo "Contacto: ".$row_Recordset1['tel_taquilla']."&nbsp;</font>";?>
			</td>
			...
			<td align="center" valign="middle"><?php
                $url='admin_usuario_ven_edit_lot.php?recordID='.$row_Recordset1['id_usuario'];?>
