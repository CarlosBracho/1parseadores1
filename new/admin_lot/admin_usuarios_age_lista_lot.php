...
if ($MM_authorizedUsers=="D") {
    $query_Recordset1 =  sprintf(
        "/* PARSEADORES1 new\admin_lot\admin_usuarios_age_lista_lot.php - QUERY 1 */ SELECT 
    ...
    $query_Recordset1 =  sprintf("/* PARSEADORES1 new\admin_lot\admin_usuarios_age_lista_lot.php - QUERY 2 */ SELECT 
			us.id_usuario, us.nom_usuario, ag.nom_agencia, ag.tel_agencia, ba.nom_banca,
			CASE us.est_usuario 
				WHEN 0 THEN '<font color=red>INACTIVO</font>' 
				WHEN 1 THEN 'ACTIVO' 
		...
		FROM 
		usuario us, agencia ag, banca ba
		WHERE us.cod_taquilla = ag.cod_agencia AND us.tip_usuario = 'G' AND ag.cod_banca = ba.cod_banca
		ORDER BY 
		us.nom_usuario ASC, ag.nom_agencia ASC");
}
  ...
  <div class="contentAdmin">
	<div style="height:100%; font-size:18px;" class="xfirefox">
        <div style="height:100%; font-size:28px; padding:10px 10px 10px 10px; float:right;">
            <a href="admin_usuario_age_add_lot.php" class="btn alert-success" 
  ...
  <?php do { ?>
		<tr class="brillo" style="border-bottom:1px solid #D5D5D5" valign="bottom">
			...
			<td align="center" valign="middle"><?php
                $url='admin_usuario_age_edit_lot.php?recordID='.$row_Recordset1['id_usuario'];?>
