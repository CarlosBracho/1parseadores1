...
$query_Recordset1 =  sprintf("/* PARSEADORES1 new\admin_lot\admin_taquillas_lista_lot.php - QUERY 1 */ SELECT 
		...
		CASE ta.est_taquilla 
			...
			ELSE 'NO DEFINIDO' 
		END AS estado,
		(/* PARSEADORES1 new\admin_lot\admin_taquillas_lista_lot.php - QUERY 2 */ SELECT cod_taopclot FROM taquilla_opc_lot tp WHERE tp.cod_taquilla = ta.cod_taquilla LIMIT 1) AS codtaopclot,
		(/* PARSEADORES1 new\admin_lot\admin_taquillas_lista_lot.php - QUERY 3 */ SELECT al.id_agelot FROM agencialoterias al WHERE ag.cod_agencia = al.id_agencia LIMIT 1) AS idagelot,
		(/* PARSEADORES1 new\admin_lot\admin_taquillas_lista_lot.php - QUERY 4 */ SELECT bl.id_banlot FROM bancaloterias bl WHERE ag.cod_banca = bl.id_banca LIMIT 1) AS idbanlot
	FROM 
