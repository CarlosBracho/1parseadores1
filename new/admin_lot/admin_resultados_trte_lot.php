...
if ((isset($_POST["MM_updateRes"])) && ($_POST["MM_updateRes"] == "form2") && (isset($_POST["guardar_Res"]))) {
            ...
            for ($i = 0; $i < count($pTriple); ++$i) {
                ...
                if (strlen(trim($x_ganador[$i]))==3) {
                    $query_Recordset13 =  sprintf(
                        "/* PARSEADORES1 new\admin_lot\admin_resultados_trte_lot.php - QUERY 1 */ SELECT re.num_resultado, re.sig_resultado, re.id_resultado
                    ...
                    if ($totalRows_Recordset13==0) {
                        $insertSQL2 = sprintf(
                            "/* PARSEADORES1 new\admin_lot\admin_resultados_trte_lot.php - QUERY 2 */ INSERT 
							INTO resultados_lot 
                        ...
                        $insertSQL1 = sprintf(
                            "/* PARSEADORES1 new\admin_lot\admin_resultados_trte_lot.php - QUERY 3 */ UPDATE resultados_lot SET num_resultado=%s, sig_resultado=%s
    ...
    if (isset($_POST["cod_animal"])) {
        ...
        foreach ($_POST["cod_animal"] as $cod_animal) {
            ...
            $signo=$_POST["res_signo"][$t];
            if ($cod_animal>0) {
                $query_Recordset13 =  sprintf(
                    "/* PARSEADORES1 new\admin_lot\admin_resultados_trte_lot.php - QUERY 4 */ SELECT re.num_resultado, re.sig_resultado, re.id_resultado
					FROM resultados_lot re WHERE id_loteria=%s AND fec_resultado=%s AND id_banca=%s LIMIT 1",
                ...
                if ($totalRows_Recordset13==0) {
                    $insertSQL2 = sprintf(
                        "/* PARSEADORES1 new\admin_lot\admin_resultados_trte_lot.php - QUERY 5 */ INSERT 
						INTO resultados_lot 
						(num_resultado, sig_resultado, id_loteria, fec_resultado, id_banca) 
						VALUES (%s, %s, %s, %s, %s)",
                        GetSQLValueString(trim($termin), "text"),
                        GetSQLValueString($signo, "int"),
                        GetSQLValueString($cod_animal, "int"),
                        GetSQLValueString($_POST["fec_resultado"], "date"),
                        GetSQLValueString($id_banca, "int")
                    );
                    ...
                    $insertSQL1 = sprintf(
                        "/* PARSEADORES1 new\admin_lot\admin_resultados_trte_lot.php - QUERY 6 */ UPDATE resultados_lot SET num_resultado=%s, sig_resultado=%s 
						WHERE id_resultado=%s",
                        GetSQLValueString(trim($termin), "text"),
                        GetSQLValueString($signo, "int"),
                        GetSQLValueString($row_Recordset13['id_resultado'], "int")
...
if ((isset($_POST["MM_update"])) && (isset($_POST["guardar"]))) {
    ...
    foreach ($_POST["id_loteria"] as $id_loteria) {
        ...
        $query_Recordset13 =  sprintf(
            "/* PARSEADORES1 new\admin_lot\admin_resultados_trte_lot.php - QUERY 7 */ SELECT re.num_resultado, re.sig_resultado, re.id_resultado
		FROM resultados_lot re WHERE id_loteria=%s AND fec_resultado=%s AND id_banca=0 LIMIT 1",
    ...
    if ($guarda==100) {
        ...
        if (isset($_POST["modificar"])&&$_POST["modificar"]==0) {
            ...
            foreach ($_POST["id_loteria"] as $id_loteria) {
                $insertSQL2 = sprintf(
                    "/* PARSEADORES1 new\admin_lot\admin_resultados_trte_lot.php - QUERY 8 */ INSERT 
					INTO resultados_lot 
					(num_resultado, id_loteria, sig_resultado, fec_resultado, id_banca) 
					VALUES (%s, %s, %s, %s, %s)",
            ...
            foreach ($_POST["id_resultado"] as $id_resultado) {
                $insertSQL1 = sprintf(
                    "/* PARSEADORES1 new\admin_lot\admin_resultados_trte_lot.php - QUERY 9 */ UPDATE resultados_lot 
					SET
...
if (isset($_POST["fec_resultado"])) {
    $fechahoy=$_POST["fec_resultado"];
} else {
...
if ((isset($_POST["MM_inserto"])) && ($_POST["MM_inserto"] == "formo") && (isset($_POST["od_sorteo"]))) {
    ...
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 new\admin_lot\admin_resultados_trte_lot.php - QUERY 10 */ SELECT 
		...
		CASE lo.tip_loteria
			WHEN 4 THEN (/* PARSEADORES1 new\admin_lot\admin_resultados_trte_lot.php - QUERY 11 */ SELECT ani.nom_animal FROM animales ani WHERE ani.id_animal=re.num_resultado LIMIT 1)
			WHEN 5 THEN (/* PARSEADORES1 new\admin_lot\admin_resultados_trte_lot.php - QUERY 12 */ SELECT fru.nom_fruta  FROM frutas fru   WHERE fru.id_fruta=re.num_resultado  LIMIT 1)
			ELSE '_'
		END AS nom_anifru
		FROM 
			sorteos so,
    ...
    $cDia=loteriaHoyAdmin($diahoy);
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 new\admin_lot\admin_resultados_trte_lot.php - QUERY 13 */ SELECT 
		lo.nom_loteria, lo.id_loteria, lo.id_terminal, lo.tip_loteria, lo.let_loteria,
		so.nom_sorteo, so.hor_sorteo, so.id_sorteo,
		re.id_resultado, re.fec_resultado, re.num_resultado, re.sig_resultado,
		CASE lo.tip_loteria
			WHEN 4 THEN (/* PARSEADORES1 new\admin_lot\admin_resultados_trte_lot.php - QUERY 14 */ SELECT ani.nom_animal FROM animales ani WHERE ani.id_animal=re.num_resultado LIMIT 1)
			WHEN 5 THEN (/* PARSEADORES1 new\admin_lot\admin_resultados_trte_lot.php - QUERY 15 */ SELECT fru.nom_fruta  FROM frutas fru   WHERE fru.id_fruta=re.num_resultado  LIMIT 1)
			ELSE '_'
		END AS nom_anifru
		FROM 
			sorteos so,
...
$query_Recordset2 = "/* PARSEADORES1 new\admin_lot\admin_resultados_trte_lot.php - QUERY 16 */ SELECT id_signo, nom_corto FROM signos ORDER BY id_signo";
$Recordset2 = mysqli_query($conexionbanca, $query_Recordset2) or die(mysqli_error($conexionbanca));
$row_Recordset2 = mysqli_fetch_assoc($Recordset2);
$totalRows_Recordset2 = mysqli_num_rows($Recordset2);
$query_Recordset3 =  sprintf("/* PARSEADORES1 new\admin_lot\admin_resultados_trte_lot.php - QUERY 17 */ SELECT so.id_sorteo, so.nom_sorteo
...
$query_Recordset4 = "/* PARSEADORES1 new\admin_lot\admin_resultados_trte_lot.php - QUERY 18 */ SELECT id_palo, nom_palo FROM palos_cartas ORDER BY id_palo";
                                ...
                                <td width="14%" align="left" valign="bottom" nowrap>
                                    ...
                                    <div style="float:left; margin:1px 0 0 10px;">
                                        <select name="bu_sorteo" id="bu_sorteo" class="btn"
                                            ...
                                            <option value="1">LOTERIAS-ANIMALITOS</option>
                                            <option value="2">SOLO LOTERIAS</option>
                                            <option value="3">SOLO ANIMALITOS</option>
                                        </select>
                                    </div>
                                </td>
                        ...
                        do {
                            ...
                            if ($row_Recordset1['tip_loteria']==3 or $row_Recordset1['tip_loteria']==6) {
                                if ($row_Recordset1['tip_loteria']==3) {?>
                                	...
                                	<div style="width:0.1px;font-size:12px;float:left;text-align:center;height:89px"></div>
									<div style="width:70px;font-size:12px;float:left;background:#D5D5D5;text-align:left;">
										Carta
										<select name="signo[]" style="width:60px; height:30px" disabled="disabled" 
											class="<?php echo $idsorteo?>">
