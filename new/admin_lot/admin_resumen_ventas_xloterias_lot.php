...
if (isset($_POST["MM_inserto"])&&$_POST["MM_inserto"]=="formo"&&isset($_POST["buscar"])) {
    if (isset($_POST["od_sorteo"])&&isset($_POST["od_agencia"])&&isset($_POST["fechai"])&&isset($_POST["fechaf"])) {
        ...
        $query_Recordset1 = sprintf(
            "/* PARSEADORES1 new\admin_lot\admin_resumen_ventas_xloterias_lot.php - QUERY 1 */ SELECT 
			lo.nom_loteria, lo.id_triple, nom_sorteo, lo.id_sorteo, 
			SUM(mon_apuesta_lot) AS ven_triple,
			CASE lo.tip_loteria
				WHEN 1 THEN (/* PARSEADORES1 new\admin_lot\admin_resumen_ventas_xloterias_lot.php - QUERY 2 */ SELECT SUM(mon_apuesta_lot) FROM venta_lot ven WHERE ven.id_loteria=lo.id_terminal AND
					us.id_usuario=ve.id_usuario)
				WHEN 3 THEN (/* PARSEADORES1 new\admin_lot\admin_resumen_ventas_xloterias_lot.php - QUERY 3 */ SELECT SUM(mon_apuesta_lot) FROM venta_lot ven WHERE ven.id_loteria=lo.id_terminal AND
					us.id_usuario=ve.id_usuario)
				WHEN 4 THEN '0'
				WHEN 5 THEN '0'
				WHEN 6 THEN '0'
...
$query_Recordset3 =  sprintf("/* PARSEADORES1 new\admin_lot\admin_resumen_ventas_xloterias_lot.php - QUERY 4 */ SELECT so.id_sorteo, so.nom_sorteo
...
$query_Recordset4 =  sprintf("/* PARSEADORES1 new\admin_lot\admin_resumen_ventas_xloterias_lot.php - QUERY 5 */ SELECT ag.cod_agencia, ag.nom_agencia 
