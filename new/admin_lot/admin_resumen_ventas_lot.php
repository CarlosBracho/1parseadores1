...
if (isset($_POST["MM_inserto"])&&$_POST["MM_inserto"]=="formo"&&isset($_POST["buscar"])) {
    if (isset($_POST["od_agencia"])&&isset($_POST["fechai"])&&isset($_POST["fechaf"])) {
        ...
        $query_Recordset1 = sprintf(
            "/* PARSEADORES1 new\admin_lot\admin_resumen_ventas_lot.php - QUERY 1 */ SELECT 
			lo.nom_loteria, lo.id_triple, nom_sorteo, lo.id_sorteo,
			SUM(mon_apuesta_lot) AS ven_triple,
			CASE lo.tip_loteria
				WHEN 1 THEN (/* PARSEADORES1 new\admin_lot\admin_resumen_ventas_lot.php - QUERY 2 */ SELECT SUM(mon_apuesta_lot) FROM venta_lot ven WHERE ven.id_loteria=lo.id_terminal AND
								ven.id_usuario = us.id_usuario)
				WHEN 3 THEN (/* PARSEADORES1 new\admin_lot\admin_resumen_ventas_lot.php - QUERY 3 */ SELECT SUM(mon_apuesta_lot) FROM venta_lot ven WHERE ven.id_loteria=lo.id_terminal AND
								ven.id_usuario = us.id_usuario)
				WHEN 4 THEN '0'
				WHEN 5 THEN '0'
				WHEN 6 THEN '0'
			...
			LEFT JOIN loterias lo ON lo.tip_loteria!=2 
...
$query_Recordset4 =  sprintf("/* PARSEADORES1 new\admin_lot\admin_resumen_ventas_lot.php - QUERY 4 */ SELECT ag.cod_agencia, ag.nom_agencia 
...
<script>
 $(document).ready(function() {$("#reloj").load('../includes/reloj.php?&js='+Math.random());var refreshId1 = setInterval(function() {$("#reloj").load('../includes/reloj.php?&js='+Math.random());}, 60000);$('#divE').fadeOut(12000);
	...
	$( "#od_agencia" ).change(function() {
		if ($("#od_agencia option:selected").text()=="") $("#buscar").attr('disabled','disabled');
		else $("#buscar").removeAttr('disabled');
	});
 });
            ...
            <div id="mostrar" style="width:100%; float:left; border-top: 1px solid #C1BDBE;font-family:'Lucida Grande','Lucida Sans Unicode','Lucida Sans','DejaVu Sans',Verdana,sans-serif;"><?php
                	...
                	<div style="width:100%; float:left; padding:0; overflow:auto;overflow-x:hidden;">
							<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-size:16px;">
                                <tr align="center" style="line-height: 1;">
                                    <td height="38" colspan="4" valign="bottom"><?php
                                        echo '<font size="3">RESUMEN DE VENTAS DESDE:'.fechanueva($fechahoyi).' ';
                                        echo 'HASTA:'.fechanueva($fechahoyf).'</font>';
                                        ?>
                                    </td>
                                </tr>
                              <tr align="center" valign="bottom" style="background: #DBDBDB;line-height: 1">
                                    <td height="26">AGENTE</td>
									<td>VENTAS</td>
                                    <td>PREMIOS</td>
                                    <td>%</td>
                                </tr><?php
                ...
                <?php
                    ...
                    if (isset($totalRows_Recordset1)) {
                        ...
                        echo 'SELECCIONE AGENTES, INDIQUE RANGO DE FECHA<br/>Y PRESIONE EL BOTON BUSCAR';
                    }
                    echo '</div>';
                }?>
            </div>
