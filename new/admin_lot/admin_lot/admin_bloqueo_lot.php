...
$query_Recordset1 =  sprintf("/* PARSEADORES1 new\admin_lot\admin_lot\admin_bloqueo_lot.php - QUERY 1 */ SELECT 
	ba.nom_banca, ba.nom_representante, ba.cod_banca,
		(/* PARSEADORES1 new\admin_lot\admin_lot\admin_bloqueo_lot.php - QUERY 2 */ SELECT 
			GROUP_CONCAT(bl.num_bloqueado, 'x',
				(CASE bl.id_loteria
					WHEN 0 THEN 'TODAS'
					ELSE (/* PARSEADORES1 new\admin_lot\admin_lot\admin_bloqueo_lot.php - QUERY 3 */ SELECT lo.nom_loteria FROM loterias lo WHERE lo.id_loteria = bl.id_loteria LIMIT 1)
				END)
			SEPARATOR ']&nbsp;&nbsp;[')
