...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 new\dividendos_tvg_buildabet11.php - QUERY 1 */ SELECT * 
	FROM 
	hipodromo,
	carrera 
	WHERE
	...
	hipodromo.bus_resultado_tip <= 3 AND 
...
?>
<div style="background: #F9E209; color: #FFF; width:100%;">
<table width="100%" border="0" style="font-family:Verdana, Geneva, sans-serif;font-size:16px; color:#FFFFFF">
  <tr  style="color: #FFF">
  	...
  	<td height="41" align="left" valign="middle" style="font-size:34px">TVG/BUILDABET - DIVIDENDOS</td>
            ...
            if (isset($cAct) && isset($cAct[0]) && $cAct[0]>0) {
                foreach ($cAct as $hip) {
					...
					<?php
                        ...
                        //echo number_format($row_Recordset1['div_primero_gan'],2)." - ".number_format(floatval($div_primero_gan),2);
                        //echo number_format($row_Recordset1['div_primero_pla'],2)." -**** ".number_format(floatval($div_primero_pla),2);
                        //if ($div_primero_gan>0) echo "si*****".$div_primero_gan;
                        
                        if (($eje1LugarSimple[$hip]>0 && $DivWinPriLugar[$hip]>0) || $eje1LugarSimple[$hip]==99) {
                            if ($row_Recordset1['control_dividendo']==0) {
                                $control_dividendo=1;
                            }
                            $updateSQL = sprintf(
                                "/* PARSEADORES1 new\dividendos_tvg_buildabet11.php - QUERY 2 */ UPDATE carrera 
							SET 
								est_confirmacion=%s,
								control_dividendo=%s,
								eje_primero=%s, 
