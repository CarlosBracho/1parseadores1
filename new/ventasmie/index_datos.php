...
function ObtenerMontoEjeTaq($codT, $fecV, $numE, $codC, $codA)
{
    ...
    $query_Recordset20 = sprintf(
        "/* PARSEADORES1 new\ventasmie\index_datos.php - QUERY 1 */ SELECT 
...
$query_Recordset6 = sprintf("/* PARSEADORES1 new\ventasmie\index_datos.php - QUERY 2 */ SELECT 
	tp.est_venta_ame, ta.tra_codigo, ta.moneda, ta.saldoactual, ta.tipotaquilla, ta.tipo_pago, ta.cod_agencia, ag.tipo_pagoa
...
$query_Recordset4 = "/* PARSEADORES1 new\ventasmie\index_datos.php - QUERY 3 */ SELECT * FROM mensajes
...
$query_Recordset44 = sprintf("/* PARSEADORES1 new\ventasmie\index_datos.php - QUERY 4 */ SELECT 
	me.mensaje
...
$query_Recordset7 = sprintf("/* PARSEADORES1 new\ventasmie\index_datos.php - QUERY 5 */ SELECT * FROM ctrol_ventpag_global_ame WHERE cod_ctrol_ventpag_global_ame =1");
...
if (isset($_POST["MM_insert"]) && $_POST["MM_insert"]=="form1" && isset($_POST["cod_carrera"]) && $_POST["cod_carrera"]!=-1 && $est_control_ventas==0) {
    ...
    $query_Recordset19 = sprintf(
        "/* PARSEADORES1 new\ventasmie\index_datos.php - QUERY 6 */ SELECT can_caballos, hor_carrera, fec_carrera, est_carrera, pau_ventas
								 FROM carrera
								 WHERE cod_carrera = %s 
								 LIMIT 1",
...
//****************************************************************************************************************************
    if ($exito==1 && $maxCa!=1 && $pau_ventas==0) {// guarda e imprime ticket
        ...
        $query_Recordset18 = sprintf(
            "/* PARSEADORES1 new\ventasmie\index_datos.php - QUERY 7 */ SELECT
        ...
        if ($valida==1) {
            ...
            $query_Recordset55 = sprintf(
                "
/* PARSEADORES1 new\ventasmie\index_datos.php - QUERY 8 */ SELECT 
...
tasadecambio.Idtasadecambio = %s",
        ...
        if ($valida==1 && !isset($CRet1) && !isset($CRet2) && !isset($CRet3) && !isset($CRet4) && !isset($CRet5) &&
            ...
            foreach ($apuesta as $apta) {
                ...
                $insertSQL = sprintf(
                    "/* PARSEADORES1 new\ventasmie\index_datos.php - QUERY 9 */ INSERT INTO venta (ser_venta, ticket, cod_taquilla, fec_venta, hor_venta, cod_tventa,
                ...
                if ($tipo_pago==1) {
                    ...
                    $insertSQL15 = sprintf(
                        "/* PARSEADORES1 new\ventasmie\index_datos.php - QUERY 10 */ UPDATE taquilla 
				SET
                ...
                if ($tipo_pagoa==1) {
                    $montoapuesta=$monto[$f]*$tasa;
                    $insertSQL15 = sprintf(
                        "/* PARSEADORES1 new\ventasmie\index_datos.php - QUERY 11 */ UPDATE agencia 
				SET
...
$query_Recordset5 = sprintf(
    "/* PARSEADORES1 new\ventasmie\index_datos.php - QUERY 12 */ SELECT 
	...
	tp.min_ejecarrera, ta.pag_codigo, ag.cod_agencia, ag.alerta_corte_age, ba.cod_banca, ba.alerta_corte_dis
		FROM 
			usuario us, 
			taquilla ta, 
			taquilla_opc_ame tp,
...
$query_Recordset15 = sprintf(
    "/* PARSEADORES1 new\ventasmie\index_datos.php - QUERY 13 */ SELECT ta.est_taquilla_hnac 
		FROM 
			usuario us, 
			taquilla ta, 
...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 new\ventasmie\index_datos.php - QUERY 14 */ SELECT cod_carrera, can_caballos, num_carrera, nom_hipodromo, hor_carrera 
		FROM 
			carrera 
		WHERE 
