...
$query_Recordset2 = sprintf("/* PARSEADORES1 new\ventasmie\ventas_reimprimir_ultimo.php - QUERY 1 */ SELECT ticket FROM venta
...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 new\ventasmie\ventas_reimprimir_ultimo.php - QUERY 2 */ SELECT 
venta.fec_venta, 
venta.hor_venta,
venta.ser_venta,
venta.tra_codigo,
...
if ($horacarrerabd >= horaactual() && $row_Recordset1['fec_venta']==fechaactualbd() && $totalRows_Recordset1>0 &&
    ...
    if ($rimp==1 && $row_Recordset1['rei_ticket']==0) {
        if ($conte==1) {
            $insertSQL = sprintf(
                "/* PARSEADORES1 new\ventasmie\ventas_reimprimir_ultimo.php - QUERY 3 */ INSERT 
					INTO reimpresion 
            ...
            $insertSQL1 = sprintf(
                "/* PARSEADORES1 new\ventasmie\ventas_reimprimir_ultimo.php - QUERY 4 */ UPDATE reimpresion 
					SET can_actual=%s
        ...
        $insertSQL2 = sprintf(
            "/* PARSEADORES1 new\ventasmie\ventas_reimprimir_ultimo.php - QUERY 5 */ UPDATE venta 
				SET rei_ticket=%s
				WHERE num_ticket=%s",
            GetSQLValueString(1, "int"),
...
<DIV id="noprint" align="center" style="float:left; width:250px;">
	<DIV align="center">
		<FORM name="form1">
        	<?php
            if ($tra_codigo==0) {?>
			<INPUT style="FONT-STYLE: normal; FONT-FAMILY: 'MS Sans Serif'; FONT-SIZE: 15px; FONT-WEIGHT: normal" id="cmdPrint" class="boton" onclick="doPrint()" name="cmdPrint" value="Imprimir" type="button">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <?php
            } ?>
			<A onclick=window.close() href="#"><FONT color=#1b007f><SPAN style="FONT-SIZE: 16pt"><B>Cerrar</B></SPAN></FONT></A>
		</FORM>
	</DIV>
</DIV>
    <?php
