...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 new\ventasmie\ventas_configurar_ticket.php - QUERY 1 */ SELECT tp.lar_ticket, tp.tam_ticket, tp.cod_taopcame, tp.tip_ticket, ta.tra_codigo, ta.cod_taquilla
	FROM 
		usuario us,
		taquilla ta, 
		taquilla_opc_ame tp 
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
    $updateSQL = sprintf(
        "/* PARSEADORES1 new\ventasmie\ventas_configurar_ticket.php - QUERY 2 */ UPDATE taquilla_opc_ame 
			SET 
			tip_ticket=%s,
			lar_ticket=%s,
			tam_ticket=%s 
    ...
    $updateSQL = sprintf(
        "/* PARSEADORES1 new\ventasmie\ventas_configurar_ticket.php - QUERY 3 */ UPDATE taquilla 
			SET 
			tra_codigo=%s  
			WHERE 
			cod_taquilla=%s",
