...
$query_Recordset18D =  sprintf(
	"/* PARSEADORES1 logrosresultados\parley.la_resultados.php - QUERY 1 */ SELECT  
*
...
$fechahora=$inicio.' '.$horasistema;
  ...
  $query_RecordsetAE =  sprintf(
	"/* PARSEADORES1 logrosresultados\parley.la_resultados.php - QUERY 2 */ SELECT
...
"
  ...
  $query_RecordsetAJ =  sprintf(
	"/* PARSEADORES1 logrosresultados\parley.la_resultados.php - QUERY 3 */ SELECT
  ...
  echo '<br>'.$equipo1.' juego completo1: '.$juegocompleto1.' juego medio1: '.$juegomedio1.' Anota primero: '.$anotaprimero.' SI NO: '.$datoscurlprimer[21][0].' HRE: '.$HRE.'<br>';
	...
	$yaestaequipo1 = 0;
     ...
     $query_Recordset177 = sprintf(
      "/* PARSEADORES1 logrosresultados\parley.la_resultados.php - QUERY 4 */ SELECT *
    FROM  p2juegos
...
GetSQLValueString($yaestaequipo2cod, "int"),
    ...
    $row_Recordset177['iniciodtp2'] = strtotime('+6 hour', strtotime($row_Recordset177['iniciodtp2']));

    $row_Recordset177['iniciodtp2'] = date("h:i A", $row_Recordset177['iniciodtp2'] );

    $horajuego2 = strtotime('+0 hour', strtotime($horajuego));

    $horajuego2 = date("h:i A", $horajuego2);
   
    if($horajuego2==$row_Recordset177['iniciodtp2']){


      echo $row_Recordset177['Id_p2juegosp2'];  
  }else{
    $juegoduplicado=$row_Recordset177['Id_p2juegosp2'];
      $query_Recordset177 = sprintf(
        "/* PARSEADORES1 logrosresultados\parley.la_resultados.php - QUERY 5 */ SELECT *
      FROM  p2juegos
      WHERE Id_p2juegosp2 <> %s AND idequipo1p2 = %s AND idequipo2p2 = %s AND iniciodtp2 >= %s AND iniciodtp2 <= %s ",
      GetSQLValueString($juegoduplicado, "int"),
      GetSQLValueString($yaestaequipo1cod, "int"),
GetSQLValueString($yaestaequipo2cod, "int"),
      GetSQLValueString($FechaTxtayer.' 00:00:02', "date"),
      GetSQLValueString($FechaTxtayer.' 23:59:59', "date")
      ...
      echo $row_Recordset177['Id_p2juegosp2']; 
                        ...
                        $query_Recordset1444 = sprintf(
                          "/* PARSEADORES1 logrosresultados\parley.la_resultados.php - QUERY 6 */ SELECT * FROM p5resultadosj WHERE 
...
if(isset($datoscurlprimer[1][0])){
  ...
  echo '<br>'.$equipo1.' juego completo1: '.$primeramitad.' juego medio1: '.$juegomedio1.'<br>';
	...
	$yaestaequipo1 = 0;
     ...
     $query_Recordset177 = sprintf(
      "/* PARSEADORES1 logrosresultados\parley.la_resultados.php - QUERY 7 */ SELECT *
    FROM  p2juegos
    WHERE idequipo1p2 = %s AND idequipo2p2 = %s AND iniciodtp2 >= %s AND iniciodtp2 <= %s ",
GetSQLValueString($yaestaequipo1cod, "int"),
GetSQLValueString($yaestaequipo2cod, "int"),
    ...
    $query_Recordset1444 = sprintf(
      "/* PARSEADORES1 logrosresultados\parley.la_resultados.php - QUERY 8 */ SELECT * FROM p5resultadosj WHERE 
      iniciodtp5 >= %s AND iniciodtp5 <= %s AND juegop5 = %s",
      GetSQLValueString($FechaTxtayer.' 00:00:02', "date"),
      GetSQLValueString($FechaTxtayer.' 23:59:59', "date"),
      GetSQLValueString($row_Recordset177['Id_p2juegosp2'], "int")
 ...
 if($totalRows_Recordset1444==0){
  ...
  if ($row_Recordset177['Id_p2juegosp2']>0) {
      ...
      $insertSQL1555 = sprintf(
          " /* PARSEADORES1 logrosresultados\parley.la_resultados.php - QUERY 9 */ INSERT INTO p5resultadosj  
...
$query_Recordset177 = sprintf(
"/* PARSEADORES1 logrosresultados\parley.la_resultados.php - QUERY 10 */ SELECT *
FROM  p2juegos
WHERE idequipo1p2 = %s AND idequipo2p2 = %s AND iniciodtp2 >= %s AND iniciodtp2 <= %s ",
GetSQLValueString($yaestaequipo1cod, "int"),
GetSQLValueString($yaestaequipo2cod, "int"),
...
$query_Recordset1444 = sprintf(
"/* PARSEADORES1 logrosresultados\parley.la_resultados.php - QUERY 11 */ SELECT * FROM p5resultadosj WHERE 
iniciodtp5 >= %s AND iniciodtp5 <= %s AND juegop5 = %s",
GetSQLValueString($FechaTxtayer.' 00:00:02', "date"),
GetSQLValueString($FechaTxtayer.' 23:59:59', "date"),
GetSQLValueString($row_Recordset177['Id_p2juegosp2'], "int")
...
echo 'creando';
$insertSQL1555 = sprintf(
  " /* PARSEADORES1 logrosresultados\parley.la_resultados.php - QUERY 12 */ INSERT INTO p5resultadosj  
(deportep5, juegop5, equipo1p5, equipo2p5, iniciodtp5, tiemposjugadosp5,
r1p5, r2p5, r3p5, r4p5, r5p5, r6p5, r7p5, r8p5, r9p5, r10p5, 
r11p5, r12p5, r13p5, r14p5, r15p5, r16p5, r17p5, r18p5, r19p5, r20p5, 
r21p5, r22p5, r23p5, r24p5, r25p5, r26p5, r27p5, r28p5, r29p5, r30p5, quienmete)
    ...
    if(!strpos($datoscurl33, '4to-tt-CUARTO') && !strpos($datoscurl33, '2da-tt-MITAD') && !strpos($datoscurl33, 'H+R+E')){
      ...
      if(isset($datoscurlprimer[1][0])){
        ...
        if(strlen($equipo1)>0 OR strlen($equipo2)>0){ 
          ...
          $query_Recordset177 = sprintf(
          "/* PARSEADORES1 logrosresultados\parley.la_resultados.php - QUERY 13 */ SELECT *
          FROM  p2juegos
          WHERE idequipo1p2 = %s AND idequipo2p2 = %s AND iniciodtp2 >= %s AND iniciodtp2 <= %s ",
          GetSQLValueString($yaestaequipo1cod, "int"),
          GetSQLValueString($yaestaequipo2cod, "int"),
          ...
          $query_Recordset1444 = sprintf(
          "/* PARSEADORES1 logrosresultados\parley.la_resultados.php - QUERY 14 */ SELECT * FROM p5resultadosj WHERE 
          iniciodtp5 >= %s AND iniciodtp5 <= %s AND juegop5 = %s",
          GetSQLValueString($FechaTxtayer.' 00:00:02', "date"),
          GetSQLValueString($FechaTxtayer.' 23:59:59', "date"),
          GetSQLValueString($row_Recordset177['Id_p2juegosp2'], "int")
          ...
          echo 'creando';
          $insertSQL1555 = sprintf(
            " /* PARSEADORES1 logrosresultados\parley.la_resultados.php - QUERY 15 */ INSERT INTO p5resultadosj  
          (deportep5, juegop5, equipo1p5, equipo2p5, iniciodtp5, tiemposjugadosp5,
          r1p5, r2p5, r3p5, r4p5, r5p5, r6p5, r7p5, r8p5, r9p5, r10p5, 
          r11p5, r12p5, r13p5, r14p5, r15p5, r16p5, r17p5, r18p5, r19p5, r20p5, 
          r21p5, r22p5, r23p5, r24p5, r25p5, r26p5, r27p5, r28p5, r29p5, r30p5, quienmete)
