...
*   **Módulo de Alertas y Trazabilidad**: En scripts de control y alertas (CLI/Cron), implementa la captura de IP multi-entorno mediante fallback dinámico (Web a CLI) y finalizadores seguros (Regla 9.3) para evitar bloqueos. En vistas de edición y listados, rediseña utilizando clases nativas de Bootstrap 4 y delega cargas de datos pesadas/historiales a endpoints asíncronos AJAX (`alertas_historial_ajax.php`) para no sobrecargar el hilo de renderizado principal. Valida siempre la sintaxis con el linter de PHP CLI ante cualquier refactorización lógica para evitar desbalances de llaves, y asegura que la bandera `statusEnvio` y la función `chequearEnvio()` estén definidas en todas las vistas con formularios para evitar ReferenceError en el cliente, incluyendo un temporizador de seguridad de 5 segundos para su desbloqueo en caso de fallas de red. Para el modal de historial, utiliza controladores 100% nativos en JS (abrirHistorial, cerrarHistorial y XMLHttpRequest) desacoplados de jQuery o Bootstrap JS para garantizar cero colisiones de librerías. Para optimizar el rendimiento y mostrar tiempos transcurridos en tiempo real, utiliza subconsultas SQL directas con `MAX(fecha_hora)` sobre `alertas_registros` combinadas con funciones PHP de tiempo relativo, y aplica un offset de +6 horas (`+6 hour`) al renderizar cualquier marca temporal absoluta para homologar las horas a horario de Venezuela (VET), ordenando el listado por fecha de última ejecución de forma descendente (`ultima_ejecucion DESC`) para priorizar visualmente las alertas ejecutadas más recientemente, habiendo sido removidos por completo los botones interactivos de "INICIAR" / "PAUSAR" del listado de acciones.
