...
if (!isset($_SESSION)) {
	...
	$query_Recordset1 =  sprintf(
        "/* PARSEADORES1 parley\adeditarequipo.php - QUERY 1 */ SELECT  
*
				FROM  p1equipos
...
if (isset($_POST['nomequipo'])) {
    $insertSQL1 = sprintf(
        "/* PARSEADORES1 parley\adeditarequipo.php - QUERY 2 */ UPDATE p1equipos 
				SET nomequipop1=%s,
			nomdimp1=%s,
			nommara=%s,
			nomsella=%s,
...
if($_POST['liga']<>$row_Recordset1['liga']){
	$insertSQL2 = sprintf(
        "/* PARSEADORES1 parley\adeditarequipo.php - QUERY 3 */ UPDATE p1equipos 
				SET 
			liga=%s			
				WHERE Id_p1equiposp1=%s",
	...
	$insertSQL2 = sprintf(
        "/* PARSEADORES1 parley\adeditarequipo.php - QUERY 4 */ UPDATE p1equipos 
				SET 
			liga=%s			
				WHERE Id_p1equiposp1=%s",
...
if($_POST['pais']<>$row_Recordset1['pais']){
	$insertSQL3 = sprintf(
        "/* PARSEADORES1 parley\adeditarequipo.php - QUERY 5 */ UPDATE p1equipos 
				SET 
			pais=%s			
				WHERE Id_p1equiposp1=%s",
		GetSQLValueString($_POST['pais'], "text"),
	...
	$insertSQL3 = sprintf(
        "/* PARSEADORES1 parley\adeditarequipo.php - QUERY 6 */ UPDATE p1equipos 
				SET 
			pais=%s			
				WHERE Id_p1equiposp1=%s",
...
"/* PARSEADORES1 parley\adeditarequipo.php - QUERY 7 */ SELECT DISTINCT  liga
FROM  p1equipos
WHERE deportep1=%s
ORDER BY liga",
...
$query_Recordset3 =  sprintf(
	"/* PARSEADORES1 parley\adeditarequipo.php - QUERY 8 */ SELECT DISTINCT  pais
	FROM  p1equipos
	ORDER BY pais",
	GetSQLValueString($row_Recordset1['deportep1'], "int")
	);
