...
function verificarjuego($cod_wuaosite)
{
    ...
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 1 */ SELECT Id_p2juegosp2, idequipo1p2, idequipo2p2, pichee1p2, pichee2p2, cod_wuaosite_empate
    FROM p2juegos 
    WHERE  
    cod_wuaosite = %s ",
...
function verificarlogro($Id_p2juegosp2, $equipo, $tipol)
{
    ...
    $query_Recordset2 = sprintf(
        "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 2 */ SELECT Id_p3logrosp3, logrop3, logroABoRLp3, equipop3
    FROM  p3logros
    WHERE 
    equipop3 = %s AND
    idjuegop3 = %s AND
...
function updatepiche($matches10px, $todox, $equipo)
{
    ...
    if ($equipo==1) {
        ...
        $datetime=$FechaTxt.' '.$horaTxt;
        $insertSQL1 = sprintf(
            "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 3 */ UPDATE p2juegos  
SET pichee1p2=%s			
WHERE cod_wuaosite=%s",
    ...
    if ($equipo==2) {
        ...
        $datetime=$FechaTxt.' '.$horaTxt;
        $insertSQL1 = sprintf(
            "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 4 */ UPDATE p2juegos  
SET pichee2p2=%s			
WHERE cod_wuaosite=%s",
            GetSQLValueString($matches10px, "text"),
            GetSQLValueString($todox, "int")
...
function updatelogro($todolo1, $todolo2, $Id_p3logrosp3)
{
    ...
    $datetime=$FechaTxt.' '.$horaTxt;
    $insertSQL1 = sprintf(
        "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 5 */ UPDATE p3logros 
...
function insertlogro($Id_p2juegosp2in, $Id_p1equiposp1in, $equipoin, $todoin1, $todoin2, $datetimein, $todoin3)
{
    ...
    $datetime=$FechaTxt.' '.$horaTxt;
    $insertSQL = sprintf(
        "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 6 */ INSERT 
INTO p3logros
...
$query_Recordsetuno1 = sprintf(
    "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 7 */ SELECT Id_p2juegosp2 
FROM p2juegos 
WHERE  
iniciodtp2 >= %s  AND
iniciodtp2 <= %s",
...
if ($totalRows_Recordsetuno1==0) {
    $insertSQL = sprintf(
        "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 8 */ INSERT 
INTO p3logros
(idjuegop3, Id_p1equiposp3, equipop3, tipojugadap3, logrop3, logrodtp3, logroABoRLp3) 
VALUES (%s, %s, %s, %s, %s, %s, %s)",
        ...
        if ($todo[9]==3) {
            ...
            if ($empate=="E" && $totaltotalRows==0) {
                ...
                if ($totaltotalRows==0) {
                    ...
                    $insertSQL = sprintf(
                        "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 9 */ INSERT 
				INTO p2juegos
            ...
            if ($empate!="E" && $totaltotalRows==1) {
                ...
                $query_Recordset21 = sprintf(
                    "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 10 */ SELECT Id_p1equiposp1
FROM p1equipos
WHERE  
nomwuaosite = %s",
                ...
                if ($totalRows_Recordset21==0) {
                    ...
                    echo " . . .";
                    $insertSQL = sprintf(
                        "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 11 */ INSERT 
				INTO p1equipos
				...
				VALUES (%s, %s, %s, %s, %s)",
                    ...
                    $Result1 = mysqli_query($conexionbanca, $insertSQL) or die(mysqli_error($conexionbanca));
                    $query_Recordset21 = sprintf(
                        "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 12 */ SELECT Id_p1equiposp1
FROM p1equipos
WHERE  
nomwuaosite = %s",
                        GetSQLValueString(trim($todo[5]), "text")
                ...
                if ($idequipo1p2=='' && $equipo==1) {
                    $insertSQL1 = sprintf(
                        "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 13 */ UPDATE p2juegos 
				SET idequipo1p2=%s		
				WHERE
cod_wuaosite = %s",
                ...
                if ($idequipo2p2=='' && $equipo==2) {
                    $insertSQL1 = sprintf(
                        "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 14 */ UPDATE p2juegos 
				SET idequipo2p2=%s		
				WHERE
cod_wuaosite = %s",
                        GetSQLValueString($row_Recordset21['Id_p1equiposp1'], "text"),
        ...
        if ($todo[9]==4) {
            ...
            echo " . . .";
            $query_Recordset21 = sprintf(
                "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 15 */ SELECT Id_p1equiposp1
FROM p1equipos
WHERE  
nomwuaosite = %s",
                GetSQLValueString(trim($todo[5]), "text")
            ...
            if ($totalRows_Recordset21==0) {
                ...
                echo " . . .";
                $insertSQL = sprintf(
                    "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 16 */ INSERT 
				INTO p1equipos
				(nomequipop1, nomdimp1, nomwuaosite, deportep1, ordenp1) 
				VALUES (%s, %s, %s, %s, %s)",
                    GetSQLValueString(trim(strtoupper($todo[5])), "text"),
                ...
                $Result1 = mysqli_query($conexionbanca, $insertSQL) or die(mysqli_error($conexionbanca));
                $query_Recordset21 = sprintf(
                    "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 17 */ SELECT Id_p1equiposp1
FROM p1equipos
WHERE  
nomwuaosite = %s",
                    GetSQLValueString(trim($todo[5]), "text")
            ...
            if ($totaltotalRows==0) {
                ...
                if ($totaltotalRows==0) {
                    ...
                    $datetime = date('Y-m-j H:i:s', $datetime);
                    $insertSQL = sprintf(
                        "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 18 */ INSERT 
				INTO p2juegos
            ...
            $query_Recordset22 = sprintf(
                "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 19 */ SELECT Id_p1equiposp1, nomwuaosite
FROM p1equipos
WHERE  
Id_p1equiposp1 = %s",
            ...
            if ($idequipo2p2=="" && $row_Recordset22['nomwuaosite']!=$nombresege) {
                ...
                echo " . . .";
                $insertSQL1 = sprintf(
                    "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 20 */ UPDATE p2juegos 
				SET idequipo2p2=%s		
        ...
        if ($todo[9]==6) {
            ...
            echo " . . .";
            $query_Recordset21 = sprintf(
                "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 21 */ SELECT *
FROM p1equipos
WHERE  
nomwuaosite = %s",
                GetSQLValueString(trim($todo[5]), "text")
            ...
            if ($totalRows_Recordset21==0) {
                ...
                echo " . . .";
                $insertSQL = sprintf(
                    "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 22 */ INSERT 
				INTO p1equipos
				(nomequipop1, nomdimp1, nomwuaosite, deportep1, ordenp1) 
				VALUES (%s, %s, %s, %s, %s)",
                    GetSQLValueString(trim(strtoupper($todo[5])), "text"),
                ...
                $Result1 = mysqli_query($conexionbanca, $insertSQL) or die(mysqli_error($conexionbanca));
                $query_Recordset21 = sprintf(
                    "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 23 */ SELECT *
FROM p1equipos
WHERE  
nomwuaosite = %s",
                    GetSQLValueString(trim($todo[5]), "text")
            ...
            if ($totaltotalRows==0) {
                ...
                if ($totaltotalRows==0) {
                    ...
                    $datetime = date('Y-m-j H:i:s', $datetime);
                    $insertSQL = sprintf(
                        "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 24 */ INSERT 
				INTO p2juegos
				(idequipo1p2, deportep2, cod_wuaosite, iniciodtp2) 
				VALUES (%s, %s, %s, %s)",
                        GetSQLValueString($row_Recordset21['Id_p1equiposp1'], "int"),
            ...
            $query_Recordset22 = sprintf(
                "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 25 */ SELECT *
FROM p1equipos
WHERE  
Id_p1equiposp1 = %s",
                GetSQLValueString($idequipo1p2, "int")
            ...
            if ($idequipo2p2=="" && $row_Recordset22['nomwuaosite']!=$nombresege) {
                ...
                echo " . . .";
                $insertSQL1 = sprintf(
                    "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 26 */ UPDATE p2juegos 
				SET idequipo2p2=%s		
				WHERE Id_p2juegosp2=%s",
                    GetSQLValueString($row_Recordset21['Id_p1equiposp1'], "int"),
                    GetSQLValueString($Id_p2juegosp2, "int")
        ...
        if ($todo[9]==1) {
            ...
            echo " . . .";
            $query_Recordset21 = sprintf(
                "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 27 */ SELECT *
FROM p1equipos
WHERE  
nomwuaosite = %s",
                GetSQLValueString(trim($todo[5]), "text")
            ...
            if ($totalRows_Recordset21==0) {
                ...
                echo " . . .";
                $insertSQL = sprintf(
                    "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 28 */ INSERT 
				INTO p1equipos
				(nomequipop1, nomdimp1, nomwuaosite, deportep1, ordenp1) 
				VALUES (%s, %s, %s, %s, %s)",
                    GetSQLValueString(trim(strtoupper($todo[5])), "text"),
                ...
                $Result1 = mysqli_query($conexionbanca, $insertSQL) or die(mysqli_error($conexionbanca));
                $query_Recordset21 = sprintf(
                    "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 29 */ SELECT *
FROM p1equipos
WHERE  
nomwuaosite = %s",
                    GetSQLValueString(trim($todo[5]), "text")
            ...
            if ($totaltotalRows==0) {
                ...
                if ($totaltotalRows==0) {
                    ...
                    $datetime = date('Y-m-j H:i:s', $datetime);
                    $insertSQL = sprintf(
                        "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 30 */ INSERT 
				INTO p2juegos
				(idequipo1p2, deportep2, cod_wuaosite, iniciodtp2) 
				VALUES (%s, %s, %s, %s)",
                        GetSQLValueString($row_Recordset21['Id_p1equiposp1'], "int"),
            ...
            $query_Recordset22 = sprintf(
                "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 31 */ SELECT *
FROM p1equipos
WHERE  
Id_p1equiposp1 = %s",
                GetSQLValueString($idequipo1p2, "int")
            ...
            if ($idequipo2p2=="" && $row_Recordset22['nomwuaosite']!=$nombresege) {
                ...
                echo " . . .";
                $insertSQL1 = sprintf(
                    "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 32 */ UPDATE p2juegos 
				SET idequipo2p2=%s		
				WHERE Id_p2juegosp2=%s",
                    GetSQLValueString($row_Recordset21['Id_p1equiposp1'], "int"),
                    GetSQLValueString($Id_p2juegosp2, "int")
            ...
            if ($todo[1]=="5RL") {
                ...
                if ($Id_p3logrosp3>0) {
                    ...
                    $insertSQL = sprintf(
                        "/* PARSEADORES1 parley\logros\wuao.site.php - QUERY 33 */ INSERT 
