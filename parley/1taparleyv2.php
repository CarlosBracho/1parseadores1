...
$dtime2 =$datetime; 
$dtime2 = strtotime(' -0 hour , -30 minute', strtotime($dtime2)); 
$dtime2 = date('Y-m-d H:i:s', $dtime2);
//
$query_Recordset5 = sprintf(
    "/* PARSEADORES1 parley\1taparleyv2.php - QUERY 1 */ SELECT * FROM usuario, taquilla, taquilla_opc_parley, agencia,
			banca
...
$comb_hembra=$row_Recordset5['comb_hembra']/1;
$monto_apuesta=$row_Recordset5['monto_apuesta']/1;
$factor_pago=$row_Recordset5['factor_pago']/1;
$efectivoOt=$row_Recordset5['efectivoO']/1;
$tipo_pago=$row_Recordset5['tipo_pago'];
$moneda=$row_Recordset5['moneda'];
$scroll=$row_Recordset5['scrollp'];
$ttl=$row_Recordset5['taptipologro'];
$factordehembra=$row_Recordset5['factor_de_hembra'];
$factordemacho=$row_Recordset5['factor_de_macho'];
$tipot=$row_Recordset5['ImpSI0NO1']; //agregado
...
$totalRows_Recordset1=0;
                                            ...
                                            $query_Recordset13 = sprintf(
                                                "/* PARSEADORES1 parley\1taparleyv2.php - QUERY 2 */ SELECT MAX(Idbalancecli) 
    ...
    $Idbalancecli=((int)$row_Recordset13['MAX(Idbalancecli)']);
                                    ...
                                    $query_Recordset14 = sprintf(
                                        "/* PARSEADORES1 parley\1taparleyv2.php - QUERY 3 */ SELECT saldoactualc
					FROM balanceclientes
					WHERE monedac = %s AND
					Idbalancecli = %s",
                                        GetSQLValueString($moneda, "int"),
...
$query_Recordset1111 = sprintf(
    "/* PARSEADORES1 parley\1taparleyv2.php - QUERY 4 */ SELECT logrop6, idp6logrosind, logroABoRLp6, idjuegop6, equipop6, tipojugadap6
FROM  p6logrosind
...
//final logros colocados por agente
    $query_Recordset111 = sprintf(
        "/* PARSEADORES1 parley\1taparleyv2.php - QUERY 5 */ SELECT logrop3, Id_p3logrosp3, logroABoRLp3, idjuegop3, equipop3, tipojugadap3
    FROM  p3logros
...
$query_Recordset1b = sprintf(
    "/* PARSEADORES1 parley\1taparleyv2.php - QUERY 6 */ SELECT * FROM p2juegos WHERE 
deportep2 = %s AND 
iniciodtp2 > %s AND
idequipo1p2 > 0 AND
idequipo1p2 > 0 AND
...
$query_Recordset101 = sprintf(
    "/* PARSEADORES1 parley\1taparleyv2.php - QUERY 7 */ SELECT * FROM p2juegos WHERE 
deportep2 = %s AND 
iniciodtp2 > %s AND
idequipo1p2 > 0 AND
idequipo1p2 > 0 AND
...
$query_Recordset201 = sprintf(
    "/* PARSEADORES1 parley\1taparleyv2.php - QUERY 8 */ SELECT * FROM p2juegos WHERE 
deportep2 = %s AND 
iniciodtp2 > %s AND
idequipo1p2 > 0 AND
idequipo2p2 > 0 AND
...
$query_Recordset301 = sprintf(
    "/* PARSEADORES1 parley\1taparleyv2.php - QUERY 9 */ SELECT * FROM p2juegos WHERE 
deportep2 = %s AND 
iniciodtp2 > %s AND
idequipo1p2 > 0 AND
idequipo2p2 > 0 AND
...
$query_Recordset401 = sprintf(
    "/* PARSEADORES1 parley\1taparleyv2.php - QUERY 10 */ SELECT * FROM p2juegos WHERE 
deportep2 = %s AND 
iniciodtp2 > %s AND
idequipo1p2 > 0 AND
idequipo2p2 > 0 AND
...
<span>
  ...
  <li class="nav-item">
    <a class="nav-link btn-outline-primary" data-toggle="tab" href="#baloncesto"><i class="fa-solid fa-basketball"></i>Baloncesto <?php echo $totalRows_Recordset101;?></a>
  </li>
  <li class="nav-item">
    <a class="nav-link btn-outline-success" data-toggle="tab" href="#futbol"><i class="fa-solid fa-futbol"></i>Futbol <?php echo $totalRows_Recordset201;?></a>
  </li>
  <li class="nav-item">
    <a class="nav-link btn-outline-info" data-toggle="tab" href="#futbolamericano"><i class="fa-solid fa-football"></i>Futbol Americano <?php echo $totalRows_Recordset401;?></a>
  </li>
  <li class="nav-item">
    <a class="nav-link btn-outline-danger" data-toggle="tab" href="#hockey"><i class="fa-solid fa-hockey-puck"></i>Hockey <?php echo $totalRows_Recordset301;?></a>
  </li>
  ...
  <div class="card">
                        <div class="card-body border border-info" style="padding: 0;">
                            <div class="row">
                                <div class="col-lg-12 mb-4 table-responsive">
                                <div id="imp4">
<?php require_once('apc_futbolamericano.php'); ?>
</div style="background-color:#d4eefd;padding:12px;margin:12px 0 12px 0;">
								</div>
                            </div>
                        </div>
...
if ($totalRows_Recordset1b>0) {
    $quiensm=1; ?>
<a class="nav-item btn btn-outline-warning active show" id="nav-home-tab" data-toggle="tab" href="#nav-Beisbol" role="tab" aria-controls="nav-Beisbol" aria-selected="true">
<div class="sportIcon sportIcon--baseball"></div>Beisbol</a>
<?php
}?>
<?php if ($totalRows_Recordset101>0) {?>
<a class="nav-item btn btn-outline-primary <?php if ($quiensm==1) {
        echo '';
    } else {
        echo 'active show';
...
<?php }?>
<?php if ($totalRows_Recordset201>0) {?>
<a class="nav-item  btn btn-outline-success <?php if ($quiensm==1) {
        echo '';
    } else {
        echo 'active show';
...
<?php }?>
<?php if ($totalRows_Recordset301>0) {?>
<a class="nav-item  btn btn-outline-danger <?php if ($quiensm==1) {
        echo '';
    } else {
        echo 'active show';
...
<a class="nav-item  btn btn-outline-danger <?php if ($quiensm==1) {
        ...
        echo 'active show';
    } ?>" id="nav-contact-tab" data-toggle="tab" href="#nav-futbolamericano" role="tab" aria-controls="nav-futbolamericano" aria-selected="<?php if ($quiensm==1) {
        echo 'false';
    } else {
        echo 'true';
        $quiensm=1;
    } ?>">
<div class="sportIcon sportIcon--hockey"></div>Futbol Americano</a>
<?php }?>
...
<?php if ($totalRows_Recordset401>0) {?>
                ...
                if ($quiensm2==1) {
                    ...
                    echo 'active show';
                    $quiensm2=1;
                } ?>" id="nav-futbolamericano" role="tabpanel" aria-labelledby="nav-contact-tab">
                    <div class="card">
                        <div class="card-body border border-danger">
                            <div class="row">
                                <div class="col-lg-12 mb-4">
...
?>
        ...
        <button type="button" class="btn btn-primary">El siguiente es el numeroa  indicar a la taquillera //agregado</button>

            <form action="1taparleyv2.php" method="post" accept-charset="utf-8" id="form" name="form">           
            ...
            <input type="hidden" name="tipot2" value="1">
			<input type="hidden" name="taquilla" value="<?php echo $taquilla; ?>">
            <?php if($moneda<12){?>
			<input type="hidden" name="monedacod" value="<?php echo $moneda; ?>"> <?php }elseif($moneda==12){?>
            <input type="hidden" name="monedacod" value="12"> <?php }?>
    ...
    <td class="calculadora">&nbsp;Equipo</td>-->
                ...
                <tr class="ui-state-default ui-corner-all ui-state-focus liga"><?php if ($MM_authorizedUsers=='C') {  ?><td colspan="2">SALDO DISPONIBLE:. <br><span style=" font-size:25px;color:#fc3c03"><?php echo $saldoactualc; ?></span></td> <?php   }?></tr>                <tr>
                    <td colspan="4">
                        ...
                        <input name="montoApostar" id="montoApostar" type="text" class="form-control"  size="10" style=" height:50px; font-size:30px" maxlength="10" autocomplete="OFF"  onkeyup="premio(this.value=this.value.replace(/^(\-)*(\d+)\.(\d\d).*$/,'$1$2.$3')); this.value=this.value.replace(/[^0-9.]/g, '');">
                    </td>
                </tr>
                ...
                <tr>
                    <td colspan="2">
                        <div class="row">
                            <div class="col-md-12 col-lg-7">
                                ...
                                <?php if($moneda==12){?>
                                <br><select name="monedaD" class="form-/* PARSEADORES1 parley\1taparleyv2.php - QUERY 11 */ select textbox table-primary" style="width:175px; font-size:20px; height:30px;">
                    ...
                    <?php }?> 
                            ...
                            <div class="col-md-12 col-lg-5">
                                <!--<button type="button"  id="limpiar_parley"  class="btn btn-warning" onclick="limpiarPantalla()" style="font-size:12px; height:30px;"><i class="fa fa-paint-brush" aria-hidden="true"></i> Limpiar</button>-->
                                <button class="btn btn-warning btn-block btn-sm" onclick="limpiarPantalla();return false"><i class="fa fa-paint-brush" aria-hidden="true"></i> Limpiar</button>
                            </div>
...
<!-- Modal CODIGO AUTOSERVICIO -->

<?php //agregado
echo $tipot;
if($tipot==1){
    $dtime2x=$FechaTxt.' '.$horaTxt; 
    $dtime2x = strtotime(' -0 hour , -10 minute', strtotime($dtime2x)); 
    $dtime2x = date('Y-m-d H:i:s', $dtime2x);



    $query_Recordset12x = sprintf(
        "/* PARSEADORES1 parley\1taparleyv2.php - QUERY 12 */ SELECT 
*
...
p4jugadas
            ...
            jugadadtp4 > %s AND
            tipot = 1 AND
            id_usuariop4 = %s AND lineatp4 = 1  ORDER BY nticketp4 DESC LIMIT 5
",
GetSQLValueString($dtime2x, "date"),
        GetSQLValueString($_SESSION['MM_id_usuario'], "int")
    );
    $Recordset12x = mysqli_query($conexionbanca, $query_Recordset12x) or die(mysqli_error($conexionbanca));
    $row_Recordset12x = mysqli_fetch_assoc($Recordset12x);
    $totalRows_Recordset12x = mysqli_num_rows($Recordset12x);



if($totalRows_Recordset12x>0){
...
<script type="text/javascript">
$( document ).ready(function() {
    $('#AUTOSERVICIO').modal('toggle')
});
$.get( "ultimotickettap.php?recordID="+usuario, function( data ) {
 $( "#viewTicketultimoauto" ).html( data );
});
</script>
<div class="modal fade" id="AUTOSERVICIO" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title " id="AUTOSERVICIOLabel">Ultima Jugada Creada Sin Validar Aun</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true"></span>
        </button>
      </div>
      <div class="modal-body">
          ...
          <button type="button" class="btn btn-primary">El siguiente es el numeroa  indicar a la taquillera </button>
          <button type="button" class="btn btn-primary"> # <?php  echo $row_Recordset12x['pcan_ticket'];     ?></button>
          </div>
            <div id="viewTicketultimoauto">
...
<script>
   // alert('deporte');
    ...
    <script src="../js/sweetalert2.all.js"></script>
                            ...
                            <script src="./Vendedor/taquillav2.js"></script>
    ...
    var com_bax = <?php echo  $comb_maxparley; ?>; // maximo logros 
    ...
    var imprime = "S";
    var tipot = "C";
    var tipot2 = 1;//agregado
    <?php if ($monto_apuesta<>0) {  ?>
    var monto_premio = <?php echo $monto_apuesta; ?>;
    <?php } else { ?>
    var monto_premio = 9999999.99;
    <?php } ?>
    ...
    var comb_hembra = <?php echo  $comb_hembra; ?>; //esta
    <?php if($factordehembra==0){ ?>
    var fact_mult_hem = 100;
    <?php } else { ?>
    var fact_mult_hem = <?php echo $factordehembra; ?>;
    <?php } ?>
    <?php if($factordemacho==0){ ?>
    var fact_mult_mac = 100;
    <?php } else { ?>
    var fact_mult_mac = <?php echo $factordemacho; ?>;
    <?php } ?>   
    ...
    var cupoDvend = 0.00;
    var factor_pago = <?php echo $factor_pago; ?>;
    ...
    var mxtcrep = "5"; //ticket repetidos
    var mxhemper = "N";
    var mxmachper = "N";
    var mxcombft = "N";
    var mxcombbs = "N";
