 ...
 * Function verif_hora();
 * Variable class
 * Proceso verifica la hora del juego con la hora de la referencia enlinea --- Mismarcardores
 * fecha Creada 13/07/2020
 * Autor Jose Rojas
 * ****/
    ...
    if (checked) {
      ...
      for (var m = 0; m < logrosCalc.length; m++) {
        if (
          ...
          //alert("Advertencia!. No puede seleccionar 2 ejemplares en la misma Valida.\n\nPor favor reformule su apuesta.");
          toastr.warning(
...
function premio(key, obj) {
  ...
  if (macuare === 'N') {
    ...
    with (document.forms[0]) {
      ...
      try {
        var cuenta =
          montoApostar.value != '' && parseInt(montoApostar.value) > 0
            ? true
            : false;
      ...
      for (var y = 0; y < logrosCalc.length; y++) {
        if (cuenta) {
          ...
          //apuesta = calcular_premio_factor(apuesta)
          //montoPremio.value = Math.round(apuesta);
          montoPremio.value = dosDecimales(apuesta); //Math.round(apuesta);
        }
      }
      //montoPremio2.value = Math.round(calcular_premio_factor(montoPremio.value))
        ...
        case 'P': //'PRINTER-DIRECT'
          ...
          if (navegador == 'Microsoft Internet Explorer') {
            if (!ActiveXEnabledOrUnnecessary()) {
              ...
              var consola = new ActiveXObject('WScript.Shell');
              $.post(
                "<?php  echo site_url('ingresar_venta.php') ?>",
                $('#form').serialize() +
                  '&tipo=' +
                  tipo +
                  '' +
          ...
          $.post(
            ...
            function (eData) {
              if (eData == 'Error') {
                ...
                $.post(
                  "<?php  echo site_url('home/impresion_firefox') ?>",
                  { n_ticket: nticket },
          ...
          $.post(
            ...
            function (eData) {
              if (eData > 0) {
                ...
                $.post(
                  "<?php  echo site_url('home/buscar_ventas_detalle') ?>",
                  { n_ticket: nticket },
                  function (eData) {
                    if (eData != logrosCalc.length) {
                      ...
                      $.post(
                        "<?php  echo site_url('home/anular_ticket_auto') ?>",
                        { n_ticket: nticket },
                        function (eData) {
                          alert(
      ...
      if (result) {
        ...
        $.ajax({
          ...
          error: function (e) {
            ...
            bootbox.alert({
              message:
                '<div class="text-center"><h3>NO HUBO RESPUESTA DEL SERVIDOR<h3/>Por favor intente de nuevo</div>',
              callback: function () {
                $('#btnImprimir').prop('disabled', false);
                $('#btnEmail').prop('disabled', false);
                $('#btnSms').prop('disabled', false);
