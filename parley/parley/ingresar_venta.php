...
$query_Recordset5 = sprintf(
    "
/* PARSEADORES1 parley\parley\ingresar_venta.php - QUERY 1 */ SELECT * FROM  taquilla, taquilla_opc_parley, agencia,
			banca
WHERE 
...
$ip_ventap=getRealIP();
$query_Recordset1 = "/* PARSEADORES1 parley\parley\ingresar_venta.php - QUERY 2 */ SELECT MAX(Id_p4jugadasp4) FROM p4jugadas";
...
foreach ($data as $key => $value) {
    $query_Recordset1cc =  sprintf(
        "/* PARSEADORES1 parley\parley\ingresar_venta.php - QUERY 3 */ SELECT 
*
FROM
p2juegos
WHERE
Id_p2juegosp2 = %s AND   iniciodtp2 < %s",
    ...
    $query_Recordset1lc =  sprintf(
        "/* PARSEADORES1 parley\parley\ingresar_venta.php - QUERY 4 */ SELECT 
*
FROM
p3logros
WHERE
...
if ($juegocerrado==0) {   //1verifica si no hay juegos cerrados
    ...
    $query_Recordset13 = sprintf(
        "/* PARSEADORES1 parley\parley\ingresar_venta.php - QUERY 5 */ SELECT MAX(Idbalancecli) 
    ...
    $query_Recordset14 = sprintf(
        "/* PARSEADORES1 parley\parley\ingresar_venta.php - QUERY 6 */ SELECT saldoactualc
					FROM balanceclientes
					WHERE monedac = %s AND
					Idbalancecli = %s",
        GetSQLValueString($moneda, "int"),
    ...
    $query_Recordset12 = sprintf(
        "/* PARSEADORES1 parley\parley\ingresar_venta.php - QUERY 7 */ SELECT MAX(nticketp4) 
					FROM p4jugadas
    ...
    $query_Recordset15 = sprintf(
        "/* PARSEADORES1 parley\parley\ingresar_venta.php - QUERY 8 */ SELECT 
	*
FROM 
	p4jugadas
WHERE 
nticketp4 = %s AND
    ...
    $insertSQL155 = sprintf(
        "/* PARSEADORES1 parley\parley\ingresar_venta.php - QUERY 9 */ INSERT INTO balanceclientes  
