...
function ObtenerMontoEjeTaq($codT, $fecV, $numE, $codC, $codA)
{
    ...
    $query_Recordset20 = sprintf(
        "/* PARSEADORES1 ventasmie\index_datos.php - QUERY 1 */ SELECT 
...
$query_Recordset6 = sprintf("/* PARSEADORES1 ventasmie\index_datos.php - QUERY 2 */ SELECT 
	tp.est_venta_ame, ta.tra_codigo, ta.moneda, ta.saldoactual, ta.tipotaquilla, ta.tipo_pago, ta.cod_agencia, ag.tipo_pagoa, us.hor_inicio, us.hor_fin, us.24hr
	FROM taquilla ta, usuario us, taquilla_opc_ame tp , agencia ag 
	WHERE  ta.cod_agencia = ag.cod_agencia AND tp.cod_taquilla = us.cod_taquilla AND ta.cod_taquilla = us.cod_taquilla AND us.id_usuario = %s LIMIT 1", GetSQLValueString($usuarioVenta, "int"));
$Recordset6 = mysqli_query($conexionbanca, $query_Recordset6) or die(mysqli_error($conexionbanca));
$row_Recordset6 = mysqli_fetch_assoc($Recordset6);
$totalRows_Recordset6 = mysqli_num_rows($Recordset6);
$tipotaquilla=$row_Recordset6['tipotaquilla']/1;
$tra_codigo=$row_Recordset6['tra_codigo']/1;
$saldoactual=$row_Recordset6['saldoactual']/1;
$tipo_pago=$row_Recordset6['tipo_pago'];
$tipo_pagoa=$row_Recordset6['tipo_pagoa'];
$cod_agencia=$row_Recordset6['cod_agencia'];
...
$selCarrera="";
if ($row_Recordset6['tra_codigo']==0) {
    $editFormAction = $_SERVER['PHP_SELF'];
    if (isset($_SERVER['QUERY_STRING'])) {
        $editFormAction .= "?" . htmlentities($_SERVER['QUERY_STRING']);
    }
} else {
    $row_Recordset6['tra_codigo']." ".$editFormAction = 'rapuesta_codigo_clientes.php';
}
$query_Recordset4 = "/* PARSEADORES1 ventasmie\index_datos.php - QUERY 3 */ SELECT * FROM mensajes
...
$query_Recordset44 = sprintf("/* PARSEADORES1 ventasmie\index_datos.php - QUERY 4 */ SELECT 
	me.mensaje
...
if (($row_Recordset4['est_mensaje']==1 or $row_Recordset4['est_mensaje']==9)) {
    $mensaje1=$mensaje1;
} else {
    $mensaje1="";
}
$query_Recordset7 = sprintf("/* PARSEADORES1 ventasmie\index_datos.php - QUERY 5 */ SELECT * FROM ctrol_ventpag_global_ame WHERE cod_ctrol_ventpag_global_ame =1");
$Recordset7 = mysqli_query($conexionbanca, $query_Recordset7) or die(mysqli_error($conexionbanca));
$row_Recordset7 = mysqli_fetch_assoc($Recordset7);
$totalRows_Recordset7 = mysqli_num_rows($Recordset7);
$cod_control=$row_Recordset7['cod_ctrol_ventpag_global_ame'];
...
if(isset($_SESSION['Carrera'])){
    ...
    $query_Recordset180 = sprintf(
        "/* PARSEADORES1 ventasmie\index_datos.php - QUERY 6 */ SELECT  mon_venta, hor_venta, num_caballo, efectivoO, cod_carrera
        FROM venta
    WHERE 
...
$query_Recordset180D = sprintf(
    "/* PARSEADORES1 ventasmie\index_datos.php - QUERY 7 */ SELECT  tp.limit_ticket_ame, us.cod_taquilla
...
if (isset($_POST["MM_insert"]) && $_POST["MM_insert"]=="form1" && isset($_POST["cod_carrera"]) && $_POST["cod_carrera"]!=-1 && $est_control_ventas==0) {
    ...
    $query_Recordset19 = sprintf(
        "/* PARSEADORES1 ventasmie\index_datos.php - QUERY 8 */ SELECT can_caballos, hor_carrera, fec_carrera, est_carrera, pau_ventas
								 FROM carrera
								 WHERE cod_carrera = %s 
								 LIMIT 1",
    ...
    if (isset($_POST["efectivoO"])) {
        $_POST["efectivoO1"]=$_POST["efectivoO"]/1;
    } else {
        $_POST["efectivoO1"]=0;
    }
    echo $_POST['efectivoO'];
...
//****************************************************************************************************************************
    if ($exito==1 && $maxCa!=1 && $pau_ventas==0) {// guarda e imprime ticket
        ...
        $query_Recordset18 = sprintf(
            "/* PARSEADORES1 ventasmie\index_datos.php - QUERY 9 */ SELECT
        ...
        if ($valida==1) {
            ...
            foreach ($apuesta as $busca) {  //busca exceso o minimos de montos
                ...
                if ($tipo[$t-1]==1 && 	$_POST['efectivoO']==3) {
                ...
                if ($tipo[$t-1]==1 && 	$_POST['efectivoO']==4) {
                ...
                if ($tipo[$t-1]==1 && 	$_POST['efectivoO']==5) {
            ...
            $query_Recordset55 = sprintf(
                "
/* PARSEADORES1 ventasmie\index_datos.php - QUERY 10 */ SELECT 
...
tasadecambio.Idtasadecambio = %s",
            ...
            if ($totalTicketx>$saldoactuala && $tipo_pago==1) {
                $valida=0;
                $saldobajoa=1;
                $exito=0;  //Monto de ticket excedido
            }
        ...
        if ($valida==1 && !isset($CRet1) && !isset($CRet2) && !isset($CRet3) && !isset($CRet4) && !isset($CRet5) &&
            ...
            foreach ($apuesta as $apta) {
                ...
                if(isset($_SESSION['Carrera'])){
                    $permisos=1;
if($apta==$_SESSION['Ejemplar'] && $monto[$f]==$_SESSION['Monto'] && $_POST['cod_carrera']==$_SESSION['Carrera'] && $_POST['efectivoO1']==$_SESSION['Tipomoneda'] && $timeticket > $horaTxt){
   
    $permisos=0;
}else{                        
                $insertSQL = sprintf(
                    "/* PARSEADORES1 ventasmie\index_datos.php - QUERY 11 */ INSERT INTO venta (ser_venta, ticket, cod_taquilla, fec_venta, hor_venta, cod_tventa,
					 num_caballo, mon_venta, cod_carrera, id_usuario, est_ticket, can_ticket, ip_venta, lin_ticket, efectivoO) 
                    ...
                    GetSQLValueString($_POST['efectivoO1'], "int")
                );
                
                $Result = mysqli_query($conexionbanca, $insertSQL) or die(mysqli_error($conexionbanca));
                $_SESSION['Ejemplar']=$apta;
$_SESSION['Carrera']=$_POST['cod_carrera'];
$_SESSION['Tipomoneda']=$_POST['efectivoO1'];
$_SESSION['Hora']=$horaTxt;
                if ($tipo_pago==1) {
                    $montoapuesta=$monto[$f]*$tasa;
                    $insertSQL15 = sprintf(
                        "/* PARSEADORES1 ventasmie\index_datos.php - QUERY 12 */ UPDATE taquilla 
				SET
                ...
                if ($tipo_pagoa==1) {
                    $montoapuesta=$monto[$f]*$tasa;
                    $insertSQL15 = sprintf(
                        "/* PARSEADORES1 ventasmie\index_datos.php - QUERY 13 */ UPDATE agencia 
				SET
        ...
        $insertSQL = sprintf(
            "/* PARSEADORES1 ventasmie\index_datos.php - QUERY 14 */ INSERT INTO venta (ser_venta, ticket, cod_taquilla, fec_venta, hor_venta, cod_tventa,
             num_caballo, mon_venta, cod_carrera, id_usuario, est_ticket, can_ticket, ip_venta, lin_ticket, efectivoO) 
             VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
            GetSQLValueString($serial, "text"),
            GetSQLValueString($numerotiket2, "int"),
...
$_SESSION['Hora']=$horaTxt;
        ...
        if ($tipo_pago==1) {
            $montoapuesta=$monto[$f]*$tasa;
            $insertSQL15 = sprintf(
                "/* PARSEADORES1 ventasmie\index_datos.php - QUERY 15 */ UPDATE taquilla 
        SET
        saldoactual=saldoactual-%s
        WHERE 
        cod_taquilla=%s",
        ...
        if ($tipo_pagoa==1) {
            $montoapuesta=$monto[$f]*$tasa;
            $insertSQL15 = sprintf(
                "/* PARSEADORES1 ventasmie\index_datos.php - QUERY 16 */ UPDATE agencia 
        SET
        saldoactuala=saldoactuala-%s
        WHERE 
        cod_agencia=%s",
    ...
    if (isset($_SESSION['Carrera']) && $permisos==0) {
        $iR=1;
        $_SESSION['MM_mensaje3']="Ticket posiblemente duplicado intente de nuevo en ".$timeecho." segundos";
    }
...
$query_Recordset5 = sprintf(
    "/* PARSEADORES1 ventasmie\index_datos.php - QUERY 17 */ SELECT 
	ta.cod_taquilla, ta.nom_taquilla, ta.tipotaquilla, ta.efectivoO, ta.tra_codigo, ta.saldoactual, ta.alerta_corte_taq, us.nom_completo, 
	tp.apu_maxgan, tp.apu_maxpla, tp.apu_maxsho, tp.apu_mingan, tp.apu_minpla, tp.apu_minsho, tp.min_ejecarrera, 
	tp.min_ejecarrera, tp.pag_codigo, ag.cod_agencia, ag.alerta_corte_age, ba.cod_banca, ba.alerta_corte_dis, us.hor_fin, us.hor_inicio
		FROM 
			usuario us, 
			taquilla ta, 
			taquilla_opc_ame tp,
...
$pedirCodigoPago=$row_Recordset5['pag_codigo'];
...
$query_Recordset15 = sprintf(
    "/* PARSEADORES1 ventasmie\index_datos.php - QUERY 18 */ SELECT ta.est_taquilla_hnac 
		FROM 
			usuario us, 
			taquilla ta, 
...
if ($totalRows_Recordset15>0) {
    $est_hnac=$row_Recordset15['est_taquilla_hnac'];
} else {
    $est_hnac=0;
}


$query_Recordset44D = sprintf("/* PARSEADORES1 ventasmie\index_datos.php - QUERY 19 */ SELECT 
	ta.cod_taquilla
	...
	ORDER BY RAND() LIMIT 1", GetSQLValueString($DAMG, "int"));
$Recordset44D = mysqli_query($conexionbanca, $query_Recordset44D) or die(mysqli_error($conexionbanca));
$row_Recordset44D = mysqli_fetch_assoc($Recordset44D);
$totalRows_Recordset44D = mysqli_num_rows($Recordset44D);

$query_Recordset51 = sprintf(
    "/* PARSEADORES1 ventasmie\index_datos.php - QUERY 20 */ SELECT 
...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 ventasmie\index_datos.php - QUERY 21 */ SELECT cod_carrera, can_caballos, num_carrera, nom_hipodromo, hor_carrera, cod_hipodromo
		FROM 
			carrera 
		WHERE 
...
if ($totalRows_Recordset1>0) {
    do {
        ...
        $canRetirados=cantRetirados($row_Recordset1['cod_carrera']);
        $vanEnCarrera=$row_Recordset1['can_caballos']-$canRetirados;
        if ($h==0 && $m<59 && $ejeMinCar<=$vanEnCarrera) {
