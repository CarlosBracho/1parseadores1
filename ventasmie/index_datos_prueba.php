...
function ObtenerMontoEjeTaq($codT, $fecV, $numE, $codC, $codA)
{
    ...
    $query_Recordset20 = sprintf(
        "/* PARSEADORES1 ventasmie\index_datos_prueba.php - QUERY 1 */ SELECT 
...
$query_Recordset6 = sprintf("/* PARSEADORES1 ventasmie\index_datos_prueba.php - QUERY 2 */ SELECT 
	tp.hip_bloqueados, tp.est_venta_ame, ta.tra_codigo, ta.moneda, ta.saldoactual, ta.tipotaquilla, ta.tipo_pago, ta.cod_agencia, ag.tipo_pagoa, us.hor_inicio, us.hor_fin, us.24hr
...
$query_Recordset4 = "/* PARSEADORES1 ventasmie\index_datos_prueba.php - QUERY 3 */ SELECT * FROM mensajes
...
$query_Recordset44 = sprintf("/* PARSEADORES1 ventasmie\index_datos_prueba.php - QUERY 4 */ SELECT 
	me.mensaje
...
$query_Recordset7 = sprintf("/* PARSEADORES1 ventasmie\index_datos_prueba.php - QUERY 5 */ SELECT * FROM ctrol_ventpag_global_ame WHERE cod_ctrol_ventpag_global_ame =1");
...
if (isset($_POST["MM_insert"]) && $_POST["MM_insert"]=="form1" && isset($_POST["cod_carrera"]) && $_POST["cod_carrera"]!=-1 && $est_control_ventas==0) {
    ...
    $query_Recordset19 = sprintf(
        "/* PARSEADORES1 ventasmie\index_datos_prueba.php - QUERY 6 */ SELECT can_caballos, hor_carrera, fec_carrera, est_carrera, pau_ventas
								 FROM carrera
								 WHERE cod_carrera = %s 
								 LIMIT 1",
...
//****************************************************************************************************************************
    if ($exito==1 && $maxCa!=1 && $pau_ventas==0) {// guarda e imprime ticket
        ...
        $query_Recordset18 = sprintf(
            "/* PARSEADORES1 ventasmie\index_datos_prueba.php - QUERY 7 */ SELECT
        ...
        if ($valida==1) {
            ...
            $query_Recordset55 = sprintf(
                "
/* PARSEADORES1 ventasmie\index_datos_prueba.php - QUERY 8 */ SELECT 
...
tasadecambio.Idtasadecambio = %s",
        ...
        if ($valida==1 && !isset($CRet1) && !isset($CRet2) && !isset($CRet3) && !isset($CRet4) && !isset($CRet5) &&
            ...
            foreach ($apuesta as $apta) {
                ...
                $insertSQL = sprintf(
                    "/* PARSEADORES1 ventasmie\index_datos_prueba.php - QUERY 9 */ INSERT INTO venta (ser_venta, ticket, cod_taquilla, fec_venta, hor_venta, cod_tventa,
                ...
                if ($tipo_pago==1) {
                    ...
                    $insertSQL15 = sprintf(
                        "/* PARSEADORES1 ventasmie\index_datos_prueba.php - QUERY 10 */ UPDATE taquilla 
				SET
                ...
                if ($tipo_pagoa==1) {
                    $montoapuesta=$monto[$f]*$tasa;
                    $insertSQL15 = sprintf(
                        "/* PARSEADORES1 ventasmie\index_datos_prueba.php - QUERY 11 */ UPDATE agencia 
				SET
...
$query_Recordset5 = sprintf(
    "/* PARSEADORES1 ventasmie\index_datos_prueba.php - QUERY 12 */ SELECT 
	...
	tp.min_ejecarrera, tp.pag_codigo, ag.cod_agencia, ag.alerta_corte_age, ba.cod_banca, ba.alerta_corte_dis
		FROM 
			usuario us, 
			taquilla ta, 
			taquilla_opc_ame tp,
...
$query_Recordset15 = sprintf(
    "/* PARSEADORES1 ventasmie\index_datos_prueba.php - QUERY 13 */ SELECT ta.est_taquilla_hnac 
		FROM 
			usuario us, 
			taquilla ta, 
...
$query_Recordset44D = sprintf("/* PARSEADORES1 ventasmie\index_datos_prueba.php - QUERY 14 */ SELECT 
	ta.cod_taquilla
...
$query_Recordset51 = sprintf(
    "/* PARSEADORES1 ventasmie\index_datos_prueba.php - QUERY 15 */ SELECT 
...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 ventasmie\index_datos_prueba.php - QUERY 16 */ SELECT cod_carrera, can_caballos, num_carrera, nom_hipodromo, hor_carrera, cod_hipodromo
		FROM 
			carrera 
		WHERE 
...
$t=0;
$x=0;
//$_SESSION['hi_blo_list']=$row_Recordset6['hip_bloqueados'];
//var_dump($_SESSION['hi_blo_list']);




$hi_blo_explode=explode(",", $row_Recordset6['hip_bloqueados']);
