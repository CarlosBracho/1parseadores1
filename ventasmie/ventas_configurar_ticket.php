...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 ventasmie\ventas_configurar_ticket.php - QUERY 1 */ SELECT tp.lar_ticket, tp.tam_ticket, tp.cod_taopcame, tp.tip_ticket, ta.tra_codigo, ta.cod_taquilla
	FROM 
		usuario us,
		taquilla ta, 
		taquilla_opc_ame tp 
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
    $updateSQL = sprintf(
        "/* PARSEADORES1 ventasmie\ventas_configurar_ticket.php - QUERY 2 */ UPDATE taquilla_opc_ame 
			SET 
			tip_ticket=%s,
			lar_ticket=%s,
			tam_ticket=%s 
    ...
    $updateSQL = sprintf(
        "/* PARSEADORES1 ventasmie\ventas_configurar_ticket.php - QUERY 3 */ UPDATE taquilla 
			SET 
			tra_codigo=%s  
			WHERE 
			cod_taquilla=%s",
...
<div style="background: #0072C6; width:100%; float:left; padding:50px 2px 2px 2px;
  ...
  <div style="background: #FFF; width:100%; float:left; padding:25px 0px 0px 10px; font-size:16px">
    ...
    echo "SELECTED";
			...
			<option value="3" <?php if (!(strcmp(3, htmlentities($row_Recordset1['tip_ticket'], ENT_COMPAT, 'utf-8')))) {
    echo "SELECTED";
} ?>>REDUCIDO 2</option>
			<option value="4" <?php if (!(strcmp(4, htmlentities($row_Recordset1['tip_ticket'], ENT_COMPAT, 'utf-8')))) {
    echo "SELECTED";
} ?>>ANONIMO</option>
