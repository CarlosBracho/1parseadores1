...
$query_Recordset1 = sprintf("/* PARSEADORES1 admin_lot\admin_reimprimir_ticket_lot.php - QUERY 1 */ SELECT 
	ta.nom_taquilla,
	ta.nom_taquilla,
 	ve.fec_venta_lot,
	ve.hor_venta_lot,
	...
	lt.nom_loteria,
	CASE lt.tip_loteria  
		WHEN 2 THEN (/* PARSEADORES1 admin_lot\admin_reimprimir_ticket_lot.php - QUERY 2 */ SELECT sor.nom_corto FROM signos sor WHERE sor.id_signo = ve.id_signo LIMIT 1)
		WHEN 3 THEN (/* PARSEADORES1 admin_lot\admin_reimprimir_ticket_lot.php - QUERY 3 */ SELECT sor.nom_corto FROM signos sor WHERE sor.id_signo = ve.id_signo LIMIT 1)
		WHEN 4 THEN (/* PARSEADORES1 admin_lot\admin_reimprimir_ticket_lot.php - QUERY 4 */ SELECT ani.nom_animal FROM animales ani WHERE ani.num_animal = ve.num_apuesta_lot LIMIT 1)
		WHEN 5 THEN (/* PARSEADORES1 admin_lot\admin_reimprimir_ticket_lot.php - QUERY 5 */ SELECT fru.nom_corto FROM frutas fru WHERE fru.id_fruta = ve.id_signo LIMIT 1)
		WHEN 6 THEN (/* PARSEADORES1 admin_lot\admin_reimprimir_ticket_lot.php - QUERY 6 */ SELECT pal.nom_corto FROM palos_cartas pal WHERE pal.id_palo = ve.id_signo LIMIT 1)
		ELSE ''
	END AS nsigno,
	ba.hor_cierre
FROM 
