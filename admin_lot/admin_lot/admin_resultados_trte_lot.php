...
if ((isset($_POST["MM_updateRes"])) && ($_POST["MM_updateRes"] == "form2") && (isset($_POST["guardar_Res"]))) {
            ...
            for ($i = 0; $i < count($pTriple); ++$i) {
                ...
                if (strlen(trim($x_ganador[$i]))==3) {
                    $query_Recordset13 =  sprintf(
                        "/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 1 */ SELECT re.num_resultado, re.sig_resultado, re.id_resultado
                    ...
                    if ($totalRows_Recordset13==0) {
                        $insertSQL2 = sprintf(
                            "/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 2 */ INSERT 
							INTO resultados_lot 
                        ...
                        $insertSQL1 = sprintf(
                            "/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 3 */ UPDATE resultados_lot SET num_resultado=%s, sig_resultado=%s
                ...
                if ($premiar==1) {
                    $query_Recordset14 = sprintf(
                        "/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 4 */ SELECT
						ve.num_ticket_lot, ve.tip_loteria_lot, ve.num_apuesta_lot, ve.mon_apuesta_lot, ve.id_signo, ve.id_loteria,
						(/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 5 */ SELECT pre_loteria FROM bancaloterias bl WHERE bl.id_loteria=ve.id_loteria AND 
    ...
    if (isset($_POST["cod_animal"])) {
        ...
        foreach ($_POST["cod_animal"] as $cod_animal) {
            ...
            if ($cod_animal>0) {
                $query_Recordset13 =  sprintf(
                    "/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 6 */ SELECT re.num_resultado, re.sig_resultado, re.id_resultado
					FROM resultados_lot re WHERE id_loteria=%s AND fec_resultado=%s AND id_banca=%s LIMIT 1",
                ...
                if ($totalRows_Recordset13==0) {
                    $insertSQL2 = sprintf(
                        "/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 7 */ INSERT 
						INTO resultados_lot 
                    ...
                    $insertSQL1 = sprintf(
                        "/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 8 */ UPDATE resultados_lot SET num_resultado=%s 
						WHERE id_resultado=%s",
            ...
            if ($premiar==1) {
                $query_Recordset14 = sprintf(
                    "/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 9 */ SELECT
					ve.num_ticket_lot, ve.tip_loteria_lot, ve.num_apuesta_lot, ve.mon_apuesta_lot, ve.id_signo, ve.id_loteria,
					(/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 10 */ SELECT pre_loteria FROM bancaloterias bl WHERE bl.id_loteria=ve.id_loteria AND 
						bl.id_banca = ag.cod_banca LIMIT 1) AS pPremio
					FROM 
						venta_lot ve, banca ba, agencia ag, taquilla ta, usuario us
    ...
    if ($u>0) { //guardar
        foreach ($ticketPre as $xdatos) {
            $insertSQL1 = sprintf(
                "/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 11 */ UPDATE venta_lot ve 
				SET
...
if ((isset($_POST["MM_update"])) && (isset($_POST["guardar"]))) {
    ...
    if ($guarda==100) {
        ...
        if (isset($_POST["modificar"])&&$_POST["modificar"]==0) {
            ...
            foreach ($_POST["id_loteria"] as $id_loteria) {
                $insertSQL2 = sprintf(
                    "/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 12 */ INSERT 
					INTO resultados_lot 
					(num_resultado, id_loteria, sig_resultado, fec_resultado, id_banca) 
					VALUES (%s, %s, %s, %s, %s)",
            ...
            foreach ($_POST["id_resultado"] as $id_resultado) {
                $insertSQL1 = sprintf(
                    "/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 13 */ UPDATE resultados_lot 
					SET
...
if ((isset($_POST["MM_inserto"])) && ($_POST["MM_inserto"] == "formo") && (isset($_POST["od_sorteo"]))) {
    ...
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 14 */ SELECT 
		...
		CASE lo.tip_loteria
			WHEN 4 THEN (/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 15 */ SELECT ani.nom_animal FROM animales ani WHERE ani.id_animal=re.num_resultado LIMIT 1)
			WHEN 5 THEN (/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 16 */ SELECT fru.nom_fruta  FROM frutas fru   WHERE fru.id_fruta=re.num_resultado  LIMIT 1)
			ELSE '_'
		END AS nom_anifru
		FROM 
			sorteos so,
    ...
    $cDia=loteriaHoyAdmin($diahoy);
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 17 */ SELECT 
		lo.nom_loteria, lo.id_loteria, lo.id_terminal, lo.tip_loteria, lo.let_loteria,
		so.nom_sorteo, so.hor_sorteo, so.id_sorteo,
		re.id_resultado, re.fec_resultado, re.num_resultado, re.sig_resultado,
		CASE lo.tip_loteria
			WHEN 4 THEN (/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 18 */ SELECT ani.nom_animal FROM animales ani WHERE ani.id_animal=re.num_resultado LIMIT 1)
			WHEN 5 THEN (/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 19 */ SELECT fru.nom_fruta  FROM frutas fru   WHERE fru.id_fruta=re.num_resultado  LIMIT 1)
			ELSE '_'
		END AS nom_anifru
		FROM 
			sorteos so,
...
$query_Recordset2 = "/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 20 */ SELECT id_signo, nom_corto FROM signos ORDER BY id_signo";
$Recordset2 = mysqli_query($conexionbanca, $query_Recordset2) or die(mysqli_error($conexionbanca));
$row_Recordset2 = mysqli_fetch_assoc($Recordset2);
$totalRows_Recordset2 = mysqli_num_rows($Recordset2);
$query_Recordset3 =  sprintf("/* PARSEADORES1 admin_lot\admin_lot\admin_resultados_trte_lot.php - QUERY 21 */ SELECT so.id_sorteo, so.nom_sorteo
