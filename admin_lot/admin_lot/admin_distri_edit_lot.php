...
if (isset($_POST["guardar"])) {
    ...
    if ($graba==31) {
        ...
        $query_Recordset7 =  sprintf(
            "/* PARSEADORES1 admin_lot\admin_lot\admin_distri_edit_lot.php - QUERY 1 */ SELECT so.id_sorteo, so.nom_sorteo
        ...
        foreach ($_POST["idtr_banlot"] as $idl) {
            ...
            $insertSQL2 = sprintf(
                "/* PARSEADORES1 admin_lot\admin_lot\admin_distri_edit_lot.php - QUERY 2 */ UPDATE bancaloterias 
					SET
					hor_cierre=%s,
					est_banlot=%s,
					lun_loteriabanca=%s,
            ...
            if ($tip_loteria[$cta]<4) {
                $insertSQL3 = sprintf(
                    "/* PARSEADORES1 admin_lot\admin_lot\admin_distri_edit_lot.php - QUERY 3 */ UPDATE bancaloterias 
						SET
						hor_cierre=%s,
						est_banlot=%s,
						lun_loteriabanca=%s,
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1" && isset($_POST["guardarDis"]))) {
    ...
    if ($graba==31) {
        $insertSQL1 = sprintf(
            "/* PARSEADORES1 admin_lot\admin_lot\admin_distri_edit_lot.php - QUERY 4 */ UPDATE banca 
			SET
			por_banca_lot=%s, 
			mod_resultado=%s
			WHERE cod_banca=%s",
...
if ((isset($_POST["MM_insert2"])) && ($_POST["MM_insert2"] == "form2")) {
    $insertSQL1 = sprintf(
        "/* PARSEADORES1 admin_lot\admin_lot\admin_distri_edit_lot.php - QUERY 5 */ UPDATE banca 
		SET
		mod_resultado=%s
		WHERE cod_banca=%s",
    ...
    $Result1 = mysqli_query($conexionbanca, $insertSQL1) or die(mysqli_error($conexionbanca));
    $query_Recordset6 =  sprintf("/* PARSEADORES1 admin_lot\admin_lot\admin_distri_edit_lot.php - QUERY 6 */ SELECT 
			...
			CASE lo.tip_loteria  
				WHEN 2 THEN (/* PARSEADORES1 admin_lot\admin_lot\admin_distri_edit_lot.php - QUERY 7 */ SELECT lot.tip_loteria FROM loterias lot WHERE lo.id_triple = lot.id_loteria LIMIT 1)
				ELSE '0'
			END AS tipo
    ...
    if ($totalRows_Recordset6>0) {
        ...
        do {
            ...
            $insertSQL2 = sprintf(
                "/* PARSEADORES1 admin_lot\admin_lot\admin_distri_edit_lot.php - QUERY 8 */ INSERT 
				INTO bancaloterias 
...
if ((isset($_POST["MM_inserto"])) && ($_POST["MM_inserto"] == "formo") && (isset($_POST["od_sorteo"]))) {
    ...
    $query_Recordset1 =  sprintf(
        "/* PARSEADORES1 admin_lot\admin_lot\admin_distri_edit_lot.php - QUERY 9 */ SELECT ba.cod_banca, ba.nom_banca, ba.por_banca_lot, ba.mod_resultado,
		...
		CASE lo.tip_loteria
			WHEN 1 THEN (/* PARSEADORES1 admin_lot\admin_lot\admin_distri_edit_lot.php - QUERY 10 */ SELECT blo.pre_loteria FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal LIMIT 1)
			WHEN 3 THEN (/* PARSEADORES1 admin_lot\admin_lot\admin_distri_edit_lot.php - QUERY 11 */ SELECT blo.pre_loteria FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal LIMIT 1)
			ELSE 0
		END AS pre_terminal,	
		CASE lo.tip_loteria
			WHEN 1 THEN (/* PARSEADORES1 admin_lot\admin_lot\admin_distri_edit_lot.php - QUERY 12 */ SELECT blo.id_banlot FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal LIMIT 1)
			WHEN 3 THEN (/* PARSEADORES1 admin_lot\admin_lot\admin_distri_edit_lot.php - QUERY 13 */ SELECT blo.id_banlot FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal LIMIT 1)
			ELSE 0
    ...
    $osorteo=array();
    $query_Recordset1 =  sprintf(
        "/* PARSEADORES1 admin_lot\admin_lot\admin_distri_edit_lot.php - QUERY 14 */ SELECT ba.cod_banca, ba.nom_banca, ba.por_banca_lot, ba.mod_resultado,
		bl.lun_loteriabanca, bl.mar_loteriabanca, bl.mie_loteriabanca, bl.jue_loteriabanca, bl.vie_loteriabanca, 
		bl.sab_loteriabanca, bl.dom_loteriabanca, bl.est_banlot, bl.hor_cierre, bl.pre_loteria, bl.id_banlot,
		lo.nom_loteria, lo.id_loteria, lo.id_terminal, lo.tip_loteria, so.nom_sorteo,
		CASE lo.tip_loteria
			WHEN 1 THEN (/* PARSEADORES1 admin_lot\admin_lot\admin_distri_edit_lot.php - QUERY 15 */ SELECT blo.pre_loteria FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal LIMIT 1)
			WHEN 3 THEN (/* PARSEADORES1 admin_lot\admin_lot\admin_distri_edit_lot.php - QUERY 16 */ SELECT blo.pre_loteria FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal LIMIT 1)
			ELSE 0
		END AS pre_terminal,	
		CASE lo.tip_loteria
			WHEN 1 THEN (/* PARSEADORES1 admin_lot\admin_lot\admin_distri_edit_lot.php - QUERY 17 */ SELECT blo.id_banlot FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal LIMIT 1)
			WHEN 3 THEN (/* PARSEADORES1 admin_lot\admin_lot\admin_distri_edit_lot.php - QUERY 18 */ SELECT blo.id_banlot FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal LIMIT 1)
			ELSE 0
		END AS id_banlot_ter	
		FROM banca ba, bancaloterias bl, loterias lo, sorteos so 
		WHERE ba.cod_banca = %s AND bl.id_banca = ba.cod_banca AND lo.id_loteria = bl.id_loteria AND lo.tip_loteria != 2 AND
...
$query_Recordset2 =  sprintf(
    "/* PARSEADORES1 admin_lot\admin_lot\admin_distri_edit_lot.php - QUERY 19 */ SELECT so.id_sorteo, so.nom_sorteo
	FROM banca ba, bancaloterias bl, loterias lo, sorteos so 
	WHERE ba.cod_banca = %s AND bl.id_banca = ba.cod_banca AND lo.id_loteria = bl.id_loteria AND lo.tip_loteria != 2 AND
	lo.est_loteria = 1 AND lo.id_sorteo = so.id_sorteo AND so.est_sorteo = 1
	GROUP BY so.id_sorteo
