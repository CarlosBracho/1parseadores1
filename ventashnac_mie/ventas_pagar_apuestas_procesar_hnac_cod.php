...
$mensaje="";$ver=1;
$Permisos=2;
$query_Recordset7 = sprintf("/* PARSEADORES1 ventashnac_mie\ventas_pagar_apuestas_procesar_hnac_cod.php - QUERY 1 */ SELECT est_control_pagos_hnac FROM ctrol_ventpag_global_hnac WHERE cod_ctrol_ventpag_global_hnac =1");
...
$query_Recordset2 = sprintf(
    "/* PARSEADORES1 ventashnac_mie\ventas_pagar_apuestas_procesar_hnac_cod.php - QUERY 2 */ SELECT venta_hnac.est_ticket_hnac, venta_hnac.ser_venta_hnac 
...
if ($totalRows_Recordset2==0 && $est_control_pagos_hnac==0) {
    $query_Recordset8 = sprintf(
        "/* PARSEADORES1 ventashnac_mie\ventas_pagar_apuestas_procesar_hnac_cod.php - QUERY 3 */ SELECT cod_taquilla
								 FROM usuario 
    ...
    $query_Recordset1 = sprintf(
        "/* PARSEADORES1 ventashnac_mie\ventas_pagar_apuestas_procesar_hnac_cod.php - QUERY 4 */ SELECT 
						...
						ve.num_caballo_hnac, tp.cab_min_hnac, ca.est_confirmacion_hnac, tp.pag_codigo_hnac, redondeo_hnac
					FROM 
					venta_hnac ve, 
					carrera_hnac ca, 
					usuario us,
    ...
    $pag_codigo_hnac=$row_Recordset1['pag_codigo_hnac'];
    ...
    if (isset($_POST["ser_venta"]) && $_POST["ser_venta"]==substr($row_Recordset1['ser_venta_hnac'], 0, 3)) { 
        $Permisos=0;
    }else{ $Permisos=1;}
    ...
    if ((($totalRows_Recordset1>0 && $row_Recordset1['est_carrera_hnac']==0 && $xserDB==$xserTic &&
     ...
     $row_Recordset1['est_ticket_hnac']==1) && $pau_ventas_hnac==0) {
        ...
        if ($defi_regla==0) {
            $query_Recordset13 = sprintf("/* PARSEADORES1 ventashnac_mie\ventas_pagar_apuestas_procesar_hnac_cod.php - QUERY 5 */ SELECT num_caballo_hnac FROM resultados_hnac WHERE 
			cod_carrera_hnac = %s", GetSQLValueString($xCarrera, "int"));
        } else {
            $query_Recordset13 = sprintf("/* PARSEADORES1 ventashnac_mie\ventas_pagar_apuestas_procesar_hnac_cod.php - QUERY 6 */ SELECT num_caballo_hnac FROM resultados_oficiales_hnac WHERE 
			cod_carrera_hnac = %s", GetSQLValueString($xCarrera, "int"));
        }
...
$redondeo_hnac=$row_Recordset1['redondeo_hnac'];
            ...
            if ($montoapagar>0) {
                do {
                    $updateSQL = sprintf(
                        "/* PARSEADORES1 ventashnac_mie\ventas_pagar_apuestas_procesar_hnac_cod.php - QUERY 7 */ UPDATE venta_hnac 
...
<?php
	...
	<?php
    if (isset($_GET["pagoSIN"]) && $ver==1 && $est_control_pagos_hnac==0) {
        $width="50%"; ?>
		<form method="post" name="form1" action="<?php echo $editFormAction; ?>" onsubmit="return chequearEnvio();">
            ...
            <?php
    if ($totalRows_Recordset2==0 && $Return==1) {
         ?>
...
if (isset($_GET["pagoSIN"])) {?>
	<div style="width:<?php echo $width; ?>;float:left;text-align:right;color:#FFF;font-size:28px; text-align:center;">
		...
		class="btn-primary" 
		onclick="location.href = '../ventashnac_mie/pag_tic_hnac.php?recordID=<?php echo "1";?>&uVenta=<?php echo $usuarioPago;?>'"/>	
	</div>
<?php } ?>
    </div>
    </div>
