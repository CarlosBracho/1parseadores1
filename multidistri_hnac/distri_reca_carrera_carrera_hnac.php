...
if (($codagencia>0 or $codagencia=="todos") && ($codcarrera>0 or $codcarrera=="todas") && $in!="" && $codHipodromo!=-1 && $codHipodromo!="") {
    ...
    if ($codcarrera!="todas") {
        $query_Recordset7 = sprintf(
            "/* PARSEADORES1 multidistri_hnac\distri_reca_carrera_carrera_hnac.php - QUERY 1 */ SELECT 
        ...
        $query_Recordset7 = sprintf(
            "/* PARSEADORES1 multidistri_hnac\distri_reca_carrera_carrera_hnac.php - QUERY 2 */ SELECT 
			ca.can_caballos_hnac, 
			ca.cod_carrera_hnac, 
			ca.est_carrera_hnac, 
			ca.est_cierre_hnac, 
        ...
        do {
            if ($totalRows_Recordset7==1) {
                ...
                $query_Recordset10 = sprintf(
                    "/* PARSEADORES1 multidistri_hnac\distri_reca_carrera_carrera_hnac.php - QUERY 3 */ SELECT 
					re.cod_tventa_hnac,
					re.num_caballo_hnac,
					re.div_pago_hnac,
					re.fac_div_hnac
            ...
            for ($numCab = 1; $numCab <= $row_Recordset7['can_caballos_hnac']; $numCab++) {
                if ($codagencia!="todos") {
                    $query_Recordset8 = sprintf(
                        "/* PARSEADORES1 multidistri_hnac\distri_reca_carrera_carrera_hnac.php - QUERY 4 */ SELECT 
                    ...
                    $query_Recordset8 = sprintf(
                        "/* PARSEADORES1 multidistri_hnac\distri_reca_carrera_carrera_hnac.php - QUERY 5 */ SELECT 
						SUM(ve.mon_venta_hnac) AS apuestas,
						SUM(CASE WHEN ve.est_ticket_hnac = 2 
							THEN ve.pag_premio_hnac ELSE 0 END) AS tpremios,
						SUM(CASE WHEN ve.est_ticket_hnac >= 4 AND ve.est_ticket_hnac <= 5
