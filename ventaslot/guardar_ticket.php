...
$query_Recordset19 = sprintf("/* PARSEADORES1 ventaslot\guardar_ticket.php - QUERY 1 */ SELECT est_control_ventas_lot, est_control_pagos_lot
...
if (isset($_POST["MM_insert"]) && $_POST["MM_insert"] == "form1" && $_SESSION["ocarrito"]->sumar_carrito()>0 &&
    ...
    $query_Recordset6 = sprintf("/* PARSEADORES1 ventaslot\guardar_ticket.php - QUERY 2 */ SELECT tp.mon_minticket_lot
        ...
        if (sizeof($cLot)>0) {
            if ($_POST['con_tope']==2) {
                $query_Recordset11 = sprintf(
                    "/* PARSEADORES1 ventaslot\guardar_ticket.php - QUERY 3 */ SELECT ba.top_triple_lot, ba.top_terminal_lot FROM banca ba
                ...
                if ($xlot!=0) {
                    ...
                    if ($xhAc<$xhCi) {
                        if ($xquedanAgencia>0) { // pregunta si no ha excedido el limite en agencia
                            ...
                            $yu++;
                            $insertSQL = sprintf(
                                "/* PARSEADORES1 ventaslot\guardar_ticket.php - QUERY 4 */ INSERT INTO venta_lot (num_apuesta_lot, mon_apuesta_lot, id_loteria, id_signo, fec_venta_lot, hor_venta_lot, id_usuario, est_ticket_lot, tip_loteria_lot, ticket_lot, ip_venta_lot, ser_ticket_lot, can_ticket_lot, lin_ticket_lot) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
