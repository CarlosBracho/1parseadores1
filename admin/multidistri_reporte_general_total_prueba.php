...
$totSistemaA=0;
$query_Recordset555 = sprintf("/* PARSEADORES1 admin\multidistri_reporte_general_total_prueba.php - QUERY 1 */ SELECT * FROM tasadecambio WHERE Idtasadecambio = 1");
...
$query_Recordset18 = sprintf("/* PARSEADORES1 admin\multidistri_reporte_general_total_prueba.php - QUERY 2 */ SELECT 
nom_multidistriMD, cod_multidistriMD  
FROM multidistriMD 
ORDER BY multidistriMD.nom_multidistriMD");
...
$query_Recordset12 = sprintf(
    "/* PARSEADORES1 admin\multidistri_reporte_general_total_prueba.php - QUERY 3 */ SELECT  nom_multidistriMD, multi_por_ameMD, multipor_banca_hnacMD, multi_por_parleyMD FROM multidistriMD WHERE cod_multidistriMD = %s",
...
/*/
$query_Recordset12 = sprintf(
    "SELECT  nom_banca, dist_por_ame, dist_cob_hnac, dist_por_parley FROM banca WHERE cod_banca = %s",
    GetSQLValueString($codigobanca=$_POST['Banca'], "int")
);
...
$query_Recordset3 = sprintf(
    "/* PARSEADORES1 admin\multidistri_reporte_general_total_prueba.php - QUERY 4 */ SELECT *
	FROM  agencia ag, banca ba, multidistriMD mu
	WHERE ag.cod_banca = ba.cod_banca AND ba.cod_multidistriMDBA = %s AND ba.cod_multidistriMDBA = mu.cod_multidistriMD
    ...
    if (isset($_POST['fecha_inicio']) && isset($_POST['fecha_fin'])) {
        if ($_POST['fecha_inicio']!="" && $_POST['fecha_fin']!="") {
            ...
            if ($_POST['id_agencia']!="todos") {
                $query_Recordset3 = sprintf(
                    "/* PARSEADORES1 admin\multidistri_reporte_general_total_prueba.php - QUERY 5 */ SELECT *
					FROM agencia ag
...
$query_Recordset2 = sprintf(
    "/* PARSEADORES1 admin\multidistri_reporte_general_total_prueba.php - QUERY 6 */ SELECT ag.cod_agencia, ag.nom_agencia
	FROM agencia ag, banca ba, multidistriMD mu
	WHERE mu.cod_multidistriMD = %s AND ba.cod_multidistriMDBA = mu.cod_multidistriMD AND ag.cod_banca = ba.cod_banca
	ORDER BY ag.nom_agencia ASC",
    GetSQLValueString($codigobanca=$_POST['Banca'], "int")
);
...
<?php
$query_Recordset44 = sprintf("/* PARSEADORES1 admin\multidistri_reporte_general_total_prueba.php - QUERY 7 */ SELECT 
	me.mensaje
  ...
  <div class="header" style="height:100px; background:#333">
        ...
        <table width="100%" HEIGHT=100 border="0" >
	     ...
	     <td VALIGN=BOTTOM bgcolor="#333" style="color:#FFF" ALIGN=RIGHT>
          ...
          <tr>
             <td VALIGN=BOTTOM bgcolor="#333" style="color:#FFF">
                Reporte General Distribuidor	<br/><br/>
             </td>
	     <td>
	     </td>
          </tr>
       ...
       <div style="background: #FFF; width:100%; float:left; padding:5px 0px 0px 10px;
           ...
           <form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1" autocomplete="off"  
             ...
             <select name="Banca" id="soflow" style="height:40px; width:320px; margin:-9px 0px 0px 0px ">
                      <option value="todos" ><?php echo $row_Recordset12['nom_multidistriMD']?></option>
                      <?php
                do {
                    ?>
               <option value="<?php echo $row_Recordset18['cod_multidistriMD']?>"
               <?php if (strtoupper($row_Recordset18['nom_multidistriMD'])==($row_Recordset12['nom_multidistriMD'])) {
                        echo "SELECTED";
                    } ?>>
                    <?php echo strtoupper($row_Recordset18['nom_multidistriMD']); ?>
                    </option>
            ...
            <td>
                ...
                <a style="color:#fff; width:100px; height:40px; "href="../admin/distri_reporte_general_total.php" target="_blank"> Ir a Reportes de Distribuidores</a>
                </button>
	        </td>
            <br><br>
            <td>
                <button class="btn-warning" style=float:right;>
                <a style="color:#fff; width:100px; height:40px; "href="../admin/agente_reporte_general_total.php" target="_blank"> Ir a Reportes de Agentes</a>
                </button>
	        </td>
                    <br><br>
                Desde:
		...
		<table width="100%" border="0" style="color:#000; font-size:12px;" cellpadding="0" cellspacing="0">
			<tr style="font-size:16px" valign="middle" align="center">
				...
				<td width="33%" colspan="-2" bgcolor="#333" style="color:#FFF"><br/>
                    AMERICANAS: <?php echo number_format($row_Recordset12['multi_por_ameMD'], 2, ",", ".")."%<br/>"; ?>
                    NACIONALES: <?php echo number_format($corbro, 2, ",", ".")." Bs/dia<br/>"; ?>
                    PARLEY: <?php echo number_format($row_Recordset12['multi_por_parleyMD'], 2, ",", ".")."%<br/><br/><br/>"; ?>
           ...
           <td width="21%">TAQUILLA</td>
            ...
            <td width="19%">TOTAL A COBRAR Al<br/> TAQUILLA AMERICANA (%)<br/> NACIONALES(X) TOTAL ( )</td>
                    ...
                    $query_Recordset30 = sprintf(
                       "/* PARSEADORES1 admin\multidistri_reporte_general_total_prueba.php - QUERY 8 */ SELECT cod_taquilla, nom_taquilla
						FROM taquilla
						WHERE cod_agencia = %s
                   ...
                   if ($totalRows_Recordset30>0) {
                       do {
                           ...
                           $query_Recordset1 = sprintf(
                               "/* PARSEADORES1 admin\multidistri_reporte_general_total_prueba.php - QUERY 9 */ SELECT
					...
					SUM(CASE WHEN ve.est_ticket = 0 AND ve.fec_venta >= %s AND ve.fec_venta <= %s AND ve.lin_ticket = 1  AND ve.efectivoO = 5
						THEN 1 ELSE 0 END) AS con_tic_elisol
                           ...
                           $query_Recordset21 = sprintf(
                               "/* PARSEADORES1 admin\multidistri_reporte_general_total_prueba.php - QUERY 10 */ SELECT
					...
					SUM(CASE WHEN veN.est_ticket_hnac = 0 AND veN.fec_venta_hnac >= %s AND veN.fec_venta_hnac <= %s AND
						...
						THEN 1 ELSE 0 END) AS con_tic_eliNsol
                           ...
                           $query_Recordset22 =  sprintf(
                               "/* PARSEADORES1 admin\multidistri_reporte_general_total_prueba.php - QUERY 11 */ SELECT cob_taquilla_hnac 
								FROM  
                           ...
                           $newDate1 = date("Y-m-d", $timestamp1 );
                              ...
                              $query_Recordset50 = sprintf(
                               "/* PARSEADORES1 admin\multidistri_reporte_general_total_prueba.php - QUERY 12 */ SELECT 
...
SUM(CASE WHEN p4jugadas.lineatp4 = 1 AND p4jugadas.jugadadtp4 >= %s AND p4jugadas.jugadadtp4 <= %s AND (p4jugadas.estadoticketp4 = 1) AND p4jugadas.monedap4 = 5  THEN p4jugadas.premioapagarp4 ELSE 0 END) AS total_ganadorsol
           ...
           GetSQLValueString($codigoAgente, "int")
                        ...
                        $query_Recordset7 =  sprintf(
                            "/* PARSEADORES1 admin\multidistri_reporte_general_total_prueba.php - QUERY 13 */ SELECT id_usuario
                            FROM  
                            cobro_hnac
                            WHERE 
                            cod_taquilla = %s AND
        ...
        <?php  //PARLEY BSS
           if ($totVentaTaqP!=0 or $totPremiTaqP!=0 or $totAnulaTaqP!=0 or $totTaquillaP!=0 or $totPTaqPpagP!=0 or
                    ...
                    $parleypagobss=$ventasparleyBSS*$row_Recordset12['multi_por_parleyMD']/100;
           ...
           if ($totVentaTaqPUSD!=0 or $totPremiTaqPUSD!=0 or $totAnulaTaqPUSD!=0 or $totTaquillaPUSD!=0 or $totPTaqPpagPUSD!=0 or
                    $porPagarEliTaqPUSD!=0 or $totGanPerTaqPUSD!=0) {?>
                      <tr onmouseover="cambiacolor_over1(this)" onmouseout="cambiacolor_out3(this)" height="20" 
                        ...
                        <?php 
                    $parleypagobusd1=$totAnulaTaqPUSD-$porPagarEliTaqPUSD;
                   ...
                   if ($totVentaTaqPCOP!=0 or $totPremiTaqPCOP!=0 or $totAnulaTaqPCOP!=0 or $totTaquillaPCOP!=0 or $totPTaqPpagPCOP!=0 or
                            $porPagarEliTaqPCOP!=0 or $totGanPerTaqPCOP!=0) {?>
                              <tr onmouseover="cambiacolor_over1(this)" onmouseout="cambiacolor_out3(this)" height="20" 
                                ...
                                <?php 
                    $parleypagobcop=$totVentaTaqPCOP-$totAnulaTaqPCOP-$porPagarEliTaqPCOP;
                    $totalcop=$totalcop+$totVentaTaqPCOP-$totAnulaTaqPCOP-$porPagarEliTaqPCOP;
                   ...
                   if ($totVentaTaqPSOL!=0 or $totPremiTaqPSOL!=0 or $totAnulaTaqPSOL!=0 or $totTaquillaPSOL!=0 or $totPTaqPpagPSOL!=0 or
                            $porPagarEliTaqPSOL!=0 or $totGanPerTaqPSOL!=0) {?>
                              <tr onmouseover="cambiacolor_over1(this)" onmouseout="cambiacolor_out3(this)" height="20" 
                                ...
                                <?php 
                    $parleypagobsol=$totVentaTaqPSOL-$totAnulaTaqPSOL-$porPagarEliTaqPSOL;
                    $totalsol=$totalsol+$totVentaTaqPSOL-$totAnulaTaqPSOL-$porPagarEliTaqPSOL;
...
echo '<br/><br/><br/>';
               ...
               $cobroaBanca=($totGenBanca*$row_Recordset12['multi_por_ameMD'])/100;
               $cobroaBancaN=$corbro*$tPuntosBa;
               $costosistema=$cobroaBanca+$parleypagobss;
               $ganBanca1=$totCobraBanca+$totCobraBancaN;
               //$ganBanca=$ganBanca1-$costosistema;
               $ganBanca=$totCobraBancaN;
                
               $totalparleyUSD=$totalusd*$row_Recordset12['multi_por_parleyMD']/100;
               $parleypagoUSD=$parleypagobusd1*$row_Recordset555['usdabss']*$row_Recordset12['multi_por_parleyMD']/100;
               $totGenBancausd=$totVentaBancausd-$totAnulaBancausd;
               $cobroaBancausd=($totGenBancausd*$row_Recordset12['multi_por_ameMD'])/100;
               $cobroaBancaNusd=0*$tPuntosBa;
               $costosistemausd=$cobroaBancausd+$totalparleyUSD;
               $ganBanca1usd=$totCobraBancausd+$totCobraBancaNusd;
               $ganBancausd=$ganBanca1usd-$costosistemausd;

               $totalparleyCOP=$totalcop*$row_Recordset12['multi_por_parleyMD']/100;
               $parleypagoCOP=$parleypagobcop*$row_Recordset555['copabss']*$row_Recordset12['multi_por_parleyMD']/100;
               $totGenBancacop=$totVentaBancacop-$totAnulaBancacop;
               $cobroaBancacop=($totGenBancacop*$row_Recordset12['multi_por_ameMD'])/100;
               $cobroaBancaNcop=0*$tPuntosBa;
               $costosistemacop=$cobroaBancacop+$totalparleyCOP;
               $ganBanca1cop=$totCobraBancacop+$totCobraBancaNcop;
               $ganBancacop=$ganBanca1cop-$costosistemacop;
                
               $totalparleySOL=$totalsol*$row_Recordset12['multi_por_parleyMD']/100;
               $parleypagoSOL=$parleypagobsol*$row_Recordset555['solabss']*$row_Recordset12['multi_por_parleyMD']/100;
               $totGenBancasol=$totVentaBancasol-$totAnulaBancasol;
               $cobroaBancasol=($totGenBancasol*$row_Recordset12['multi_por_ameMD'])/100;
               $cobroaBancaNsol=0*$tPuntosBa;
               $costosistemasol=$cobroaBancasol+$totalparleySOL;
               $ganBanca1sol=$totCobraBancasol+$totCobraBancaNsol;
               $ganBancasol=$ganBanca1sol-$costosistemasol;

               $cambiousdabss=$costosistemausd*$row_Recordset555['usdabss']+$parleypagoUSD;
               $cambiocopabss=$costosistemacop*$row_Recordset555['copabss']+$parleypagoCOP;
               $cambiosolabss=$costosistemasol*$row_Recordset555['solabss']+$parleypagoSOL;
               ...
               $totalapagar=$costosistema+$cambiousdabss+$cambiocopabss+$cambiosolabss+$cobroaBancaN;
               ...
               $acumuladoagentesventabss=($acumuladoagentesventabss/100)*$row_Recordset12['multi_por_ameMD'];
...
document.getElementById('pagAgentodos').innerHTML = "<?php echo "<br> NACIONALES >> ". number_format($cobroaBancaN, 2, ",", ".")."&nbsp;BSS(".$taquillaspar.")"; ?>";
