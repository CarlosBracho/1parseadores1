...
$query_Recordset1 = sprintf(
    "/* PARSEADORES1 admin\minutos_carrera.php - QUERY 1 */ SELECT * FROM carrera WHERE eje_primero=0 AND fec_carrera=%s",
            ...
            do {
                ...
                if ($hipodomo[0]!="") {
                    foreach ($hipodomo as $hip) {
                        if (trim($hipodomo[$f])==trim($row_Recordset1['nom_hipodromo'])
                            ...
                            <?php
                                $updateSQL = sprintf(
                                "/* PARSEADORES1 admin\minutos_carrera.php - QUERY 2 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_carrera=%s 
                                                      WHERE cod_carrera=%s",
                ...
                if ($cont==1) {
                    if ($row_Recordset1['hor_carrera']>$horasistema && $row_Recordset1['est_carrera']==1) {
                        ...
                        $est=0;
                        $updateSQL = sprintf(
                            "/* PARSEADORES1 admin\minutos_carrera.php - QUERY 3 */ UPDATE carrera SET hor_carrera=%s, hor_mtp=%s, est_carrera=%s
                                      WHERE cod_carrera=%s",
