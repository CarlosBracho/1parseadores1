...
Se cuenta con un sistema de auditoría y monitoreo sobre las alertas de sincronización (`includes/watchandwager_creador.php` y `includes/mtp_betbird.php`). Toda invocación (Llamado - Tipo 0) o corrida exitosa tras superar los filtros de reposo (Ejecución Efectiva - Tipo 1) es registrada en la tabla `alertas_registros` indicando la IP física del servidor mediante una rutina multi-entorno robusta. El panel administrativo (`admin/alertas_lista.php` y `admin/alertas_edit.php`) ha sido reintegrado visualmente al estilo clásico naranja/gris de la suite administrativa del software, manteniendo la compatibilidad nativa y utilizando un visor modal asíncrono para el historial de ejecuciones (`admin/alertas_historial_ajax.php`) desacoplado de las librerías JS de Bootstrap mediante estilos CSS modernos y un controlador nativo en JS puro (XMLHttpRequest y funciones inline de apertura/cierre), previniendo cualquier conflicto con versiones antiguas de jQuery. Adicionalmente, el panel muestra el tiempo transcurrido desde el último llamado y la última ejecución efectiva de cada alerta mediante subconsultas SQL atómicas, y la función `chequearEnvio()` incluye un temporizador de desbloqueo de 5 segundos para evitar bloqueos del formulario. Con salvaguardas defensivas en la cabecera `includes/cabeceraadmin.php` para evitar ReferenceErrors del objeto global `alertify` en los ciclos de reloj del sistema. Toda visualización de marcas de tiempo en el listado y modal de historial se desfasa en +6 horas para homologar las horas de Hawaii (HST) al huso horario de Venezuela (VET), y el listado se ordena por fecha de última ejecución de forma descendente (`ultima_ejecucion DESC`) para priorizar visualmente las alertas ejecutadas más recientemente, habiendo sido eliminados los botones interactivos de "INICIAR" / "PAUSAR" para resguardar la consistencia y simplificar las acciones directas de la tabla.
