...
$query_Recordset111 = sprintf("/* PARSEADORES1 logros\logros_sella_solohockey.php - QUERY 1 */ SELECT * 
	FROM 
	cookies 
	WHERE
...
$query_RecordsetLG =  sprintf(
	"/* PARSEADORES1 logros\logros_sella_solohockey.php - QUERY 2 */ SELECT
  ...
  $query_RecordsetAE =  sprintf(
	"/* PARSEADORES1 logros\logros_sella_solohockey.php - QUERY 3 */ SELECT
...
"
  ...
  $query_RecordsetAJ =  sprintf(
	"/* PARSEADORES1 logros\logros_sella_solohockey.php - QUERY 4 */ SELECT
...
function verificarlogro($yaestaequipo1cod, $logro, $tipojugada, $idjuego, $equipo1o2, $logrodate, $ArrayLogros2, $Fechahoramarafun, $valorabrl)
  {
    ...
    if ($idjuego > 0 && $yaestalg == 0 && $yaestaequipo1cod > 0) {
      ...
      $insertSQL = sprintf(
        "/* PARSEADORES1 logros\logros_sella_solohockey.php - QUERY 5 */ INSERT 
    INTO p3logros
    ...
    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
      ...
      $insertSQL = sprintf(
        "/* PARSEADORES1 logros\logros_sella_solohockey.php - QUERY 6 */ INSERT 
    INTO p7histolg
    ...
    if ($idjuego > 0 && $yaestalgperonoigual == 0 &&  $Id_p3logrosp3 > 0) {
      ...
      $logroactualizado = 1;
      $insertSQL1 = sprintf(
        "/* PARSEADORES1 logros\logros_sella_solohockey.php - QUERY 7 */ UPDATE p3logros 
      ...
      $insertSQL = sprintf(
        "/* PARSEADORES1 logros\logros_sella_solohockey.php - QUERY 8 */ INSERT 
      INTO p7histolg
      (idjuegop7, Id_p1equiposp7, equipop7, tipojugadap7, logrop7, logroABoRLp7, actxp7) 
    VALUES (%s, %s, %s, %s, %s, %s, %s)",
        GetSQLValueString($idjuego, "int"),
  ...
  $query_Recordset18D =  sprintf(
	"/* PARSEADORES1 logros\logros_sella_solohockey.php - QUERY 9 */ SELECT  
*
...
$query_Recordset1 =  sprintf(
	"/* PARSEADORES1 logros\logros_sella_solohockey.php - QUERY 10 */ SELECT  
*
			FROM  opciones_parley
			WHERE id_opcionp=%s
			LIMIT 3",
...
echo $Parseo['time'].'<br>'.'<br>';
		...
		echo ' MEDIO TIEMPO LOGROS'.' ML5 '.$ml25.' RUNLINE '.$runline25.' factor '.$runline25factor.' BAJA5 '.$bajalogro5.' factor '.$bajafactor5.'<br>';
		echo $Parseo['time'].'<br>'.'<br>';
	...
	foreach ($ArrayJuegos as $clave => $valor2) {
	  ...
	  if ($valor2["idequipo1p2"] == $yaestaequipo1cod && $valor2["idequipo2p2"] == $yaestaequipo2cod) {
		...
		if ($idjuego > 0) {
			...
			$insertSQL24 = sprintf(
				"/* PARSEADORES1 logros\logros_sella_solohockey.php - QUERY 11 */ UPDATE p2juegos 
		   ...
		   if($idjuego==0){
			...
			$query_Recordsetvrfjuego =  sprintf(
				"/* PARSEADORES1 logros\logros_sella_solohockey.php - QUERY 12 */ SELECT  
deportep2
	  FROM  p2juegos
	  WHERE 
	  iniciodtp2=%s
...
echo 'ESTE JUEGO SE CREARA'.$yaestaequipo1cod.'    '.$yaestaequipo2cod;
				...
				$insertSQL = sprintf(
				  "/* PARSEADORES1 logros\logros_sella_solohockey.php - QUERY 13 */ INSERT 
INTO p2juegos
  ...
  if ($juego <> 0) {
	...
	if ($row_Recordset18D['Swicht'] == 3) {
	  $query_Recordset1b = sprintf(
		"/* PARSEADORES1 logros\logros_sella_solohockey.php - QUERY 14 */ SELECT Id_p2juegosp2 FROM p2juegos WHERE 
iniciodtp2 > %s AND
idequipo1p2 > 0 AND
idequipo1p2 > 0 AND
parseconp2 = 3 AND 
