...
$query_Recordset18D =  sprintf(
	"/* PARSEADORES1 logros\parley.la_logros2.php - QUERY 1 */ SELECT  
*
...
function verificarlogro($yaestaequipo1cod, $logro, $tipojugada, $idjuego, $equipo1o2, $logrodate, $ArrayLogros2, $Fechahoramarafun, $valorabrl)
  {
    ...
    if ($idjuego > 0 && $yaestalg == 0 && $yaestaequipo1cod > 0) {
      ...
      $insertSQL = sprintf(
        "/* PARSEADORES1 logros\parley.la_logros2.php - QUERY 2 */ INSERT 
    INTO p3logros
    ...
    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
      ...
      $insertSQL = sprintf(
        "/* PARSEADORES1 logros\parley.la_logros2.php - QUERY 3 */ INSERT 
    INTO p7histolg
    ...
    if ($idjuego > 0 && $yaestalgperonoigual == 0 &&  $Id_p3logrosp3 > 0) {
      ...
      $logroactualizado = 1;
      $insertSQL1 = sprintf(
        "/* PARSEADORES1 logros\parley.la_logros2.php - QUERY 4 */ UPDATE p3logros 
      ...
      $insertSQL = sprintf(
        "/* PARSEADORES1 logros\parley.la_logros2.php - QUERY 5 */ INSERT 
      INTO p7histolg
      (idjuegop7, Id_p1equiposp7, equipop7, tipojugadap7, logrop7, logroABoRLp7, actxp7) 
    VALUES (%s, %s, %s, %s, %s, %s, %s)",
        GetSQLValueString($idjuego, "int"),
...
$query_RecordsetLG =  sprintf(
	"/* PARSEADORES1 logros\parley.la_logros2.php - QUERY 6 */ SELECT
  ...
  $query_RecordsetAE =  sprintf(
	"/* PARSEADORES1 logros\parley.la_logros2.php - QUERY 7 */ SELECT
...
"
  ...
  $query_RecordsetAJ =  sprintf(
	"/* PARSEADORES1 logros\parley.la_logros2.php - QUERY 8 */ SELECT
...
$SI=divlogro($datoscurlprimer[17][0], 'SI', 1);
$NO=divlogro($datoscurlprimer[17][0], 'NO', 2);
$deporte=0;

echo '<br>'.$competicion.'<br>';  
echo '<br>'.$horajuego.'<br>';

echo '<br>'.$equipo1.' ML '.$ml1.' RUNLINE '.$runline1.' factor '.$runline1factor.' SRUNLINE '.$Srunline1.' factor '.$Srunline1factor.' ALTA '.$altalogro.' factor '.$altabaja.' SI '.$SI.' anota primero1 '.$AP1.' HCE '.$HCE1.' factor '.$HCE1factor.'<br>';
	echo ' MEDIO TIEMPO LOGROS'.' ML5 '.$ml15.' RUNLINE '.$runline15.' factor '.$runline15factor.' ALTA '.$altalogro5.' factor '.$altabaja5.'<br>';
	
	
	echo '<br>'.$equipo2.' ML '.$ml2.' RUNLINE '.$runline2.' factor '.$runline2factor.' SRUNLINE '.$Srunline2.' factor '.$Srunline2factor.' BAJA '.$bajalogro.' factor '.$altabaja.' NO '.$NO.' anota primero2 '.$AP2.' HCE '.$HCE2.' factor '.$HCE2factor.'<br>';
  ...
  $parseconp2=0;
    ...
    if ($parseconp2 == 2) {
		if ($idjuego > 0) {
			...
			$insertSQL24 = sprintf(
				"/* PARSEADORES1 logros\parley.la_logros2.php - QUERY 9 */ UPDATE p2juegos 
...
echo $logrodate;
        $query_Recordsetvrfjuego =  sprintf(
          "/* PARSEADORES1 logros\parley.la_logros2.php - QUERY 10 */ SELECT  
  deportep2
      FROM  p2juegos
      WHERE 
      iniciodtp2=%s
...
foreach ($ArrayJuegos as $clave => $valor2) {
  ...
  if ($idjuego > 0) {
    ...
    // SI ACTUALIZA LOS LOGROS POR MARA, ESTE CODIGO VA A SER QUE ACTUALIZE EL JUEGO
    $insertSQL24 = sprintf(
      "/* PARSEADORES1 logros\parley.la_logros2.php - QUERY 11 */ UPDATE p2juegos 
    SET  p2vecesactualizado=p2vecesactualizado+1		
    WHERE 
    Id_p2juegosp2 = %s",                  
      GetSQLValueString($idjuego, "int")
...
echo $logrodate;
      $query_Recordsetvrfjuego =  sprintf(
        "/* PARSEADORES1 logros\parley.la_logros2.php - QUERY 12 */ SELECT  
deportep2
    FROM  p2juegos
    WHERE 
    iniciodtp2=%s
  ...
  $parseconp2=0;
    ...
    if ($parseconp2 == 2) {
		if ($idjuego > 0) {
			...
			// SI ACTUALIZA LOS LOGROS POR MARA, ESTE CODIGO VA A SER QUE ACTUALIZE EL JUEGO
			$insertSQL24 = sprintf(
				"/* PARSEADORES1 logros\parley.la_logros2.php - QUERY 13 */ UPDATE p2juegos 
			SET  p2vecesactualizado=p2vecesactualizado+1		
			WHERE 
			Id_p2juegosp2 = %s",                  
				GetSQLValueString($idjuego, "int")
...
echo $logrodate;
        $query_Recordsetvrfjuego =  sprintf(
          "/* PARSEADORES1 logros\parley.la_logros2.php - QUERY 14 */ SELECT  
  deportep2
      FROM  p2juegos
      WHERE 
      iniciodtp2=%s
...
$buscado= "/".$buscado."/i";
          ...
          if ($idjuego > 0) {
            ...
            // SI ACTUALIZA LOS LOGROS POR MARA, ESTE CODIGO VA A SER QUE ACTUALIZE EL JUEGO
            $insertSQL24 = sprintf(
              "/* PARSEADORES1 logros\parley.la_logros2.php - QUERY 15 */ UPDATE p2juegos 
            SET  p2vecesactualizado=p2vecesactualizado+1		
            WHERE 
            Id_p2juegosp2 = %s",                  
              GetSQLValueString($idjuego, "int")
      ...
      echo $logrodate;
              $query_Recordsetvrfjuego =  sprintf(
                "/* PARSEADORES1 logros\parley.la_logros2.php - QUERY 16 */ SELECT  
        deportep2
            FROM  p2juegos
            WHERE 
            iniciodtp2=%s
