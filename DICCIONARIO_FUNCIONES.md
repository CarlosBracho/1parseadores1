...
| **Bypass Híbrido Historial (JS)** | Script JS | Mecanismo en `alertas_lista.php` desacoplado de las librerías JS de Bootstrap mediante CSS moderno personalizado que neutraliza estilos heredados de Bootstrap 2.0.3 (con forzado de opacidad y remoción de las clases de conflicto de Bootstrap `.modal` y `.fade`) y un disparador directo jQuery con delegación de eventos. |
| **Salvaguarda Defensiva Alertify** | JavaScript | Bloque condicional en `includes/cabeceraadmin.php` que verifica si `alertify` está definido en el contexto global antes de llamar a `alertify.error`, evitando ReferenceErrors críticos en el hilo principal de las páginas administrativas. |
