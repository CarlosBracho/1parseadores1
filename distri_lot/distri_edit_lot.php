...
if (isset($_POST["guardar"])) {
    ...
    if ($graba==31) {
        ...
        $query_Recordset8 =  sprintf(
            "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 1 */ SELECT so.hor_sorteo
        ...
        foreach ($_POST["idtr_banlot"] as $idl) {
            ...
            $insertSQL2 = sprintf(
                "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 2 */ UPDATE bancaloterias 
					SET
					hor_cierre=%s,
					est_banlot=%s,
					lun_loteriabanca=%s,
            ...
            if ($tip_loteria[$cta]<4) {
                $insertSQL3 = sprintf(
                    "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 3 */ UPDATE bancaloterias 
						SET
						hor_cierre=%s,
						est_banlot=%s,
						lun_loteriabanca=%s,
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1" && isset($_POST["guardarDis"]))) {
    ...
    if ($graba==31) {
        $insertSQL1 = sprintf(
            "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 4 */ UPDATE banca 
			SET
			top_triple_lot=%s,
			top_terminal_lot=%s,
			con_tope=%s
    ...
    if (isset($_POST["tipA"]) && $_POST["tipA"]>=0) {
        if ($_POST["tipA"]>=0 && $_POST["tipA"]<=4 && isset($_POST["od_loteria"])) {
            ...
            $query_Recordset1 =  sprintf(
                "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 5 */ SELECT 
				bl.id_banlot, lo.tip_loteria, so.hor_sorteo,
				CASE lo.tip_loteria
					WHEN 1 THEN (/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 6 */ SELECT blo.id_banlot FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal AND 
						ba.cod_banca = blo.id_banca LIMIT 1)
					WHEN 3 THEN (/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 7 */ SELECT blo.id_banlot FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal AND 
						ba.cod_banca = blo.id_banca LIMIT 1)
					ELSE 0
				END AS id_terminal	
            ...
            if ($totalRows_Recordset1>0) {
                if ($_POST["tipA"]==0) {//dias
                    ...
                    do {
                        $insertSQL2 = sprintf(
                            "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 8 */ UPDATE bancaloterias 
                        ...
                        if ($row_Recordset1['tip_loteria']==1 or $row_Recordset1['tip_loteria']==3) {
                            $insertSQL2 = sprintf(
                                "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 9 */ UPDATE bancaloterias 
								SET lun_loteriabanca=%s, mar_loteriabanca=%s, mie_loteriabanca=%s, jue_loteriabanca=%s,
									vie_loteriabanca=%s, sab_loteriabanca=%s, dom_loteriabanca=%s
									WHERE id_banlot=%s",
                                GetSQLValueString($lun, "int"),
                    ...
                    do {
                        ...
                        $insertSQL2 = sprintf(
                            "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 10 */ UPDATE bancaloterias SET hor_cierre=%s WHERE id_banlot=%s",
                            GetSQLValueString($hora, "date"),
                            GetSQLValueString($row_Recordset1['id_banlot'], "int")
                        );
                        $Result2 = mysqli_query($conexionbanca, $insertSQL2) or die(mysqli_error($conexionbanca));
                        if ($row_Recordset1['tip_loteria']==1 or $row_Recordset1['tip_loteria']==3) {
                            $insertSQL2 = sprintf(
                                "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 11 */ UPDATE bancaloterias SET hor_cierre=%s WHERE id_banlot=%s",
                                GetSQLValueString($hora, "date"),
                                GetSQLValueString($row_Recordset1['id_terminal'], "int")
                            );
                            $Result2 = mysqli_query($conexionbanca, $insertSQL2) or die(mysqli_error($conexionbanca));
                    ...
                    if ($_POST['triA']>0) {
                        do {
                            $insertSQL3 = sprintf(
                                "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 12 */ UPDATE bancaloterias 
								SET pre_loteria=%s
								WHERE id_banlot=%s",
                    ...
                    if ($_POST['terA']>0) {
                        do {
                            if ($row_Recordset1['tip_loteria']==1 or $row_Recordset1['tip_loteria']==3) {
                                $insertSQL3 = sprintf(
                                    "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 13 */ UPDATE bancaloterias 
									SET pre_loteria=%s
									WHERE id_banlot=%s",
                    ...
                    do {
                        $insertSQL2 = sprintf(
                            "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 14 */ UPDATE bancaloterias SET est_banlot=%s WHERE id_banlot=%s",
                        ...
                        if ($row_Recordset1['tip_loteria']==1 or $row_Recordset1['tip_loteria']==3) {
                            $insertSQL2 = sprintf(
                                "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 15 */ UPDATE bancaloterias SET est_banlot=%s WHERE id_banlot=%s",
                                GetSQLValueString($_POST['staA'], "int"),
                                GetSQLValueString($row_Recordset1['id_terminal'], "int")
                            );
                            $Result2 = mysqli_query($conexionbanca, $insertSQL2) or die(mysqli_error($conexionbanca));
            ...
            if ($_POST["tipA"]==5 && $_POST["toptrA"]>0) {
                ...
                $query_Recordset1 =  sprintf(
                    "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 16 */ SELECT bl.id_banlot, bl.top_venta
					FROM banca ba, bancaloterias bl, loterias lo, sorteos so 
                ...
                if ($totalRows_Recordset1>0) {
                    do {
                        ...
                        $insertSQL1 = sprintf(
                            "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 17 */ UPDATE bancaloterias 
							SET
							top_venta=%s
							WHERE id_banlot=%s",
                ...
                $sorteo=implode("','", $_POST["od_sorteo"]);
                $query_Recordset1 =  sprintf(
                    "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 18 */ SELECT bl.id_banlot, bl.top_venta
					FROM banca ba, bancaloterias bl, loterias lo, sorteos so 
					WHERE ba.cod_banca = %s AND bl.id_banca = ba.cod_banca AND lo.id_loteria = bl.id_loteria AND 
					lo.tip_loteria = 2 AND lo.est_loteria = 1 AND lo.id_sorteo = so.id_sorteo AND so.est_sorteo = 1 AND 
					so.id_sorteo IN ('$sorteo')",
                ...
                if ($totalRows_Recordset1>0) {
                    do {
                        echo $row_Recordset1['id_banlot']."<br/>";
                        $insertSQL1 = sprintf(
                            "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 19 */ UPDATE bancaloterias 
							SET
							top_venta=%s
							WHERE id_banlot=%s",
...
if ((isset($_POST["MM_inserto"])) && ($_POST["MM_inserto"] == "formo") && (isset($_POST["od_sorteo"]))) {
    ...
    $query_Recordset1 =  sprintf(
        "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 20 */ SELECT ba.cod_banca, ba.nom_banca, ba.por_banca_lot, ba.mod_resultado,
		...
		CASE lo.tip_loteria
			WHEN 1 THEN (/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 21 */ SELECT blo.pre_loteria FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal AND 
				ba.cod_banca = blo.id_banca LIMIT 1)
			WHEN 3 THEN (/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 22 */ SELECT blo.pre_loteria FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal AND 
				ba.cod_banca = blo.id_banca LIMIT 1)
			ELSE 0
		END AS pre_terminal,	
		CASE lo.tip_loteria
			WHEN 1 THEN (/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 23 */ SELECT blo.top_venta FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal AND 
				ba.cod_banca = blo.id_banca LIMIT 1)
			WHEN 3 THEN (/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 24 */ SELECT blo.top_venta FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal AND 
				ba.cod_banca = blo.id_banca LIMIT 1)
			ELSE 0
		END AS top_venta_terminal,
		CASE lo.tip_loteria
			WHEN 1 THEN (/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 25 */ SELECT blo.id_banlot FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal AND 
				ba.cod_banca = blo.id_banca LIMIT 1)
			WHEN 3 THEN (/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 26 */ SELECT blo.id_banlot FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal AND 
				ba.cod_banca = blo.id_banca LIMIT 1)
			ELSE 0
    ...
    $osorteo=array();
    $query_Recordset1 =  sprintf(
        "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 27 */ SELECT ba.cod_banca, ba.nom_banca, ba.por_banca_lot, ba.mod_resultado, 
		ba.con_tope, ba.top_triple_lot, ba.top_terminal_lot, so.hor_sorteo, 
		bl.lun_loteriabanca, bl.mar_loteriabanca, bl.mie_loteriabanca, bl.jue_loteriabanca, bl.vie_loteriabanca, 
		bl.sab_loteriabanca, bl.dom_loteriabanca, bl.est_banlot, bl.hor_cierre, bl.pre_loteria, bl.id_banlot, bl.top_venta,
		lo.nom_loteria, lo.id_loteria, lo.id_terminal, lo.tip_loteria, so.nom_sorteo,
		CASE lo.tip_loteria
			WHEN 1 THEN (/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 28 */ SELECT blo.pre_loteria FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal AND 
				ba.cod_banca = blo.id_banca LIMIT 1)
			WHEN 3 THEN (/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 29 */ SELECT blo.pre_loteria FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal AND 
				ba.cod_banca = blo.id_banca LIMIT 1)
			ELSE 0
		END AS pre_terminal,
		CASE lo.tip_loteria
			WHEN 1 THEN (/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 30 */ SELECT blo.top_venta FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal AND 
				ba.cod_banca = blo.id_banca LIMIT 1)
			WHEN 3 THEN (/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 31 */ SELECT blo.top_venta FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal AND 
				ba.cod_banca = blo.id_banca LIMIT 1)
			ELSE 0
		END AS top_venta_terminal,
		CASE lo.tip_loteria
			WHEN 1 THEN (/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 32 */ SELECT blo.id_banlot FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal AND 
				ba.cod_banca = blo.id_banca LIMIT 1)
			WHEN 3 THEN (/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 33 */ SELECT blo.id_banlot FROM bancaloterias blo WHERE blo.id_loteria = lo.id_terminal AND 
				ba.cod_banca = blo.id_banca LIMIT 1)
			ELSE 0
		END AS id_banlot_ter	
		FROM banca ba, bancaloterias bl, loterias lo, sorteos so 
...
$query_Recordset2 =  sprintf(
    "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 34 */ SELECT so.id_sorteo, so.nom_sorteo
	FROM banca ba, bancaloterias bl, loterias lo, sorteos so 
	WHERE ba.cod_banca = %s AND bl.id_banca = ba.cod_banca AND lo.id_loteria = bl.id_loteria AND lo.tip_loteria != 2 AND
	lo.est_loteria = 1 AND lo.id_sorteo = so.id_sorteo AND so.est_sorteo = 1
...
$query_Recordset3 =  sprintf(
    "/* PARSEADORES1 distri_lot\distri_edit_lot.php - QUERY 35 */ SELECT lo.nom_loteria, bl.id_banlot
