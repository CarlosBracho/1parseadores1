...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1") && isset($_POST["agregar"])) {
    ...
    if (isset($_POST['tip_bloqueo'])&&isset($_POST['cod_banca'])&&isset($_POST['num_bloqueo'])&&$_POST['num_bloqueo']!=""
        ...
        if ($guarda==80) {
            $insertSQL1 = sprintf(
                "/* PARSEADORES1 distri_lot\admin_bloqueo_edit_lot.php - QUERY 1 */ INSERT 
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1") && isset($_POST["eliminar"])) {
    $query_delete =  sprintf(
        "/* PARSEADORES1 distri_lot\admin_bloqueo_edit_lot.php - QUERY 2 */ DELETE FROM bloqueadoloterias WHERE id_bloqueado = %s",
...
$query_Recordset1 =  sprintf(
    "/* PARSEADORES1 distri_lot\admin_bloqueo_edit_lot.php - QUERY 3 */ SELECT 
...
$query_Recordset2 =  sprintf(
    "/* PARSEADORES1 distri_lot\admin_bloqueo_edit_lot.php - QUERY 4 */ SELECT 
...
$query_Recordset3 =  sprintf(
    "/* PARSEADORES1 distri_lot\admin_bloqueo_edit_lot.php - QUERY 5 */ SELECT 
...
$query_Recordset4 =  sprintf(
    "/* PARSEADORES1 distri_lot\admin_bloqueo_edit_lot.php - QUERY 6 */ SELECT 
...
$query_Recordset5 =  sprintf(
    "/* PARSEADORES1 distri_lot\admin_bloqueo_edit_lot.php - QUERY 7 */ SELECT 
...
$query_Recordset6 =  sprintf(
    "/* PARSEADORES1 distri_lot\admin_bloqueo_edit_lot.php - QUERY 8 */ SELECT 
	...
	(CASE bl.id_sorteo
		WHEN 0 THEN 
			(CASE bl.id_loteria
				WHEN -1 THEN 'TODAS'
				WHEN 0 THEN 'TODAS'
				ELSE (/* PARSEADORES1 distri_lot\admin_bloqueo_edit_lot.php - QUERY 9 */ SELECT CONCAT('LOTERIA: ',lo.nom_loteria) FROM loterias lo WHERE lo.id_loteria = bl.id_loteria LIMIT 1)
			END)
		ELSE (/* PARSEADORES1 distri_lot\admin_bloqueo_edit_lot.php - QUERY 10 */ SELECT CONCAT('SORTEO: ',so.nom_sorteo) FROM sorteos so WHERE so.id_sorteo = bl.id_sorteo LIMIT 1)
	END) AS nomloteria,
	
	(CASE bl.id_taquilla
		WHEN 0 THEN 'TODAS'
		ELSE (/* PARSEADORES1 distri_lot\admin_bloqueo_edit_lot.php - QUERY 11 */ SELECT ta.nom_taquilla FROM taquilla ta WHERE bl.id_taquilla = ta.cod_taquilla LIMIT 1)
	END) AS nomtaquilla,
	
	(CASE bl.id_agencia
		WHEN 0 THEN 'TODAS'
		ELSE (/* PARSEADORES1 distri_lot\admin_bloqueo_edit_lot.php - QUERY 12 */ SELECT ag.nom_agencia FROM agencia ag WHERE ag.cod_agencia = bl.id_agencia LIMIT 1)
	END) AS nomagencia
